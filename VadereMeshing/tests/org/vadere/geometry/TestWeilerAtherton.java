...
public class TestWeilerAtherton {
  ...
  public void testNoIntersection() {
    ...
    VPolygon poly2 =
        GeometryUtils.toPolygon(new VPoint(-0.01, 0), new VPoint(-1, 1), new VPoint(-1, -1));
  ...
  public void testIntersection() {
    VPolygon poly1 = GeometryUtils.toPolygon(new VPoint(0, 0), new VPoint(1, 1), new VPoint(1, -1));
    VPolygon poly2 =
        GeometryUtils.toPolygon(new VPoint(0.3, 0), new VPoint(-1, 1), new VPoint(-1, -1));
  ...
  public void testIntersectionFaceConstructionNoIntersections() {
    ...
    Pair<PFace, PFace> pair =
        weilerAtherton.constructIntersectionFaces(
            poly1, new PMesh(),
            poly2, new PMesh());
    ...
    assertEquals(
        expectedPoints1,
        mesh.streamPoints(face1).map(p -> new VPoint(p)).collect(Collectors.toSet()));
    assertEquals(
        expectedPoints2,
        mesh.streamPoints(face2).map(p -> new VPoint(p)).collect(Collectors.toSet()));
  }
  ...
  public void testIntersectionFaceConstructionIntersections() {
    ...
    assertEquals(
        5, mesh.streamPoints(face1).map(p -> new VPoint(p)).collect(Collectors.toSet()).size());
    assertEquals(
        5, mesh.streamPoints(face2).map(p -> new VPoint(p)).collect(Collectors.toSet()).size());
  }
}
