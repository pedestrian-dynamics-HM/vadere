...
public class PerformanceTest {
  ...
  private static void testPointerWalk() {
    ...
    IIncrementalTriangulation<PVertex, PHalfEdge, PFace> delaunay =
        IIncrementalTriangulation.createPTriangulation(IPointLocator.Type.BASE, points);
    delaunay.finish();
    log.info(
        "runtime of the Walk method, #vertices = "
            + delaunay.getVertices().size()
            + " is "
  ...
  private static void testArrayJumpAndWalk() {
    long ms = System.currentTimeMillis();
    IIncrementalTriangulation<AVertex, AHalfEdge, AFace> delaunay =
        IIncrementalTriangulation.createATriangulation(IPointLocator.Type.JUMP_AND_WALK, points);
    delaunay.finish();
    log.info(
        "runtime of the Jump & Walk method (Array), #vertices = "
            + delaunay.getVertices().size()
            + " is "
            + (System.currentTimeMillis() - ms)
            + " [ms]");
  ...
  private static void testPointerJumpAndWalk() {
    long ms = System.currentTimeMillis();
    IIncrementalTriangulation<PVertex, PHalfEdge, PFace> delaunay =
        IIncrementalTriangulation.createPTriangulation(IPointLocator.Type.JUMP_AND_WALK, points);
    delaunay.finish();
    log.info(
        "runtime of the Jump & Walk method (Pointer), #vertices = "
            + delaunay.getVertices().size()
            + " is "
            + (System.currentTimeMillis() - ms)
            + " [ms]");
  ...
  private static void testPointerDelaunayHierarchy() {
    ...
    delaunay.finish();
    log.info(
        "runtime of the Delaunay-Hierarchy (Pointer), #vertices = "
            + delaunay.getVertices().size()
            + " is "
            + (System.currentTimeMillis() - ms)
            + " [ms]");
  ...
  private static void testArrayDelaunayHierarchy() {
    ...
    delaunay.finish();
    log.info(
        "runtime of the Delaunay-Hierarchy (Array), #vertices = "
            + delaunay.getVertices().size()
            + " is "
            + (System.currentTimeMillis() - ms)
            + " [ms]");
  ...
  private static void testSweepline() {
    ...
    log.info(
        "runtime of the Sweepline method, #vertices = "
            + points.size()
            + " is "
            + (System.currentTimeMillis() - ms)
            + " [ms]");
  ...
  private static void testJTSDelaunayBuilder() {
    ...
    log.info(
        "runtime of the Quad-Tree method (JTS), #vertices = "
            + points.size()
            + " is "
            + (System.currentTimeMillis() - ms)
            + " [ms]");
