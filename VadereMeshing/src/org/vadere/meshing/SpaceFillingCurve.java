...
public class SpaceFillingCurve {
  ...
  public double compute(final IPoint point) {
    ...
    for (int i = 0; i < xCode.length(); i++) {
      ...
      Pair<State, Byte> pair =
          nextState(
              currentState,
              Byte.parseByte(xCode.charAt(i) + ""),
              Byte.parseByte(yCode.charAt(i) + ""));
  ...
  */
  ...
  public Pair<State, Byte> nextState(final State current, Byte x, Byte y) {
    ...
    switch (current) {
      case TopLeft:
        if (x == 0 && y == 1) return Pair.of(State.TopLeft, one);
        else if (x == 1 && y == 1) return Pair.of(State.TopLeft, two);
        else if (x == 0 && y == 0) return Pair.of(State.TopRight, zero);
        else return Pair.of(State.BottomLeft, three);
      case TopRight:
        if (x == 0 && y == 1) return Pair.of(State.BottomRight, three);
        else if (x == 1 && y == 1) return Pair.of(State.TopRight, one);
        else if (x == 0 && y == 0) return Pair.of(State.TopLeft, zero);
        else return Pair.of(State.TopRight, two);
      case BottomLeft:
        if (x == 0 && y == 1) return Pair.of(State.BottomLeft, one);
        else if (x == 1 && y == 1) return Pair.of(State.BottomRight, zero);
        else if (x == 0 && y == 0) return Pair.of(State.BottomLeft, two);
        else return Pair.of(State.TopLeft, three);
      case BottomRight:
        if (x == 0 && y == 1) return Pair.of(State.TopRight, three);
        else if (x == 1 && y == 1) return Pair.of(State.BottomLeft, zero);
        else if (x == 0 && y == 0) return Pair.of(State.BottomRight, two);
        else return Pair.of(State.BottomRight, one);
    }
