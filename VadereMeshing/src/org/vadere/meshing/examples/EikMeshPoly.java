...
public class EikMeshPoly {
  ...
  public static void meshPoly(@NotNull final String fileName)
    ...
    var ruppert = new PRuppertsTriangulator(pslg, p -> Double.POSITIVE_INFINITY, 0, true);
    ruppert.generate();
    ...
    Predicate<PFace> alertPredicate =
        f -> {
          return !meshImprover.getMesh().isBoundary(f)
              && distanceFunction.apply(meshImprover.getMesh().toTriangle(f).midPoint()) > 0;
        };
    ...
    System.out.println(
        String.format(
            "Mesh generation complete: %d vertices", meshImprover.getMesh().getNumberOfVertices()));

    System.out.println("Writing TikZ file...");
    write(
        toTexDocument(
            TexGraphGenerator.toTikz(
                meshImprover.getMesh(), f -> lightBlue, null, vertexColorFunction, 1.0f, true)),
        fileName + ".tex");
  ...
  public static void displayPolyFile(@NotNull final String fileName) throws IOException {
    ...
    MeshPolyReader<PVertex, PHalfEdge, PFace> meshPolyWriter =
