...
/**
 * A poly-connectivity {@link IPolyConnectivity} is the connectivity of a mesh of non-intersecting
 * connected polygons including holes. So it is more abstract than a tri-connectivity {@link
 * ITriConnectivity}. The mesh {@link IMesh} stores all the date of the base elements, vertices
 * {@link V}, half-edges {@link E} and faces {@link F}) and offers factory method to create new base
 * elements. The connectivities, i.e. {@link IPolyConnectivity} and {@link ITriConnectivity} offers
 * all the operations manipulating the connectivity of the mesh. The connectivity is the relation
 * between vertices and edges which define faces which therefore define the mesh structure.
 *
...
public interface IPolyConnectivity<V extends IVertex, E extends IHalfEdge, F extends IFace>
  ...
  /**
   * Returns the mesh of this poly-connectivity {@link IPolyConnectivity}.
   *
   ...
   * faces of the mesh. For each polygon of the mesh the contained method will be evaluated until it
   * returns <tt>true</tt> which is rather time consuming.
   *
   * <p>Does not change the connectivity.
   *
  ...
  /**
   * Adjust the edge of a vertex in O(d) where d is the degree of the vertex. If there is an
   * half-edge e which is at the boundary (i.e. hole or border) and has the vertex v as its end
   * point, this method will set the half-edge of v to e. This is helpful to speed up the test
   * whether a vertex is a boundary vertex!
   *
   * <p>Does not change the connectivity.
   *
   * @param vertex v
  ...
  default void adjustVertex(@NotNull final V vertex) {
    getMesh()
        .streamEdges(vertex)
        .filter(edge -> getMesh().isBoundary(edge))
        .findAny()
        .ifPresent(edge -> getMesh().setEdge(vertex, edge));
  }

  /**
   * Returns a half-edge (begin, end) where end is its end point and begin is the end point of its
   * predecessor. This requires O(d), where d is the degree of begin.
   *
   * <p>Does not change the connectivity.
   *
  ...
  default Optional<E> findEdge(@NotNull final V begin, @NotNull final V end) {
    IMesh<V, E, F> mesh = getMesh();
    return mesh.getIncidentEdges(mesh.getEdge(begin))
        .stream()
        .filter(edge -> mesh.getPrev(edge).equals(end))
        .map(edge -> mesh.getTwin(edge))
        .findAny();
  }

  /**
   * Tests if the half-edge is the only link / part of the full-edge between the face of the
   * half-edge and the face of its twin. This requires O(n), where n is the number of edges of the
   * face of the edge.
   *
   * <p>Does not change the connectivity.
   *
  ...
  /**
   * Splits the edge (s) to (e) into two edges (s) to (p) to (e) in O(1).
   *
  ...
  /**
   * Tests if there is any face which shares more than one edge with the face we are checking. This
   * requires O(n), where n is the number of edges of the face.
   *
   * <p>Does not change the connectivity.
   *
   * @param face the face we are checking
   * @return true if there is no face which shares more than one edge with this face, false
   *     otherwise
   */
  ...
  /**
   * Splitting the face i.e. a polygon into as many faces as the face has edges which triangulates
   * the face / polygon. This requires the time to locate the face which is O(n), where n is the
   * number of faces for a basic implementation and O(log(n)) for more sophisticated point location
   * algorithms see {@link IPointLocator} and the actual split which requires O(1) but which might
   * require additional changes e.g. in case for a Delaunay Triangulation see {@link
   * IncrementalTriangulation}.
   *
   * <p>Assumption: the vertex is valid i.e. it is contained in some face.
   *
   * <p>Changes the connectivity.
   *
   * @param vertex the vertex which spilts the face which triangleContains the vertex. It has to be
   *     contained any face.
   */
  default void split(@NotNull final V vertex) {
    ...
    if (!optFace.isPresent()) {
      throw new IllegalArgumentException(
          vertex + " is not contained in any face. Therefore, no face found to split into faces.");
    } else {
  ...
  /**
   * Splitting the face i.e. a polygon into as many faces as the face has edges. This will
   * essentially triangulate the polygon.
   *
   * <p>Assumption: the vertex is valid i.e. it is contained in the face.
   *
   * <p>Changes the connectivity.
   *
  ...
  /**
   * Removes all links between the face and the otherFace. This essentially merges these two faces
   * together if and only if there share a common edge. If one of these faces is the outer boundary
   * i.e. the border the other one will be deleted. This requires O(max(n,m)), where n and m is the
   * number of edges of the involved faces.
   *
   * <p>Assumption: both faces aren't destroyed.
   *
   * <p>Changes the connectivity.
   *
   * @param face face one
   * @param otherFace face two
   * @param deleteIsolatedVertices if true, vertices with degree zero will be removed from the mesh
   *     data structure otherwise they will not.
   * @return (optional) the remaining face (which might be face or otherFace) or empty if both edges
   *     share no common edge and therefore nothing changes
   */
  default Optional<F> removeEdges(
      @NotNull final F face, @NotNull F otherFace, final boolean deleteIsolatedVertices) {
    // TODO: test it!
    ...
    List<E> toDeleteEdges =
        getMesh()
            .getEdges(remFace)
            .stream()
            .filter(e -> getMesh().getTwinFace(e).equals(finalFace))
            .collect(Collectors.toList());

    assert getMesh().getEdges(remFace).size() > toDeleteEdges.size()
        : "can not remove all of the edges, since this could lead to an invalid mesh";

    // List<E> survivalEdges = getMesh().streamEdges(remFace).filter(e ->
    // !getMesh().getTwinFace(e).equals(finalFace)).collect(Collectors.toList());
   ...
   * the mergeCondition does no longer hold. This requires in the worst case O(n), where n is the
   * number of edges of all involved faces (i.e. the face and the merged faces).
   *
   * <p>Changes the connectivity.
   *
   * @param face the face
  ...
  /**
   * A virus like working algorithm which merges neighbouring faces by starting at the face until
   * the mergeCondition does no longer hold or the maximal dept is reached. This requires in the
   * worst case O(n), where n is the number of edges of all involved faces (i.e. the face and the
   * merged faces).
   *
   * <p>Changes the connectivity.
   *
   ...
   * @param deleteIsolatedVertices if true, vertices with degree zero will be removed from the mesh
   *     data structure otherwise they will not.
   * @param errorCondition a predicate which indicates that the merge process did merge faces which
   *     should not be merged, and therefore leading to an illegal mesh. If this condition is ever
  ...
  default Optional<F> mergeFaces(
    ...
    while (modified) {
      ...
      assert neighbouringFaces.isEmpty()
              || neighbouringFaces.stream().anyMatch(f -> !f.equals(neighbouringFaces.get(0)))
          : "each edge of both faces is a link to the other face";
      ...
      for (F neighbouringFace : neighbouringFaces) {
        ...
        if (!getMesh().isDestroyed(neighbouringFace) && mergeCondition.test(neighbouringFace)) {
          Optional<F> optionalMergeResult =
              removeEdges(currentFace, neighbouringFace, deleteIsolatedVertices);
  ...
  // TODO: improve performance by remembering faces
  /**
   * A virus like working algorithm which searches for neighbouring faces by starting at the face
   * until the <tt>markCondition</tt> does no longer hold or the maximal dept is reached. This
   * requires in the worst case O(n), where n is the number of edges of all involved faces (i.e. the
   * face and the merged faces).
   *
   * <p>Does not changes the connectivity.
   *
   * @param face the face
  ...
  default List<F> findFaces(
      @NotNull final F face, @NotNull final Predicate<F> markCondition, final int maxDept) {
    int dept = 0;
    ...
    while (!toProcess.isEmpty()) {
      ...
      // assert toProcess.isEmpty() || toProcess.stream().anyMatch(e -> getMesh().getFace(e))) :
      // "each edge of both faces is a link to the other face";
  ...
  /**
   * Creates a new hole or extends an existing hole by removing neighbouring faces by a virus
   * algorithm which consumes faces as long as the merge condition holds.
   *
   * <p>Changes the connectivity.
   *
   * @param face they face which will be transformed into a hole
   * @param mergeCondition the merge condition
   * @param deleteIsoletedVertices if true isolated vertices, i.e. vertices without any edges, will
   *     be removed from the mesh
   * @param vertexAdjust true means that boundary vertices will get their boundary edge as edge,
   *     false means there is no guarantee that this adjustment is made
   * @return (optional) the hole or face itself it the face does not fulfill the merge condition or
   *     empty if due to the creation of the hole all faces will be removed!
   */
  default Optional<F> createHole(
      @NotNull final F face,
      @NotNull final Predicate<F> mergeCondition,
      final boolean deleteIsoletedVertices,
      final boolean vertexAdjust) {
   ...
   * it is at the border (during the shrinking process) and satisfies the condition. Like a virus
   * this algorithms removes faces from outside, i.e. the border, towards inside. This requires O(n)
   ...
   * @param deleteIsolatedVertices true then isolated vertices (they are not connected to an edge)
   *     will be removed.
   * @param vertexAdjust true means that boundary vertices will get their boundary edge as edge,
   *     false means there is no guarantee that this adjustment is made
   */
  default void shrinkBorder(
      final Predicate<F> removeCondition,
      final boolean deleteIsolatedVertices,
      final boolean vertexAdjust) {
  ...
  default void shrinkBorder(
      final Predicate<F> removeCondition, final boolean deleteIsolatedVertices) {
    shrinkBorder(removeCondition, deleteIsolatedVertices, true);
  }

  default void shrinkBoundary(
      final Predicate<F> removeCondition, final boolean deleteIsolatedVertices) {
   ...
   * @param deleteIsolatedVertices true then isolated vertices (they are not connected to an edge)
   *     will be removed.
   * @param adjustVertices true means that boundary vertices will get their boundary edge as edge,
   *     false means there is no guarantee that this adjustment is made
   */
  default void shrinkBoundary(
      @NotNull final F boundary,
      final Predicate<F> removeCondition,
      final boolean deleteIsolatedVertices,
      final boolean adjustVertices) {
  ...
  default void removeFacesAtBoundary(
      @NotNull final Predicate<F> mergePredicate, @NotNull final Predicate<F> errorPredicate)
      throws IllegalMeshException {
   ...
   *
   * <p>Assumption: the edge is a simple link, if this is not the case the method will not change
   * the topology.
   *
   * <p>Changes the connectivity.
   *
   ...
   *
   * <p>Assumption: the edge is a simple link
   *
   * <p>Changes the connectivity.
   *
   * @param edge the simple link
  ...
  /**
   * Tests whether the point (x, y) is contained in the face. This requires O(n), where n is the
   * number of edges of the face.
   *
   * <p>Assumption: The ordering of the mesh is CCW and the face is a valid simple polyon.
   *
   * <p>Does not change the connectivity.
   *
   ...
   *
   * <p>Assumption: v1 -> v2 -> v3 is ccw oriented.
   *
   * @param x
   * @param y
   * @param v1
  ...
  /**
   * Returns true if the point (x1, y1) is part of the face in O(n), where n is the number of edges
   * of the face.
   *
   * <p>Does not change the connectivity.
   *
  ...
  /**
   * Returns true if the point (x1, y1) is close to a point of the face in O(n), where n is the
   * number of edges of the face.
   *
   * <p>Does not change the connectivity.
   *
  ...
  default boolean isClose(
      final double x1, final double y1, @NotNull final F face, double distance) {
    return getCloseEdge(face, x1, y1, distance).isPresent();
  }

  /**
   * (Optional) returns the half-edge of a face which ends in (x1, y1) in O(n), where n is the
   * number of edges of the face.
   *
   * <p>Does not change the connectivity.
   *
  ...
  /**
   * (Optional) returns the half-edge of a face with an end-point close to (x1, y1) in O(n), where n
   * is the number of edges of the face.
   *
   * <p>Does not change the connectivity.
   *
   * @param face the face
  ...
  default boolean isRightOf(final double x1, final double y1, @NotNull final E edge) {
    ...
    return GeometryUtils.isRightOf(
        getMesh().getX(v1), getMesh().getY(v1), getMesh().getX(v2), getMesh().getY(v2), x1, y1);
  }
  ...
  /**
   * Tests if the line-segment defined by the half-edge intersects the line defined by p1 and p2 in
   * O(1).
   *
   * <p>Does not change the connectivity.
   *
  ...
  default boolean intersects(
      @NotNull final IPoint p1, @NotNull final IPoint p2, @NotNull final E edge) {
    V v1 = getMesh().getVertex(getMesh().getPrev(edge));
    V v2 = getMesh().getVertex(edge);
  ...
  /**
   * Tests if the line-segment defined by the (v1,v2) intersects the line defined by p1 and p2 in
   * O(1).
   *
   * <p>Does not change the connectivity.
   *
  ...
  default boolean intersects(
      ...
      @NotNull final V v1,
      @NotNull final V v2) {
    return GeometryUtils.intersectLine(
        p1.getX(), p1.getY(), p2.getX(), p2.getY(), v1.getX(), v1.getY(), v2.getX(), v2.getY());
  }

  /**
   * Tests if the half-line-segment starting at p1 in the direction (p2-p1) intersects the
   * line-segment defined by the half-edge in O(1).
   *
   * <p>Does not change the connectivity.
   *
   ...
   * @param edge the half-edge defining the line-segment
   * @return true if the half-line-segment starting at p1 in the direction (p2-p1) intersects the
   *     line-segment defined by the half-edge, false otherwise
   */
  default boolean intersectsDirectional(
      @NotNull final IPoint p1, @NotNull final IPoint p2, E edge) {
    V v1 = getMesh().getVertex(getMesh().getPrev(edge));
    V v2 = getMesh().getVertex(edge);
  ...
  /**
   * Removes a face from the mesh by removing all border edges of the face. If there are no border
   * edges the face will be converted to be a hole. If there are any neighbouring holes, all of them
   * will be merged together.
   *
   * <p>Changes the connectivity.
   *
   * @param face the face that will be removed from the mesh
   * @param deleteIsolatedVertices true means that all vertices with degree smaller equals 1 will be
   *     removed as well
   */
  ...
  /**
   * Removes a face from the mesh by removing all boundary edges of the face. If there is no
   * boundary edge this method will not change the mesh topology. This requires O(n) (if the face is
   * no island) where n is the number of edges of the face. If the face is an island (a very special
   * case) this can require O(m), where m is the number of all edges of the mesh!
   *
   * <p>Changes the connectivity.
   *
   * <p>Assumption: boundary is in fact a boundary and is neighbouring the face and there is no
   * other neighbouring boundary.
   *
   * @param face the face that will be removed from the mesh
   ...
   *     false means there is no guarantee that this adjustment is made
   */
  default void removeFaceAtBoundary(
      @NotNull final F face,
      @NotNull final F boundary,
      final boolean deleteIsolatedVertices,
      final boolean adjustVertices) {
    if (!getMesh().isDestroyed(face)) {
      ...
      assert getMesh().streamFaces(face).filter(neighbour -> neighbour.equals(boundary)).count()
          > 0;
      ...
      for (E edge : getMesh().getEdgeIt(face)) {
        ...
        if (twinFace.equals(boundary)) {
          ...
          // remember an edge that will not be deleted. This edge can be used as the edge of the
          // boundary.
          survivingEdge = edge;
      ...
      // special case: all edges will be deleted && the edge of the border will be deleted as well!
      // => adjust the border edge
        ...
        if (twinFace.equals(face)) {
          ...
          if (!optBoundaryEdge.isPresent()) {
            if (getMesh().getEdges(boundary).size() == delEdges.size()) {
              log.warn(
                  face
                      + " is the last remaining face which will be deletes as well, therefore the mesh will be emoty!");
      ...
      if (!delEdges.isEmpty()) {
        ...
        for (E delEdge : delEdges) {
          ...
          // boolean isolated0 = getMesh().getTwin(h0) == getMesh().getNext(h0) ||
          // getMesh().getTwin(h0) == getMesh().getPrev(h0);
          // boolean isolated1 = getMesh().getTwin(h1) == getMesh().getNext(h1) ||
          // getMesh().getTwin(h1) == getMesh().getPrev(h1);
  ...
  default void removeFaceAtBoundary(
      @NotNull final F face, @NotNull final F boundary, final boolean deleteIsolatedVertices) {
    removeFaceAtBoundary(face, boundary, deleteIsolatedVertices, true);
  }

  /**
   * Removes a face from the mesh by removing all boundary edges of the face. If there is no border
   * edge this method will not change the mesh topology.
   *
   * <p>Changes the connectivity.
   *
   * <p>Assumption: the face is at the border
   *
   * @param face the face that will be removed from the mesh
   * @param deleteIsolatedVertices true means that all vertices with degree smaller equals 1 will be
   *     removed as well
  ...
  /**
   * Tests whether the vertex has degree smaller or equals 2. If an edge gets deleted and the vertex
   * is simple connected the vertex becomes isolated.
   *
   * <p>Does not change the connectivity.
   *
  ...
  /**
   * Returns a half-edge such that it is part of face1 and the twin of this half-edge is part of
   * face2.
   *
   * <p>Does not change the connectivity.
   *
