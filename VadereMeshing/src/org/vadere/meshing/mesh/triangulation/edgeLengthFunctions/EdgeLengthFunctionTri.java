...
public class EdgeLengthFunctionTri<V extends IVertex, E extends IHalfEdge, F extends IFace>
    implements IEdgeLengthFunction {
  private IIncrementalTriangulation<V, E, F> triangulation;

  public EdgeLengthFunctionTri(@NotNull final IIncrementalTriangulation<V, E, F> triangulation) {
  ...
  public Double apply(IPoint p) {
    ...
    if (mesh.isBoundary(face)) {
      ...
      return (triangulation.getMesh().getDoubleData(v1, propName)
              + triangulation.getMesh().getDoubleData(v2, propName))
          / 2.0;
    } else {
