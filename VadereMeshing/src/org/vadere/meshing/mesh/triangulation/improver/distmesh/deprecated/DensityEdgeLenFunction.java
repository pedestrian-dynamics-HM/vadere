...
public class DensityEdgeLenFunction implements IEdgeLengthFunction {
  ...
  public DensityEdgeLenFunction(
      final Function<IPoint, Double> densityFunc, final VRectangle regionBoundingBox) {
  ...
  public Double apply(final IPoint iPoint) {
    ...
    return 1
        / (Parameters.MINIMUM
            + (densityFunc.apply(iPoint) / maxDensity) * Parameters.DENSITYWEIGHT);
  }
  ...
  private double calculateMaxDensity() {
    ...
    for (int i = 0; i < Parameters.SAMPLENUMBER; i++) {
      for (int j = 0; j < Parameters.NPOINTS; j++) {
        double x =
            random.nextInt((int) (regionBoundingBox.getMaxX() - regionBoundingBox.getMinX()) + 1);
        double y =
            random.nextInt((int) (regionBoundingBox.getMaxY() - regionBoundingBox.getMinY()) + 1);
        int xi =
            (int)
                Math.floor(
                    x
                        / (regionBoundingBox.getMaxX() - regionBoundingBox.getMinX())
                        * (Parameters.SAMPLEDIVISION - 1));
        int yi =
            (int)
                Math.floor(
                    y
                        / (regionBoundingBox.getMaxY() - regionBoundingBox.getMinY())
                        * (Parameters.SAMPLEDIVISION - 1));
        means[yi][xi] = (means[yi][xi] + densityFunc.apply(new VPoint(x, y))) / 2;
        if (maxDensity < means[yi][xi]) maxDensity = means[yi][xi];
      }
    }
    return maxDensity;
  }
