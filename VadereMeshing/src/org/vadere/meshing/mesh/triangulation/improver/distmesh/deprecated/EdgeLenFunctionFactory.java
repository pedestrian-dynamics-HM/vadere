...
public class EdgeLenFunctionFactory {
  ...
  static IEdgeLengthFunction create(
      final VRectangle regionBoundingBox, final Function<IPoint, Double> densityFunc) {
  ...
  static IEdgeLengthFunction create(
      final VRectangle regionBoundingBox, final IDistanceFunction distanceFunc) {
    return new SimpleDistanceEdgeLenFunction(regionBoundingBox, distanceFunc);
  }

  static IEdgeLengthFunction create(double factor) {
    return vertex -> factor * (1 + vertex.getX());
  }
