...
public class GenEikMesh<V extends IVertex, E extends IHalfEdge, F extends IFace>
    implements IMeshImprover<V, E, F>, ITriangulator<V, E, F> {
  ...
  /**
   * Constructor to use EikMesh on an existing {@link org.vadere.meshing.mesh.inter.ITriangulation},
   * that is EikMesh uses this triangulation as a bases. It will refineSimplex2D the triangulation
   * by using a longest edge split strategy {@link GenRivaraRefinement} to generate desired edge
   * length determined by len(p) = <tt>edgeLengthFunc(p)</tt> if <tt>refine</tt> is set to
   * <tt>true</tt>. Since there is no {@link IDistanceFunction} function the geometry i.e. the
   * boundary and holes are defined by the given triangulation itself. Therefore, the algorithm does
   ...
   * <ol>
   *   <ul>
   *     <tt>edgeLengthFunc</tt> should be something like <tt>edgeLengthFunc</tt>(p) = minEdgeLen +
   *     f(p) and should be >= minEdgeLen everywhere!
   *   </ul>
   *   <ul>
   *     <tt>triangulation</tt> should be a valid triangulation
   *   </ul>
   * </ol>
   *
  ...
  public GenEikMesh(
    ...
    this.triangulation
        .getMesh()
        .streamBoundaryEdges()
        .map(e -> triangulation.getMesh().getVertex(e))
        .forEach(v -> setFixPoint(v, true));
  ...
  /**
   * Constructor to use EikMesh on an existing {@link org.vadere.meshing.mesh.inter.ITriangulation},
   * that is EikMesh uses this triangulation as a bases. It will not refine the the triangulation
   * further. Since there is no {@link IDistanceFunction} function the geometry i.e. the boundary
   * and holes are defined by the given triangulation itself. Therefore, the algorithm does not
   * project points but uses slide points only.
   *
   * <p>Assumption:
   *
   * <ol>
   ...
   * that is EikMesh uses this triangulation as a bases. It will refineSimplex2D the triangulation
   * by using a longest edge split strategy {@link GenRivaraRefinement} to generate desired edge
   * length determined by len(p) = <tt>edgeLengthFunc(p)</tt>. Since a {@link IDistanceFunction}
   * function is given projection is used (and no sliding points).
   *
   * <p>Assumption:
   *
   * <ol>
   *   <ul>
   *     <tt>edgeLengthFunc</tt> should be something like <tt>edgeLengthFunc</tt>(p) = 1 + f(p) and
   *     should be >= 1 everywhere!
   *   </ul>
   *   <ul>
   *     <tt>triangulation</tt> should be a valid triangulation with angles >= 20 degree!
   *   </ul>
   * </ol>
   *
   ...
   * defined by a {@link IDistanceFunction} and additionally (optional) by some {@link VShape}s,
   * where each of the elements in <tt>shapes</tt> is part of the (outside) boundary area. The
   * algorithm will generate sliding points on the base of the given {@link VShape}s and will
   * project based on the given {@link IDistanceFunction}.
   *
   ...
   * @param shapes a collection of shapes used to generate fix points to improve the result on sharp
   *     corners
  ...
  public GenEikMesh(
      @NotNull final VPolygon segmentBound,
      final double initialEdgeLen,
      @NotNull final Collection<? extends VShape> shapes,
      @NotNull final IMeshSupplier<V, E, F> meshSupplier) {
    this(
        new DistanceFunction(segmentBound, shapes),
        p -> initialEdgeLen,
        initialEdgeLen,
        GeometryUtils.boundRelative(segmentBound.getPoints()),
        shapes,
        meshSupplier);
  }
  ...
  /**
   * Returns true if the final mesh has been constructed, that is if the algorithm converges or the
   * mesh quality is good enough.
   *
  ...
  public boolean isFinished() {
    synchronized (getMesh()) {
      ...
      return isInitialized() && quality >= Parameters.qualityMeasurement && converged
          || (maxMovement > 0 && maxMovement / initialEdgeLen < Parameters.DPTOL)
          || nSteps >= MAX_STEPS;
    }
  }
          ...
          .filter(
              v ->
                  getMesh()
                      ...
                      .allMatch(f -> faceToQuality(f) > Parameters.qualityMeasurement))
          .forEach(v -> setFixPoint(v, true));
      getMesh()
          .streamFaces()
          .filter(f -> getMesh().streamVertices(f).allMatch(v -> isFixPoint(v)))
          .forEach(f -> getMesh().setBooleanData(f, "frozen", true));
    }
  }
    ...
    synchronized (getMesh()) {
      ...
      if (!isInitialized()) {
        ...
        if (allowFaceCollapse) {
          getTriangulation()
              .collapseBoundaryFaces(
                  f -> true,
                  e ->
                      !isFixPoint(getMesh().getVertex(e))
                          && getTriangulation().isLargeAngle(e, Parameters.MAX_COLLAPSE_ANGLE),
                  v -> {
                    if (useSlidingLines) {
                      E edge = getMesh().getBoundaryEdge(v).get();
                      if (isSlidePoint(getMesh().getVertex(getMesh().getNext(edge)))
                          && isSlidePoint(getMesh().getVertex(getMesh().getPrev(edge)))) {
                        pointToSlidingLine.put(
                            v,
                            pointToSlidingLine.get(getMesh().getVertex(getMesh().getNext(edge))));
                      }
                    }
                  });
        }
  ...
  /**
   * Returns true if the algorithm is based on a distance function. Otherwise it is based on a PSLG
  ...
  private void computeForces() {
    streamEdges()
        .forEach(e -> getForce(getMesh().toPoint(e), getMesh().toPoint(getMesh().getPrev(e))));
  }
  ...
  /**
   * Computes and sets the overall force acting on a vertex. This force is determined by all
   * adjacent real edges and in case of points 'close' to the boundary by a virtual edge.
   *
    ...
    for (E edge : getMesh().getEdgeIt(vertex)) {
      ...
      if (useVirtualEdges && !isAtBoundary) {
        ...
        if (getMesh().isAtBoundary(prev) || isSlidePoint(getMesh().getVertex(prev))) {
          ...
          VPoint m =
              GeometryUtils.projectOntoLine(
                  p1.getX(), p1.getY(), q1.getX(), q1.getY(), q2.getX(), q2.getY());
          ...
          // IPoint projection = GeometryUtils.projectOnto(virtualForce.getX(), virtualForce.getY(),
          // q2.getX() - q1.getX(), q2.getY() - q1.getY());
  ...
  /**
   * Unused: see (A Matlab mesh generator for the two-dimensional finite element method, doi:
   * 10.1016/j.amc.2014.11.009)
   *
   * @param edge
   */
  private void computeForcesBossan(final E edge) {
    ...
    double desiredLen =
        edgeLengthFunc.apply(
                new VPoint((v1.getX() + v2.getX()) * 0.5, (v1.getY() + v2.getY()) * 0.5))
            * Parameters.FSCALE
            * scalingFactor;
   ...
   * and resets their forces / velocities. A vertex might be broken (removed by edge collapse) if
   * the forces acting on an boundary vertex are to strong.
   */
  ...
  /**
   * Updates a vertex which is not a fix point, that is, the computed force is applied and the
   * vertex move according to this force. Additionally, the vertex might get back projected if it is
   * outside or it might break if the forces / the pressure are too large.
   *
  ...
  private void updateVertex(final V vertex) {
    ...
    if (canBreak(vertex) && isBreaking(vertex)) {
      // TODO: if the algorithm runs in parallel this might lead to unexpected results! synchronized
      // required!
      ...
      if (isSlidePoint(vertex)) {
        ...
        IPoint projection =
            GeometryUtils.projectOnto(
                movement.getX(),
                movement.getY(),
                lineEdge.getX2() - lineEdge.getX1(),
                lineEdge.getY2() - lineEdge.getY1());
  ...
  /**
   * Returns true if and only if the vertex is a sliding point which means it can only slide on a so
   * called sliding line.
   *
   * @param vertex the vertex of interest.
  ...
  public boolean isSlidePoint(@NotNull V vertex) {
    boolean slidePoint = /*getMesh().isAtBoundary(vertex) &&*/
        !isFixPoint(vertex) && pointToSlidingLine.containsKey(vertex);
    return slidePoint;
  }
  ...
  private void updateFace(@NotNull F face) {
    if (canBreak(face) && isBreaking(face)) {
      ...
      getTriangulation()
          .splitTriangle(
              face, getMesh().createPoint(circumcenter.getX(), circumcenter.getY()), false);
    }
  }
  ...
  private boolean updateEdge(@NotNull final E edge) {
    if (getTriangulation().isShortestHalfEdge(edge)
        && (faceToQuality(edge) < Parameters.MIN_COLLAPSE_QUALITY)) {
      V v1 = getMesh().getVertex(edge);
      ...
      if (!isFixPoint(v1) || !isFixPoint(v2)) {
        ...
        if (isSlidePoint(v1)
            && isSlidePoint(v2)
            && !pointToSlidingLine.get(v1).equals(pointToSlidingLine.get(v2))) {
          return false;
        }
  ...
  private void updateBoundaryEdge(@NotNull final E edge) {
    if (canBreak(edge) && isBreaking(edge)) {
      ...
      boolean isFixedSlicePoint =
          useSlidingLines && isFixPoint(v1) && isFixPoint(v2) && !hasDistanceFunction();
  ...
  private boolean isBreaking(@NotNull final F face) {
    if (faceToQuality(face) > 0.95) {
      ...
      if (edgeLengthFunc.apply(getMesh().toLine(edge).midPoint()) * 2.1
          <= getMesh().toLine(edge).length()) {
        VPoint circumcenter = getMesh().toCircumcenter(face);
  ...
  /**
   * Returns true if the edge can be split, that is the edge can be replaced by two new edges by
   * splitting the edge and its face into two. The edge has to be at the boundary.
   *
   * @param edge the edge
   * @return true if the edge can be collapsed / break.
   */
  private boolean canBreak(@NotNull final E edge) {
    return allowEdgeSplits /*&& getMesh().isAtBoundary(edge) */
        && (getMesh().isBoundary(edge) || isConstrained(edge));
  }

  /**
   * Returns true if the edge should be split. That is the case if the edge is long with respect to
   * the other two edges of the face, i.e. it has to be the longest edge and the quality of the face
   * is low. The analogy is that the edge breaks because it can not become any longer.
   *
  ...
  /**
   * Returns true if the vertex can be collapsed, that is the vertex can be removed by removing one
   * edge and collapsing the other two. The vertex has to be at the boundary and has to have degree
   * equal to three. We say that this vertex can break under the pressure of the forces.
   *
   * @param vertex the vertex
  ...
  /**
   * Returns true if the vertex should be collapsed. That is the case if the resulting force acting
   * on the vertex is low but the sum of all absolute partial forces is high. An analogy might be
   * that the vertex breaks under the pressure of the forces.
   *
  ...
  /**
   * Computes the projection of Projects vertices. The projection acts towards the boundary of the
   * boundary. EikMesh projects only boundary vertices. Furthermore, to improve the convergence rate
   * EikMesh additionally projects vertices which are inside if the projection is inside a valid
   * circle segment.
   *
  ...
  private VPoint computeProjection(@NotNull final V vertex) {
    ...
    if (isBoundary(vertex)) {
      ...
      // back projection towards the inside if the point is inside (to improve the convergence rate
      // of the algorithm)
   ...
   * distance function available. By default, the distance function is required to project points
   * onto the boundary. We replace this requirement by projecting points onto the boundary segments
   * i.e. points can move on the lines of the segment-bound or a hole of a PSLG, that is a
   * triangulation. Therefore, this method constructs a map : boundary vertex -> line which gives
   * quick access to the projection line.
   */
  private void computeSlidingLines() {
    ...
    for (F boundaryFace : getMesh().getBoundaryAndHoles()) {
      ...
      Optional<V> optionalV =
          getMesh().streamVertices(boundaryFace).filter(v -> isFixPoint(v)).findFirst();
    ...
    assert getMesh()
        .getBoundaryVertices()
        .stream()
        .filter(v -> isSlidePoint(v))
        .allMatch(v -> pointToSlidingLine.containsKey(v));
  }
  ...
  /**
   * Tests if the inside projection is valid which is the case if the angle3D at the vertex (at the
   * boundary) is greater than 180 degree or the result of the projection lies inside the segment
   * spanned by the vertex and its two neighbouring border vertices.
   *
   * @param vertex the vertex
   ...
   */
  private boolean isInsideProjectionValid(
      @NotNull final V vertex, final double newX, final double newY) {
    ...
    if (!boundaryEdgeOpt.isPresent()) {
      ...
      return angle > Math.PI
          || (GeometryUtils.isLeftOf(r, p, newX, newY) && GeometryUtils.isLeftOf(p, q, newX, newY));
    }
  }

  /**
   * Computes the anchor to vertex partial relation. This relation gives some vertices their anchor
   * point. If a vertex has an anchor point it will be driven towards this point (instead of the
   * normal movement).
   */
  ...
  /**
   * Flips all edges which do not fulfill the Delaunay criterion and therefore being illegal. Note
   * that this is not a recursive flipping and therefore the result might not be a Delaunay
   * triangulation. However, due to the nature of the EikMesh algorithm (high quality initial mesh)
  ...
  private double computeEdgeScalingFactor(@NotNull final IEdgeLengthFunction edgeLengthFunc) {
    double edgeLengthSum =
        streamEdges().map(edge -> getMesh().toLine(edge)).mapToDouble(line -> line.length()).sum();

    double desiredEdgeLenSum =
        streamEdges()
            .map(edge -> getMesh().toLine(edge))
            .map(line -> line.midPoint())
            .mapToDouble(midPoint -> edgeLengthFunc.apply(midPoint))
            .sum();
  ...
  /**
   * Returns the force which is currently i.e. which was computed by {@link
   * GenEikMesh#computeForce(IVertex)} applied to the vertex.
   *
   * @param vertex the vertex of interest
  ...
  /**
   * Applies the force of the vertex to the vertex which results in an displacement of the vertex.
   *
   * @param vertex the vertex of interest
   */
  ...
  /**
   * Computes the set of anchor points. An anchor point replaces fix points in EikMesh. Instead of
   * inserting fix points EikMesh pushes (via forces) close points of an anchor point towards this
   * anchor point. For each shape the the points of the path defining the shape will be added to the
   ...
   *
   * <p>This takes O(n) time where n is the number of removed faces which will be consumed.
   */
  private void removeFacesAtBoundary() {
    ...
    Predicate<F> isOfLowQuality =
        f -> faceToQuality(f) < Parameters.MIN_TRIANGLE_QUALITY && !isShortBoundaryEdge(f);
  ...
  private void updateProjectionLines() {
    // this is a bad code which just updates the pointToSlidingLine requiring to run on the border
    // possibly in both directions
    getMesh()
        .streamVertices()
        .filter(v -> isSlidePoint(v))
        .filter(v -> !pointToSlidingLine.containsKey(v))
        .forEach(
            v -> {
              E edge = getMesh().getBoundaryEdge(v).get();
              ...
              if (isFixPoint(v1) && isFixPoint(v2)) {
                ...
                if (!pointToSlidingLine.containsKey(v)) {
                  EdgeIteratorReverse<V, E, F> reverseEdgeIterator =
                      new EdgeIteratorReverse<>(getMesh(), edge);
  ...
  /**
   * Shrinks the boundary such that there are no more triangles outside the boundary i.e. where the
   * distance is positive.
   */
  private void shrinkBoundary() {
    Predicate<F> removePredicate =
        face -> distanceFunc.apply(getTriangulation().getMesh().toMidpoint(face)) > 0;
    ...
    for (F face : faces) {
      if (!getTriangulation().getMesh().isDestroyed(face)
        ...
        getTriangulation()
            .createHole(
                face,
                f -> distanceFunc.apply(getTriangulation().getMesh().toMidpoint(f)) > 0,
                true);
      }
    }
  }
  ...
  /**
   * Unused old strategy to deal with very obtuse triangles at the boundary. This is replaced by the
   * concept of virtual edges.
   *
   * @param edge
   */
  private void computeBoundaryForces(final E edge) {
    ...
    if (getMesh().isBoundary(edge)) {
      ...
      if ((getMesh().toLine(next).length() + getMesh().toLine(prev).length()) * 1.1
          < getMesh().toLine(edge).length()) {
        ...
        VPoint q =
            new VPoint((midPoint.getX() + v3.getX()) * 0.5, (midPoint.getY() + v3.getY()) * 0.5);
