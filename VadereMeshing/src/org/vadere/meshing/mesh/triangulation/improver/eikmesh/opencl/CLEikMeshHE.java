...
public class CLEikMeshHE
    implements IMeshImprover<AVertex, AHalfEdge, AFace>, ITriangulator<AVertex, AHalfEdge, AFace> {
  ...
  public void initialize() throws OpenCLException {
    ...
    // UniformRefinementTriangulator uniformRefinementTriangulation = new
    ...
    GenUniformRefinementTriangulatorSFC<AVertex, AHalfEdge, AFace> uniformRefinementTriangulation =
        new GenUniformRefinementTriangulatorSFC(
            meshSupplier, bound, obstacleShapes, edgeLengthFunc, distanceFunc);
    ...
    for (AFace face : faces) {
      if (faceToQuality(face) < Parameters.MIN_TRIANGLE_QUALITY) {
        ...
        if (optEdge.isPresent()
            && !getMesh().isBoundary(getMesh().getTwin(getMesh().getNext(optEdge.get())))) {
  ...
  public boolean isMovementIllegal() {
    for (AFace face : getMesh().getFaces()) {
      if (!getMesh().isBoundary(face)
          && !getMesh().isDestroyed(face)
          && !triangulation.isCCW(face)) {
        return true;
      }
    }
    return false;
