...
public class Distmesh {
  ...
  public void reTriangulate(boolean force) {
    if (force || firstStep || maxMovementLen / initialEdgeLen > Parameters.TOL) {
      ...
      Collection<Coordinate> coords =
          points.stream().map(p -> new Coordinate(p.getX(), p.getY())).collect(Collectors.toList());
      fact = new GeometryFactory();
      builder = new DelaunayTriangulationBuilder();

      synchronized (this) {
        builder.setSites(coords);
        com.vividsolutions.jts.geom.GeometryCollection multiTris =
            (com.vividsolutions.jts.geom.GeometryCollection) builder.getTriangles(fact);
      ...
      for (MPoint point : points) {
        if (!point.equals(p1) && !point.equals(p2) && !point.equals(p3)) {
          if (GeometryUtils.isInsideCircle(
              p1, p2, p3, point.getX(), point.getY(), GeometryUtils.DOUBLE_EPS)) {
            count++;
            break;
          }
        }
  ...
  public void step() {
    ...
    lineStream.forEach(
        line -> {
          ...
          double desiredLen =
              relativeDesiredEdgeLengthFunc.apply(line.midPoint())
                  * Parameters.FSCALE
                  * scalingFactor;
          ...
          IPoint forceDirection =
              new VPoint(line.getP1()).subtract(new VPoint(line.getP2())).norm();
          VPoint directedForce = new VPoint(forceDirection.scalarMultiply(lenDiff));

          // VPoint directedForce = new VPoint((lenDiff / len) * (line.getX1() - line.getX2()),
          // (lenDiff / len) * (line.getY1() - line.getY2()));
        ...
        point -> {
          if (!point.isFixPoint()) {
            ...
            if (distance > 0) {
              ...
              // point.subtract(new VPoint(initialEdgeLen * distance * dGradPX / dGrad2,
              // initialEdgeLen * distance * dGradPY / dGrad2));
            } else {
  ...
  private Set<EikMeshPoint> generatePoints() {
    ...
    Function<IPoint, Double> pointDensityFunc =
        vertex ->
            1
                / (relativeDesiredEdgeLengthFunc.apply(vertex)
                    * relativeDesiredEdgeLengthFunc.apply(vertex));
     ...
     */
    Set<EikMeshPoint> generatedPoints =
        gridPoints
            .stream()
  ...
  private Set<EikMeshPoint> generateGridPoints() {
    ...
    int elementsInRow =
        (int) Math.ceil((regionBoundingBox.getHeight()) / (initialEdgeLen * Math.sqrt(3) / 2));
    ...
    for (int j = 0; j < elementsInRow; j++) {
      for (int i = 0; i < elementsInCol; i++) {
        ...
        if (j != 0 && j % 2 != 0) {
          point =
              new EikMeshPoint(
                  startX + i * initialEdgeLen + initialEdgeLen / 2,
                  startY + j * initialEdgeLen * sqrt3 / 2,
                  false);
        } else {
          point =
              new EikMeshPoint(
                  startX + i * initialEdgeLen, startY + j * initialEdgeLen * sqrt3 / 2, false);
        }
    ...
    for (VShape shape : obstacles) {
      pointSet.addAll(
          shape
              .getPath()
              .stream()
              .map(p -> new EikMeshPoint(p.getX(), p.getY(), true))
  ...
  private List<EikMeshPoint> divLine(double x1, double y1, double x2, double y2, int segments) {
    ConstantLineIterator lineIterator =
        new ConstantLineIterator(new VLine(x1, y1, x2, y2), initialEdgeLen);
  ...
  private VTriangle tripleToTriangle(
      final Triple<EikMeshPoint, EikMeshPoint, EikMeshPoint> triple) {
    return new VTriangle(
        triple.getLeft().toVPoint(), triple.getMiddle().toVPoint(), triple.getRight().toVPoint());
  }
    ...
    synchronized (this) {
      return triangleT
          ...
          .map(
              triple ->
                  new VTriangle(
                      new VPoint(triple.getLeft()),
                      new VPoint(triple.getMiddle()),
                      new VPoint(triple.getRight())))
          .collect(Collectors.toList());
    }
  }
}
