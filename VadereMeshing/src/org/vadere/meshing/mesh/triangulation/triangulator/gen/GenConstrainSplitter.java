...
public class GenConstrainSplitter<V extends IVertex, E extends IHalfEdge, F extends IFace>
    implements ITriangulator<V, E, F> {

  private static final Logger logger = Logger.getLogger(GenConstrainSplitter.class);
  ...
  private void enforceConstrain(@NotNull final Pair<V, V> constrain) {
    ...
    for (E edge : intersectingEdges) {
      ...
      if (GeometryUtils.intersectLineSegment(
        ...
        // the edge is co-linear that means we add the complete edge to be part of the constrains or
        // one vertex is very close to (v1--v2).
