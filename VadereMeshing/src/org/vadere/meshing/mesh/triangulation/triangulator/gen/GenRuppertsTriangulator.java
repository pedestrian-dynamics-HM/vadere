...
/**
 * Ruperts-Algorithm: not jet finished: Slow implementation!
 *
 ...
 */
public class GenRuppertsTriangulator<V extends IVertex, E extends IHalfEdge, F extends IFace>
    implements ITriangulator<V, E, F>, ITriEventListener<V, E, F> {
  ...
  /**
   * True if Ruppert's algorithm has been initialized i.e. the constrained Delaunay triangulation is
  ...
  /**
   * The angle3D which guarantees that Ruppert's algorithm terminates. If the {@link
   * GenRuppertsTriangulator#minAngle} is smaller the algorithm might not terminate.
   */
  public static double MIN_ANGLE_TO_TERMINATE = 20.6;

  /**
   * If true, all triangles inside holes and outside the segment-bound generated during the
   * construction will be removed before termination.
   */
  ...
  public GenRuppertsTriangulator(
      @NotNull final Supplier<IMesh<V, E, F>> meshSupplier, @NotNull final PSLG pslg) {
  ...
  public void refineSimplex2D() {
    ...
    // split the next skinny triangle at its circumcenter TODO: order by quality ie worst triangle
    // first!
    if (!badTriangles.isEmpty() || !largeTriangles.isEmpty()) {
      ...
      if ((handleBad && isBad(face)) || (!handleBad && isLarge(face))) {
        ...
        assert getMesh().toTriangle(face).midPoint().distance(triangle.midPoint())
  ...
  private void findEncrocedSegments(@NotNull final VPoint circumCenter) {
    segments
        .stream()
        .filter(e -> isEncroached(e, circumCenter))
        .forEach(e -> encroachedSegements.add(e));
    // segments.stream().filter(e -> isEncroached(e, circumCenter)).forEach(e ->
    // encroachedSegements.add(e));
  }
  ...
  private void removeOutsideTriangles() {
    ...
    for (VPolygon hole : pslgBound.getHoles()) {
      ...
      Optional<F> optFace =
          getMesh().streamFaces(f -> !getMesh().isHole(f)).filter(mergeCondition).findAny();
    ...
    if (pslgBound.getSegmentBound() != null) {
      Predicate<F> mergeCondition =
          f -> !pslgBound.getSegmentBound().contains(getMesh().toTriangle(f).midPoint());
  ...
  private void markOutsideTriangles() {
    for (VPolygon hole : pslgBound.getHoles()) {
      Predicate<F> markCondition =
          f -> !isMarked(f) && hole.contains(getMesh().toTriangle(f).midPoint());
      Optional<F> optFace =
          getMesh().streamFaces(f -> !getMesh().isHole(f)).filter(markCondition).findAny();
      if (optFace.isPresent()) {
  ...
  public void step() {
    if (!initialized) {
      ...
      triangulation.setCanIllegalPredicate(
          edge -> !segments.contains(edge) && !segments.contains(getMesh().getTwin(edge)));
      ...
      getMesh()
          .streamFaces()
          .filter(f -> isLarge(f) && !isBad(f))
          .forEach(f -> addLargeTriangle(f));
  ...
  private void refineSimplex1D() {
    ...
    // segments.stream().filter(e -> isEncroachedExpensive(e)).forEach(e ->
    // encroachedSegements.addFirst(e));
    deEncrocheSgements();
  }
  ...
  private Pair<E, E> split(@NotNull final E segment) {
    ...
    if (mark) {
      // we have to mark the correct face, this depends on the call above i.e. V v1 =
      // getMesh().getVertex(splitSegment);
  ...
  private boolean isEncroached(@NotNull final E segment, @NotNull final VPoint p) {
    ...
    return p.distance(line.getVPoint1()) > GeometryUtils.DOUBLE_EPS
        && p.distance(line.getVPoint2()) > GeometryUtils.DOUBLE_EPS
        && diameterCircle.contains(p);
  }
  ...
  private boolean isEncroachedExpensive(@NotNull final E segment) {
    ...
    return getMesh()
        .streamPoints()
        .anyMatch(p -> isEncroached(segment, new VPoint(p.getX(), p.getY())));
  }
