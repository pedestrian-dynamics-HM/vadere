 ...
 */
@Deprecated
public class UniformSFCTriangulator<V extends IVertex, E extends IHalfEdge, F extends IFace>
  ...
  private void removeTrianglesInsideObstacles() {
    ...
    for (F face : faces) {
      if (!triangulation.getMesh().isDestroyed(face)
          && distFunc.apply(triangulation.getMesh().toTriangle(face).midPoint()) > 0) {
        triangulation.removeFaceAtBorder(face, true);
      }
    }
  }
  ...
  private boolean isCompleted(E edge) {
    ...
    return (line.length() <= lenFunc.apply(line.midPoint()) && random.nextDouble() < 0.96)
        || (!triangle.intersectsRectangleLine(bbox)
            && (mesh.isBoundary(twin) || !mesh.toTriangle(twin).intersectsRectangleLine(bbox)))
        || boundary
            .stream()
            .anyMatch(
                shape ->
                    shape.contains(triangle.getBounds2D())
                        || (!mesh.isBoundary(twin)
                            && shape.contains(mesh.toTriangle(twin).getBounds2D())));
  }
