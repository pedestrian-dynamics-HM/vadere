...
/**
 * Triangulation creator: This class is implements an algorithm which refines a given triangulation
 * (which might be empty), by recursively splitting existing triangles (starting with the super
 * triangle if the triangulation is empty) into parts. More precisely, each tringle which should be
 * split will be split at its longest edge in one iteration. Which generates a new set of triangles.
 * The process is repeated until there is no more triangle which has to be split left.
 *
 ...
 */
public class GenUniformRefinementTriangulator<
   ...
   * Default constructor.
   *
   * @param supplier a supplier to construct a completely fresh and empty triangulation {@link
  ...
  /**
   * Initializes this triangulator. This has to be called before {@link
   * GenUniformRefinementTriangulator#step()} can be called.
   */
  public void init() {
    triangulation.init();
  ...
  /** Implements one split-iteration. */
  public void step() {
    synchronized (mesh) {
      if (!initialized) {
        init();
  ...
  /**
   * Returns true if there is no more triangle which has to be split.
   *
   ...
   *
   * <ol>
   *   <li>{@link GenUniformRefinementTriangulator#init()}
   *   <li>{@link GenUniformRefinementTriangulator#step()} until {@link
   *       GenUniformRefinementTriangulator#isFinished()}
   *   <li>{@link GenUniformRefinementTriangulator#finish()}
   * </ol>
   *
  ...
  /** Removes useless and unwanted triangles. */
  public void finish() {
    synchronized (mesh) {
  ...
  private void removeTrianglesOutsideBBox() {
    triangulation.shrinkBorder(
        f -> distFunc.apply(triangulation.getMesh().toTriangle(f).midPoint()) > 0, true);
  }
    ...
    for (F face : faces) {
      if (!triangulation.getMesh().isDestroyed(face) && !triangulation.getMesh().isHole(face)) {
        triangulation.createHole(
            face, f -> distFunc.apply(triangulation.getMesh().toTriangle(f).midPoint()) > 0, true);
      }
    }
  }
