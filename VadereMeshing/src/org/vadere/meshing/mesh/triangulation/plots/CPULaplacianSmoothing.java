...
public class CPULaplacianSmoothing extends JFrame {
  ...
  private CPULaplacianSmoothing() {
    ...
    // IDistanceFunction distanceFunc1 = p -> 2 - Math.sqrt((p.getX()-1) * (p.getX()-1) + p.getY() *
    // p.getY());
    // IDistanceFunction distanceFunc3 = p -> 2 - Math.sqrt((p.getX()-5) * (p.getX()-5) + p.getY() *
    // p.getY());
    // IDistanceFunction distanceFunc = p -> -10+Math.sqrt(p.getX() * p.getX() + p.getY() *
    // p.getY());
    ...
    // IDistanceFunction distanceFunc4 = p -> Math.max(Math.abs(p.getY()) - 4, Math.abs(p.getX()) -
    // 25);
    ...
    // Math.abs(distanceFunc.apply(p)));
    // IEdgeLengthFunction edgeLengthFunc = p -> 1.0 + Math.abs(distanceFunc.apply(p)*0.5);

    // IDistanceFunction distanceFunc = p -> Math.max(Math.max(Math.max(distanceFunc1.apply(p),
    // distanceFunc2.apply(p)), distanceFunc3.apply(p)), distanceFunc4.apply(p));
    ...
    LaplacianSmother meshGenerator =
        new LaplacianSmother(distanceFunc, edgeLengthFunc, 0.5, bbox, new ArrayList<>());

    Predicate<PFace> predicate =
        face -> meshGenerator.getTriangulation().getMesh().toTriangle(face).isNonAcute();
    MeshPanel<PVertex, PHalfEdge, PFace> distmeshPanel =
        new MeshPanel(meshGenerator.getMesh(), predicate, 1000, 800);
    ...
    while (counter <= 2000) {
      // obscuteTriangles = meshGenerator.getTriangles().stream().filter(tri ->
      // tri.isNonAcute()).count();
      // PriorityQueue<PFace<MeshPoint>> priorityQueue = meshGenerator.getQuailties();
      // avgQuality = priorityQueue.stream().reduce(0.0, (aDouble, meshPointPFace) -> aDouble +
      // meshGenerator.faceToQuality(meshPointPFace), (d1, d2) -> d1 + d2) / priorityQueue.size();
    ...
    // System.out.print(TexGraphGenerator.meshToGraph(meshGenerator.getMesh()));
    // System.out.print("finished:" + meshGenerator.getMesh().getVertices().stream().filter(v ->
    // !meshGenerator.getMesh().isDestroyed(v)).count());
