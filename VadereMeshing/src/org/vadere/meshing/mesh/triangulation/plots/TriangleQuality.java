...
public class TriangleQuality {
  ...
  private static void stepUniformRing(double startLen) {
    ...
    IDistanceFunction distanceFunc =
        p -> Math.abs(7 - Math.sqrt(p.getX() * p.getX() + p.getY() * p.getY())) - 3;
    ...
    System.out.println(
        "#steps: ["
            + steps.stream().map(n -> n + "").reduce("", (s1, s2) -> s1 + "," + s2).substring(1)
            + "]");
    System.out.println(
        "quality: ["
            + quality.stream().map(n -> n + "").reduce("", (s1, s2) -> s1 + "," + s2).substring(1)
            + "]");
  }
    ...
    System.out.println(
        "quality: ["
            + meshGenerator
                ...
                .getFaces()
                .stream()
                .map(f -> meshGenerator.faceToQuality(f))
                .map(n -> n + "")
                .reduce("", (s1, s2) -> s1 + "," + s2)
                .substring(1)
            + "]");
    System.out.println(
        "#none acute triangles: "
            + meshGenerator
                .getMesh()
                .getFaces()
                .stream()
                .map(f -> meshGenerator.getMesh().toTriangle(f))
                .filter(t -> t.isNonAcute())
                .count());
  }
  ...
  private static void resultAdaptiveRing(double startLen) {
    ...
    IDistanceFunction distanceFunc =
        p -> Math.abs(7 - Math.sqrt(p.getX() * p.getX() + p.getY() * p.getY())) - 3;
    List<VShape> obstacles = new ArrayList<>();
    IEdgeLengthFunction edgeLengthFunc =
        p -> initialEdgeLength + Math.abs(distanceFunc.apply(p)) * 0.5;
