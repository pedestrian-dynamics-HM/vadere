...
public class VisualTestCPU {
  ...
  private static void overallUniformRingA() {
    ...
    // IDistanceFunction distanceFunc = IDistanceFunction.intersect(p -> Math.sqrt(p.getX() *
    // p.getX() + p.getY() * p.getY()) - 10, IDistanceFunction.create(bbox, hex));
    List<VShape> obstacles = new ArrayList<>();

    GenEikMesh meshGenerator =
        new GenEikMesh(
            distanceFunc,
            p -> 1.0 + (distanceFunc.apply(p) * distanceFunc.apply(p) / 6.0),
            initialEdgeLength,
            bbox,
            new ArrayList<>(),
            supplier);
    ...
    MeshPanel<AVertex, AHalfEdge, AFace> distmeshPanel =
        new MeshPanel<>(meshGenerator.getMesh(), f -> false, 1000, 800, colorFunction);
