...
public class DistMeshTests extends JFrame {
  ...
  private static void testVisual() {
    ...
    // IDistanceFunction distanceFunc = p -> Math.sqrt(p.getX() * p.getX() + p.getY() * p.getY()) -
    // 1.0;
    // IDistanceFunction distanceFunc = p -> Math.abs(0.7 - Math.sqrt(p.getX() * p.getX() + p.getY()
    // * p.getY())) - 0.3;
    ...
    Distmesh meshGenerator =
        new Distmesh(distanceFunc, p -> 1.0, initialEdgeLength, bbox, obstacles);
