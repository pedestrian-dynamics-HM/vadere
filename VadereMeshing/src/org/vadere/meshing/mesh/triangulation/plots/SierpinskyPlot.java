...
public class SierpinskyPlot {
  ...
  private static void uniformCircle(final double initialEdgeLength) {
    ...
    IEdgeLengthFunction edgeLengthFunc =
        p -> 1.0 + (Math.abs(distanceFunc.apply(p)) * Math.abs(distanceFunc.apply(p)));
    ...
    GenUniformRefinementTriangulatorSFC<AVertex, AHalfEdge, AFace> uniformRefinementTriangulation =
        new GenUniformRefinementTriangulatorSFC(
            supplier, bbox, obstacles, edgeLengthFunc, distanceFunc);

    IIncrementalTriangulation<AVertex, AHalfEdge, AFace> triangulation =
        uniformRefinementTriangulation.init();
    MeshPanel<AVertex, AHalfEdge, AFace> panel =
        new MeshPanel<>(triangulation.getMesh(), f -> false, 1000, 800);
