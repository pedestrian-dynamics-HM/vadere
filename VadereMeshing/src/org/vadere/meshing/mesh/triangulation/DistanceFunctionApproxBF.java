...
public class DistanceFunctionApproxBF<V extends IVertex, E extends IHalfEdge, F extends IFace>
    implements IDistanceFunctionCached {
  ...
  public DistanceFunctionApproxBF(
      @NotNull final IMesh<V, E, F> mesh, @NotNull final IDistanceFunction exactDistanceFunc) {
  ...
  public DistanceFunctionApproxBF(
    ...
    var ruppertsTriangulator =
        new GenRuppertsTriangulator<V, E, F>(
            meshSupplier, boundedPSLG, 10, circumRadiusFunc, false, false);
  ...
  public void printPython() {
    System.out.println(
        triangulation
            .getMesh()
            .toPythonTriangulation(v -> triangulation.getMesh().getDoubleData(v, propName)));
  ...
  private double apply(@NotNull final IPoint p, @NotNull final F face) {
    ...
    if (mesh.isBoundary(face)) {
      ...
      return InterpolationUtil.barycentricInterpolation(
          x1, y1, val1, x2, y2, val2, x3, y3, val3, totalArea, p.getX(), p.getY());
    }
  }
