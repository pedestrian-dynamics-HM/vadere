...
public class MeshRenderer<V extends IVertex, E extends IHalfEdge, F extends IFace> {
  ...
  /**
   * A {@link Collection} of {@link F} from the mesh. Ths collection exist to avoid the {@link
   ...
   * @param alertPred a {@link Predicate} of {@link F} which marks a face to be drawn in a special
   *     way.
   ...
   *     way.
   */
  public MeshRenderer(@NotNull final IMesh<V, E, F> mesh, @NotNull final Predicate<F> alertPred) {
   ...
   * @param mesh the mesh which will be rendered
   */
  public MeshRenderer(@NotNull final IMesh<V, E, F> mesh) {
    this(mesh, f -> false, null);
  }

  public MeshRenderer(@NotNull final IMesh<V, E, F> mesh, boolean renderFaces) {
    this(mesh, f -> false, null);
  ...
  public void render(
      @NotNull final Graphics2D targetGraphics2D, final int width, final int height) {
    render(targetGraphics2D, 0, 0, width, height);
  }

  public void render(
      @NotNull final Graphics2D targetGraphics2D,
      final int x,
      final int y,
      final int width,
      final int height) {
  ...
  private void renderGraphics(
      @NotNull final Graphics2D graphics, final int width, final int height) {
  ...
  public void renderPostTransform(@NotNull final Graphics2D graphics, VRectangle bound) {
    ...
    synchronized (mesh) {
      ...
      minEdgeLen =
          (float) edges.stream().mapToDouble(e -> mesh.toLine(e).length()).min().orElse(0.0);
    }
    // graphics.translate(-bound.getMinX()+(0.5*Math.max(0, bound.getWidth()-bound.getHeight())),
    ...
    for (V vertex : vertices) {
      ...
      graphics.fill(
          new Ellipse2D.Double(
              vertex.getX() - ptdiameter / 2,
              vertex.getY() - ptdiameter / 2,
              ptdiameter,
              ptdiameter));
    }
  ...
  private void renderGraphics(
      @NotNull final Graphics2D graphics, final int width, final int height, VRectangle bound) {
