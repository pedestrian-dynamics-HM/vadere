 ...
 *
 * The {@link DelaunayHierarchyPointLocator} is also a {@link ITriEventListener}. It listens to
 ...
 */
public class DelaunayHierarchyPointLocator<V extends IVertex, E extends IHalfEdge, F extends IFace, Mesh extends ITriangleMesh<V, E, F>> implements ITriangleMeshPointLocator<V, E, F> {
	private static Logger log = Logger.getLogger(DelaunayHierarchyPointLocator.class);
	 ...
	 */
	public DelaunayHierarchyPointLocator(
	...
	private F getStartFace(@NotNull final IPoint endPoint, @NotNull final IIncrementalTriangulation<V, E, F> triangulation) {
		List<V> vertices = triangulation.getMesh().vertices().getAll();
		int n = vertices.size();

		if(n < 20) {
			return triangulation.getMesh().faces().getFirst();
		}
		else {
            return takeRandomStartVertexThatIsClosest(endPoint, triangulation, n, vertices);
        }
	}

    private F takeRandomStartVertexThatIsClosest(@NotNull IPoint endPoint, @NotNull IIncrementalTriangulation<V, E, F> triangulation, int n, List<V> vertices) {
        V result = null;
        ...
        if(result == null) {
            ...
            return triangulation.getMesh().faces().getOf(result);
        }
    }
    ...
    public void postInsertEvent(@NotNull final V vertex) {
        IPoint p = base.getMeshBuilder().getMesh().vertices().toMutablePoint(vertex);
        V prev = vertex;
        ...
        for(int i = 1; i <= vertexLevel; i++) {
            ...
            if(locatedFaces.hasNext()) {
                ...
                E inserted = tri.insert(p, f);
                v = tri.getMesh().vertices().getEndOf(inserted);
            }
            else {
                v = tri.getMesh().vertices().getEndOf(tri.insert(p));
            }
    ...
    private void setDown(V src, V dest, int srcLevel) {
        getLevel(srcLevel).getMeshBuilder().getMesh().setDown(src, dest);
    }
    ...
    private V getDown(V src, int srcLevel) {
        ...
        return getLevel(srcLevel-1).getMeshBuilder().getMesh().getDown(src);
    }
	...
	private LinkedList<F> locateAll(final IPoint point) {
        ...
        while (level > 1 && getLevel(level).getMeshBuilder().getMesh().vertices().count() < minSize) {
            level--;
        }
        ...
        while (level >= 1) {
            ...
            if(v == null) {
                if(level == 1) {
                    face = tri.getMesh().readConnectivity().straightWalk2D(point.getX(), point.getY(), getStartFace(point, tri));
                }
                else {
                    face = tri.getMesh().readConnectivity().locateMarch(point.getX(), point.getY(), getStartFace(point, tri)).get();
                }

                v = tri.getMesh().vertices().closestOfFaceTo(face, point.getX(), point.getY());
                ...
                //TODO: SE-Architecture dirty here!
                if(level == 1) {
                    face = tri.getMesh().readConnectivity().straightWalk2D(point.getX(), point.getY(), tri.getMesh().faces().getOf(v));
                }
                else {
                    face = tri.getMesh().readConnectivity().locateMarch(point.getX(), point.getY(), tri.getMesh().faces().getOf(v)).get();
        ...
        assert getLevel(1).getMeshBuilder().getMesh().readConnectivity().faceContains(point.getX(), point.getY(), faces.peekFirst());
    ...
    private boolean isOnEdge(@NotNull final IIncrementalTriangulation<V, E, F> tri, @NotNull F face, @NotNull IPoint point, final double tolerance) {
		E edge = tri.getMesh().edges().closestOfFaceTo(face, point.getX(), point.getY());
	    IPoint p1 = tri.getMesh().edges().getMutableEndPoint(tri.getMesh().edges().getPrev(edge));
	    IPoint p2 = tri.getMesh().edges().getMutableEndPoint(edge);
    ...
    public ITriangleMeshPointLocator<V, E, F> withCache() {
        return new CachedPointLocator<>(this, base.getMesh());
    }
    ...
    public ITriangleMeshPointLocator<V, E, F> copyFor(ITriangleMesh<V, E, F> mesh) {
        return new DelaunayHierarchyPointLocator<>(base.copy(), triangulationSupplier);
    }
    ...
    public String toString() {
        ...
        for(int i = 0; i < hierarchySets.size(); i++) {
            builder.append("["+i+"]:" + hierarchySets.get(i).getMesh().vertices().count()+"\n");
        }
    ...
    public V getNearestPoint(final IIncrementalTriangulation<V, E, F> triangulation, final F face, final IPoint point) {
        ...
        return mesh.edges().streamEdgesOf(face).map(edge -> mesh.vertices().getEndOf(edge)).reduce((p1, p2) -> p1.distance(point) > p2.distance(point) ? p2 : p1).get();
	}
	...
	public String print() {
		...
		for(int i = 0; i < hierarchySets.size(); i++) {
			builder.append("T_["+i+"]:" + hierarchySets.get(i).getMesh().vertices().count()+"\n");
