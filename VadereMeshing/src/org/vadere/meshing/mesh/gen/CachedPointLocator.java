...
public class CachedPointLocator<V extends IVertex, E extends IHalfEdge, F extends IFace>
  ...
  public CachedPointLocator(
      @NotNull final IPointLocator<V, E, F> pointLocator,
      @NotNull final ITriConnectivity<V, E, F> triConnectivity) {
  ...
  public F locatePoint(@NotNull final IPoint point, @NotNull final Object caller) {
    ...
    if (cache.containsKey(caller) && !triConnectivity.getMesh().isDestroyed(cache.get(caller))) {
      face =
          triConnectivity.locateMarch(point.getX(), point.getY(), cache.get(caller)).orElse(null);
    } else {
  ...
  public Optional<F> locate(double x, double y, Object caller) {
    ...
    if (optFace.isPresent()
        && !(contains && optFace.get().equals(starFace))
