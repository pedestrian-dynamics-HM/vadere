...
public class CLGatherer {
	...
	public static IntBuffer getHalfEdges(@NotNull final AMesh mesh) {
        Collection<AHalfEdge> edges = mesh.edges().getAll();
        ...
        for(AHalfEdge edge : edges) {
            ...
            edgeBuffer.put(index+3, edge.getFaceId());
            /*if(edge.getFace()==-1){
    ...
    public static void scatterFaces(@NotNull final IMesh<AVertex, AHalfEdge, AFace> mesh, @NotNull final IntBuffer faceBuffer) {
        Collection<AFace> faces = mesh.faces().getAll();
    ...
    public static IntBuffer getFaces(@NotNull final AMesh mesh) {
        Collection<AFace> faces = mesh.faces().items;;
	...
	public static IntBuffer getBorderVertices(@NotNull final AMesh mesh) {
        ...
        for(AVertex vertex : vertices) {
            assert vertex.getId() == index;
            vertexBuffer.put(index, mesh.vertices().isAtBoundary(vertex) ? 1 : 0);
            index++;
        }
        return vertexBuffer;
    }
    ...
    public static IntBuffer getEdges(@NotNull final AMesh mesh) {
        ...
        int index = 0;
        for(AHalfEdge edge : edges) {
            edgeBuffer.put(index, mesh.edges().getPrev(edge).getEnd());
            index++;
            edgeBuffer.put(index, edge.getEnd());
            index++;
            if(mesh.faces().isBoundary(mesh.faces().getOf(edge))) {
                edgeBuffer.put(index, -1);
                index++;
                edgeBuffer.put(index, mesh.faces().getOf(mesh.edges().getTwin(edge)).getId());
                index++;
                if(mesh.faces().isBoundary(mesh.faces().getOf(mesh.edges().getTwin(edge)))) {
                    throw new IllegalArgumentException("invalid mesh!");
                }
            }
            else if(mesh.faces().isBoundary(mesh.faces().getOf(mesh.edges().getTwin(edge)))) {
                edgeBuffer.put(index, mesh.faces().getOf(edge).getId());
                index++;
                edgeBuffer.put(index, -1);
                index++;
                if(mesh.faces().isBoundary(mesh.faces().getOf(edge))) {
	...
	public static IntBuffer getTwins(@NotNull final AMesh mesh) {
        ...
        int index = 0;
        for(AHalfEdge edge : edges) {
            edgeBuffer.put(index, mesh.edges().getTwin(edge).getId());
            index++;
        }
        return edgeBuffer;
    }
	...
	public static IntBuffer getTriangles(@NotNull final AMesh mesh) {
        ...
        int index = 0;
        for(AFace face : faces) {
            AHalfEdge edge = mesh.edges().getAnyOf(face);
