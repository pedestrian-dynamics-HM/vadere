 ...
 *     <p>The Delaunay-Hierarchy is a data structure which accelerates the point location problem
 *     i.e. given a point p=(x,y) find the triangle which contains p. Let P be the set of all points
 *     of the triangulation. The Delaunay-Hierarchy is a hierachy of Delaunay-Triangulations T_0,
 *     T_1, ..., T_k of point sets P_0, P_1, ..., P_k such that P_0 = P, P_k is a subset of P_{k-1}.
 *     <p>To find the triangle t which contains p the algorithm starts at the hierarchy k and finds
 *     the triangle t_k which contains p (by starting from some t_k' triangle of T_k and walking
 *     towards t_k). By using some point p_k of t_k the next search (walk) starts from triangle
 *     t_{k-1}' which has p_k as its point towards t_{k-1}. This will be repeated until the hierachy
 *     0 and t is reached.
 *     <p>For more informations see devillers-2002 (The Delaunay Hierarchy).
 *     <p>The {@link DelaunayHierarchy} is also a {@link ITriEventListener}. It listens to vertex
 *     insertVertex events to update itself.
 *     <p>Note that any insertion / deletion of a point into / from the triangulation has to be
 *     propagated to its Delaunay-Hierarchy.
 *     <p>In the current state the Delaunay-Hierarchy does only support triangulations without
 *     holes.
 * @param <V> the type of the vertices
 * @param <E> the type of the half-edges
 * @param <F> the type of the faces
 */
public class DelaunayHierarchy<V extends IVertex, E extends IHalfEdge, F extends IFace>
    implements IPointLocator<V, E, F> {
  ...
  /**
   * T_1, which contains all the points of T_0 but no holes. Therefore we can walk through T_1 to
   * find the face of T_0.
   */
  ...
  /**
   * Default constructor of the Delaunay-Hierarchy. The base might be a triangulation which already
   * contains points.
   *
   * @param base T_0
  ...
  /**
   * Initializes the Delaunay-Hierarchy by connecting all virtual vertices of the triangulations
   * (connecting T_k -> T_{k-1}). And inserting all vertices of the base triangulation which might
   * already contain some points.
   */
  private void init() {
    base.init();
  ...
  private F getStartFace(
      @NotNull final IPoint endPoint,
  ...
  private V getDown(V src, int srcLevel) {
    // srcLevel-1 since the resulting vertex is contained in the mesh one level below the src
    // vertex!
  ...
  /**
   * Locates for some level k >= 1 all triangles t_k, t_{k-1}, ..., t_0 which are part of T_k,
   * T_{k-1}, ..., T_k. The location of some t_j uses the result of locating t_{j+1}.
   *
   * @param point the point we want to locate.
   * @return returns a list of faces [t_k, t_{k-1}, ..., t_0] for which p is contained in any of
   *     these faces. Only t_0 might be not a triangle.
   */
  ...
  private boolean isOnEdge(
      @NotNull final IIncrementalTriangulation<V, E, F> tri,
      @NotNull F face,
   ...
   */
  public V getNearestPoint(
      final IIncrementalTriangulation<V, E, F> triangulation, final F face, final IPoint point) {
    IMesh<V, E, F> mesh = triangulation.getMesh();
    return mesh.streamEdges(face)
        .map(edge -> mesh.getVertex(edge))
        .reduce((p1, p2) -> p1.distance(point) > p2.distance(point) ? p2 : p1)
        .get();
  }
  ...
  public String print() {
    ...
    for (int i = 0; i < hierarchySets.size(); i++) {
      builder.append(
          "T_[" + i + "]:" + hierarchySets.get(i).getMesh().getNumberOfVertices() + "\n");
