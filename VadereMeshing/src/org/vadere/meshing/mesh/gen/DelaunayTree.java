...
/**
 * An implementation of the so called Delaunay Tree which does not suppport deletion of points from
 * the triangulation {@link IIncrementalTriangulation}.
 *
 * <p>The Delaunay Tree see Computational Geometry: Algorithms and Applications (berg-2008) page
 * 191.
 *
 ...
 */
public class DelaunayTree<V extends IVertex, E extends IHalfEdge, F extends IFace>
  ...
  public void postSplitTriangleEvent(F original, F f1, F f2, F f3, V v) {
    ...
    DAG<DAGElement<V, E, F>> newFaceDag1 =
        new DAG<>(
            new DAGElement<V, E, F>(
                face, Triple.of(points1.get(0), points1.get(1), points1.get(2))));
    faceDag.addChild(newFaceDag1);
    map.put(face, newFaceDag1);

    face = f2;
    List<V> points2 = mesh.getVertices(face);
    DAG<DAGElement<V, E, F>> newFaceDag2 =
        new DAG<>(
            new DAGElement<V, E, F>(
                face, Triple.of(points2.get(0), points2.get(1), points2.get(2))));
    faceDag.addChild(newFaceDag2);
    map.put(face, newFaceDag2);

    face = f3;
    List<V> points3 = mesh.getVertices(face);
    DAG<DAGElement<V, E, F>> newFaceDag3 =
        new DAG<>(
            new DAGElement<V, E, F>(
                face, Triple.of(points3.get(0), points3.get(1), points3.get(2))));
  ...
  public void postFlipEdgeEvent(final F f1, final F f2) {
    ...
    DAG<DAGElement<V, E, F>> newf1Dag =
        new DAG<>(
            new DAGElement<V, E, F>(f1, Triple.of(points1.get(0), points1.get(1), points1.get(2))));
    DAG<DAGElement<V, E, F>> newf2Dag =
        new DAG<>(
            new DAGElement<V, E, F>(f2, Triple.of(points2.get(0), points2.get(1), points2.get(2))));
