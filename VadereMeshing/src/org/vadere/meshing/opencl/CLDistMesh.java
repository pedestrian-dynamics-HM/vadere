...
public class CLDistMesh extends CLOperation {
  ...
  private void buildProgram() throws OpenCLException {
    try (MemoryStack stack = stackPush()) {
      ...
      clProgram =
          clCreateProgramWithSource(clContext, clProgramStrings, clProgramLengths, errcode_ret);
      ...
      CLInfo.checkCLError(errcode_ret);
      clGetKernelWorkGroupInfo(
          clKernelForces, clDevice, CL_KERNEL_PREFERRED_WORK_GROUP_SIZE_MULTIPLE, pp, null);
  ...
  private void createMemory() throws OpenCLException {
    try (MemoryStack stack = stackPush()) {
      ...
      CLInfo.checkCLError(errcode_ret);
      clTriangles =
          clCreateBuffer(clContext, CL_MEM_READ_WRITE | CL_MEM_COPY_HOST_PTR, t, errcode_ret);
      CLInfo.checkCLError(errcode_ret);
      clForces =
      ...
      clIsBoundaryVertex =
          clCreateBuffer(
              clContext, CL_MEM_READ_WRITE | CL_MEM_COPY_HOST_PTR, boundaryVertices, errcode_ret);
      CLInfo.checkCLError(errcode_ret);
      clTriLocks =
          clCreateBuffer(
              clContext, CL_MEM_READ_WRITE | CL_MEM_COPY_HOST_PTR, triLocks, errcode_ret);
      CLInfo.checkCLError(errcode_ret);
      ...
      CLInfo.checkCLError(errcode_ret);
      clTwins =
          clCreateBuffer(clContext, CL_MEM_READ_WRITE | CL_MEM_COPY_HOST_PTR, twins, errcode_ret);
      CLInfo.checkCLError(errcode_ret);
    }
  }
  ...
  public boolean step(final boolean flipAll) throws OpenCLException {
    try (MemoryStack stack = stackPush()) {
      ...
      enqueueNDRangeKernel(
          ...
          1,
          null,
          clGloblWorkSizeForces,
          clLocalWorkSizeForces,
          null,
          null);
      // log.info("(default) computed forces");

      // enqueueNDRangeKernel("compute forces", clQueue, clKernelForces, 1, null,
      // clGlobalWorkSizeEdges, null, null, null);
      // log.info("(default) computed forces");
      ...
      // enqueueNDRangeKernel(clQueue, clKernelRemoveTriangles, 1, null, clGlobalWorkSizeEdges,
      // null, null, null);
  ...
  private void fixBorderFace(
      @NotNull final AFace borderFace, @NotNull final Set<AHalfEdge> toRemoveEdges) {
  ...
  private void printResult() {
    ...
    // log.info("scalingFactor:" + (doublePrecision ? scalingFactorD.get(0) :
    // scalingFactorF.get(0)));
  }
  ...
  private void removeFaceAtBorder(@NotNull final AFace face) {
    if (!getMesh().isDestroyed(face)) {
      ...
      if (getMesh().getTwinFace(getMesh().getEdge(boundary)) == face && delEdges.size() == count) {
        ...
        EdgeIterator<AVertex, AHalfEdge, AFace> edgeIterator =
            new EdgeIterator<>(getMesh(), borderEdge);
        ...
        if (getMesh().getTwinFace(borderEdge) == face) {
          borderEdge =
              getMesh()
                  .streamEdges()
                  .filter(e -> getMesh().getTwinFace(e) != face)
                  .filter(e -> getMesh().isBoundary(e))
                  .findAny()
                  .get();
          // throw new IllegalArgumentException("could not adjust border edge! Deletion of " + face
          // + " is not allowed.");
        }
