 ...
 * (https://en.wikipedia.org/wiki/Weiler%E2%80%93Atherton_clipping_algorithm) enables boolean
 * operations on polygons, i.e.:
 *
 * <ul>
 *   <li>INTERSECTION of polygon A (subject) and B (clipper).
 *   <li>UNION of polygon A (subject) and B (clipper).
 *   <li>SUBTRACTION of polygon B (subject) from polygon A (clipper).
 * </ul>
 *
 * Note that holes are not supported, i.e. if UNION produces a polygon with holes this will be
 * filled. If SUBTRACTION produces a hole an error will be thrown since {@link VPolygon} do not
 * support holes. Furthermore, it is assumed that each {@link VPolygon} which is involved is a
 * simple non-self intersecting polygon. Co-linear and duplicated points, i.e. useless points, will
 * be removed.
 *
...
public class WeilerAtherton {
  ...
  /**
   * Constructs two faces which represents two polygons including their intersection points. Each
   * intersection point is connected to his twin (the connection point of the other face which is
   * geometrically the same point).
   *
   ...
   * @return two faces which represents two polygons including their intersection points (in CW
   *     order)
   */
    ...
    subjectMesh
        ...
        .forEach(
            v -> {
              ...
              subjectMesh.setData(
                  v, propNameInside, clipPointSet.contains(v) || clipping.contains(v));
            });

    PFace clippingFace =
        clippingMesh.toFace(clipping.getPath().stream().collect(Collectors.toList()));
    clippingMesh
        .streamVertices()
        .forEach(
            v -> {
              clippingMesh.setData(v, propNameIntersection, false);
              clippingMesh.setData(
                  v, propNameInside, subPointSet.contains(v) || subject.contains(v));
            });
    ...
    while (intersectionFound) {
      ...
      for (PHalfEdge clippingEdge : clippingEdges) {
        for (PHalfEdge subjectEdge : subjectEdges) {
          Optional<VPoint> optIntersectionPoint =
              equalIntersectionPoints(subjectEdge, subjectMesh, clippingEdge, clippingMesh);
          if (!optIntersectionPoint.isPresent()) {
            ...
            if (!(l1.getP1().equals(l2.getP1())
              ...
              if (GeometryUtils.distanceToLineSegment(
                      new VPoint(l1.getP1()), new VPoint(l1.getP2()), new VPoint(l2.getP1()))
                  <= EPSILON) {
                intersectionPoint = new VPoint(l2.getP1());
              } else if (GeometryUtils.distanceToLineSegment(
                      new VPoint(l1.getP1()), new VPoint(l1.getP2()), new VPoint(l2.getP2()))
                  <= EPSILON) {
                intersectionPoint = new VPoint(l2.getP2());
              } else if (GeometryUtils.distanceToLineSegment(
                      new VPoint(l2.getP1()), new VPoint(l2.getP2()), new VPoint(l1.getP1()))
                  <= EPSILON) {
                intersectionPoint = new VPoint(l1.getP1());
              } else if (GeometryUtils.distanceToLineSegment(
                      new VPoint(l2.getP1()), new VPoint(l2.getP2()), new VPoint(l1.getP2()))
                  <= EPSILON) {
                intersectionPoint = new VPoint(l1.getP2());
              } else if (GeometryUtils.intersectLineSegment(
                  l1.getP1().getX(),
                  l1.getP1().getY(),
                  l1.getP2().getX(),
                  l1.getP2().getY(),
  ...
  private PVertex splitEdge(
      @NotNull final PHalfEdge edge, @NotNull final IPoint p, @NotNull final PMesh mesh) {
   ...
   * @return a point which is the intersection point or null / empty if the two edges did not have
   *     an geometrical equal point
   */
  ...
  private List<VPolygon> construct(
    ...
    VPolygon subject =
        GeometryUtils.isCCW(subjectCandidat) ? subjectCandidat : subjectCandidat.revertOrder();
    VPolygon clipping;
    switch (operation) {
      case SUBTRACTION:
        clipping =
            GeometryUtils.isCCW(clippingCandidat)
                ? clippingCandidat.revertOrder()
                : clippingCandidat;
        break;
      case UNION:
      case INTERSECTION:
    ...
    Set<PHalfEdge> subjectExitingEdges =
        subjectMesh
            .streamEdges(subjectFace)
            .filter(
                edge ->
                    subjectMesh
                        .getData(subjectMesh.getVertex(edge), propNameIntersection, Boolean.class)
                        .get())
            .filter(
                edge ->
                    !clipping.contains(subjectMesh.toLine(subjectMesh.getNext(edge)).midPoint()))
    ...
    Set<PHalfEdge> subjectEnteringEdges =
        subjectMesh
            .streamEdges(subjectFace)
            .filter(
                edge ->
                    subjectMesh
                        ...
                        .get())
            .filter(
                edge -> clipping.contains(subjectMesh.toLine(subjectMesh.getNext(edge)).midPoint()))
            .filter(edge -> !clipping.contains(subjectMesh.toLine(edge).midPoint()))
            /*.filter(edge ->
            		contains(clipping, subjectMesh.getPoint(subjectMesh.getNext(edge))) &&
            		!contains(clipping, subjectMesh.getPoint(subjectMesh.getPrev(edge)))
    ...
    switch (operation) {
      case INTERSECTION:
        intersectionSet = subjectEnteringEdges;
        break;
      case UNION:
      case SUBTRACTION:
      default:
        intersectionSet = subjectExitingEdges;
        break;
    }
    ;
    ...
    if (intersectionSet.isEmpty()) {
      boolean subInClip =
          subjectMesh.streamPoints(subjectFace).allMatch(p -> contains(clipping, p));

      boolean clipInSub =
          clippingMesh.streamPoints(clippingFace).allMatch(p -> contains(subject, p));
        ...
        default:
          {
            if (clipInSub) {
              throw new IllegalArgumentException(
                  "subtracting a polygon which is contained in its counterpart will produce a polygon with a hole which is not supported.");
            }
          }
          break;
      }
    ...
    while (!intersectionSet.isEmpty()) {
      ...
      PHalfEdge subjectTwin =
          clippingMesh.getEdge(
              subjectMesh
                  .getData(subjectMesh.getVertex(subjectEdge), propNameTwin, PVertex.class)
                  .get());
      ...
      do {
        ...
        if (mesh.getData(v, propNameIntersection, Boolean.class).get()) {
          ...
          if (!prevTwinPoint.equals(prevPoint)) {
            ...
            next =
                mesh.getEdge(mesh.getData(mesh.getVertex(next), propNameTwin, PVertex.class).get());
          }
        }
  ...
  public List<VPolygon> cap(
      @NotNull final VPolygon subjectCandidat, @NotNull final VPolygon clippingCandidat) {
    return construct(subjectCandidat, clippingCandidat, Operation.INTERSECTION);
  }

  // private PVertex<VPoint, Object, Object> getTwin(@NotNull final PVertex<VPoint, Object, Object>
  // v, PMesh<VPoint, Object, >)
   ...
   * @return a pair of polygon where the second element is null if the polygons got merged. This is
   *     not the case if there do not overlap.
   */
  public List<VPolygon> cup(
      @NotNull final VPolygon subjectCandidat, @NotNull final VPolygon clippingCandidat) {
  ...
  public Optional<VPolygon> subtraction(
    ...
    if (result.size() == 1) {
      ...
      throw new IllegalStateException(
          "subtraction of two polygons should never produce more than one polygon.");
    }
  }
}
