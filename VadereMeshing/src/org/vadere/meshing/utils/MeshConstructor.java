...
public class MeshConstructor {
  ...
  public IMesh<PVertex, PHalfEdge, PFace> pslgToCoarsePMesh(
      @NotNull final PSLG pslg,
      final Function<IPoint, Double> circumRadiusFunc,
      final boolean viszalize) {
    ...
    var ruppertsTriangulator =
        new PRuppertsTriangulator(boundedPSLG, pslg, circumRadiusFunc, 10, false, false);
  ...
  public IMesh<PVertex, PHalfEdge, PFace> pslgToAdaptivePMesh(
    ...
    IEdgeLengthFunction edgeLengthFunction =
        p -> hmin + smoothness * Math.abs((distanceFunctionApproximation).apply(p));
    EdgeLengthFunctionApprox edgeLengthFunctionApprox =
        new EdgeLengthFunctionApprox(pslg, edgeLengthFunction, p -> hmax);
  ...
  public IMesh<PVertex, PHalfEdge, PFace> pslgToUniformPMesh(
      @NotNull final PSLG pslg, final double h0, final boolean viszalize) {
    Collection<VPolygon> holes = pslg.getHoles();
    VPolygon segmentBound = pslg.getSegmentBound();
    IDistanceFunction distanceFunction = IDistanceFunction.create(segmentBound, holes);
    logger.info("construct distance function");
    ...
    // (3) use EikMesh to improve the mesh
    var meshImprover =
        new PEikMesh(distanceFunctionApproximation, p -> h0, h0, pslg.getBoundingBox(), polygons);
  ...
  public IMesh<PVertex, PHalfEdge, PFace> pslgToUniformOptimalPMesh(
      @NotNull final PSLG pslg, final double h0, final boolean viszalize) {
    ...
    EdgeLengthFunctionApprox edgeLengthFunctionApprox =
        new EdgeLengthFunctionApprox(pslg, p -> Double.POSITIVE_INFINITY, p -> h0);
    edgeLengthFunctionApprox.smooth(smoothness);
    logger.info("construct element size function");
