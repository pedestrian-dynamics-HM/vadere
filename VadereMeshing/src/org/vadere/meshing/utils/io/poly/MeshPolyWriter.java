...
/**
 * The {@link MeshPolyWriter} transforms a {@link IMesh} into a ascii-poly {@link String} and writes
 * into a file. It is the counterpart to {@link MeshPolyReader}.
 *
...
public class MeshPolyWriter<V extends IVertex, E extends IHalfEdge, F extends IFace> {
  ...
  private void to2DPoly(
    ...
    for (V v : mesh.getVertices()) {
      ...
      appender.append(
          String.format(
              Locale.US,
              "%d" + SEPARATOR + "%d" + SEPARATOR + "%d" + SEPARATOR + "%f" + SEPARATOR + "%f",
              id,
              boundary,
              target,
              v.getX(),
  ...
  public void to2DPoly(
    ...
    PrintWriter stream = new PrintWriter(new FastBufferedOutputStream(outputStream));
  }
