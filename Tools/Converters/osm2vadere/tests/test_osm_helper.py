...
from osm_helper import Node, OsmLookup, OsmLookupError, Tag, Nd, Way, PathToPolygon, PolyObjectWidthId, OsmData
...
class TestWay(unittest.TestCase):
    ...
    def test_create(self):
        way = Way.create(-3, [Node.create_node(1, 1, 2),
                              Node.create_node(2, 3, 4),
                              Node.create_node(3, 5, 6)],
                            att={'foo': 'bar'}, tags={'baz': 42})
        self.assertEqual(way.id, -3)
        self.assertEqual(way.action, "modify")
        self.assertEqual(way.visible, "true")
        self.assertDictEqual(way.attr, {'foo': 'bar'})
...
class TestPathToPolygon(unittest.TestCase):
    ...
    def test_strait_line(self):
        ...
        self.assertTrue((path.polygon[0] == [0., 0.1]).all())
        self.assertTrue((path.polygon[1] == [2., 0.1]).all())
        self.assertTrue((path.polygon[2] == [3., 0.1]).all())
        self.assertTrue((path.polygon[3] == [3., -0.1]).all())
        self.assertTrue((path.polygon[4] == [2., -0.1]).all())
        self.assertTrue((path.polygon[5] == [0., -0.1]).all())
        self.assertTrue((path.polygon[6] == [0., 0.1]).all())
...
class TestOsmLookup(unittest.TestCase):
    ...
    def test_load(self):
        nodes = [Node.create_node(2, 48.16256981067, 11.5868239213).to_xml(),
    ...
    def test_multiple_utm_zones(self):
        nodes = [Node.create_node(2, 48.16256981067, 11.5868239213).to_xml(),
                 Node.create_node(1, 48.16254537153, 11.58697933476).to_xml(),
                 Node.create_node(-3, 48.16255438099, 33.58692204178).to_xml()]  # wrong zone
        lookup = OsmLookup()
...
class TestOsmData(unittest.TestCase):
    ...
    def setUp(self):
        self.valid001 = OsmData(input_file=os.path.join(os.path.dirname(__file__), "./valid001.osm"))
        self.valid002 = OsmData(input_file=os.path.join(os.path.dirname(__file__), "./valid002.osm"))
        self.valid003 = OsmData(input_file=os.path.join(os.path.dirname(__file__), "./valid003.osm"))
        self.valid004 = OsmData(input_file=os.path.join(os.path.dirname(__file__), "./valid004.osm"))
        self.invalid001 = OsmData(input_file=os.path.join(os.path.dirname(__file__), "./invalid001.osm"))
        self.invalid002 = OsmData(input_file=os.path.join(os.path.dirname(__file__), "./invalid002.osm"), strict=False)
        self.invalid003 = OsmData(input_file=os.path.join(os.path.dirname(__file__), "./invalid003.osm"), strict=False)
        self.invalid004 = OsmData(input_file=os.path.join(os.path.dirname(__file__), "./invalid004.osm"), strict=False)
        self.invalid005 = OsmData(input_file=os.path.join(os.path.dirname(__file__), "./invalid005.osm"), strict=False)
    ...
    def test__xpath_k_v_tags(self):
        self.assertEqual(OsmData._xpath_k_v_tags([("Foo", "Bar"), ("Baz", "Tar")]),
    ...
    def test__add_ignore(self):
        ...
        self.assertEqual(OsmData._add_ignore([("Foo", "Bar"), ("Baz", "Tar")]),
                         " and not (./tag[@k='Foo' and @v='Bar'] and ./tag[@k='Baz' and @v='Tar'])")

    def test__tag_matcher(self):
        self.assertTupleEqual(OsmData._tag_matcher([("Foo", "Bar"), ("Bazz",)], [], truth_operator=("and", "and")),
                              ("./tag[@k='Bazz'] and ./tag[@k='Foo' and @v='Bar']", "")
                              )
        self.assertTupleEqual(OsmData._tag_matcher([("Foo", "Bar"), ("Bazz",)], [], truth_operator=("or", "and")),
        ...
        self.assertTupleEqual(OsmData._tag_matcher([], [("Foo", "Bar"), ("Bazz",)], truth_operator=("and", "or")),
                              ("", "./tag[@k='Bazz'] or ./tag[@k='Foo' and @v='Bar']")
                              )

        self.assertTupleEqual(OsmData._tag_matcher([], [], truth_operator=("and", "or")),
                              ("", ""))

        self.assertTupleEqual(OsmData._tag_matcher([("A", "A"), ("B", "B")], [("C", "C"), ("D", "D")],
    ...
    def test_filter_tag(self):
        ...
        ret = self.valid001.filter_tag(include=[("test",)], exclude=[("rover:id", "-133507")])
        self.assertEqual(len(ret), 1)

        ret = self.valid001.filter_tag(include=[("test",)], exclude=[("rover:id",), ("building", "yes")])
        self.assertEqual(len(ret), 2)
        ...
        ret = self.valid001.filter_tag(exclude=[("rover:id",), ("building", "yes")],
                                       truth_operator=("and", "or"))
        self.assertEqual(len(ret), 0)

        ret = self.valid001.filter_tag(include=[("rover:id",), ("building", "yes")], truth_operator=("and", "and"))
        self.assertEqual(len(ret), 0)

        ret = self.valid001.filter_tag(include=[("rover:id",), ("building", "yes")], truth_operator=("or", "and"))
        self.assertEqual(len(ret), 2)
    ...
    def test_tags_to_dict(self):
        ...
        self.assertDictEqual(OsmData.tags_to_dict(element, "rover:"),
                             {'id': '-133507',
                              'obstacle': 'yes',
    ...
    def test_latlon_lookup(self):
        self.assertEqual(self.valid001.latlon_lookup(1318876746), (48.1608727, 11.5891088))
        self.assertEqual(self.valid001.latlon_lookup(-127425), (48.16255438099, 11.58692204178))
    ...
    def test_element(self):
        ...
        self.assertEqual(element.get("origin"), "CGImap 0.7.5 (31203 thorn-02.openstreetmap.org)")
    ...
    def test_node_add(self):
        ...
        self.assertEqual(self.valid001.node_add((48.16256981067, 11.5868239213)), -127405)
        self.assertEqual(len(self.valid001.nodes), 6)
        ...
        self.assertEqual(self.valid001.lookup.latlon_to_node.get((48.16, 11.59)), node_id)
    ...
    def test_nodes_find_with_tags(self):
        ...
        self.assertEqual(len(self.valid001.nodes_find_with_tags(exc_tag=[('rover:id', )])), 5)
    ...
    def test_nodes_not_used(self):
        self.assertSetEqual(self.valid001.nodes_not_used([-127405, 1318876746, 1324414637]), set())
        self.assertSetEqual(self.invalid001.nodes_not_used([-127433, -127405, 1318876746, 1324414637]), {-127433})

    def test_nodes_to_latlon(self):
        self.assertListEqual(self.valid001.nodes_to_latlon(
            [-127405, 1318876746]),
    ...
    def test_nodes_to_utm(self):
        self.assertListEqual(
            ...
            [(692351.2680216449, 5337605.595388093), (692527.5088385276, 5337422.705304472)]
        )
    ...
    def test_way(self):
        ...
        self.assertEqual(self.valid001.way(-133507, rover_id=True).get('id'), '-999069167')
    ...
    def test_way_node_refs(self):
        self.assertListEqual(self.valid001.way_node_refs(-999069167), [-127405, -127407, -127425, -127405])
    ...
    def test_create_convex_hull(self):
        el = self.valid003.ways_find_with_tags(inc_tag=[("rover:obstacle:hull", ), ("rover:obstacle:ignore", "yes")])
        self.assertEqual(len(el), 0)
        hull_id = self.valid003.create_convex_hull([39057410, 36942797])
        el = self.valid003.ways_find_with_tags(inc_tag=[("rover:obstacle:hull", ), ("rover:obstacle:ignore", "yes")])
        self.assertEqual(len(el), 2)

        self.assertListEqual(self.valid003.way_node_refs(hull_id),
                             [429583851, 429583850, 429583279, 429583282, 429583298, 429583853, 429583852, 429583851])
        self.assertEqual(len(self.valid003.ways_find_with_tags(inc_tag=[
            ("rover:id", str(np.abs(hull_id))), ("rover:obstacle", "convex-hull"), ("rover:obstacle:hull", "-1")
        ])), 1)

    def test_lint_add_ids(self):
        self.assertEqual(len(self.invalid002.xml.xpath("/osm/way[@id < 0]")), 1)
        self.assertEqual(len(self.invalid002.xml.xpath("/osm/way[@id < 0 and not (./tag[@k='rover:id'])]")), 1)

        self.assertEqual(len(self.invalid002.xml.xpath("/osm/node[@id < 0]")), 4)
        self.assertEqual(len(self.invalid002.xml.xpath("/osm/node[@id < 0 and not (./tag[@k='rover:id'])]")), 3)
        ...
        self.assertEqual(len(self.invalid002.xml.xpath("/osm/way[@id < 0]")), 1)
        self.assertEqual(len(self.invalid002.xml.xpath("/osm/way[@id < 0 and not (./tag[@k='rover:id'])]")), 0)

        self.assertEqual(len(self.invalid002.xml.xpath("/osm/node[@id < 0]")), 4)
        self.assertEqual(len(self.invalid002.xml.xpath("/osm/node[@id < 0 and not (./tag[@k='rover:id'])]")), 0)
