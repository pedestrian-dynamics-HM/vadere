...
class FileStorageObserver(RunObserver):
    VERSION = "FileStorageObserver-own"
    ...
    def __init__(
        ...
        if template is not None:
            ...
            template = Path(basedir / "template.html")
            if not template.exists():
                template = None

        self.initialize(basedir, resource_dir, source_dir, template, logger, priority)
    ...
    def _make_dir(self, _id):
        now = datetime.now()
        date = "{}-{}-{}_{}-{}-{}".format(now.year, now.month, now.day, now.hour, now.minute, now.second)
        new_dir = os.path.join(self.basedir, date)
        os.mkdir(new_dir)
    ...
    def started_event(
        ...
        if self.logger is not None:
            self.file_handler = FileHandler(os.path.join(self.dir, 'run.log'))
            self.file_handler.setFormatter(Formatter('%(levelname)s - %(name)s - %(message)s'))
...
@cli_option('-z', '--filestorage')
def filestorage_cli(args, run):
    """Add a file-storage observer to the experiment."""
    if not os.path.exists(args):
        os.makedirs(args)

    run.observers.append(FileStorageObserver(args, logger=run.root_logger, template=pkg_resources.resource_filename('utils.sacred', 'template.txt')))
