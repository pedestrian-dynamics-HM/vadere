...
usage (run from target_learning directory)
    ...
    data input path : path should be relative to the directory target_learning
...
from rf.regression import ingredient as rf_ingredient, multiple_forests as multiple, single_forest as single

from sacred.experiment import Experiment
from utils.sacred.file_storage import FileStorageObserver, filestorage_cli

# create experiment
ex = Experiment('target_learning', ingredients=[density_ingredient, rf_ingredient],
                additional_cli_options=[filestorage_cli])
use_multiple = False
...
def main(_run, _log, _config):
    ...
    # load dataset depending on command line input _config
    path = dataset.get('path', None)  # path should be relative to the directory target_learning
    print(path)
    print(os.getcwd())
    ...
    if path is not None:
        ...
        train_samples, test_samples, train_targets, test_targets = train_test_split(maps, distributions,
    ...
    if use_multiple:
        ...
        errors = multiple(
                        ...
                        prediction_model=_config.get('prediction_model', None))
    else:
        regressors, \
        normed_prediction, \
        (score_training, score_test, score_oob), \
