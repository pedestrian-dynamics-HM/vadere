...
/**
 * CompoundObject implementation based on TraCI as described in
 * https://sumo.dlr.de/docs/TraCI/Protocol.html#atomar_types
 *
 * <p>This implementation consist of two equally long arrays {@link #type} and {@link #data}. The
 * {@link #type} array saves Type of the objects at the same index in {@link #data}.
 *
 * <p>At time of creation it must be stated how many objects will be tide together. See @{@link
...
public class CompoundObject {
  ...
  public Object getData(int index, TraCIDataType type) {
    ...
    if (!this.type[index].equals(type))
      throw new TraCIException(
          "Type mismatch of CompoundObject element %s != %s  at index %d",
  ...
  public boolean equals(Object o) {
    ...
    return Arrays.equals(type, that.type) && Arrays.equals(data, that.data);
  }
  ...
  private class Iter implements Iterator<Pair<TraCIDataType, Object>> {
    ...
    public Pair<TraCIDataType, Object> next() {
      ...
      if (!this.typeAssertionTest.test(p.getLeft())) {
        throw new TraCIException(
            "Type mismatch in CompoundObject. Expected '%s' but found '%s'",
