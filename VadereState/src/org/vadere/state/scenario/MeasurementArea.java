...
public class MeasurementArea extends ScenarioElement<AttributesMeasurementArea> {
  ...
  public boolean isRectangular() {
    ...
    if (shape instanceof VRectangle) return true;
  ...
  public VRectangle asVRectangle() {
    VShape shape = attributes.getShape();
    if (shape instanceof VRectangle) return (VRectangle) shape;
    if (shape instanceof VPolygon) {
  ...
  /**
   * Compare {@link MeasurementArea}s based on their shape. Important {@link VPolygon} != {@link
   * VRectangle} even if all points are the same.
   *
   * @param other
   * @return
   */
