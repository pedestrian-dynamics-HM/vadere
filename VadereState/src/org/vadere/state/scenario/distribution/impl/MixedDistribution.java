...
public class MixedDistribution extends VDistribution<AttributesMixedDistribution> {
  ...
  protected void setValues(AttributesMixedDistribution parameter, RandomGenerator randomGenerator)
    ...
    final boolean tooManyFewSwitchPoints =
        parameter.getSwitchpoints().size() != parameter.getDistributions().size() - 1;
    if (tooManyFewSwitchPoints) {
      throw new Exception(
          "There should be exactly one switchpoint for"
  ...
  private void setDistributions(
      List<AttributesDistribution> distributions, RandomGenerator randomGenerator)
      throws Exception {
    this.distributions = new ArrayList<>();

    for (AttributesDistribution distribution : distributions) {
      VDistribution<?> dist = DistributionFactory.create(distribution, randomGenerator);
  ...
  private boolean intervallAndTimeIsValidAt(double timeCurrentEvent) {
    return iscCurrentIntervalInBound()
        && isEventInCurrentInterval(timeCurrentEvent)
