...
public class DistributionRegistry {
  ...
  public static RegisteredDistribution get(String name) throws Exception {
    if (!REGISTRY.containsKey(name)) {
      throw new Exception(
          "There is no distribution with name "
              + name
  ...
  private static HashMap<String, RegisteredDistribution> findDistributions() {
    ...
    HashMap<String, RegisteredDistribution> registry =
        new HashMap<String, RegisteredDistribution>();
    ...
    distributions.forEach(
        clazz -> {
          ...
          if (VDistribution.class.isAssignableFrom(clazz)) {
            ...
            Class<? extends VDistribution<?>> vadereDistributionclazz =
                (Class<? extends VDistribution<?>>) clazz;
            RegisteredDistribution a =
                new RegisteredDistribution(annotation.parameter(), vadereDistributionclazz);
            String name =
            ...
            System.out.println(
                DistributionRegistry.class.getName()
                    + ": "
                    + clazz
