...
public class TargetPedestrian extends Target implements DynamicElementRemoveListener<Pedestrian> {
  ...
  // Static Variables
  /**
   * {@link Target}s and {@link TargetPedestrian}s share the same address space for ids! This can
   * cause conflicts when the target potential is retrieved in
   * "PotentialFieldTarget.getPotential()". "PotentialFieldTarget" stores potentials in a
   * map<targetId, potentials>. Therefore, use a fixed offset to discriminate {@link Target}s and
   * {@link TargetPedestrian}s.
   */
  ...
  public boolean equals(Object o) {
    ...
    if (!pedestrian.equals(that.pedestrian)) return false;
