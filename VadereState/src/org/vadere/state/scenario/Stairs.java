...
public class Stairs extends ScenarioElement<AttributesStairs> {
  ...
  private Tread[] computeTreads() {
    ...
    PathIterator iterator =
        this.getShape().getPathIterator(AffineTransform.getRotateInstance(-angle));
    ...
    for (int i = 0; i < treadsResult.length; i++) {
      ...
      // ---------------------------------- << line that pedestrians walk on (centered between
      // edges)
      ...
      double x =
          rotatedBounds.getMinX()
              - treadDepth / 2
              + factor * (rotatedBounds.getWidth() + treadDepth * 2);
