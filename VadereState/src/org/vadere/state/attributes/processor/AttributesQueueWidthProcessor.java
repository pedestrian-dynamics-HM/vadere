...
public class AttributesQueueWidthProcessor extends AttributesProcessor {
  ...
  public VPoint getReferencePoint() {
  ...
  public void setDirection(VPoint direction) {
    if (direction.distanceToOrigin() != 1.0
        || Math.max(Math.abs(direction.getX()), Math.abs(direction.getY())) != 1.0) {
      System.out.println(
          "Warning! Only the following vectors are allowed: [1,0],[-1,0],[0,1],[0,-1]. It is continued with [1,0].");
