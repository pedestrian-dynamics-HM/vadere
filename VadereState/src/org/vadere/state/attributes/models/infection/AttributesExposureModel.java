...
/**
 * AttributesExposureModel contains user-defined properties describing the <code>
 * AbstractExposureModel</code>.
 */
public abstract class AttributesExposureModel extends Attributes {
  ...
  /**
   * Contains the Ids of pedestrians that are directly set into the topography (and not spawned by
   * sources). Any agent contained in the list is infectious. All others (not spawned by sources)
   * are not infectious.
   */
  private ArrayList<Integer> infectiousPedestrianIdsNoSource;

  public AttributesExposureModel() {
    this.exposureModelSourceParameters =
        new ArrayList<>(Arrays.asList(new AttributesExposureModelSourceParameters()));
