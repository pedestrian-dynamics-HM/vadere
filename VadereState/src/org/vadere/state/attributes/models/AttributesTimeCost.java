 ...
 *
 * <p>TODO: split AttributesTimeCost timeCostAttributes into multiple classes see comment in {@link
...
public class AttributesTimeCost extends Attributes {
  ...
  public enum LoadingType {
    ...
    CONSTANT,
    /** use c_D and c_D' for differ pedestrians of the some target and other target. */
    CONSTANT_RESPECT_TARGETS,
    /** use the dynamic loading strategy to calculate for each pedestrian an individual loading. */
    DYNAMIC,
    QUEUEGAME;
  }
  ...
  /**
   * (hartmann-2012, c_D) = the weight of the pedestrian that has the some target as the potential
   * field that will be generated.
   */
  private double pedestrianSameTargetDensityWeight = 3.5;
  /**
   * (hartmann-2012, c_D') = the weight of the pedestrian that has on other target than the
   * potential field that will be generated.
   */
  ...
  /**
   * (queueing only) the factor that influences the queue width. = 1 > means lower width. The value
   * should be in the interval [1;infty[.
   */
   ...
   * pedestrian or uses a constant loading strategy. This will be done by the loading strategies. If
   * the loadingType=CONSTANT_RESPECT_TARGETS than pedestrianSameTargetDensityWeight and
   * pedestrianOtherTargetDensityWeight has to be set. If loadingType=CONSTANT than pedestrianWeight
   * has to be set.
   */
