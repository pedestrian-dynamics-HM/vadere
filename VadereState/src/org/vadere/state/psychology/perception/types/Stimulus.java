 ...
 *
 * <p>A stimulus has a time and possibly additional information.
 *
 * <p>The additional information depend on the type of the stimulus and should be added by
 * subclasses. For instance, a stimulus "ElapsedTime" can provide the current time. A stimulus
 * "Threat" can have a loudness and a polygon which describes where the threat can be perceived.
 *
 * <p>This class and its subclasses should be de-/serialized as JSON. Therefore, provide some
 * annotations so that serialized objects do not reveal Java type information like "util.ArrayList".
 *
 * <p>See @link http://www.baeldung.com/jackson-inheritance
 *
 * <p>Watch out: subclasses require a default constructor so that de-/serialization works!
 */
...
// "time" is set when the stimulus is injected into the simulation run and must not be
// de-/serialized.
...
public abstract class Stimulus implements Cloneable {
  ...
  // Static Methods
  public static boolean listContainsStimulus(
      List<Stimulus> stimuli, Class<? extends Stimulus> eventToCheck) {
