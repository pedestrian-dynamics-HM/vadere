 ...
 *
 * <p>This stimulus allows to script
 *
 * <ul>
 *   <li>when agents change their target to a new target.
 *   <li>how much agents should change their target to a new target.
 * </ul>
 *
 * For instance, a stimulus description looks like this:
 *
 * <ul>
 *   <li>allowedTimeDelta = 1.0
 *   <li>changeRemainingPedestrians = true
 *   <li>originalTargetIds = [1, 2]
 *   <li>newTargetIds = [3, 4]
 *   <li>simTimesToChangeTarget = [10.0, 20.0]
 *   <li>totalAgentsToChangeTarget = [1, 2]
 * </ul>
 *
 * On the cognition layer, this leads to following actions:
 *
 * <ul>
 *   <li>At sim time 10.0 second, for 1 agent with target id 1 or 2 change target id to 3.
 *   <li>At sim time 20.0 second, for 2 agents with target id 1 or 2 change target id to 4.
 *   <li>At each sim time from "simTimesToChangeTarget", change also the target of all remaining
 *       agents (i.e., whose target id is not in list "originalTargetIds") if
 *       "changeRemainingPedestrians = true". For instance, at sim time 10.0 second the target id of
 *       agent a1 changed from 1 to 3 and at sim time 20.0 the target id is changed from 3 to 4
 *       (since 3 is not in list [1, 2].
 * </ul>
 *
 * Note: "allowedTimeDelta" is necessary to make sure that stimulus is injected properly. For
 * instance, if "simTimeStepLength = 0.4" the simulated times are [0.0, 0.4, 0.8, 1.2, ...]. If
 * "ChangeTargetScripted" should be injected at "start = 1.0" and "end = 1.1" this would mean it
 * could not be injected because the "simTimeStepLength" is too coarse. Setting "allowedTimeDelta =
 * 0.2" causes that the stimulus is injected at "simTime = 1.2".
 */
public class ChangeTargetScripted extends Stimulus {
  ...
  public boolean getChangeRemainingPedestrians() {
    return changeRemainingPedestrians;
  }
  ...
  public LinkedList<Integer> getNewTargetIds() {
    return newTargetIds;
  }
