...
/**
 * AirTransmissionModelHealthStatus that is used in combination with the <code>AirTransmissionModel
 * </code>.
 */
public class AirTransmissionModelHealthStatus extends ExposureModelHealthStatus {
   ...
   */
  private static final VPoint RESET_EXHALATION_POSITION = null;
  ...
  public AirTransmissionModelHealthStatus(
      boolean breathingIn, double respiratoryTimeOffset, VPoint exhalationStartPosition) {
    super();
  ...
  public boolean equals(Object obj) {
    ...
    return breathingIn == other.breathingIn
        && Double.compare(other.respiratoryTimeOffset, respiratoryTimeOffset) == 0
   ...
   * respiratoryTimeOffset, and periodLength. Assumes that periodLength for inhalation and
   * exhalation are equally long. Pedestrian inhales when sin(time) > 0 or cos(time) == 1.
   */
  public void updateRespiratoryCycle(double simTimeInSec, double periodLength) {
    double b = 2.0 * Math.PI / periodLength;
    setBreathingIn(
        (Math.sin(b * (respiratoryTimeOffset + simTimeInSec)) > 0)
            || (Math.cos(b * (respiratoryTimeOffset + simTimeInSec)) == 1));
  }
