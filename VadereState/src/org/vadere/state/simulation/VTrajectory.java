...
public class VTrajectory implements Iterable<FootStep> {
  ...
  public boolean adjustEndTime(@NotNull final double endTime) {
    if (!isEmpty()) {
      ...
      footSteps.addLast(
          new FootStep(footStep.getStart(), footStep.getEnd(), footStep.getStartTime(), endTime));
    } else {
  ...
  public boolean isInBetween(double startTime, double endTime) {
    ...
    return footSteps.peekFirst().getStartTime() <= startTime
        && footSteps.peekLast().getEndTime() >= endTime;
  }
  ...
  public boolean isInBetween(@NotNull final VTrajectory other) {
    ...
    return isInBetween(
        other.footSteps.peekFirst().getStartTime(), other.footSteps.peekLast().getEndTime());
  }
  ...
  public VTrajectory add(@NotNull final FootStep footStep) {
    ...
    assert footSteps.isEmpty()
        || (footSteps.peekLast().getEndTime() <= footStep.getStartTime()
  ...
  public VTrajectory cut(@NotNull final VRectangle rectangle) {
    ...
    boolean inside =
        !footSteps.isEmpty() ? rectangle.contains(footSteps.peekFirst().getStart()) : false;

    for (FootStep footStep : footSteps) {
      if (footStep.intersects(rectangle)) {
        Pair<FootStep, FootStep> splitStep =
            footStep.cut(footStep.computeIntersectionTime(rectangle));
  ...
  private boolean isEntering(
      @NotNull final VRectangle rectangle, @NotNull FootStep intersectionStep) {
