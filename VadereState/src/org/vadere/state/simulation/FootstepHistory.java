...
public class FootstepHistory {
  ...
  /**
   * Heading based on TraCI angle * - measured in degree * - 0 is headed north * - clockwise
   * orientation (i.e. 90 heads east, 180 heads south, etc.) * - range from 0 to 360.
   *
  ...
  public double getNorthBoundHeadingAngle(int histLength, boolean degree) {
    if (footSteps.size() < histLength) return 0.0; // not enough data. Return North heading.
    ...
    Vector2D heading =
        new Vector2D(currentLocation.x - pastLocation.x, currentLocation.y - pastLocation.y);
    ...
    // mod 2PI for [0, 2PI]
    double angel =
        (Math.abs(2 * Math.PI - heading.angleToZero()) + 0.5 * Math.PI)
            % (2 * Math.PI); // angle with y-axis

    if (degree) return angel * (180 / Math.PI);
    return angel;
  }
  ...
  public String toString() {
    ...
    String footStepString =
        footSteps.stream().map(footStep -> footStep.toString()).collect(Collectors.joining(" -> "));
