...
/**
 * Immutable class. Java Bean that store the stepNumber and the simulation time in seconds of a
 * specific time step. A simulation step is defined by <tt>simTimeStepLength</tt> that is if t is
 * the simulation time than ceil(t / <tt>simTimeStepLength</tt>) is the simulation {@link Step}.
 *
...
public class Step implements Comparable<Step> {
  ...
  private static final Step FIST_STEP = new Step(1);
  private static final double MIN_TOLERANCE = 0.001;
  private static final double MAX_TOLERANCE = 0.999;
   ...
   *
   * @param step a step
   * @return the subtraction result or the minimal step if the result would generate a step smaller
   *     than the minimal step.
   */
