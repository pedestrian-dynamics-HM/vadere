__kernel void fft2Dim(const __global float2 *input,
    ...
    for(int N2 = 4; N2 < points_per_item; N2 <<=1) {
        ...
        for(int fft_index = 0; fft_index < points_per_item; fft_index += 2*N2) {
            ...
            for(int i = 1; i < N2; i++) {
                float param = (M_PI_F * i *direction)/N2;
    ...
    for(int N2 = points_per_item; N2 < N; N2 <<=1) {
        ...
        for(int i = start; i < start + points_per_item / 2; i++) {
            float param = (M_PI_F * angle * direction)/N2;
            sine = sincos(param,&cosine);
            sine = direction*sine;
            wk = (float2) (
    ...
    for(int N2 = 4; N2 < points_per_item; N2 <<=1) {
        ...
        for(int fft_index = 0; fft_index < points_per_item; fft_index += 2*N2) {
            ...
            for(int i = 1; i < N2; i++) {
                ...
                float real = local_data[l_addr + N2 + i].s0 * cosine + local_data[l_addr + N2 + i].s1 * sine;
                float imag = local_data[l_addr + N2 + i].s1 * cosine - local_data[l_addr + N2 + i].s0 * sine;
                float2 point = local_data[l_addr + N2 + i];
                wk = (float2) (
                    local_data[l_addr + N2 + i].s0 * cosine +
                    local_data[l_addr + N2 + i].s1 * sine,
                    local_data[l_addr + N2 + i].s1 * cosine -
