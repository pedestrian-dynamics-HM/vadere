...
public class ProcessorBaseFactory<T> extends BaseFactory<T, ProcessorFactoryObject<T>> {
  ...
  public void addMember(
      Supplier supplier, String label, String desc, Class clazz, String... flags) {
    supplierMap.put(
        clazz.getCanonicalName(),
        new ProcessorFactoryObject<>(supplier, label, desc, clazz, flags));
  }
  ...
  public boolean containsFlag(Class processor, String flagStr) {
    ProcessorFactoryObject<T> processorFactoryObject =
        supplierMap.get(processor.getCanonicalName());
    if (processorFactoryObject == null) return false;
