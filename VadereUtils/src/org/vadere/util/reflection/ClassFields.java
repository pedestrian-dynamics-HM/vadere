...
public final class ClassFields {
  ...
  public static Field[] getSuperDeclaredFields(Class clazz) {
    return getSuperClassHierarchy(clazz)
        .stream()
        .map(c -> checkVadereAttributes(c))
        .reduce((c1, c2) -> ArrayUtils.addAll(c1, c2))
        .get();
  }
  ...
  public static Map<String, List<Field>> getFieldsGroupedBySemanticMeaning(
      java.util.List<Field> fieldList) {
    return fieldList
        .stream()
        .collect(
            Collectors.groupingBy(
                field -> field.getAnnotation(VadereAttribute.class).group(), Collectors.toList()));
  }
