 ...
 * fixed column names). Null entries are not allowed. If you try to get an value from an entry
 * outside the range this may result in an {@link IllegalArgumentException}. Column names are
 * unique. It is possible to merge a table in an existing table, if they both has the same number of
 * rows. One can only insert values at the end of the table. You have to complete a whole row before
 * you can insert another one. The only possibility to delete manipulate existing rows is to use the
 * iterators {@link RowIterator} or {@link RowArrayIterator}. The reason is that it is inefficient
 * to reference via index since the implementation is based on {@link LinkedList}.
 *
 * <p>Note: The column size difference is maximal +-1, if the row is not complete. This
...
public class Table implements Iterable<Row> {
  ...
  public Table(final String[] columnNames, final Object[] values, final int size) {
    ...
    if (columnNames.length != values.length) {
      throw new IllegalArgumentException(
          "values and columnNames array has not the same length: "
              + columnNames.length
              + " != "
              + values.length);
    }
  ...
  /**
   * Adds a value to the specific column of the current row identified by a column name. Note that
   * one has to fill a whole row before add a new row. After a row is complete one has to call
   * addRow(). If the column does not exist an {@link IllegalArgumentException} will be thrown. If
   * one try to add an entry to a full column an {@link IndexOutOfBoundsException} will be thrown.
   *
  ...
  /**
   * Adds a new empty row to this table. After one complete a row this method has to be called to
   * continue insert elements.
   */
  public void addRow() {
    size++;
  }
  ...
  /**
   * Merges a table into this table by add each column to this table. Both tables has to be of the
   * same size. If there a duplicated column names this table will stay with its column. Futhermore
   * the columns of the other table will be added behind the this table.
   *
  ...
  public void merge(final Table table) {
    ...
    for (String columnName : table.columnNames) {
      if (!this.columnNamesSet.contains(columnName)
          || this.table.get(columnName) == null
  ...
  /**
   * Returns a single value of this table. This is a expensive call since the data structure use
   * LinkedLists. You should use the iterator.
   *
