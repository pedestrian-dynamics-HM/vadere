...
public class Face {
  ...
  private HalfEdge handleVertexOutsideOfBorders(HalfEdge edge) {
    ...
    VPoint nextIntersection =
        getCloserOneOfTwoPoints(getBorderIntersections(reEntranceNext), vertex);
    VPoint previousIntersection =
        getCloserOneOfTwoPoints(getBorderIntersections(reEntrancePrevious), vertex);
    ...
    if (!isCloser(nextIntersection, reEntrancePrevious)) {
      ...
      createBorderEdges(reEntranceNext, reEntrancePrevious, nextIntersection, previousIntersection);
    }
  ...
  private void attachOpenEnds(HalfEdge openStart, HalfEdge openEnd) {
    ...
    if (openStart == openEnd) {
      ...
      if (MathUtil.pOverLineAB(sitePosition, borderPoint1, borderPoint2)) {
        ...
        createBorderEdges(openStart, openEnd, borderPoint2, borderPoint1);
      }
    }
    // multiple edges exist, the open edges have to be connected
      ...
      if (twin == null) {
        borderPoint1 = getOtherBorderPoint(openStart, openStart.getNext().getOrigin());
      } else {
        borderPoint1 = getBorderPointOpenHalfEdge(openStart, openStart.getNext());
      }
      ...
      if (twin == null) {
        ...
        borderPoint2 = getBorderPointOpenHalfEdge(openEnd, openEnd.getPrevious());
      }
  ...
  private VPoint getOtherBorderPoint(HalfEdge edge, VPoint takenBorderPoint) {
    ...
    if (classifyBorderPoint(takenBorderPoint) == classifyBorderPoint(intersections.get(0))) {
  ...
  private VPoint getBorderPointOpenHalfEdge(HalfEdge edge, HalfEdge neighbor) {
    ...
    // strange condition
    if (position1.distance(intersections.get(0)) < position2.distance(intersections.get(0))) {
      result = intersections.get(0);
    } else {
      result = intersections.get(1);
    }
  ...
  private void createBorderEdges(
      HalfEdge startEdge, HalfEdge endEdge, VPoint borderPointStart, VPoint borderPointEnd) {
  ...
  private int classifyBorderPoint(VPoint point) {
    ...
    if (point.x == limits.xLow) {
      ...
      throw new IllegalArgumentException("Point does not lie on any border.");
    }
  ...
  boolean isOutsideOfBorders(VPoint vertex) {
    ...
    if (vertex != null) {
      if (vertex.x < limits.xLow
          || vertex.x > limits.xHigh
          || vertex.y < limits.yLow
          || vertex.y > limits.yHigh) {
        result = true;
      }
    }
