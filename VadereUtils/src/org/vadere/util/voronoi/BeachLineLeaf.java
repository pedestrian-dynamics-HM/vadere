...
public class BeachLineLeaf implements BeachLineNode {
  ...
  boolean hasCircleEvent() {
    ...
    if (predecessor != null && successor != null) {
      ...
      double criteria = (xThis - xPre) * (yPre - ySuc) + (yThis - yPre) * (xSuc - xPre);
  ...
  void createCircleEvent() {
     ...
     */
    x =
        (xThis * xThis * (yPre - ySuc)
            + (xPre * xPre + (yPre - ySuc) * (yPre - yThis)) * (ySuc - yThis)
            + xSuc * xSuc * (-yPre + yThis));
    x = x / (2 * (xThis * (yPre - ySuc) + xPre * (ySuc - yThis) + xSuc * (-yPre + yThis)));

    // conditions I thought of my own, hence it may be wrong
    if (yThis + NUM_ERROR < yPre || yThis - NUM_ERROR > yPre) {
      y = -(xThis - xPre) / (yThis - yPre) * (x - (xPre + xThis) / 2) + (yPre + yThis) / 2;
    }
    /* else if(yThis + NUM_ERROR < ySuc || yThis - NUM_ERROR > ySuc) { */
    else {
      y = -(xSuc - xThis) / (ySuc - yThis) * (x - (xThis + xSuc) / 2) + (yThis + ySuc) / 2;
    }
    /*
     * else { throw new IllegalStateException(); }
     */

    double radius = Math.sqrt(Math.pow(xThis - x, 2) + Math.pow(yThis - y, 2));
