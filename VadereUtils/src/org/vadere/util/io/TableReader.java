...
/**
 * This class convert a stream of table lines {@link java.util.stream.Stream<String>} into a {@link
 * org.vadere.util.data.Table}. Note that a stream can not be reused, so if u first want to get the
 * headline and then construct the table based on the headline you have to use two {@link
 * java.util.stream.Stream<String>} instances, so you have to recreate the {@link
...
public class TableReader {
  ...
  public Table readTable(
      final Stream<String> lines, boolean containsHeadLine, final String... variables) {
  ...
  public String[] readHeadLine(final Stream<String> lines) {
    ...
    if (line.isPresent()) {
      ...
      if (format == null) {
        headerFormat =
            Arrays.stream(line.get().split(" ")).reduce("", (s1, s2) -> s1.concat("%s ")).trim();
      } else {
        headerFormat = format.replaceAll("%.", "%s");
      }

      long limit = headerFormat.chars().filter(c -> c == '%').count();
      return Stream.iterate(Pair.of(headerFormat, line.get()), t -> nextResidual(t))
          .limit(limit)
          .filter(pair -> pair.getValue() != null)
          .map(pair -> toValue(pair.getLeft(), pair.getRight()).toString())
          .toArray(String[]::new);
    }
    return headline;
  }

  private Stream<Row> readRows(
      final Stream<String> lines, final boolean skipHeadline, final String... variables) {
    return lines
        .filter(TableReader::isNotEmptyString)
        .skip(skipHeadline ? 1 : 0)
  ...
  private Row toRow(final String line, String format, final String... variables) {
    ...
    Row row =
        Stream.iterate(Triple.of(0, format, line), t -> nextResidual(t))
            .limit(variables.length)
            .map(
                tripel ->
                    toColumn(variables[tripel.getLeft()], tripel.getMiddle(), tripel.getRight()))
  ...
  private Row toColumn(
      final String columnName, final String residualFormatString, final String residualLine) {
    Row row = new Row();
    row.setEntry(columnName, toValue(residualFormatString, residualLine));
    return row;
  }

  private Triple<Integer, String, String> nextResidual(
      final Triple<Integer, String, String> tripel) {
    Integer varCount = tripel.getLeft();
    Pair<String, String> tupel = nextResidual(Pair.of(tripel.getMiddle(), tripel.getRight()));

    return tupel.getValue() != null
        ? Triple.of(varCount + 1, tupel.getLeft(), tupel.getRight())
  ...
  /**
   * Convert the value to an primitive DataType which is defined by format (e.g. d (for integer) or
   * f (for double)).
   *
   * @param value
   * @param format
