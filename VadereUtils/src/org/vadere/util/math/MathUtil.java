...
public class MathUtil {
  ...
  private static final List<Point> neumannNeighborhood = getNeumannNeighborhood(new Point(0, 0));

  // epsilon for finite differences see
  ...
  /**
   * The value of the gradient of the 2D smooth function with compact support given by
   * exp(1/((x/cut)^2-1)).
   *
  ...
  public static void cutExpGrad2D(double[] x, double cut, double[] grad) {
    ...
    if (absx >= 1) {
      ...
      grad[0] = -2 * Math.exp(1 / denom) * x[0] / (cut * cut) * 1 / (denom * denom);
      grad[1] = -2 * Math.exp(1 / denom) * x[1] / (cut * cut) * 1 / (denom * denom);
    }
  }

  /**
   * The value of the 2D smooth function with compact support given by exp(1/((x/cut)^2-1)).
   *
   * @param x 2D position where the gradient should be evaluated.
  ...
  /**
   * The value of the 2D smooth function with compact support given by exp(1/((x/cut)^(2p)-1)).
   *
   * @param x 2D position where the gradient should be evaluated.
   * @param cut if abs(x)>cut, the function is zero.
   * @return
  ...
  /**
   * computes the "smallest distance on the circle" of the angle3D (x+v,x,x2-x)
   *
   * @param x the starting position
   * @param v the viewing direction
   * @param x2 the position that should be evaluated
   * @return a value between 0.0 and 1.0 that indicates how much of the position is visible. 1.0 is
   *     fully visible, 0.0 is hidden.
   */
  public static double smallestViewingAngle(double[] x, double[] v, double[] x2) {
    ...
    double aa =
        ((x[0] + v[0] - x2[0]) * (x[0] + v[0] - x2[0])
            + (x[1] + v[1] - x2[1]) * (x[1] + v[1] - x2[1]));
    double b = Math.sqrt((x[0] - x2[0]) * (x[0] - x2[0]) + (x[1] - x2[1]) * (x[1] - x2[1]));
    ...
    // in NaN.
    double arg = Math.min(1, Math.max(-1, (b * b + c * c - aa) / (2 * b * c)));
  ...
  /**
   * Computes the portion of a position x2 that is visible from position x given the direction in v.
   *
   * @param x the starting position
   * @param v the viewing direction
   * @param x2 the position that should be evaluated
  ...
  /**
   * Generates quasi random numbers in two dimensions using a VERY simple algorithm.
   *
   ...
   */
  public static double[][] quasiRandom2D(
      Random rndSource, int count, double width, double height, double randomFrac) {
    if (count == 0) {
      count = 1;
    }
    ...
    for (int i = 0; i < count; i++) {
      ...
      SimpleEntry<Integer, Integer> coords = binCoodinates.remove(nextIndex);
      result[i][0] =
          Math.min(
              width, coords.getKey() * sideLenX + rndSource.nextDouble() * sideLenX * randomFrac);
      result[i][1] =
          Math.min(
              height,
              coords.getValue() * sideLenY + rndSource.nextDouble() * sideLenY * randomFrac);
    }
  ...
  /**
   * Returns a list of absolute coordinates which correspond to the Moore neighborhood of the given
   * point p.
   */
  ...
  /**
   * Returns a list of absolute coordinates which correspond to the Neumann neighborhood of the
   * given point p.
   */
  ...
  /** Returns a list of relative offsets which correspond to the Neumann neighborhood. */
  ...
  public static List<Double> solveQuadratic(double a, double b, double c) {
    ...
    if (a != 0) {
      ...
      if (discr == 0) {
        ...
        Collections.addAll(
            result, (-b + Math.sqrt(discr)) / (2.0 * a), (-b - Math.sqrt(discr)) / (2.0 * a));
      }
    } else if (b != 0) {
      result.add(-c / b);
    } else {
      // throw new IllegalArgumentException("ax^2 + bx + c = 0 is not a valid quadratic equation for
      // a=b=0.");
    }
  ...
  /**
   * Returns the max real solutions of the quadratic equation ax^2+bx+c=0 or Double.MAX_VALUE if
  ...
  public static VPoint proectVector(final VPoint origin, final VPoint projection) {
    double tmp =
        (origin.x * projection.x + origin.y * projection.y)
