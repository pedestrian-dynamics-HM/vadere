 ...
 *     <p>A signed distance function d. d(x) greater than 0 if x is outside d(x) smaller or equals 0
 *     if x is inside
 */
@FunctionalInterface
public interface IDistanceFunction extends Function<IPoint, Double> {

  static IDistanceFunction createRing(
  ...
  static IDistanceFunction createDisc(
      final double xCenter, final double yCenter, final double radius) {
    return p -> {
      double dx = p.getX() - xCenter;
      double dy = p.getY() - yCenter;
  ...
  static IDistanceFunction create(
      final VRectangle regionBoundingBox, final Collection<? extends VShape> obstacles) {
    return new DistanceFunction(regionBoundingBox, obstacles);
  }

  static IDistanceFunction create(
      final VRectangle regionBoundingBox,
      final Collection<? extends VShape> obstacles,
      final Collection<? extends VShape> targets) {
    return new DistanceFunction(regionBoundingBox, obstacles, targets);
  }

  static IDistanceFunction create(
      final VPolygon regionBoundingBox,
      final Collection<? extends VShape> obstacles,
      final Collection<? extends VShape> targets) {
    return new DistanceFunction(regionBoundingBox, obstacles, targets);
  }
  ...
  static IDistanceFunction create(
      final VPolygon regionBoundingBox, final Collection<? extends VShape> obstacles) {
    return new DistanceFunction(regionBoundingBox, obstacles);
  }
