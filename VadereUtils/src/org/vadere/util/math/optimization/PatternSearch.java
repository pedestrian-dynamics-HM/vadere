...
public class PatternSearch {
  ...
  public PatternSearch(
      @NotNull final VCircle evalArea,
    ...
    this.basePoints =
        basePoints
            .stream()
            .map(p -> p.scalarMultiply(evalArea.getRadius()))
  ...
  public double getMinValue() {
    return localOptimizers
        .stream()
        .mapToDouble(localOptimizer -> localOptimizer.getMinValue())
        .min()
        .getAsDouble();
  }
  ...
  private void globalSearch() {
    PriorityQueue<Pair<VPoint, Double>> list =
    ...
    globalLen =
        Math.min(
            globalLen,
            Math.max(
                evalArea.getRadius() / (numberOfCircles > 0 ? numberOfCircles : 1),
                2 * Math.PI * evalArea.getRadius() / (numberOfPoints > 0 ? numberOfPoints : 1)));
  ...
  private boolean contains(@NotNull final VPoint point) {
    return normCircle.getRadius() * normCircle.getRadius()
        >= normCircle.getCenter().distanceSq(point) - GeometryUtils.DOUBLE_EPS;
  }

  private boolean onCircle(@NotNull final VPoint point) {
    return Math.abs(
            normCircle.getRadius() * normCircle.getRadius()
                - normCircle.getCenter().distanceSq(point))
      ...
      if (onCircle(base)) {
        ...
        for (int i = 0; i < crossPoints.length; i++) {
          ...
          if (!contains(crossPoint)) {
            VPoint rotatedCrossPoint =
                base.rotate((crossPoints[i].x + crossPoints[i].y) / normCircle.getRadius())
                    .subtract(base);
        ...
        for (int i = 0; i < crossPoints.length; i++) {
          VPoint crossPoint = base.add(crossPoints[i]);
          if (!contains(crossPoint)) {
            Optional<VPoint> intersectionPoint =
                normCircle.getSegmentIntersectionPoints(crossPoint.x, crossPoint.y, base.x, base.y);
