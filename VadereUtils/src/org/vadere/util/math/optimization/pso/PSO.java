...
public class PSO {
  ...
  public PSO(
    ...
    assert swarmPositions
        .stream()
        .allMatch(
            p -> circle.getCenter().distance(p) < circle.getRadius() + GeometryUtils.DOUBLE_EPS);
  ...
  public boolean hasFinished() {
    return iterationCounter >= attributesPSO.maxIteration
        || (iterationCounter >= attributesPSO.minIteration && hasConverged());
  }
  ...
  public void update() {
    if (!hasFinished()) {
      ...
      double omega =
          attributesPSO.wUpperBound
              - (iterationCounter / attributesPSO.minIteration)
                  * (attributesPSO.wUpperBound - attributesPSO.wLowerBound);
  ...
  private void updateParticle(@NotNull final Particle particle, final double omega) {
    ...
    if (!circle.contains(particle.getLocation())
        && particle.getLocation().distance(currentLocation) > GeometryUtils.DOUBLE_EPS) {
      particle.setVelocity(particle.getVelocity().scalarMultiply(-0.5));
      particle.setLocation(
          circle
              .getClosestIntersectionPoint(
                  currentLocation, particle.getLocation(), particle.getLocation())
              .orElse(particle.getLocation()));
    }
  ...
  /**
   * 1) updates the best overall value of the PSO. 2) if the new best overall value is smaller than
   * the old one, particles inform each other about their best values and locations.
   */
  ...
  private void informParticles(
      @NotNull final Particle particle, @NotNull final Particle otherParticle) {
    double globalBest = particle.getGlobalBestFitnessValue();
  ...
  private List<Particle> initialSwarm(@NotNull List<VPoint> swarmPositions) {
    return swarmPositions
        .stream()
        .map(location -> locationToParticle(location))
