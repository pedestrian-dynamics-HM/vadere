...
public class Simplex1D {
  ...
  private VPoint toPoint(final double rad) {
    return new VPoint(Math.cos(rad) * circle.getRadius(), Math.sin(rad) * circle.getRadius())
  ...
  /**
   * True if the geometrically size of the simplex and the variance of values is below the
   * <tt>threshold</tt>. Or the globally best point has been found.
   *
  ...
  private double getTol() {
    return 2.0
        * Math.abs(values[1] - values[0])
        / (Math.abs(values[1]) + Math.abs(values[0]) + TINY);
  }
