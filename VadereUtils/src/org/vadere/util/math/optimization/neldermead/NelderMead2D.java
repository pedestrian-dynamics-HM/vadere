...
/**
 * Implementation of the 2-dimensional Nelder-Mead. Multiple simplexes can be solved in parallel
 * (default).
 *
...
public class NelderMead2D {
   ...
   * @param eval the evaluation function such that for illegal / invalid position p eval.apply(p) ==
   *     MAX_VAL
  ...
  public NelderMead2D(
    ...
    this.simplexes =
        simplexes
            .stream()
            .map(triangle -> new Simplex2D(eval, triangle))
            .filter(simplex2D -> !simplex2D.isInvalid())
  ...
  private void update() {
    stream()
        ...
        .filter(s -> !s.hasConverged(threshold))
        .forEach(s -> s.update());
  }
  ...
  private Simplex2D getBest() {
    ...
    for (Simplex2D simplex : simplexes) {
      if (best == null
          || (minimize && best.getValue() > simplex.getValue())
          || (!minimize && best.getValue() < simplex.getValue())) {
        best = simplex;
      }
    }
  ...
  /**
   * Returns true, if all valid simplexes have converged which depends on the <tt>threshold</tt>,
   * false otherwise
   *
  ...
  public String toString() {
    return "["
        + simplexes
            .stream()
            .map(simplex2D -> simplex2D.toString())
            .reduce((s1, s2) -> s1 + "," + s2)
            .orElse("")
        + "]";
  }
