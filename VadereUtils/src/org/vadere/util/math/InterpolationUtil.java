...
public class InterpolationUtil {
  ...
  public static <P extends IPoint> double barycentricInterpolation(
    ...
    return barycentricInterpolation(
        p1, eval.apply(p1), p2, eval.apply(p2), p3, eval.apply(p3), totalArea, x, y);
  }
  ...
  public static double barycentricInterpolation(
      ...
      final double y) {
    return barycentricInterpolation(
        p1.getX(), p1.getY(), val1, p2.getX(), p2.getY(), val2, p3.getX(), p3.getY(), val3,
        totalArea, x, y);
  }
  ...
  public static double barycentricInterpolation(
    ...
    assert Math.abs(
            totalArea
                - GeometryUtils.areaOfPolygon(new double[] {x1, x2, x3}, new double[] {y1, y2, y3}))
  ...
  public static double barycentricInterpolation(
      final IPotentialPoint[] points, final double x, final double y) {
  ...
  public static double barycentricInterpolation(
      final List<? extends IPotentialPoint> points, final double x, final double y) {
  ...
  /**
   * Computes bilinear interpolation of z = f(x,y) with z1 to z4 being the given z values at the
   * edges of a rectangle in the xy-plane. z1 refers to the left-lower edge of the rectangle. The
   * remaining z values have to be assigned counter-clockwise to the rectangle edges. t and u are
   * the normalized offsets of the left-lower edge of the rectangle along the x- and y-axis.
   */
  public static double bilinearInterpolation(
      double z1, double z2, double z3, double z4, double t, double u) {
    double result = 0;
    double value = 0;
    double weight = 0;
  ...
  /**
   * Computes bilinear interpolation for (x,y) while nodes may be undefined (=Double.MAX_VALUE).
   *
   * <p>See bilinearInterpolation for further information about the basic method. In contrast to the
   * original bilinearInterpolation(), bilinearInterpolationWithUnkown() just accumulates the nodes
   * whose values are known multiplied by its weight. The accumulated weight of the known values are
   * stored in the parameter weightOfKnown. see https://en.wikipedia.org/wiki/Bilinear_interpolation
   * (Unit square)
   *
   * @param z the values of the 4 grid points which convex hull contains (x,y) in the following
   *     order: (0,0), (1,0), (0,1), (1,1) with respect to (x,y)
   ...
   */
  public static Pair<Double, Double> bilinearInterpolationWithUnkown(
   ...
   * @return
   */
  public static double getValueByTriangleInterpolation(CellGrid pot, double x, double y) {
   ...
   *
   * <p>see http://processingjs.nihongoresources.com/bezierinfo/legendre-gauss-values.php
   */
  private static double[] GaussQuadraturePoints =
      new double[] {
        -0.993752171,
        -0.967226839,
        -0.920099334,
  ...
  private static double[] NormalQuadraturePoints =
      new double[] {
        -1.000000000000000,
        -0.933333333333333,
        -0.866666666666667,
  ...
  private static double[] ExponentialQuadraturePoints =
      new double[] {
        -1.0000, -0.8333, -0.6667, -0.5000, -0.3333, -0.1667, 0, 0.1667, 0.3333, 0.5000, 0.6667,
        0.8333, 1.0000
      };
  /**
   ...
   * <p>see http://processingjs.nihongoresources.com/bezierinfo/legendre-gauss-values.php
   */
  private static double[] GaussQuadratureWeights =
      new double[] {
  ...
  private static double[] ExponentialQuadratureWeights =
      new double[] {
        0.0419, 0.3319, -0.3449, 1.4980, -2.4841, 4.1669, -4.4195, 4.1669, -2.4841, 1.4980, -0.3449,
        0.3319, 0.0419
      };

  private static double[] NormalQuadratureWeights =
      new double[] {
  ...
  /** Integral over the two-dimensional mollifier with sigma = 1.0, from -1 to 1. */
  private static double mollifierIntegral = 0.466512;

  /**
   * Compute the gradient at the given position x, using a mollified version of the solution stored
   ...
   * @param grad
   */
  public static void getGradientMollified(
      CellGrid pot, double[] x, double[] grad, double gradientMollifierRadius) {
    ...
    for (int row = 0; row < N; row++) {
      for (int col = 0; col < N; col++) {
        ...
        double potValue =
            InterpolationUtil.getValueByTriangleInterpolation(
                pot, xDisplaced / pot.getResolution(), yDisplaced / pot.getResolution());
        if (potValue > 9999 || Double.isInfinite(potValue) || Double.isNaN(potValue)) {
          potValue = 9999;
        }
        ...
        // mollify
        grad[0] += -cgrad[0] * potValue / mollifierIntegral * weights[row] * weights[col];
        grad[1] += -cgrad[1] * potValue / mollifierIntegral * weights[row] * weights[col];
      }
    }
