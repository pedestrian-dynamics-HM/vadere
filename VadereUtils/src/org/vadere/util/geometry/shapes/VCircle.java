...
public class VCircle implements VShape, ICircleSector {
  ...
  public VPoint closestPoint(IPoint point) {
    Vector2D direction = new Vector2D(point.getX() - center.x, point.getY() - center.y);
   ...
   * @return returns an immutable list {@link ImmutableList} of all immutable points {@link VPoint}
   *     intersecting the circle by the line defined by (p, q)
   */
  public ImmutableList<VPoint> getIntersectionPoints(
      final double x11, final double y11, final double x22, final double y22) {
    ...
    assert (Math.abs(dx) > 0.0 || Math.abs(dy) > 0.0) && dr * dr > 0.0
        : "the line (" + x1 + "," + y1 + ") -- (" + x2 + "," + y2 + ") is invalid";
    ...
    if (D == 0) {
      ...
      return ImmutableList.of(
          new VPoint(x1 + this.getCenter().x, y1 + this.getCenter().y),
          new VPoint(x2 + this.getCenter().x, y2 + this.getCenter().y));
    }
  }

  /**
   * Returns, the closest of all intersection points of the intersection of this circle and the line
   * (p, q).
   *
   * @param p defining the line
   * @param q defining the line
   * @param r the point of measurement
   * @return the closest of all intersection points of the intersection of this circle and the line
   *     (p, q). There might be no such point!
   */
  public Optional<VPoint> getClosestIntersectionPoint(
      @NotNull final VPoint p, @NotNull final VPoint q, @NotNull final VPoint r) {
  ...
  /**
   * Returns, the closest of all intersection points of the intersection of this circle and the line
   * (p=(x1, y1), q = (x2, y2)) with respect to r = (x3, y3).
   */
  public Optional<VPoint> getClosestIntersectionPoint(
      final double x1,
      final double y1,
   ...
   * @return
   */
  public Optional<VPoint> getSegmentIntersectionPoints(
      final double x11, final double y11, final double x22, final double y22) {
    ...
    //		throw new IllegalArgumentException("line segment ("+x11+","+y11+") - ("+x22+","+y22+") does
    // not intersect circle " + this + ".");
  }
  ...
  public boolean equals(Object obj) {
    ...
    if (!(obj instanceof VCircle)) return false;
  ...
  public Rectangle2D getBounds2D() {
    return new Rectangle2D.Double(center.x - radius, center.y - radius, 2 * radius, 2 * radius);
  }
  ...
  public boolean intersects(VLine intersectingLine) {
    if (intersectingLine.ptSegDist(this.getCenter()) <= this.getRadius()) return true;
    return false;
  }
  ...
  public boolean intersects(VShape shape) {
    if (shape instanceof VCircle) {
      ...
      return otherCircle.getCenter().distance(this.getCenter())
          < (otherCircle.getRadius() + this.getRadius());
    } else {
  ...
  public String toString() {
    return "VCircle{" + "center=" + center + ", radius=" + radius + '}';
  }
}
