...
public class VPolygon extends Path2D.Double implements VShape {
  ...
  public boolean isCCW() {
    ...
    List<VPoint> convexPartialPolygon =
        points.stream().filter(p -> convexHull.contains(p)).collect(Collectors.toList());
    return GeometryUtils.isCCW(
        convexPartialPolygon.get(0), convexPartialPolygon.get(1), convexPartialPolygon.get(2));
  }
  ...
  public boolean intersects(VPoint center, double radius) {
    ...
    for (int i = 0; i < pointList.size() - 1; i++) {
      ...
      if (i < pointList.size() - 1) {
        intersectingLine = new VLine(pointList.get(i), pointList.get(i + 1));
      } else {
      ...
      // ball
      if (GeometryUtils.closestToSegment(intersectingLine, center).distance(center) < radius) {
        return true;
      }
    }
  ...
  public boolean intersectsRectangleLine(final VRectangle rectangle) {
    return intersects(
        ...
        || intersects(
            new VLine(
                rectangle.getMaxX(), rectangle.getMinY(), rectangle.getMaxX(), rectangle.getMaxY()))
        || intersects(
            new VLine(
  ...
  public void grow(double absolute) {
    ...
    for (PathIterator vertexItr = getPathIterator(null); !vertexItr.isDone(); vertexItr.next()) {
    ...
    for (int iVertex = 0; iVertex < curVertices.size() - 1; ++iVertex) {
      ...
      newVertices.add(new VPoint(curVertex.x + deltaNxtLast.y, curVertex.y - deltaNxtLast.x));
    ...
    if (!newVertices.isEmpty()) {
      ...
      this.append(GeometryUtils.polygonFromPoints2D(newVertices.toArray(new VPoint[0])), false);
      this.closePath();
    }
  }

  public LinkedList<VPolygon> borderAsShapes(
      double borderWidth, double shapeShrinkOffset, double segmentGrowOffset) {
    ...
    while (!vertexItr.isDone()) {
      ...
      segmentVertices.moveTo(
          lastVertex[0]
              - delta[0] * segmentGrowOffset
              - delta[1] * (borderOffset + shapeShrinkOffset + segmentGrowOffset),
          lastVertex[1]
              - delta[1] * segmentGrowOffset
              + delta[0] * (borderOffset + shapeShrinkOffset + segmentGrowOffset));
      segmentVertices.lineTo(
          lastVertex[0]
              - delta[0] * segmentGrowOffset
              + delta[1] * (borderOffset - shapeShrinkOffset + segmentGrowOffset),
          lastVertex[1]
  ...
  public VPoint closestPoint(IPoint point) {
    ...
    while (!iterator.isDone()) {
      ...
      currentClosest =
          GeometryUtils.closestToSegment(new VLine(last[0], last[1], next[0], next[1]), point);
    ...
    if (first != null) {
      currentClosest =
          GeometryUtils.closestToSegment(new VLine(next[0], next[1], first[0], first[1]), point);
      ...
      if (GeometryUtils.intersectLine(line, q1, q2)) {
        currentIntersectionPoint =
            GeometryUtils.lineIntersectionPoint(
                new VLine(last[0], last[1], next[0], next[1]),
                q1.getX(),
                q1.getY(),
                q2.getX(),
                q2.getY());
  ...
  public boolean equals(Object obj) {
    ...
    if (!(obj instanceof VPolygon)) return false;
    ...
    if (!thisPoints.equals(otherPoints)) return false;
