 ...
 *     <p>A VCircleSector is a part of a disc. The disc is defined by the center and the radius and
 *     the part by the minimum and maximum angle3D.
 */
public class VCircleSector implements ICircleSector {
  ...
  public VCircleSector(
      @NotNull final VPoint center,
      final double radius,
  ...
  public ImmutableList<VPoint> getIntersectionPoints(double x1, double y1, double x2, double y2) {
    ...
    circle
        .getIntersectionPoints(x1, y1, x2, y2)
        .stream()
        .filter(p -> insideAngle(GeometryUtils.angleTo(getCenter(), p)))
        .forEach(p -> intersectionPoints.add(p));
  ...
  public Optional<VPoint> getClosestIntersectionPoint(
    ...
    return getIntersectionPoints(x1, y1, x2, y2)
        .stream()
        .min(Comparator.comparingDouble(p -> p.distance(x3, y3)));
  }
  ...
  public boolean intersectsLine(double x1, double y1, double x2, double y2) {
    if (getLine1().intersectsLine(x1, y1, x2, y2) || getLine2().intersectsLine(x1, y1, x2, y2)) {
      ...
      return interSectionPoints
          .stream()
          .anyMatch(p -> insideAngle(GeometryUtils.angleTo(getCenter(), p)));
    }
  }
    ...
    if (line1 == null) {
      line1 =
          new VLine(
              center.getX(),
              center.getY(),
              Math.cos(minAngle) * radius,
              Math.sin(minAngle) * radius);
    }
    return line1;
  }
