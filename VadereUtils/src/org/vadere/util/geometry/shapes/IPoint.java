 ...
 * be immutable (see {@link VPoint}) or mutable (see {@link MPoint}). In the mutable case operations
 * like {@link IPoint#add(IPoint)} change the 2D-coordinates of the this point.
 */
public interface IPoint extends Cloneable, Geometry {
   ...
   *
   * <p>This does not effect the coordinates of this.
   *
    ...
    if (length <= 0) {
      if (len != 0.0) {
        throw new IllegalArgumentException(
            "a vector with zero length can not be set to a specific magnitude != 0.");
      } else {
        return this;
      }
    }
  ...
  /**
   * Clones the point. This will return a copy if the point is immutable, otherwise this will return
   * this.
   *
  ...
  // Methods used by the R-Tree, for this data structure a point is equal to a Rectangle where each
  // defining point is equal (zero area)
  @Override
