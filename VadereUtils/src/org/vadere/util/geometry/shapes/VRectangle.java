...
/** Note: A rectangle which has the same corner points as a polygon is not equals to the polygon. */
public class VRectangle extends Rectangle2D.Double implements VShape {
  ...
  public boolean contains(VCircle otherShape) {
    ...
    boolean circleFitsIntoHorizontally =
        (centerX > (this.x + radius)) && ((this.x + this.width) > (centerX + radius));
    boolean circleFitsIntoVertically =
        (centerY > (this.y + radius)) && ((this.y + this.height) > (centerY + radius));
  ...
  public Optional<VPoint> getClosestIntersectionPoint(VPoint q1, VPoint q2, VPoint r) {
    ...
    for (VLine line : getLines()) {
      if (GeometryUtils.intersectLineSegment(line, q1, q2)) {
        VPoint tmpIntersectionPoint =
            GeometryUtils.lineIntersectionPoint(line, q1.getX(), q1.getY(), q2.getX(), q2.getY());
  ...
  public VRectangle scale(final double scalar) {
    return new VRectangle(
        getX() * scalar, getY() * scalar, getWidth() * scalar, getHeight() * scalar);
  }
  ...
  public List<VPoint> getPath() {
    return Arrays.asList(
        new VPoint(x, y),
        new VPoint(x + width, y),
        new VPoint(x + width, y + height),
        new VPoint(x, y + height));
  }
