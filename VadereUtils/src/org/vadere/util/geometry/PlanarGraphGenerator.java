...
/**
 * This class generates a planar graph based on a list of possibly intersecting and co-linear lines.
 * By line we mean line-segment. There are multiple cases for line intersection:
 *
 * <p>1. two lines intersect at one point which is not an endpoint of the line (general case) 2. two
 * lines are identical (the have the same end point) 3. two lines share infinitely many points but
 * are not identical 4. two lines share exactly one endpoint
 *
...
public class PlanarGraphGenerator {
  ...
  /**
   * Computes and returns non-intersecting lines. The computation is supported by a R-Tree data
   * structure such that we avoid to test all pairs of lines.
   *
  ...
  public Collection<VLine> generate() {
    ...
    do {
      ...
      for (VLine line1 : allLines) {
        ...
        RectangleDouble bufferedMbr =
            RectangleDouble.create(mbr.x1() - tol, mbr.y1() - tol, mbr.x2() + tol, mbr.y2() + tol);
  ...
  /**
   * Computes and returns non-intersecting lines. The computation is done via a bruteforce method by
   * testing each pair of lines which can be computational expensive.
   *
   * @return non-intersecting lines.
   */
  ...
  /**
   * Resolves the possible intersection of two lines and writes the resulting lines (at most 4) back
   * into the result list.
   *
   * @param line1 the first line
   * @param line2 the second line
   * @param result an empty list of lines
   * @return true if some lines changed (a line got deleted, split, moved), otherwise (no treatment
   *     required) false
   */
  private boolean adjustLines(
      @NotNull final VLine line1, @NotNull final VLine line2, @NotNull final List<VLine> result) {
   ...
   */
  private boolean handleIntersection(
      @NotNull final VLine line1, @NotNull final VLine line2, @NotNull final List<VLine> lines) {
   ...
   * @param line2 line2
   * @param lines an empty list of lines
   * @return true if the point is not an endpoint of line1, otherwise false because no treatment is
   *     required
   */
  private boolean handleCoLinearPoint(
      @NotNull final VLine line1,
      @NotNull final VPoint point,
      @NotNull final VLine line2,
      @NotNull final List<VLine> lines) {
    assert lines.isEmpty();
    if (equals(line1.getVPoint1(), point) || equals(line1.getVPoint2(), point)) {
      ...
      // VPoint pPoint = GeometryUtils.projectOntoLine(point.getX(), point.getY(), line1.x1,
      // line1.y1, line1.x2, line1.y2);
      VPoint pPoint = point;
   ...
   * @param lines an empty list of lines
   * @return false if the two lines share exactly one endpoint (no treatment required), true
   *     otherwise
   */
  private boolean handleCoLinearLines(
      @NotNull final VLine line1, @NotNull final VLine line2, @NotNull final List<VLine> lines) {
    assert lines.isEmpty();
  ...
  private boolean closeEndPoints(@NotNull final VLine line1, @NotNull final VLine line2) {
    return GeometryUtils.distanceToLineSegment(
                line1.getVPoint1(), line1.getVPoint2(), line2.getVPoint1())
            < tol
        || GeometryUtils.distanceToLineSegment(
                line1.getVPoint1(), line1.getVPoint2(), line2.getVPoint2())
            < tol
        || GeometryUtils.distanceToLineSegment(
                line2.getVPoint1(), line2.getVPoint2(), line1.getVPoint1())
            < tol
        || GeometryUtils.distanceToLineSegment(
                line2.getVPoint1(), line2.getVPoint2(), line1.getVPoint2())
            < tol;
  }
  ...
  private boolean equals(@NotNull final VLine line1, @NotNull final VLine line2) {
    return equals(line1.getVPoint1(), line2.getVPoint1())
            && equals(line1.getVPoint2(), line2.getVPoint2())
        || equals(line1.getVPoint1(), line2.getVPoint2())
            && equals(line1.getVPoint2(), line2.getVPoint1());
  }
