...
/**
 * A grid augmenting the position of generic objects, for faster access. O(1) instead of O(n) for
 * one fixed radius check. See {@link LinkedCellsGrid#getObjects(VPoint, double)}.
 */
public class LinkedCellsGrid<T extends PointPositioned> implements Iterable<T> {
  ...
  /**
   * One cell in the grid. It triangleContains a mapping from points to lists of objects. This means
   * that one can store multiple objects in one cell.
   *
  ...
  private class ContainerisedElement {
    ...
    public String toString() {
      return "ContainerisedElement{"
          ...
          + object
          + ", pos= "
          + ((PointPositioned) object).getPosition()
          + '}';
    }
  }
   ...
   * @param top y-position of top left corner // TODO is it not bottom left corner?
    ...
    int iX =
        (int)
            Math.max(
                0,
                Math.min(
                    this.gridSize[0] - 1, Math.floor((pos.x - left) / width * this.gridSize[0])));
    int iY =
        (int)
            Math.max(
                0,
                Math.min(
                    this.gridSize[1] - 1, Math.floor((pos.y - top) / height * this.gridSize[1])));
  ...
  /**
   * Adds a given object to the grid at position of the object. The position is discretized
   * automatically to fit in the cells.
   *
  ...
  /**
   * Removes the objects equal to the given object from the grid regardless of their position. Note
   * that this function has complexity O(N), with N = number of objects in the grid.
   *
   * @param object
   */
  ...
  /**
   * Tests whether the linked cells grid triangleContains an object that equals(the given object).
   * The complexity of this operation is O(N), N = number of objects in the grid.
   *
   * @param element
   * @return
   */
  ...
  public boolean equals(Object obj) {
    ...
    if (java.lang.Double.doubleToLongBits(cellSize[0])
        != java.lang.Double.doubleToLongBits(other.cellSize[0])) {
      return false;
    }
    ...
    if (java.lang.Double.doubleToLongBits(height)
        != java.lang.Double.doubleToLongBits(other.height)) {
      return false;
    }
    if (java.lang.Double.doubleToLongBits(left) != java.lang.Double.doubleToLongBits(other.left)) {
      return false;
    }
    if (java.lang.Double.doubleToLongBits(top) != java.lang.Double.doubleToLongBits(other.top)) {
      return false;
    }
    if (java.lang.Double.doubleToLongBits(width)
        != java.lang.Double.doubleToLongBits(other.width)) {
      return false;
    }
    return true;
  }
