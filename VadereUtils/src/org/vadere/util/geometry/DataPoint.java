...
/**
 * Point class representing one {@link VPoint} in 2D space with an additional double-valued data
 * store.
 *
 * <p>An order can be applied with the isGreaterThan method.
 */
public class DataPoint extends VPoint implements Comparable<VPoint> {

  private static Comparator<DataPoint> comparator =
      (d1, d2) -> {
  ...
  private static Comparator<DataPoint> pointComparator =
      (d1, d2) -> {
        if (Math.abs(d1.data - d2.data) < GeometryUtils.DOUBLE_EPS) {
          // compare coordinates
  ...
  /**
   * Creates a comparator for the data values. If the values agree, the Point.compareTo is called to
   * compare x,y values.
   *
   * @return
   */
