...
public class SimpleUniformIntegerPointProvider implements IPointProvider {
  ...
  public SimpleUniformIntegerPointProvider(final Random random, int xUpperBound, int yUpperBound) {
    xDist =
        new UniformIntegerDistribution(new JDKRandomGenerator(random.nextInt()), 0, xUpperBound);
    yDist =
        new UniformIntegerDistribution(new JDKRandomGenerator(random.nextInt()), 0, yUpperBound);
  }
