...
/** Derive a new point, uniformly selected within a given radius around the given based point. */
public class UniformRadialPointOffset implements IPointOffsetProvider {
  ...
  public UniformRadialPointOffset(final Random random, double maxOffsetRadius) {
    ...
    this.angleDist =
        new UniformRealDistribution(new JDKRandomGenerator(random.nextInt()), 0, 2 * Math.PI);
  }
