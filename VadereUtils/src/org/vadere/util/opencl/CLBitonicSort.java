 ...
 *     <p>This class implements the bitonic sort using the GPU via OpenCL.
 */
  ...
  public void sort(@NotNull final int[] keys, @NotNull final int[] values) throws OpenCLException {
    ...
    try (MemoryStack stack = stackPush()) {
      ...
      if (padKeys.length <= max_work_group_size) {
        ...
        CLInfo.checkCLError(
            clSetKernelArg(clBitonicSortLocal, 6, padKeys.length * 4)); // local memory
        CLInfo.checkCLError(
            clSetKernelArg(clBitonicSortLocal, 7, padKeys.length * 4)); // local memory
        ...
        CLInfo.checkCLError(
            clSetKernelArg(clBitonicSortLocal1, 4, max_work_group_size * 4)); // local memory
        CLInfo.checkCLError(
            clSetKernelArg(clBitonicSortLocal1, 5, max_work_group_size * 4)); // local memory
        ...
        for (int size = 2 * max_work_group_size; size <= padKeys.length; size <<= 1) {
          for (int stride = size / 2; stride > 0; stride >>= 1) {
            if (stride >= max_work_group_size) {
              ...
              CLInfo.checkCLError(
                  clSetKernelArg(clBitonicMergeLocal, 8, max_work_group_size * 4)); // local memory
              CLInfo.checkCLError(
                  clSetKernelArg(clBitonicMergeLocal, 9, max_work_group_size * 4)); // local memory
