...
public final class CLGLInteropDemo {
	...
	public static void main(String... args) {
		...
		Collections.sort(platforms, (p1, p2) -> {
			...
			return InfoUtils.getPlatformInfoStringUTF8(p1, CL_PLATFORM_VENDOR).compareTo(InfoUtils.getPlatformInfoStringUTF8(p2, CL_PLATFORM_VENDOR));
		});
		...
		if (platformCaps.cl_khr_icd) {
			platformID = InfoUtils.getPlatformInfoStringASCII(platform, CL_PLATFORM_ICD_SUFFIX_KHR); // less spammy
		} else {
			platformID = InfoUtils.getPlatformInfoStringUTF8(platform, CL_PLATFORM_VENDOR);
		}

		boolean hasCPU = false;
		boolean hasGPU = false;
		for (Long device : getDevices(platform, CL_DEVICE_TYPE_ALL)) {
			long type = InfoUtils.getDeviceInfoLong(device, CL_DEVICE_TYPE);
	...
	private static List<Long> getDevices(long platform, int deviceType) {
		...
		try (MemoryStack stack = stackPush()) {
			...
			if (errcode == CL_DEVICE_NOT_FOUND) {
				...
				InfoUtils.checkCLError(clGetDeviceIDs(platform, deviceType, deviceIDs, (IntBuffer)null));
