...
public class Mandelbrot {
	...
	static {
		try {
			source = IOUtil.ioResourceToByteBuffer("demo/Mandelbrot.cl", 4096);
		} catch (IOException e) {
		...
		while (!glfwWindowShouldClose(window.handle)) {
			...
			if (startTime > System.currentTimeMillis()) {
				...
				log(String.format(
						...
						InfoUtils.getPlatformInfoStringUTF8(platform, CL_PLATFORM_VENDOR),
						fps,
						fps / (timeUsed / 1000f)
				));
				fps = 0;
	...
	private void buildProgram() {
		...
		int errcode = clBuildProgram(clProgram, device, options, buildCallback = CLProgramCallback.create((program, user_data) -> {
			log(String.format(
					...
					program,
					InfoUtils.getProgramBuildInfoInt(program, device, CL_PROGRAM_BUILD_STATUS) == CL_SUCCESS ? "successfully" : "unsuccessfully"
			));
			String log = InfoUtils.getProgramBuildInfoStringASCII(program, device, CL_PROGRAM_BUILD_LOG);
			if (!log.isEmpty()) {
	...
	private void initGLObjects() {
		if (clTexture != NULL) {
			InfoUtils.checkCLError(clReleaseMemObject(clTexture));
