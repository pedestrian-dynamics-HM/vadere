...
public final class CLDemo {
  ...
  private static void demo(MemoryStack stack) {
    ...
    for (int p = 0; p < platforms.capacity(); p++) {
      ...
      for (int d = 0; d < devices.capacity(); d++) {
        ...
        System.out.printf("\n\t** NEW DEVICE: [0x%X]\n", device);
        System.out.println(
            "\tCL_DEVICE_TYPE = " + InfoUtils.getDeviceInfoLong(device, CL_DEVICE_TYPE));
        System.out.println(
            "\tCL_DEVICE_VENDOR_ID = " + InfoUtils.getDeviceInfoInt(device, CL_DEVICE_VENDOR_ID));
        System.out.println(
            "\tCL_DEVICE_MAX_COMPUTE_UNITS = "
                + InfoUtils.getDeviceInfoInt(device, CL_DEVICE_MAX_COMPUTE_UNITS));
        System.out.println(
            "\tCL_DEVICE_NATIVE_VECTOR_WIDTH_DOUBLE = "
                + InfoUtils.getDeviceInfoLong(device, CL_DEVICE_NATIVE_VECTOR_WIDTH_DOUBLE));
        ...
        System.out.println(
            "\tCL_DEVICE_MAX_WORK_ITEM_DIMENSIONS = "
                + InfoUtils.getDeviceInfoInt(device, CL_DEVICE_MAX_WORK_ITEM_DIMENSIONS));
        System.out.println(
            "\tCL_DEVICE_MAX_WORK_GROUP_SIZE = "
                + InfoUtils.getDeviceInfoPointer(device, CL_DEVICE_MAX_WORK_GROUP_SIZE));
        System.out.println(
            "\tCL_DEVICE_LOCAL_MEM_SIZE = "
                + InfoUtils.getDeviceInfoLong(device, CL_DEVICE_LOCAL_MEM_SIZE));
        System.out.println(
            "\tCL_DEVICE_MAX_CLOCK_FREQUENCY = "
                + InfoUtils.getDeviceInfoInt(device, CL_DEVICE_MAX_CLOCK_FREQUENCY));
        System.out.println(
            "\tCL_DEVICE_ADDRESS_BITS = "
                + InfoUtils.getDeviceInfoInt(device, CL_DEVICE_ADDRESS_BITS));
        System.out.println(
            "\tCL_DEVICE_AVAILABLE = "
                + (InfoUtils.getDeviceInfoInt(device, CL_DEVICE_AVAILABLE) != 0));
        System.out.println(
            "\tCL_DEVICE_COMPILER_AVAILABLE = "
                + (InfoUtils.getDeviceInfoInt(device, CL_DEVICE_COMPILER_AVAILABLE) != 0));
        System.out.println(
            "\tCL_DEVICE_VENDOR_ID = " + InfoUtils.getDeviceInfoInt(device, CL_DEVICE_VENDOR_ID));
  ...
  private static void printDeviceInfo(long device, String param_name, int param) {
    System.out.println(
        "\t" + param_name + " = " + InfoUtils.getDeviceInfoStringUTF8(device, param));
  }
