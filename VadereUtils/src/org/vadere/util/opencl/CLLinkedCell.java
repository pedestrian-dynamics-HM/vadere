...
public class CLLinkedCell extends CLOperation {
   ...
   */
  public CLLinkedCell(
      final int numberOfElements, final VRectangle bound, final double cellSize, final int device)
      throws OpenCLException {
    super(device);
   ...
   * @throws OpenCLException
   */
  public CLLinkedCell(final int numberOfElements, final VRectangle bound, final double cellSize)
      throws OpenCLException {
   ...
   * (reorderedPositions[cellStart[i]*2], reorderedPositions[cellStart[i]*2+1]) and
   * (reorderedPositions[(cellEnds[i]-1)*2], reorderedPositions[(cellEnds[i]-1)*2+1]).
   */
  public class LinkedCell {
    ...
    /**
     * the mapping between the unordered (original) positions and the reorderedPositions, i.e.
     * reorderedPositions[i] == positions[indices[i]]
     */
    public int[] indices;
  ...
  public LinkedCell calcLinkedCell(@NotNull final List<VPoint> positions) throws OpenCLException {
    ...
    return gridCells;
    // clBitonicSort(clHashes, clIndices, clHashes, clIndices, numberOfElements, 1);
    // clFindCellBoundsAndReorder(clCellStarts, clCellEnds, clReorderedPositions, clHashes,
    // clIndices, clPositions, numberOfElements, numberOfGridCells);
  }
  ...
  public int[] calcSortedHashes(@NotNull final List<VPoint> positions) throws OpenCLException {
    ...
    clCalcHash(
        clHashes, clIndices, clPositions, clCellSize, clWorldOrigin, clGridSize, numberOfElements);
    clBitonicSort(clHashes, clIndices, clHashes, clIndices, numberOfElements, 1);
    clEnqueueReadBuffer(clQueue, clHashes, true, 0, hashes, null, null);
    ...
    try (MemoryStack stack = stackPush()) {
      ...
      long maxWorkGroupSize =
          getMaxWorkGroupSizeForKernel(
              clDevice, clFindCellBoundsAndReorder, 4); // local 4 byte (integer)
    ...
    try (MemoryStack stack = stackPush()) {
      ...
      CLInfo.checkCLError(
          clGetKernelWorkGroupInfo(
              clBitonicMergeLocal, clDevice, CL_KERNEL_LOCAL_MEM_SIZE, rBitonicMergeLocal, null));

      LongBuffer rBitonicSortLocal1 = stack.mallocLong(1);
      CLInfo.checkCLError(
          clGetKernelWorkGroupInfo(
              clBitonicSortLocal1, clDevice, CL_KERNEL_LOCAL_MEM_SIZE, rBitonicSortLocal1, null));

      LongBuffer rBitonicSortLocal = stack.mallocLong(1);
      CLInfo.checkCLError(
          clGetKernelWorkGroupInfo(
              clBitonicSortLocal, clDevice, CL_KERNEL_LOCAL_MEM_SIZE, rBitonicSortLocal, null));

      return rBitonicMergeLocal.get() >= MIN_LOCAL_SIZE
          && rBitonicSortLocal1.get() >= MIN_LOCAL_SIZE
          && rBitonicSortLocal.get() >= MIN_LOCAL_SIZE;
    }
  }
  ...
  private void clBitonicSort(
    ...
    try (MemoryStack stack = stackPush()) {
      ...
      long maxWorkGroupSize =
          Math.min(Math.min(maxWorkGroupSize1, maxWorkGroupSize2), maxWorkGroupSize3);
  ...
  private long getMaxWorkGroupSizeForKernel(long clDevice, long clKernel, long workItemMem)
    ...
    try (MemoryStack stack = stackPush()) {
      ...
      logger.debug(
          "required memory for each work item = " + (workItemMem + kernelLocalMemory) + " byte");
      long maxWorkGroupSizeForLocalMemory =
          max_local_memory_size / (workItemMem + kernelLocalMemory);
