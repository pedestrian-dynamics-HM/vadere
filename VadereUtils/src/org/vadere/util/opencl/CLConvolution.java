...
/**
 * This class offers the convolution operation via OpenCL i.e. the use of the GPU to accelerate the
 * computation. Successive convolutions can be computed by reusing memory if the size of the
 * involved matrices do not change, that is, {@link CLConvolution#clearCL()} can be called after
 * multiple convolutions are computed.
 *
...
public class CLConvolution extends CLOperation {
   ...
   * @param kernel a 1D error representing the kernel (this can be a 1D kernel or a 2D kernel
   *     depending on the <tt>type</tt>)
  ...
  public CLConvolution(
      ...
      final int kernelWidth,
      @NotNull final float[] kernel)
      throws OpenCLException, UnsatisfiedLinkError {
    this.type = type;
  ...
  public void init() throws OpenCLException, BootstrapMethodError {
    ...
    switch (type) {
      case NonSeparate:
        clKernel = clKernelConvolve;
        break;
      case Col:
        clKernel = clKernelConvolveCol;
        break;
      case Row:
        clKernel = clKernelConvolveRow;
        break;
      case Separate:
        clKernel = -1;
        break;
      default:
        throw new IllegalArgumentException("unsupported kernel type = " + type);
    }
  ...
  /**
   * Executes the convolution operation this might be a 2D convolution realized by one 2D kernel or
   * by two 1D kernels or just one 1D convolution which depends on the constructor arguments of this
   * class.
   *
  ...
  public float[] convolve(final float[] input) throws OpenCLException {
    ...
    switch (type) {
      case NonSeparate:
        ...
        break;
      case Col:
        convolve(clKernelConvolveCol);
        break;
      case Row:
        convolve(clKernelConvolveRow);
        break;
      case Separate:
        convolveSeparate();
        break;
    ...
    try (MemoryStack stack = stackPush()) {
      ...
      CLInfo.checkCLError(
          clEnqueueNDRangeKernel(
              clQueue, clKernelConvolveCol, 2, null, clGlobalWorkSizeEdges, null, null, null));
      CLInfo.checkCLError(
          clEnqueueNDRangeKernel(
              clQueue, clKernelConvolveRow, 2, null, clGlobalWorkSizeEdges, null, null, null));
      clFinish(clQueue);
    }
  }
  ...
  private void convolve(final long clKernel) throws OpenCLException {
    try (MemoryStack stack = stackPush()) {
      ...
      CLInfo.checkCLError(
          clEnqueueNDRangeKernel(
              clQueue, clKernel, 2, null, clGlobalWorkSizeEdges, null, null, null));
  ...
  private void setArguments(final long clKernel) throws OpenCLException {
    try (MemoryStack stack = stackPush()) {
      ...
      clInput =
          clCreateBuffer(clContext, CL_MEM_READ_WRITE, 4 * matrixWidth * matrixHeight, errcode_ret);
      clOutput =
          clCreateBuffer(clContext, CL_MEM_WRITE_ONLY, 4 * matrixWidth * matrixHeight, errcode_ret);
      clGaussianKernel =
          clCreateBuffer(
              clContext,
  ...
  /**
   * Works like a C++ destructor, i.e. frees host and GPU / device memory. This has to be called if
   * no more convolutions are computed. Note that successive convolutions can be computed by reusing
   * memory if the size of the involved matrices do not change.
   *
   * @throws OpenCLException if there is any problem with OpenCL e.g. no OpenCL support
   */
