...
public class UniformRefinementTriangulatorSFC<P extends IPoint, V extends IVertex<P>, E extends IHalfEdge<P>, F extends IFace<P>> implements ITriangulator<P, V, E, F> {
	...
	/**
	 * Constructs the starting point of this algorithm:
	 * A triangulation containing 2 triangles which are the result
	 * of splitting a square which contains the bounding box.
	 *
	...
	/**
	 * Applies one iteration in the construction of the space filling curve i.e. the next level will be
	 * reached. If there are n candidates at most 2*n new candidates will be created:
	 *
	 * (1) update the SFC:  if the node k is a candidate which is not complete the node will be replaces in the
	 *                      SFC by two new nodes. The order of these two nodes and the directions depends on the
	 *                      direction of k.
	 *
	 * (2) refine edges:    After the SFC is updated we can refine edges. This won't destroy the edges of the nodes of
	 *                      the SFC.
	 *
	...
	/**
	 * There are two half-edges for each edge but we want to have only one half-edge for each edge
	 * for a split. This method helps do filter one of those edges.
	 *
	...
	/**
	 * Refines an edge, i.e. splits the edge at its midpoint if the midpoint is not already contained in
	 * the list of points. The triangulation notifies its listeners about this event.
	 *
	...
	/**
	 * Called after the splitting is completed. This method creates all holes, removes triangles outside the bounding box,
	 * inserts fix points and re-arranges the mesh data structure such that it is cache friendly.
	 */
	public void finish() {
	...
	/**
	 * Insert fix points. If there is a point p very close to the fix, instead of
	 * inserting the fix point, the point p will be moved to the position of the fix point
	 * and will be marked as fix point.
	 *
	 * @param fixPoints
	 */
	...
	/**
	 * Returns the lonngest edge of a face.
	 *
	 * @param face the face
	...
	/**
	 * Shrinks the border such that there are no more triangles outside the boundary i.e. where the distance is positive.
	 * Note the border is part of the whole boundary which is defined by the border and the holes.
	 */
	...
	/**
	 * Creates holes everywhere where the distance function is positive. Neighbouring holes will be merged.
	 */
