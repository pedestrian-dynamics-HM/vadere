...
public class TestIOUtils {
  ...
  public void testReadTableFormat1() {
    ...
    String[] headline =
        reader.readHeadLine(
            Stream.generate(() -> generateLine(dataFormat, headerFormat)).limit(401));
    lineNumber = 0;
    Table t =
        reader.readTable(
            Stream.generate(() -> generateLine(dataFormat, headerFormat)).limit(401), headline);

    Consumer<Integer> checkRow =
        i -> {
          ...
          assertEquals(
              "wrong data in the table, pos name", "s" + (i + 1) + "d", t.getEntry("name", i));
        };
  ...
  public void testReadTable() {
    ...
    Table t =
        reader.readTable(
            Stream.of(tableDataString1.split(System.lineSeparator())), false, "a", "b", "c", "d");
  ...
  private String generateLine(final String dataFormat, final String headerFormat) {
    ...
    if (lineNumber == 0) {
      result =
          new Formatter(Locale.ENGLISH).format(headerFormat, "x", "y", "id", "name").toString();
    } else {
      result =
          new Formatter(Locale.ENGLISH)
              .format(
                  dataFormat,
                  lineNumber * 10.0,
                  lineNumber * 3.0,
