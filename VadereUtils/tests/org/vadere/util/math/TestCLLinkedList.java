...
public class TestCLLinkedList {
  ...
  public void testCalcHashSmall() throws OpenCLException {
    ...
    // logger.info("number of cells = " + clUniformHashedGrid.getGridSize()[0] *
    // clUniformHashedGrid.getGridSize()[1]);
      ...
      int hash =
          getGridHash(
              getGridPosition(
                  ...
                  clUniformHashedGrid.getWorldOrign()),
              clUniformHashedGrid.getGridSize());
  ...
  private void testGridCell(final int deviceType, int size) throws OpenCLException {
    CLLinkedCell clUniformHashedGrid =
        new CLLinkedCell(size, new VRectangle(0, 0, 10, 10), 0.6, deviceType);
    ArrayList<VPoint> positions = new ArrayList<>();
    for (int i = 0; i < size; i++) {
      positions.add(new VPoint(0.5 + random.nextFloat() * 9, 0.5 + random.nextFloat() * 9));
    }
    ...
    for (int cell = 0; cell < numberOfCells; cell++) {
      ...
      for (int i = cellStart; i < cellEnd; i++) {
        VPoint point =
            new VPoint(
                gridCells.reorderedPositions[i * 2], gridCells.reorderedPositions[i * 2 + 1]);
        int[] gridPosition =
            getGridPosition(
                point, clUniformHashedGrid.getCellSize(), clUniformHashedGrid.getWorldOrign());
