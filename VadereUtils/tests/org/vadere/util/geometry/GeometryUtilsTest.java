...
public class GeometryUtilsTest {
  ...
  public void testDistanceToLineSegment() {
    ...
    assertEquals(
        Math.sqrt(1 * 1 + 4 * 4),
        GeometryUtils.distanceToLineSegment(p, q, 6, -1),
        GeometryUtils.DOUBLE_EPS);

    assertEquals(
        Math.sqrt(3 * 3 + 3 * 3),
        GeometryUtils.distanceToLineSegment(p1, p2, 4, 4),
        GeometryUtils.DOUBLE_EPS);
    assertEquals(
        Math.sqrt(4 * 4 + 4 * 4),
        GeometryUtils.distanceToLineSegment(p1, p2, -4, -4),
    ...
    assertEquals(
        ...
        GeometryUtils.distanceToLineSegment(p1, p2, 1, 0),
        GeometryUtils.DOUBLE_EPS);

    assertEquals(
        line1.distance(new VPoint(3.21, -123.123)),
        GeometryUtils.distanceToLineSegment(p, q, 3.21, -123.123),
        GeometryUtils.DOUBLE_EPS);
    assertEquals(
        line2.distance(new VPoint(313.21, 3.123)),
        GeometryUtils.distanceToLineSegment(p1, p2, 313.21, 3.123),
        GeometryUtils.DOUBLE_EPS);
  }
  ...
  public void testDistanceToLine() {
    ...
    assertEquals(
        Math.sqrt(0.5 * 0.5 + 0.5 * 0.5),
        GeometryUtils.distanceToLine(p1, p2, 1, 0),
        GeometryUtils.DOUBLE_EPS);
  }
  ...
  public void testHalfLineIntersect() {
    ...
    assertTrue(
        GeometryUtils.intersectHalfLineSegment(new VPoint(1, 0), new VPoint(0.5, 0.5), p1, p2));
    assertTrue(GeometryUtils.intersectHalfLineSegment(new VPoint(1, 0), new VPoint(1, 1), p1, p2));
    assertTrue(
        GeometryUtils.intersectHalfLineSegment(new VPoint(2, 1), new VPoint(-1, 31), p1, p2));
  }
  ...
  public void testCentroidOnTriangle() {
    ...
    assertTrue(
        "Centroid Center and CircumCircle Center not the same for Triangle",
        Math.abs(diff.x) < 0.001);
    assertTrue(
        "Centroid Center and CircumCircle Center not the same for Triangle",
        Math.abs(diff.y) < 0.001);
  }
  ...
  public void testCentroidOnRectangle() {
    ...
    assertTrue(
        "Centroid Center and CircumCircle Center not the same for VRectangle",
        Math.abs(diff.x) < 0.001);
    assertTrue(
        "Centroid Center and CircumCircle Center not the same for VRectangle",
        Math.abs(diff.y) < 0.001);
