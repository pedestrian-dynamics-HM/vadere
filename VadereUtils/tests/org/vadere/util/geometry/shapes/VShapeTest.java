...
public class VShapeTest {
  ...
  @Test(timeout = 1000) // ms
  /**
   * Error at magic number. With given Shapes in this test the AWT Area equals comparision will
   * start loop forever. Small changes (0.005 --> 0.008) will fix this. Problem occurs if the
   * contains check involves circles because they get transformed to awt Paths for the comparision
   * (bezier curve) which is probably the reason. Current fix introduces a new contains(VCircle
   * otherShape) overload in which specialized check should take place. Currently only VRectangle
   * implement this. Other shapes will still use the numerical unstable version.
   */
