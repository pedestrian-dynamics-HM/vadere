...
/**
 * Wrapper around a given TraCIValueSubscriptionCommand to execute the subscription todo: merge
 * multiple subscription so no unnecessary duplicates are send send
 */
public class Subscription {
  ...
  public void executeSubscription(RemoteManager remoteManager) {
    ...
    TraCISubscriptionResponse subResponse =
        new TraCISubscriptionResponse(
            new StatusResponse(valueSubscriptionCommand.getTraCICmd(), TraCIStatusResponse.OK, ""),
            responseIdentifier,
            valueSubscriptionCommand.getElementIdentifier(),
            valueSubscriptionCommand.getNumberOfVariables());
    ...
    for (TraCIGetCommand getCmd : valueSubscriptionCommand.getGetCommands()) {
      ...
      if (getResponse.getStatusResponse().getResponse().equals(TraCIStatusResponse.ERR)) {
        logger.warn(
            "Get command returned error: " + getResponse.getStatusResponse().getDescription());
        if (getResponse
            .getStatusResponse()
            .getDescription()
            .equals(CommandHandler.ELEMENT_ID_NOT_FOUND)) {
      ...
      subResponse.addVariableResponse(
          getResponse.getVariableIdentifier(),
  ...
  public String getSubscriptionId() {
    return "SubID_"
        + valueSubscriptionCommand.getTraCICmd().name()
        + "-"
        + valueSubscriptionCommand.getElementIdentifier();
  }
