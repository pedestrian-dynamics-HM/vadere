 ...
 *
 * <p>Within the {@link StateAccessHandler#execute(RemoteManager, SimulationState)} method the
 * {@link SimulationState} is save to access and change. Be really careful what you change!
 */
public class RemoteManager implements RunnableFinishedListener {
  ...
  public void loadScenario(String scenarioString, Map<String, ByteArrayInputStream> cacheData) {
    ...
    try {
      ...
      scenarioPath =
          defaultOutputdir
              .resolve(IOUtils.SCENARIO_DIR)
              .resolve(scenario.getName() + IOUtils.SCENARIO_FILE_EXTENSION);
    ...
    currentSimulationRun =
        new RemoteScenarioRun(scenario, outputDir, this, scenarioPath, scenarioCache);
  }
  ...
  private ScenarioCache buildScenarioCache(
      final Scenario scenario, Map<String, ByteArrayInputStream> cacheData) {
  ...
  public boolean accessState(StateAccessHandler stateAccessHandler) {
    if (currentSimulationRun == null) return false;
  ...
  public boolean nextStep(double simTime) {
    if (simulationFinished) return false;
  ...
  public void finished(Runnable runnable) {
    ...
    if (guiSupport) ServerView.close();
  }
  ...
  public void startSimulation() {
    ...
    if (currentSimulationThread != null && currentSimulationThread.isAlive())
      throw new TraCIExceptionInternal(
          "A simulation is already running. Stop current simulation before starting new one.");

    simulationFinished = false;
    currentSimulationThread = new Thread(currentSimulationRun);
    currentSimulationThread.setUncaughtExceptionHandler(
        (t, ex) -> {
    ...
    logger.infof(
        "Start Scenario %s with remote control...", currentSimulationRun.getScenario().getName());
