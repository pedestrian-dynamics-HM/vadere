...
public class PointConverter {
  ...
  public PointConverter(CompoundObject obj) {
    if (obj.size() != 2) throw new TraCIException("Expected 3 elements for PointConverter");
  ...
  public Pair<TraCIDataType, VPoint> convert(ReferenceCoordinateSystem coord) {
    ...
    switch (conversionType) {
      case POSITION_2D:
        ret =
            Pair.create(
                TraCIDataType.POS_2D, coord.convertToCartesian(p.y, p.x)); // LAT LONG axis order!!!
        break;
