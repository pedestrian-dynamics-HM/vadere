 ...
 *
 * <p>The class expects that the given buffer only contains *one* command. The command length filed
  ...
  public int readCmdIdentifier() {
    if (cmdIdentifierRead)
      throw new TraCIExceptionInternal(
          "TraCI Command Identifier already consumed. readCmdIdentifier() must only be called once. Something went wrong in the TraCI message handling.");
