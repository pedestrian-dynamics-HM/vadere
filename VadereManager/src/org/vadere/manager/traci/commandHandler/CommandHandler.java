 ...
 *
 * <p>See {@link CommandExecutor} on how commands are dispatched to the correct {@link
 * CommandHandler} subclass. These classes implement methods which adhere to the TraCICmdHandler
 * Interface. These methods are used by the {@link CommandExecutor} for dispatching.
 */
public abstract class CommandHandler<VAR extends Enum> {
  ...
  public static final String ELEMENT_ID_NOT_FREE =
      "There is already an element with the given object id ";
  public static final String FILE_NOT_FOUND = "No file with given path ";
  public static final String COULD_NOT_PARSE_OBJECT_FROM_JSON =
      "Could not parse object from given json ";
  public static final String COULD_NOT_MAP_OBJECT_FROM_JSON =
      "Could not map object from given json ";
  ...
  protected void init(
      Class<? extends Annotation> singleAnnotation, Class<? extends Annotation> multAnnotation) {
  ...
  protected TraCICommand invokeHandler(
      Method m, Object obj, TraCICommand cmd, RemoteManager manager) {
    try {
  ...
  public TraCICommand process_NotImplemented(TraCICommand cmd, RemoteManager remoteManager) {
    return cmd.setNOK_response(
        TraCIPacket.sendStatus(
            cmd.getTraCICmd(),
  ...
  public TraCIGetResponse responseOK(
      TraCIDataType responseDataType,
      Object responseData,
      TraCICmd apiCmd,
      TraCICmd apiCmdResponse) {
    TraCIGetResponse res =
        new TraCIGetResponse(
            new StatusResponse(apiCmd, TraCIStatusResponse.OK, ""), apiCmdResponse);
  ...
  public TraCIGetResponse responseERR(String err, TraCICmd apiCmd, TraCICmd apiCmdResponse) {
    return new TraCIGetResponse(
        new StatusResponse(apiCmd, TraCIStatusResponse.ERR, err), apiCmdResponse);
  }
