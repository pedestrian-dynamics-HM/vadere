...
public class CommandExecutor {
  ...
  public CommandExecutor(RemoteManager remoteManager) {
    ...
    cmdMap.put(
        TraCICmd.SUB_SIMULATION_VALUE.id, SimulationCommandHandler.instance::processValueSub);
    ...
    cmdMap.put(
        TraCICmd.SET_VEHICLE_STATE.id,
        (cmd, manager) -> ((TraCISetCommand) cmd).setOK()); // todo just say ok but do nothing
  }
  ...
  public TraCIPacket execute(TraCICommand cmd) {
    ...
    try {
      ...
      String err =
          String.format("error building response for: %s", cmd.getTraCICmd().logShort(varId));
      logger.errorf(err);
      e.printStackTrace();
      return TraCIPacket.createErr(cmd.getTraCICmd().id, err + "see Vadere log for details");
    }
