...
public class VadereCommandHandler extends CommandHandler<VadereVar> {
  ...
  public TraCIGetResponse responseOK(TraCIDataType responseDataType, Object responseData) {
    return responseOK(
        responseDataType,
        responseData,
        TraCICmd.GET_VADERE_VALUE,
        TraCICmd.RESPONSE_GET_VADERE_VALUE);
  }
  ...
  @VadereHandler(
    cmd = TraCICmd.SET_VADERE_STATE,
    var = VadereVar.ADD_TARGET_CHANGER,
    dataTypeStr = "String",
    name = "createTargetChanger",
    ...
    try {
      atc =
          (AttributesTargetChanger)
              StateJsonConverter.deserializeScenarioElementType(
                  data, ScenarioElementType.TARGET_CHANGER);
      remoteManager.accessState(
          (manager, state) -> {
            // todo[TargetChanger] the creation of a TargetChanger with controller is complex and
            // bad to read, can a pattern be applied?
            TargetChanger tc = new TargetChanger(atc);
            // todo[random]: use Random object from context for now. This should be replaced by the
            // meta seed.
            VadereContext ctx = VadereContext.getCtx(state.getTopography());
            Random rnd = (Random) ctx.get("random");
            TargetChangerController tcc =
                new TargetChangerController(state.getTopography(), tc, rnd);
  ...
  @VadereHandler(
    cmd = TraCICmd.SET_VADERE_STATE,
    var = VadereVar.ADD_STIMULUS_INFOS,
    dataTypeStr = "String",
    name = "addStimulusInfos",
    ignoreElementId = true
  )
  ...
  @VadereHandler(
    cmd = TraCICmd.GET_VADERE_VALUE,
    var = VadereVar.GET_ALL_STIMULUS_INFOS,
    name = "getAllStimulusInfos",
    ignoreElementId = true
  )
  public TraCICommand process_getAllStimulusInfos(
    ...
    remoteManager.accessState(
        (manager, state) -> {
          ...
          List<StimulusInfo> lsi =
              ListUtils.union(sic.getRecurringStimuli(), sic.getOneTimeStimuli());
  ...
  @VadereHandler(
    cmd = TraCICmd.SET_VADERE_STATE,
    var = VadereVar.REMOVE_TARGET_CHANGER,
    name = "removeTargetChanger"
  )
  public TraCICommand process_removeTargetChanger(
      TraCISetCommand cmd, RemoteManager remoteManager) {
    remoteManager.accessState(
        (manager, state) -> {
          LinkedList<TargetChanger> lltc =
              (LinkedList<TargetChanger>) state.getTopography().getTargetChangers();
          if (lltc.remove(
              lltc.stream()
                  .filter(p -> p.getId() == Integer.parseInt(cmd.getElementId()))
                  .findFirst()
                  .orElse(null))) cmd.setOK();
          else cmd.setErr("ID not found.");
        });
  ...
  public TraCICommand processValueSub(TraCICommand rawCmd, RemoteManager remoteManager) {
    return processValueSub(
        ...
        this::processGet,
        TraCICmd.GET_VADERE_VALUE,
        TraCICmd.RESPONSE_SUB_VADERE_VALUE);
  }
