...
public class PersonCommandHandler extends CommandHandler<PersonVar> {
    ...
    remoteManager.accessState(
        (manager, state) -> {
          ...
          if (checkIfPedestrianExists(ped, cmd)) {
            ...
            cmd.setResponse(
                responseOK(PersonVar.POSITION3D.type, new Vector3D(pos2d.x, pos2d.y, 0.0)));
            logger.debugf(
                "time: %f Pedestrian: %s Position: %s",
                state.getSimTimeInSec(), cmd.getElementIdentifier(), ped.getPosition().toString());
          }
        });
    return cmd;
  }
  ...
  public TraCICommand process_getMaximumSpeed(TraCIGetCommand cmd, RemoteManager rmeoteManager) {
    rmeoteManager.accessState(
        (manager, state) -> {
          ...
          if (checkIfPedestrianExists(ped, cmd)) {
            cmd.setResponse(
                responseOK(PersonVar.MAXSPEED.type, ped.getAttributes().getMaximumSpeed()));
          }
        });
    return cmd;
  }
  ...
  public TraCICommand process_setStimulus(TraCISetCommand cmd, RemoteManager remoteManager) {
    ...
    // tmp.stream().map(Integer::parseInt).collect(Collectors.toCollection(LinkedList::new));
    remoteManager.accessState(
        (manager, state) -> {
          Pedestrian ped =
              state
              ...
              if (checkIfMainModelIsPresent(state, cmd)) {
                Pedestrian oldPed =
                    state
                        .getTopography()
                        .getPedestrianDynamicElements()
                        .getElement(Integer.parseInt(idList.get(0)));
                Pedestrian newDynamicElement =
                    (Pedestrian)
                        state
                            .getMainModel()
                            .get()
                            .createElement(
                                generalPed.getPosition(), generalPed.getId(), oldPed.getClass());
  ...
  public TraCICommand processValueSub(TraCICommand rawCmd, RemoteManager remoteManager) {
    return processValueSub(
        ...
        this::processGet,
        TraCICmd.GET_PERSON_VALUE,
        TraCICmd.RESPONSE_SUB_PERSON_VARIABLE);
  }
