...
public class ByteArrayOutputStreamTraCIWriter implements TraCIWriter {
  ...
  private TraCIWriter writeString(String val, Charset c) {
    ...
    if (byteString.length > 0) writeBytes(byteString);
    return this;
  }
  ...
  public TraCIWriter writePolygon(List<VPoint> points) {
    ...
    if (points.size() > 255)
      throw new TraCIException(
          "Polygon to big. " + "TraCI only supports polygon up to 255 points.");
  ...
  public TraCIWriter writeTrafficLightPhaseList(List<TrafficLightPhase> phases) {
    ...
    if (phases.size() > 255)
      throw new TraCIException(
          "Traffic Light Phase List to big. " + "TraCI only supports list up to 255 elements.");
  ...
  /**
   * Check if the given cmdLen fits into a single byte. If not use the extended cmdLen format which
   * nulls the first byte and introduces a int field for the cmdLen.
   *
