...
public class TraCIPacket extends ByteArrayOutputStreamTraCIWriter {
  ...
  public static TraCIPacket createErr(int cmdIdentifier, String description) {
    return new TraCIPacket()
        .addEmptyLengthField()
        .add_Err_StatusResponse(cmdIdentifier, description);
  }
  ...
  public static TraCIPacket sendStatus(
      TraCICmd cmd, TraCIStatusResponse status, String description) {
  ...
  private void throwIfFinalized() {
    if (finalized) throw new TraCIExceptionInternal("Cannot change finalized TraCIPacket");
  }

  private TraCIPacket addEmptyLengthField() {
    if (emptyLengthField) throw new TraCIExceptionInternal("Should only be called at most once.");
    writeInt(-1);
  ...
  public TraCIPacket wrapCommand(
    ...
    cmdBuilder
        .writeUnsignedByte(commandIdentifier.id)
  ...
  public TraCIPacket wrapGetResponse(TraCIGetResponse res) {
    ...
    TraCIWriter cmdBuilder = getCmdBuilder();
    cmdBuilder
        .writeUnsignedByte(res.getResponseIdentifier().id)
  ...
  private void wrapSubscription(TraCISubscriptionResponse res) {
    ...
    cmdBuilder
        .writeUnsignedByte(
            res.getResponseIdentifier().id) // (i.e. TraCICmd.RESPONSE_SUB_PERSON_VARIABLE)
    ...
    res.getResponses()
        .forEach(
            var -> {
              cmdBuilder
                  .writeUnsignedByte(var.getVariableId())
  ...
  public TraCIPacket wrapGetVersionCommand(TraCIGetVersionCommand cmd) {
    ...
    if (res.isOKResponseStatus()) add_OK_StatusResponse(cmd.getTraCICmd());
    else addStatusResponse(res.getStatusResponse());
  ...
  public TraCIPacket addStatusResponse(
      int cmdIdentifier, TraCIStatusResponse response, String description) {
    throwIfFinalized();
