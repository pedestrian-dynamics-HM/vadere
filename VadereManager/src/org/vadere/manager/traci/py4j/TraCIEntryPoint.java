...
public class TraCIEntryPoint implements Runnable {
  ...
  public static void main(String[] args) throws IOException, InterruptedException {
    ...
    try {
      ...
      //			logger.infof("Start Server(%s) with Loglevel: %s",
      // VadereServer.currentVersion.getVersionString(), logger.getLevel().toString());
      //			Thread serverThread = new Thread(new VadereServer(serverSocket, pool,
      // Paths.get(ns.getString("output-dir")), ns.getBoolean("guiMode")));
      //			serverThread.start();
      TraCIEntryPoint entryPoint =
          new TraCIEntryPoint(
              ns.getInt("port"),
              ns.getString("bind"),
              ns.getInt("javaPort"),
              ns.getInt("pythonPort"));
  ...
  private static ArgumentParser createArgumentParser() {
    ArgumentParser parser =
        ArgumentParsers.newArgumentParser("Vadere Server")
            .defaultHelp(true)
  ...
  private synchronized void handleConnection() throws IOException, InterruptedException {
    Runtime.getRuntime()
        .addShutdownHook(
            new Thread(
                () -> {
    ...
    try {
      ...
      GatewayServer gatewayServer =
          new GatewayServer.GatewayServerBuilder(this)
              .javaPort(javaPort)
      ...
      if (traCISocket != null) traCISocket.close();
    }
  }
