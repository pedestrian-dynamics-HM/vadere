...
public class Manager {
  ...
  public static void main(String[] args) {
    ...
    try {
      ...
      ServerSocket serverSocket =
          new ServerSocket(ns.getInt("port"), 50, InetAddress.getByName(ns.getString("bind")));
      logger.infof(
          "Start Server(%s) with Loglevel: %s",
          VadereServer.currentVersion.getVersionString(), logger.getLevel().toString());
      AbstractVadereServer server;
      if (ns.getBoolean("singleClient")) {
        server =
            new VadereSingleClientServer(
                serverSocket,
                Paths.get(ns.getString("output-dir")),
                ns.getBoolean("guiMode"),
                ns.getBoolean("trace"),
  ...
  private static void addOptionsToParser(ArgumentParser parser) {
    ...
    // no action required call to  Logger.setMainArguments(args) already configured Logger.
    parser
        .addArgument("--clientNum")
        .required(false)
        .type(Integer.class)
        .setDefault(4)
        .dest("clientNum")
        .help(
            "Set number of clients to manager. Important: Each client has a separate simulation. No communication between clients");
    ...
    parser
        ...
        .dest("trace")
        .help(
            "Activate additional TRACE information in low level components. Ensure correct --loglevel setting to see additional information");

    parser
        .addArgument("--single-client")
        .required(false)
        .action(Arguments.storeTrue())
        .type(Boolean.class)
        .dest("singleClient")
    ...
    parser
        .addArgument("--gui-mode")
        .required(false)
        .action(Arguments.storeTrue())
        .type(Boolean.class)
        .dest("guiMode")
        .help(
            "Start server with GUI support. If a scenario is received show the current state of the scenario");
    ...
    parser
        ...
        .type(String.class)
        .help(
            "Supply path to scenario. This will start this scenario and waits for first simstep command");
  }
}
