...
public class ControlCommandHandlerTest extends CommandHandlerTest {
  ...
  private void process_close_(SimThreadState simThreadState) {
    ...
    assertThat(
        closeCommand.getResponse().getStatusResponse().getResponse(),
  ...
  public void process_simStep() {
    ...
    TraCISimStepCommand rawCmd =
        (TraCISimStepCommand) getFirstCommand(TraCISimStepCommand.build(targetTime));
    RemoteManager rm = mock(RemoteManager.class, Mockito.RETURNS_DEEP_STUBS);
    ...
    assertThat(
        simStepCommand.getResponse().getStatusResponse().getResponse(),
        equalTo(TraCIStatusResponse.OK));
  ...
  public void process_getVersion() {
    ...
    TraCIGetVersionCommand rawCmd =
        (TraCIGetVersionCommand) getFirstCommand(TraCIGetVersionCommand.build());
    RemoteManager rm =
  ...
  public void process_load_file() {
    ...
    TraCISendFileCommand rawCmd =
        (TraCISendFileCommand)
            getFirstCommand(TraCISendFileCommand.TraCISendFileCommand(fileName, file));
    RemoteManager rm = mock(RemoteManager.class, Mockito.RETURNS_DEEP_STUBS);
