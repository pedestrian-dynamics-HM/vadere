...
public class ClientAnnotationProcessor extends AbstractProcessor {
  ...
  public boolean process(Set<? extends TypeElement> annotations, RoundEnvironment roundEnv) {
    ...
    for (TypeElement annotation : annotations) {
      ...
      try {
        FileObject file =
            processingEnv.getFiler().createResource(StandardLocation.CLASS_OUTPUT, "", fileName);
    ...
    try {
      ...
      if (element == null) writeAbstractTestClient();
    } catch (IOException e) {
      e.printStackTrace();
    }
    return true;
  ...
  protected void writeAbstractTestClient() throws IOException {
    JavaFileObject jFile =
        processingEnv.getFiler().createSourceFile("org.vadere.manager.client.AbstractTestClient");
    try (PrintWriter writer = new PrintWriter(jFile.openWriter())) {
      ...
      writer.println();
      writer
          .append("\tpublic void init(TraCISocket socket, ConsoleReader consoleReader){")
          .println();
  ...
  private void writePythonBinding(Element apiClass, StringBuilder pythonConstants)
      throws IOException {
    ...
    pythonConstants
        .append(traCIApi.cmdGetVarName)
        .append(" = 0x")
        .append(Integer.toHexString(traCIApi.cmdGet))
        .append("\n");
    pythonConstants
        .append(traCIApi.cmdResponseGetVarName)
        .append(" = 0x")
        .append(Integer.toHexString(traCIApi.cmdResponseGet))
        .append("\n");
    pythonConstants
        .append(traCIApi.cmdSetVarName)
        .append(" = 0x")
        .append(Integer.toHexString(traCIApi.cmdSet))
        .append("\n");
    pythonConstants
        .append(traCIApi.cmdSubVarName)
        .append(" = 0x")
        .append(Integer.toHexString(traCIApi.cmdSub))
        .append("\n");
    pythonConstants
        .append(traCIApi.cmdResponseSubVarName)
        .append(" = 0x")
        .append(Integer.toHexString(traCIApi.cmdResponseSub))
        .append("\n");
    pythonConstants
        .append(traCIApi.cmdCtxVarName)
        .append(" = 0x")
        .append(Integer.toHexString(traCIApi.cmdCtx))
        .append("\n");
    pythonConstants
        .append(traCIApi.cmdResponseCtxVarName)
        .append(" = 0x")
        .append(Integer.toHexString(traCIApi.cmdResponseCtx))
        .append("\n\n");
    ...
    try (PrintWriter w = new PrintWriter(file.openWriter())) {
      ...
      w.append(t(2))
          .append("Domain.__init__(self, ")
          .append(qq(traCIApi.domainName))
          .append(",")
          .append("tc.")
          .append(traCIApi.cmdGetVarName)
          .append(", tc.")
        ...
        if (anMirrors != null) {
          for (AnnotationMirror anMirror : anMirrors) {
            ...
            String singeAn =
                traCIApi
                    .singleAnnotation
                    .substring(traCIApi.singleAnnotation.lastIndexOf(".") + 1)
                    .trim();
  ...
  private void writePythonGET(PrintWriter w, ApiHandler apiHandler) {
    if (apiHandler.dataTypeStr.isEmpty()) {
      ...
      if (apiHandler.ignoreElementId) {
        ...
        w.append(t(2))
            .append("return self._getUniversal(")
            .append(apiHandler.varIdPython)
            .append(", ")
            .append(qq(""))
            .append(")")
            .println();
        ...
        w.append(t(1))
            .append("def ")
            .append(apiHandler.namePython)
            .append("(self, element_id):")
            .println();
        w.append(t(2))
            .append("return self._getUniversal(")
            .append(apiHandler.varIdPython)
      ...
      if (apiHandler.ignoreElementId) {
        ...
        w.append(t(1))
            .append("def ")
            .append(apiHandler.namePython)
            .append("(self, element_id, data):")
            .println();
        w.append(t(2))
            .append("return self._getUniversal(")
            .append(apiHandler.varIdPython)
            .append(", element_id, data)")
            .println();
      }
    }
    w.println();
  ...
  protected void writeApiClass(Element apiClass) throws IOException {
    ...
    JavaFileObject jFile =
        processingEnv.getFiler().createSourceFile(traCIApi.packageName + '.' + traCIApi.name);
    ...
    try (PrintWriter writer = new PrintWriter(jFile.openWriter())) {
      ...
      writer
          .append("public class ")
          .append(traCIApi.name)
          .append(" extends ")
          .append(traCIApi.extendedClassName)
          .append(" {")
          .println();
            ...
            if (anName.equals(singeAn)) {
              ...
              apiMapping.append(
                  String.format(
                      ...
                      traCIApi.nameShort.toLowerCase(),
                      apiHandler.name,
                      traCIApi.name.toLowerCase(),
                      apiHandler.name));
              apiAbstract.append(
                  String.format(
  ...
  protected void writeGET(PrintWriter writer, ApiHandler apiHandler) {
    if (apiHandler.dataTypeStr.isEmpty()) {
      ...
      if (apiHandler.ignoreElementId) {
        writer
            .append("\tpublic TraCIResponse ")
            .append(apiHandler.name)
            .append("() throws IOException {")
            .println();
        writer
            .append("\t\tTraCIPacket p = TraCIGetCommand.build(")
            .append(apiHandler.cmd)
            .append(", ")
            .append(apiHandler.varId)
            .append(", \"-1\");")
            .println();
      } else {
        writer
            .append("\tpublic TraCIResponse ")
            .append(apiHandler.name)
            .append("(String elementID) throws IOException {")
            .println();
        writer
            .append("\t\tTraCIPacket p = TraCIGetCommand.build(")
            .append(apiHandler.cmd)
      ...
      if (apiHandler.ignoreElementId) {
        writer
            ...
            .append(apiHandler.name)
            .append("(")
            .append(apiHandler.dataTypeStr)
            .append(" data) throws IOException {")
            .println();
        writer
            .append("\t\tTraCIPacket p = TraCIGetCommand.build(")
            .append(apiHandler.cmd)
  ...
  protected void writeSET(PrintWriter writer, ApiHandler apiHandler) {
    if (apiHandler.ignoreElementId) {
      ...
      writer
          .append("\t\tTraCIPacket p = TraCISetCommand.build(")
          .append(apiHandler.cmd)
          .append(", \"-1\", ")
          .append(apiHandler.varId)
          .append(", ")
  ...
  class TraCiApiWrapper {
    ...
    TraCiApiWrapper(Element apiClass) {
      ...
      packageName =
          StringUtils.isNotBlank(traCIApi.packageName())
