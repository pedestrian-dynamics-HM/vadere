...
public class ModelFactoryProcessor extends AbstractFactoryProcessor {
  ...
  protected void addLastConstructor(Set<? extends Element> elements, PrintWriter writer) {
    ...
    for (TypeElement e : mainModels) {
      writer
          .append("		mainModels.put(")
          .append(quote(e.getQualifiedName().toString()))
          .append(", ")
  ...
  protected void writeFactory(Set<? extends Element> elements) throws IOException {
    JavaFileObject jFile =
        processingEnv.getFiler().createSourceFile(factoryPackage + "." + factoryClassName);
  ...
  /**
   * Filter Models based on {@link ModelClass#isMainModel()} and group them based on their package
   * name.
   */
