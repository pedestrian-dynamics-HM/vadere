...
public class TestIFIMGrid {
  ...
  public void setUp() throws Exception {
    cellGrid =
        new CellGrid(
            ...
            -0.9,
            -0.9);
    // FloorDiscretizer.setGridValuesForShape(cellGrid, targetShape, new CellState(0.0,
    // PathFindingTag.Target));
    cellGrid.setValue(
        new Point((cellGrid.getNumPointsX() / 2), (cellGrid.getNumPointsY()) / 2),
        new CellState(0.0, PathFindingTag.Target));
    distFunc = p -> -p.distanceToOrigin();
    timeCostFunction =
        p ->
            1.0 / (0.8 * Math.sin(2 * Math.PI * p.getX()) * Math.sin(2 * Math.PI * p.getY()) + 1.0);
  }
  ...
  public void TestFMM() {
    ...
    EikonalSolver eikonalSolverFMM =
        new EikonalSolverFMM(cellGrid, distFunc, false, timeCostFunction, unknownPenalty, weight);
  ...
  public void TestFIM() {
    ...
    EikonalSolver eikonalSolverFIM =
        new EikonalSolverFIM(cellGrid, distFunc, false, timeCostFunction, unknownPenalty, weight);
  ...
  public void TestIFIM() {
    double ms = System.currentTimeMillis();
    EikonalSolver eikonalSolverFIM =
        new EikonalSolverIFIM(cellGrid, distFunc, timeCostFunction, unknownPenalty, weight);
    eikonalSolverFIM.solve();
    double runtimeInMs = System.currentTimeMillis() - ms;
