 ...
 *     <p>Comparing the performance of the fast marching method FMM and the simpliefied fast
 *     marchning mehtod SFMM.
 */
public class PerformanceSFMM {

  private static final List<VShape> obstacles =
      Arrays.asList(
    ...
    public void doSetup() {
      ...
      distanceFunction =
          p -> {
            VPoint dp =
                new VPoint(
                    cellGrid.getWidth() / (cellGrid.getNumPointsX() - 1) / 2.0,
      ...
      for (VShape obstacle : obstacles) {
        FloorDiscretizer.setGridValuesForShape(
            cellGrid, obstacle, new CellState(Double.MAX_VALUE, PathFindingTag.Obstacle));
      }
    }
  }
  ...
  public double testFMM(PerformanceSFMM.StateCellGrid stateCellGrid) {
    EikonalSolverFMM ffm =
        new EikonalSolverFMM(
            stateCellGrid.cellGrid,
            stateCellGrid.distanceFunction,
            true,
  ...
  public double testSFMM(PerformanceSFMM.StateCellGrid stateCellGrid) {
    EikonalSolverSFMM ffm =
        new EikonalSolverSFMM(
            stateCellGrid.cellGrid,
            stateCellGrid.distanceFunction,
            true,
            new UnitTimeCostFunction(),
