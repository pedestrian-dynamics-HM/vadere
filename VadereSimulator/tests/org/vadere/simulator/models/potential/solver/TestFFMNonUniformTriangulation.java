...
public class TestFFMNonUniformTriangulation {
  ...
  @Before
  public void setUp() {
    // IDistanceFunction distanceFunc = p -> Math.abs(7 - Math.sqrt(p.getX() * p.getX() + p.getY() *
    // p.getY())) - 3;
    distanceFunc = p -> Math.abs(6 - Math.sqrt(p.getX() * p.getX() + p.getY() * p.getY())) - 4;

    // distanceFunc = p -> -10+Math.sqrt(p.getX() * p.getX() + p.getY() * p.getY());
    // IDistanceFunction distanceFunc = p -> Math.abs(7 - Math.max(Math.abs(p.getX()),
    // Math.abs(p.getY()))) - 3;
  ...
  private GenEikMesh<PVertex, PHalfEdge, PFace> createEikMesh(
      @NotNull final IEdgeLengthFunction edgeLengthFunc, final double initialEdgeLen) {
    IMeshSupplier<PVertex, PHalfEdge, PFace> meshSupplier = () -> new PMesh();
    GenEikMesh<PVertex, PHalfEdge, PFace> eikMesh =
        new GenEikMesh<>(
            distanceFunc, edgeLengthFunc, initialEdgeLen, bbox, new ArrayList<>(), meshSupplier);
  ...
  public void testTriangulationFMM() {
    ...
    // IEdgeLengthFunction edgeLengthFunc = p -> 1.0 + 0.5*Math.min(Math.abs(distanceFunc.apply(p) +
    // 4), Math.abs(distanceFunc.apply(p)));
    IEdgeLengthFunction edgeLengthFunc = p -> 1.0;
    ...
    try {
      ...
      FileWriter potentialFieldWriter =
          new FileWriter("./output/" + timestamp.getTime() + "potentialField_adapt_0_7.csv");
      ...
      for (double y = bbox.getMinY() + 2; y <= bbox.getMaxY() - 2; y += 0.1) {
        for (double x = bbox.getMinX() + 2; x < bbox.getMaxX() - 2; x += 0.1) {
          ...
          if (val >= 0.0 && val < Double.MAX_VALUE) {
            double side =
                Math.min(
                    (new VPoint(x, y).distanceToOrigin() - 2.0),
                    (10 - new VPoint(x, y).distanceToOrigin()));
    ...
    log.info(
        "max edge length: "
            + triangulation
                .getMesh()
                .streamEdges()
                .map(e -> triangulation.getMesh().toLine(e).length())
    ...
    log.info(
        "max distance to boundary: "
            + triangulation
                ...
                .getBoundaryVertices()
                .stream()
                .map(p -> Math.abs(distanceFunc.apply(p)))
                .max(Comparator.comparingDouble(d -> d)));
                    ...
                    p ->
                        Math.abs(
                            Math.abs(
                                triangulation
                                        .getMesh()
                                        .getDoubleData(p, MeshEikonalSolverFMM.namePotential)
                                    + distanceFunc.apply(p))))
                .max(Comparator.comparingDouble(d -> d)));
                                ...
                                Math.abs(
                                    Math.abs(
                                        triangulation
                                                .getMesh()
                                                .getDoubleData(
                                                    p, MeshEikonalSolverFMM.namePotential)
                                            + distanceFunc.apply(p))))
                        .map(val -> val * val)
                        .reduce(0.0, (d1, d2) -> d1 + d2)
                    / triangulation.getMesh().getNumberOfVertices()));
  ...
  public void testTriangulationFMMCase3() {
    ...
    // IEdgeLengthFunction edgeLengthFunc = p -> 1.0 + Math.min(Math.abs(distanceFunc.apply(p) + 4)
    // * 2, Math.abs(distanceFunc.apply(p)) * 2);
  ...
  private double computeL2Error(
    ...
    for (PVertex vertex : triangulation.getMesh().getVertices()) {
      double diff =
          triangulation.getMesh().getDoubleData(vertex, MeshEikonalSolverFMM.namePotential)
              + distanceFunc.apply(vertex);
      sum += diff * diff;
    }
  ...
  public void testRegularFMM() {
    ...
    CellGrid cellGrid =
        new CellGrid(
            bbox.getWidth() - 3, bbox.getHeight() - 3, resolution, new CellState(), xMin, yMin);
    cellGrid
        .pointStream()
    ...
    EikonalSolver solver =
        new org.vadere.simulator.models.potential.solver.calculators.cartesian.EikonalSolverFMM(
            cellGrid, distanceFunc, false, new UnitTimeCostFunction(), 0.1, 1.0);
    ...
    try {
      ...
      p.toFile().mkdirs();
      FileWriter writer =
          new FileWriter(p.resolve("./potentialField_reg_0_4.csv").toAbsolutePath().toString());
    ...
    log.info("max error: " + maxError);
    Stream<Point> resultPoints =
        cellGrid.pointStream().filter(p -> cellGrid.getValue(p).tag != PathFindingTag.Obstacle);
        ...
        "max error-2: "
            + resultPoints
                .map(
                    p ->
                        Math.abs(
                            cellGrid.getValue(p).potential
                                - distanceFunc.apply(cellGrid.pointToCoord(p))))
                .max(Comparator.comparingDouble(d -> d)));
    log.info("L2-error: " + Math.sqrt(sum / counter));
  ...
  public void testRegularFMMCase2() {
    ...
    cellGrid
        ...
        .forEach(
            p -> cellGrid.setValue(p, new CellState(Double.MAX_VALUE, PathFindingTag.Obstacle)));
