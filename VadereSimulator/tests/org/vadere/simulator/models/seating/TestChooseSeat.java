...
public class TestChooseSeat {
  ...
  public void testChooseSeat0() {
    ...
    final Map<SeatPosition, Double> probabilities =
        FractionProbabilityNormalization.normalize(new AttributesSeating().getSeatChoice0());

    for (int i = 0; i < tallySheet.getKeys().size(); i++) {
      Seat s = seatGroup.getSeat(i);
      assertEquals(
          probabilities.get(getSeatPosition(s)), (double) tallySheet.getCount(s) / nTrials, 0.05);
    }
  }

  private SeatPosition getSeatPosition(Seat seat) {
    for (SeatPosition pos : SeatPosition.values())
      if (seatGroup.getSeatByPosition(pos) == seat) return pos;
  ...
  public void testChooseSeat1() {
    ...
    for (int i = 0; i < 4; i++) {
      ...
      Map<SeatRelativePosition, Double> map =
          FractionProbabilityNormalization.normalize(new AttributesSeating().getSeatChoice1());
      assertEquals(
          map.get(SeatRelativePosition.DIAGONAL),
          (double) tallySheet.getCount(diagonallyOppositeSeat) / nTrials,
          0.05);
    }
  }
