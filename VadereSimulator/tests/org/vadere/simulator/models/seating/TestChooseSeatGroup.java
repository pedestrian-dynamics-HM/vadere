...
public class TestChooseSeatGroup {
  ...
  public void testChooseSeatGroupBetween0123() {
    ...
    List<Integer> sortedKeys = tallySheet.getKeys().stream().sorted().collect(Collectors.toList());
  ...
  private double[] getSeatGroupPersonCountProbabilities() {
    final Map<Boolean, Double> probabilities =
        FractionProbabilityNormalization.normalize(new AttributesSeating().getSeatGroupChoice());
