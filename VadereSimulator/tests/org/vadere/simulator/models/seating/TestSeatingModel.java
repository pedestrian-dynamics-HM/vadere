...
public class TestSeatingModel {
  ...
  public void testInitializeWithWrongTopography() {
    new SeatingModel()
        .initialize(attributes, new Domain(new Topography()), attributesPedestrian, random);
  }

  @Test(expected = AttributesNotFoundException.class)
  public void testInitializeWithNoAttributes() {
    new SeatingModel()
        .initialize(new ArrayList<>(0), new Domain(topography), attributesPedestrian, random);
  }
