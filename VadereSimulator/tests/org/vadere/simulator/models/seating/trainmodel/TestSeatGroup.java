...
public class TestSeatGroup {
  ...
  private void testRelativePosition(int actual, int seat, SeatRelativePosition relativePosition) {
    assertEquals(
        aSeatGroup.getSeat(actual),
        aSeatGroup.seatRelativeTo(aSeatGroup.getSeat(seat), relativePosition));
  }
  ...
  public void testAvailableSeatAtFacingDirection() {
    ...
    sitDownTestPerson(2);
    assertEquals(
        aSeatGroup.getSeat(3),
        aSeatGroup.availableSeatAtFacingDirection(SeatFacingDirection.FORWARD));

    clearTestSeats();
    sitDownTestPerson(3);
    assertEquals(
        aSeatGroup.getSeat(2),
        aSeatGroup.availableSeatAtFacingDirection(SeatFacingDirection.FORWARD));

    clearTestSeats();
    sitDownTestPerson(1);
    assertEquals(
        aSeatGroup.getSeat(0),
        aSeatGroup.availableSeatAtFacingDirection(SeatFacingDirection.BACKWARD));

    clearTestSeats();
    sitDownTestPerson(0);
    assertEquals(
        aSeatGroup.getSeat(1),
        aSeatGroup.availableSeatAtFacingDirection(SeatFacingDirection.BACKWARD));
  }
  ...
  public void testGetSeatByPosition() {
    ...
    assertEquals(
        rightSeatGroup.getSeat(0), rightSeatGroup.getSeatByPosition(SeatPosition.AISLE_BACKWARD));
    assertEquals(
        rightSeatGroup.getSeat(1), rightSeatGroup.getSeatByPosition(SeatPosition.WINDOW_BACKWARD));
    assertEquals(
        rightSeatGroup.getSeat(2), rightSeatGroup.getSeatByPosition(SeatPosition.AISLE_FORWARD));
    assertEquals(
        rightSeatGroup.getSeat(3), rightSeatGroup.getSeatByPosition(SeatPosition.WINDOW_FORWARD));
  }

  private void testAvailableSeat(
      SeatGroup seatGroup, SeatSide side, int personSeatIndex, int expectedSeatIndex) {
