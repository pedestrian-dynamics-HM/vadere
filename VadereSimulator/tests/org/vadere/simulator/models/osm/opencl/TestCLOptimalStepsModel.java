...
public class TestCLOptimalStepsModel {
  ...
  public void setUp() throws IOException {
    ...
    obstacleDistancePotential =
        new PotentialFieldDistanceEikonalEq(
            topography
                .getObstacles()
                .stream()
    ...
    targetPotentialField =
        new PotentialFieldSingleTargetGrid(
            new Domain(topography), attributesAgent, attributesFloorField, 1);
    targetPotentialField.preLoop(0.4f);
    pedestrians = new ArrayList<>();

    for (int i = 0; i < numberOfElements - 1; i++) {
      VPoint randomPosition =
          new VPoint(
              (float) (bound.getMinX() + random.nextDouble() * bound.getWidth()),
              (float) (bound.getMinY() + random.nextDouble() * bound.getHeight()));
      CLParallelOptimalStepsModel.PedestrianOpenCL pedestrian =
          new CLParallelOptimalStepsModel.PedestrianOpenCL(randomPosition, maxStepSize);
      pedestrians.add(pedestrian);
    }

    CLParallelOptimalStepsModel.PedestrianOpenCL lastPedestrian =
        pedestrians.get(pedestrians.size() - 1);

    CLParallelOptimalStepsModel.PedestrianOpenCL pedestrian =
        new CLParallelOptimalStepsModel.PedestrianOpenCL(
            lastPedestrian.position.add(new VPoint(0.01, 0.1)), maxStepSize);
    pedestrians.add(pedestrian);
  }
  ...
  public void testIdentity() throws OpenCLException {
    CLParallelOptimalStepsModel clOptimalStepsModel =
        new CLParallelOptimalStepsModel(
            attributesOSM,
            attributesFloorField,
