...
public class TestOutputFile {
    ...
    try {
      String json =
          IOUtils.readTextFile(
              new File(getClass().getResource("/data/basic_1_chicken_osm1.scenario").toURI())
                  .getAbsolutePath());
      ...
      MainModelBuilder modelBuilder =
          new MainModelBuilder(testScenario.getScenarioStore(), null, null);
  ...
  public void testFileFormatAfterMultipleSimulationRuns() {
    ...
    ProcessorManager manager =
        testScenario.getDataProcessingJsonManager().createProcessorManager(mainModel, topography);
  ...
  public void testHandlingNameConflict() {
    ...
    List<String> header =
        testScenario.getDataProcessingJsonManager().getOutputFiles().get(0).getEntireHeader();
