  ...
  public void loadDefaultSimulationStateMocks() {
    ...
    addSimState(
        new SimulationStateMock(1) {
          @Override
          public void mockIt() {
            b.clear().add(1, new VPoint(1.0, 1.0)).add(2, new VPoint(3.5, 2.5));
            when(state.getTopography().getElements(Pedestrian.class)).thenReturn(b.getList());
            when(state.getScenarioStore().getAttributesSimulation().getSimTimeStepLength())
                .thenReturn(0.4);
          }
        });
    ...
    addSimState(
        new SimulationStateMock(2) {
          @Override
          public void mockIt() {
            b.clear().add(1, new VPoint(2.0, 2.0)).add(2, new VPoint(3.5, 3.5));
            when(state.getTopography().getElements(Pedestrian.class)).thenReturn(b.getList());
            when(state.getScenarioStore().getAttributesSimulation().getSimTimeStepLength())
                .thenReturn(0.4);
          }
    ...
    addSimState(
        new SimulationStateMock(3) {
          @Override
          public void mockIt() {
            b.clear().add(1, new VPoint(3.0, 3.0)).add(2, new VPoint(3.5, 4.5));
            when(state.getTopography().getElements(Pedestrian.class)).thenReturn(b.getList());
            when(state.getScenarioStore().getAttributesSimulation().getSimTimeStepLength())
                .thenReturn(0.4);
          }
