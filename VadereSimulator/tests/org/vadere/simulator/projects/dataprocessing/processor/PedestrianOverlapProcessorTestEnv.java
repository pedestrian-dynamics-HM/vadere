...
public class PedestrianOverlapProcessorTestEnv
    extends ProcessorTestEnv<TimestepPedestrianIdOverlapKey, OverlapData> {
    ...
    addSimState(
        new SimulationStateMock(1) {
          ...
          public void mockIt() {
            ...
            Mockito.when(state.getTopography().getSpatialMap(DynamicElement.class))
                .thenReturn(getCellGridMock(b));
            Mockito.when(state.getTopography().getAttributesPedestrian().getRadius())
                .thenReturn(attributesAgent.getRadius());
          }
        });
    ...
    addSimState(
        new SimulationStateMock(2) {
          ...
          public void mockIt() {
            ...
            addToExpectedOutput(
                new TimestepPedestrianIdOverlapKey(step, 1, 5), b.overlapData(1, 5, minDist));
            addToExpectedOutput(
                new TimestepPedestrianIdOverlapKey(step, 5, 1), b.overlapData(5, 1, minDist));
            addToExpectedOutput(
                new TimestepPedestrianIdOverlapKey(step, 1, 3), b.overlapData(1, 3, minDist));
            addToExpectedOutput(
                new TimestepPedestrianIdOverlapKey(step, 3, 1), b.overlapData(3, 1, minDist));
          }
        });

    addSimState(
        new SimulationStateMock(3) {
          @Override
          public void mockIt() {
            b.clear().add(1, new VPoint(1.0, 1.0));
            ...
            int step = state.getStep();
            addToExpectedOutput(
                new TimestepPedestrianIdOverlapKey(step, 1, 2), b.overlapData(1, 2, minDist));
            addToExpectedOutput(
                new TimestepPedestrianIdOverlapKey(step, 1, 3), b.overlapData(1, 3, minDist));
            addToExpectedOutput(
                new TimestepPedestrianIdOverlapKey(step, 1, 4), b.overlapData(1, 4, minDist));

            addToExpectedOutput(
                new TimestepPedestrianIdOverlapKey(step, 2, 1), b.overlapData(2, 1, minDist));
            addToExpectedOutput(
                new TimestepPedestrianIdOverlapKey(step, 2, 3), b.overlapData(2, 3, minDist));
            addToExpectedOutput(
                new TimestepPedestrianIdOverlapKey(step, 2, 4), b.overlapData(2, 4, minDist));

            addToExpectedOutput(
                new TimestepPedestrianIdOverlapKey(step, 3, 1), b.overlapData(3, 1, minDist));
            addToExpectedOutput(
                new TimestepPedestrianIdOverlapKey(step, 3, 2), b.overlapData(3, 2, minDist));
            addToExpectedOutput(
                new TimestepPedestrianIdOverlapKey(step, 3, 4), b.overlapData(3, 4, minDist));

            addToExpectedOutput(
                new TimestepPedestrianIdOverlapKey(step, 4, 1), b.overlapData(4, 1, minDist));
            addToExpectedOutput(
                new TimestepPedestrianIdOverlapKey(step, 4, 2), b.overlapData(4, 2, minDist));
            addToExpectedOutput(
                new TimestepPedestrianIdOverlapKey(step, 4, 3), b.overlapData(4, 3, minDist));
          }
        });
  }
  ...
  void verySmallOverlapping() {
    ...
    addToExpectedOutput(
        new TimestepPedestrianIdOverlapKey(step, 4, 3), b.overlapData(4, 3, minDist));
    addToExpectedOutput(
        new TimestepPedestrianIdOverlapKey(step, 5, 6), b.overlapData(5, 6, minDist));
    addToExpectedOutput(
        new TimestepPedestrianIdOverlapKey(step, 6, 5), b.overlapData(6, 5, minDist));
  }
