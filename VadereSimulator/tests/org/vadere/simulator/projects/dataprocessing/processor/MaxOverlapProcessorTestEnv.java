...
public class MaxOverlapProcessorTestEnv extends ProcessorTestEnv<NoDataKey, Double> {
  ...
  void initializeDependencies() {
    ...
    int pedestrianOverlapProcessorId =
        addDependentProcessor(PedestrianOverlapProcessorTestEnv::new);
    AttributesMaxOverlapProcessor attr =
        (AttributesMaxOverlapProcessor) testedProcessor.getAttributes();
    ...
    addSimState(
        new SimulationStateMock(2) {
          ...
          public void mockIt() {
            ...
            double maxDist = 0;
            if (dist1.getOverlap() > maxDist) maxDist = dist1.getOverlap();

            if (dist2.getOverlap() > maxDist) maxDist = dist2.getOverlap();

            if (dist3.getOverlap() > maxDist) maxDist = dist3.getOverlap();

            if (dist4.getOverlap() > maxDist) maxDist = dist4.getOverlap();
