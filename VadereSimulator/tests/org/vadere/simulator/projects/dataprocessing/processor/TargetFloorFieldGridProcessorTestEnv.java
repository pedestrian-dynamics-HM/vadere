...
public class TargetFloorFieldGridProcessorTestEnv
    extends ProcessorTestEnv<TimestepRowKey, FloorFieldGridRow> {
    ...
    addSimState(
        new SimulationStateMock(1) {
          ...
          public void mockIt() {
            ...
            when(state.getTopography().getBounds())
                .thenReturn(new Rectangle2D.Double(0.0, 0.0, 3.0, 2.0));
            when(pft.needsUpdate()).thenReturn(false);
            b.clear().add(1);
            when(state.getTopography().getPedestrianDynamicElements().getElements())
