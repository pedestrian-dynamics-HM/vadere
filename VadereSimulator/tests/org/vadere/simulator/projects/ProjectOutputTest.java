...
public class ProjectOutputTest implements TestResourceHandlerScenario {
  ...
  public void listSelectedOutputDirs() {
    ...
    assertEquals("The directory name is false", "testOutput2", selectedOutputDirs.get(0).getName());
  }
  ...
  public void updateWithNew() throws IOException, URISyntaxException {
    Path backup =
        Paths.get(getClass().getResource("/data/simpleProject/output/testOutput2").toURI());
    FileUtils.copyDirectory(
        backup.toFile(), project.getOutputDir().resolve("testOutput3").toFile());
