...
public class ThreatCognitionModelTest {
  ...
  private void movePedestrianAlongXaxis(
      Pedestrian pedestrian, int totalFootsteps, double stepSize) {
  ...
  public void updateSetsLatestThreatUnhandledFalsIfSameThreatOcurred() {
    ...
    for (int i = 0; i < 5; i++) {
      ...
      // First occurrence is a new threat. Subsequent occurrences are no old threats (and must not
      // be handled).
  ...
  @Test
  public void
      updateSetsOutGroupMembersToTargetOrientedUponElapsedTimeStimulusIfNoThreatOccurredBefore() {
    initializeTopography();
