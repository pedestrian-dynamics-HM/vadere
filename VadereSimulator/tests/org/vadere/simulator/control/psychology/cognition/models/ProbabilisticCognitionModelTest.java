...
public class ProbabilisticCognitionModelTest {
  ...
  public void initializeReactionBehavior() {
    ...
    AttributesProbabilisticCognitionModel probModelAttr =
        new AttributesProbabilisticCognitionModel();
  ...
  @NotNull
  private AttributesRouteChoiceDefinition getRouteChoiceDefinition(
      int targetId1, int targetId2, double prob1, double prob2, String instruction) {
  ...
  public void updateSetsElapsedTimeIfNoOtherStimulusIsPresent() {
    ...
    LinkedList<Integer> targetIds =
        probabilisticPerceptionModel.getFilteredAttributes("A").getTargetIds();
    LinkedList<Double> targetProbs =
        probabilisticPerceptionModel.getFilteredAttributes("A").getTargetProbabilities();
    ...
    double prob1Is =
        1.0
            * pedestrians
                .stream()
                .filter(ped -> ped.getTargets().getFirst() == targetIds.get(0))
                .count()
            / sampleSize;
    double prob2Is =
        1.0
            * pedestrians
                .stream()
                .filter(ped -> ped.getTargets().getFirst() == targetIds.get(1))
                .count()
            / sampleSize;
