...
public class TexGraphGenerator extends org.vadere.meshing.utils.io.tex.TexGraphGenerator {
  ...
  public static <V extends IVertex, E extends IHalfEdge, F extends IFace> String toTikz(
    ...
    String cObstacle =
        "{" + obsColor.getRed() + "," + obsColor.getGreen() + "," + obsColor.getBlue() + "}";
    String cTarget =
        "{"
            + targetColor.getRed()
            + ","
    ...
    for (Pedestrian pedestrian : topography.getElements(Pedestrian.class)) {
      builder.append(
          "\\filldraw[color=black,fill=cPedestrian]("
              + pedestrian.getPosition().getX()
              + ","
  ...
  private static String toTikz(
      @NotNull final ScenarioElement element, @NotNull final String tikzColor) {
    StringBuilder builder = new StringBuilder();
