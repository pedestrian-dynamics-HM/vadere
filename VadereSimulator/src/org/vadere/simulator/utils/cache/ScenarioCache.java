...
public class ScenarioCache {
  ...
  private Path resolveCachePath(Path cacheParentDir, String scenarioFileConfig) {
    ...
    if (scenarioConfig.isAbsolute()) {
      logger.infof(
          "Cache path overwritten: Scenario specified absolute cache path. %s",
      ...
      if (attFF != null) {
        ...
        this.cachePath =
            resolveCachePath(
                cacheParentDir,
                attFF
                    .getCacheDir()); // cacheParentDir.resolve(CACHE_DIR_NAME).resolve(attFF.getCacheDir());
        empty = !attFF.isUseCachedFloorField(); // deactivate cache object if caching is not active.
        if (!empty) findCacheOnFileSystem();
  ...
  private void addCacheByType(
      EikonalSolverType eikType, CacheType cacheType, String cacheIdentifier) {
  ...
  public ScenarioCache addReadOnlyCache(String cacheIdentifier, ByteArrayInputStream stream) {
    if (empty) throw new IllegalStateException("Empty cache object.");
    switch (attFF.getCacheType()) {
      case BIN_CACHE:
        cacheMap.put(
            cacheIdentifier,
            new CellGridBinaryCacheObject(
                cacheIdentifier, null, new DataInputStream(new FastBufferedInputStream(stream))));
