 ...
 *
 * <ul>
 *   <li>to define the cognitive state of an agent or
 *   <li>to maneuver and agent through the {@link org.vadere.state.scenario.Topography}
 * </ul>
 *
...
public class TopographyHelper {
  ...
  public static Target findClosestTargetToSource(
      Topography topography, ScenarioElement source, Threat threat) {
  ...
  public static List<Pedestrian> getNeighborsWithSelfCategory(
      Pedestrian pedestrian, SelfCategory expectedSelfCategory, Topography topography) {
    VPoint positionOfPedestrian = pedestrian.getPosition();

    List<Pedestrian> closestPedestrians =
        topography
  ...
  public static Pedestrian getNeighborCloserToTargetCentroid(
      Pedestrian pedestrian, Topography topography) {
    VPoint positionOfPedestrian = pedestrian.getPosition();
    ...
    if (targetList.size() > 0) {
      ...
      closestPedestrians =
          closestPedestrians
              ...
              .filter(
                  closestPed ->
                      closestPed.getPosition().distance(targetCentroid)
                          < pedestrian.getPosition().distance(targetCentroid))
              .collect(Collectors.toList());
  ...
  @NotNull
  public static List<Pedestrian> getNeighborsCloserToTarget(
      PedestrianOSM pedestrian, Topography topography) {
    VPoint positionOfPedestrian = pedestrian.getPosition();
    ...
    closestPedestrians =
        closestPedestrians
            ...
            .filter(
                candidate ->
                    pedestrian.getTargetPotential(candidate.getPosition())
  ...
  public static boolean otherPedestrianIsCloserToTarget(
      Topography topography, PedestrianBHM me, Pedestrian other) {
    ...
    if (me.isPotentialFieldInUse()) { // Use geodetic distance
      ...
      double otherDistanceToTarget =
          me.getPotentialFieldTarget().getPotential(other.getPosition(), other);
  ...
  public static boolean pedestrianIsBlockedByObstacle(
      Pedestrian pedestrian, Topography topography) {
    ...
    if (footstepHistory.size() >= requiredFootSteps) {
      if (footstepHistory.getAverageSpeedInMeterPerSecond() <= requiredSpeedToBeBlocked) {
        // Watch out: This is probably a very expensive call but Gerta suggests to include it to get
  ...
  public static boolean walkingDirectionDiffers(
      Pedestrian pedestrian1, Pedestrian pedestrian2, Topography topography) {
    boolean directionIsDifferent = false;

    double angleInRadian =
        calculateAngleBetweenWalkingDirections(pedestrian1, pedestrian2, topography);
    if (angleInRadian == -1
        || Math.toDegrees(angleInRadian)
            > pedestrian1.getAttributes().getWalkingDirectionSameIfAngleLessOrEqual()) {
  ...
  public static double calculateAngleBetweenWalkingDirections(
      Pedestrian pedestrian1, Pedestrian pedestrian2, Topography topography) {
      ...
      case BY_GRADIENT:
        angleInRadian =
            calculateAngleBetweenTargetGradients(
                (PedestrianOSM) pedestrian1, (PedestrianOSM) pedestrian2);
        break;
  ...
  public static double calculateAngleBetweenTargetGradients(
      PedestrianOSM pedestrian1, PedestrianOSM pedestrian2) {
    double angleInRadian = -1;
    ...
    double multipliedMagnitudes =
        targetGradientPedestrian1.distanceToOrigin() * targetGradientPedestrian2.distanceToOrigin();
  ...
  /**
   * Calculate the angle3D between the two vectors v1 and v2 where v1 = (TargetPedestrian1 -
   * pedestrian1) and v2 = (TargetPedestrian2 - pedestrian2):
   *
   * <pre>
   *     T2 o   o T1
   *        ^   ^
   ...
   * </pre>
   *
   * This is required to decide if pedestrian1 and pedestrian2 can be swapped because they have
   * different walking directions.
   *
   * @return An angle3D between 0 and <i>pi</i> radian or -1 if at least one of the given
   *     pedestrians has no target.
   */
  public static double calculateAngleBetweenTargets(
      Pedestrian pedestrian1, Pedestrian pedestrian2, Topography topography) {
    double angleInRadian = -1;
    ...
    if (pedestrian1.hasNextTarget() && pedestrian2.hasNextTarget()) {
      ...
      double multipliedMagnitudes =
          targetVectorPed1.distanceToOrigin() * targetVectorPed2.distanceToOrigin();
  ...
  public static VPoint calculateVectorPedestrianToTarget(Pedestrian pedestrian, Target target) {
    ...
    if (pedestrian.getAttributes().getWalkingDirectionCalculation()
    ...
    } else if (pedestrian.getAttributes().getWalkingDirectionCalculation()
      ...
      throw new IllegalArgumentException(
          String.format(
              "Unsupported angle3D calculation type: \"%s\"",
              pedestrian.getAttributes().getWalkingDirectionCalculation()));
    }
