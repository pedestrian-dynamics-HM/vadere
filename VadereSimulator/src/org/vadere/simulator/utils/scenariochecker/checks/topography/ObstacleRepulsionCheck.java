...
public class ObstacleRepulsionCheck extends AbstractScenarioCheck implements TopographyCheckerTest {
  // minimal corridor width at  |gradient(target potential)| < |gradient(wall repulsion)| -> agents
  // might get stuck
  ...
  public PriorityQueue<ScenarioCheckerMessage> runScenarioCheckerTest(Topography topography) {
    ...
    while (shapes.size() > 1) {
      ...
      List<Double> distances =
          shapes.stream().map(shape -> shape.distance(vShape)).collect(Collectors.toList());
      distances =
          distances
              .stream()
              .map(
        ...
        if (distMin < threshold) {
          ...
          ret.add(
              msgBuilder
                  .topographyWarning()
                  .target(
                      topography.getObstacles().get(counter),
                      topography.getObstacles().get(counter + index))
                  .reason(ScenarioCheckerReason.NARROW_BOTTLENECK, msg)
                  .build());
        }
        index++;
      }
