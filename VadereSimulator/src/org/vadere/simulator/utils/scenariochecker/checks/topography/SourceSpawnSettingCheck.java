...
public class SourceSpawnSettingCheck extends AbstractScenarioCheck
      ...
      if (attr == null) {
        messages.add(
            msgBuilder
                .topographyError()
                .target(source)
                .reason(ScenarioCheckerReason.SOURCE_NEEDS_SPAWNER)
                .build());
      } else {
        if (attr.isEventPositionRandom() && !attr.isEventPositionFreeSpace()) {
          messages.add(
              msgBuilder
                  .topographyWarning()
                  .target(source)
                  .reason(ScenarioCheckerReason.SOURCE_SPAWN_RND_POS_NOT_FREE_SPACE)
                  .build());
          ...
          messages.add(
              msgBuilder
                  .topographyWarning()
                  .target(source)
                  .reason(ScenarioCheckerReason.SOURCE_SPAWN_USE_NOT_FREE_SPACE)
                  .build());
        }
      }
    }
