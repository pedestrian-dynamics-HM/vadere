...
public class SimulationTimeStepLengthCheck extends AbstractScenarioCheck {
  ...
  private static final double LOW_BOUND = 0.01;
  private static final double HIGH_BOUND = 1.0;
    ...
    if (simTimeStep <= LOW_BOUND || simTimeStep >= HIGH_BOUND) {
      ...
      ret.add(
          msgBuilder
              .simulationAttrError()
              .reason(
                  ScenarioCheckerReason.SIM_TIME_STEP_WRONG,
                  String.format(
                      " [%3.1f - %3.1f] current value: %3.1f", LOW_BOUND, HIGH_BOUND, simTimeStep))
              .build());
    }
    return ret;
  }
