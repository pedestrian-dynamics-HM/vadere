...
public class PedestrianSpeedSetupCheck extends AbstractScenarioCheck
    implements TopographyCheckerTest {
  @Override
    ...
    topography
        ...
        .forEach(
            p -> {
              ...
              if (speedMean < p.getAttributes().getMinimumSpeed()
                  || speedMean > p.getAttributes().getMaximumSpeed()) {
                ret.add(
                    msgBuilder
                        .topographyError()
                        .target(p)
                        .reason(
                            ScenarioCheckerReason.PEDESTRIAN_SPEED_SETUP,
                            String.format(
              ...
              if (p.getAttributes().getMinimumSpeed()
                      > Pedestrian.PEDESTRIAN_MAX_SPEED_METER_PER_SECOND
                  || p.getAttributes().getMaximumSpeed()
                      > Pedestrian.PEDESTRIAN_MAX_SPEED_METER_PER_SECOND) {
                ret.add(
                    msgBuilder
                        ...
                        .target(p)
                        .reason(
                            ScenarioCheckerReason.PEDESTRIAN_SPEED_NOT_LOGICAL,
                            String.format(
                                "[max: %.2f min: %.2f threshold: %.2f]",
                                p.getAttributes().getMinimumSpeed(),
                                p.getAttributes().getMaximumSpeed(),
                                Pedestrian.PEDESTRIAN_MAX_SPEED_METER_PER_SECOND))
                        .build());
              }
              if (p.getAttributes().getMinimumSpeed() < 0
                  || p.getAttributes().getMaximumSpeed() < 0) {
                ret.add(
                    msgBuilder
                        .topographyError()
                        .target(p)
