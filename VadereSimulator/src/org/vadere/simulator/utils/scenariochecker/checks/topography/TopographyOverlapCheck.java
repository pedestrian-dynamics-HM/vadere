...
public class TopographyOverlapCheck extends AbstractScenarioCheck implements TopographyCheckerTest {
      ...
      for (int j = i + 1; j < all.size(); j++) {
        ...
        if (isTestActive(
          ...
          handelObstacleTargetOverlap(elementA, elementB, ret);
            ...
            elementB,
            ScenarioElementType.OBSTACLE,
            ScenarioElementType.STAIRS)) {

          handelObstacleStairsOverlap(elementA, elementB, ret);

        } else if (isTestActive(
            useTest, elementA, elementB, ScenarioElementType.SOURCE, ScenarioElementType.SOURCE)) {

          handelSourceSourceOverlap(elementA, elementB, ret);

        } else if (isTestActive(
            useTest, elementA, elementB, ScenarioElementType.SOURCE, ScenarioElementType.TARGET)) {

          handelSourceTargetOverlap(elementA, elementB, ret);

        } else if (isTestActive(
            useTest, elementA, elementB, ScenarioElementType.SOURCE, ScenarioElementType.STAIRS)) {

          handelSourceStairsOverlap(elementA, elementB, ret);

        } else if (isTestActive(
            useTest, elementA, elementB, ScenarioElementType.TARGET, ScenarioElementType.TARGET)) {

          handelTargetTargetOverlap(elementA, elementB, ret);

        } else if (isTestActive(
            useTest, elementA, elementB, ScenarioElementType.TARGET, ScenarioElementType.STAIRS)) {

          handelTargetStairsOverlap(elementA, elementB, ret);

        } else if (isTestActive(
            useTest, elementA, elementB, ScenarioElementType.STAIRS, ScenarioElementType.STAIRS)) {
  ...
  private boolean isTestActive(
      BiFunction<ScenarioElementType, ScenarioElementType, Boolean> checkTest,
      ScenarioElement eA,
      ScenarioElement eB,
      ScenarioElementType type1,
      ScenarioElementType type2) {
  ...
  private ScenarioElement elementOfType(
      ScenarioElement e1, ScenarioElement e2, ScenarioElementType type) {
    return e1.getType().equals(type) ? e1 : e2;
  }

  private void handelObstacleObstacleOverlap(
      ScenarioElement e1, ScenarioElement e2, PriorityQueue<ScenarioCheckerMessage> ret) {
    ...
    if (e1.enclosesScenarioElement(e2)) {
      ret.add(
          msgBuilder
              .topographyWarning()
              .target(e1, e2)
              .reason(ScenarioCheckerReason.OVERLAP_OBSTACLE_OBSTACLE)
              .build());
  ...
  private void handelObstacleSourceOverlap(
      ScenarioElement e1, ScenarioElement e2, PriorityQueue<ScenarioCheckerMessage> ret) {
    ...
    if (source.overlapWith(obstacle)) {
      ret.add(
          msgBuilder
              ...
              .reason(ScenarioCheckerReason.OVERLAP_OBSTACLE_SOURCE)
              .build());
    }
  }

  private void handelObstacleTargetOverlap(
      ScenarioElement e1, ScenarioElement e2, PriorityQueue<ScenarioCheckerMessage> ret) {
    ...
    if (obstacle.enclosesScenarioElement(target)) {
      ret.add(
          msgBuilder
              ...
              .reason(ScenarioCheckerReason.OVERLAP_OBSTACLE_TARGET_ERR)
              .build());
      ...
      ret.add(
          msgBuilder
              .topographyWarning()
              .target(obstacle, target)
              .reason(ScenarioCheckerReason.OVERLAP_OBSTACLE_TARGET_WARN)
              .build());
    }
  }

  private void handelObstacleStairsOverlap(
      ScenarioElement e1, ScenarioElement e2, PriorityQueue<ScenarioCheckerMessage> ret) {
    ...
    if (obstacle.enclosesScenarioElement(stairs)) {
      ret.add(
          msgBuilder
              .topographyError()
              .target(obstacle, stairs)
              .reason(ScenarioCheckerReason.OVERLAP_OBSTACLE_STAIRS_ERR)
              .build());
      ...
      ret.add(
          msgBuilder
              .topographyWarning()
              .target(obstacle, stairs)
              .reason(ScenarioCheckerReason.OVERLAP_OBSTACLE_STAIRS_WARN)
              .build());
    }
  }

  private void handelSourceSourceOverlap(
      ScenarioElement e1, ScenarioElement e2, PriorityQueue<ScenarioCheckerMessage> ret) {
    if (e1.overlapWith(e2)) {
      ret.add(
          msgBuilder
              .topographyWarning()
              .target(e1, e2)
              .reason(ScenarioCheckerReason.OVERLAP_SOURCE_SOURCE)
              .build());
    }
  }

  private void handelSourceTargetOverlap(
      ScenarioElement e1, ScenarioElement e2, PriorityQueue<ScenarioCheckerMessage> ret) {
    ...
    if (source.overlapWith(target)) {
      ret.add(
          msgBuilder
              ...
              .reason(ScenarioCheckerReason.OVERLAP_SOURCE_TARGET)
              .build());
    }
  }

  private void handelSourceStairsOverlap(
      ScenarioElement e1, ScenarioElement e2, PriorityQueue<ScenarioCheckerMessage> ret) {
    ...
    if (source.overlapWith(stairs)) {
      ret.add(
          msgBuilder
              .topographyWarning()
              .target(source, stairs)
              .reason(ScenarioCheckerReason.OVERLAP_SOURCE_STAIR)
              .build());
    }
  }

  private void handelTargetTargetOverlap(
      ScenarioElement e1, ScenarioElement e2, PriorityQueue<ScenarioCheckerMessage> ret) {
    if (e1.overlapWith(e2)) {
      ret.add(
          msgBuilder
              .topographyWarning()
              .target(e1, e2)
              .reason(ScenarioCheckerReason.OVERLAP_TARGET_TARGET)
              .build());
    }
  }

  // TOPOGRAPHY_WARN total overlap == partial overlap
  private void handelTargetStairsOverlap(
      ScenarioElement e1, ScenarioElement e2, PriorityQueue<ScenarioCheckerMessage> ret) {
    ...
    if (target.overlapWith(stairs)) {
      ret.add(
          msgBuilder
              .topographyWarning()
              .target(target, stairs)
              .reason(ScenarioCheckerReason.OVERLAP_TARGET_STAIR)
              .build());
    }
  }

  // TOPOGRAPHY_ERROR total overlap == partial overlap
  private void handelStairStairOverlap(
      ScenarioElement e1, ScenarioElement e2, PriorityQueue<ScenarioCheckerMessage> ret) {
    if (e1.overlapWith(e2)) {
      ret.add(
          msgBuilder
              .topographyError()
              .target(e1, e2)
              .reason(ScenarioCheckerReason.OVERLAP_STAIR_STAIR)
              .build());
    }
  }
}
