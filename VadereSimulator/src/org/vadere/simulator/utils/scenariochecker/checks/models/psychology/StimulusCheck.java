...
/** @author Christina Maria Mayr Warnings if social distance is out or range */
public class StimulusCheck extends AbstractScenarioCheck {

  private static final double LOW_BOUND = 1.25;
  private static final double HIGH_BOUND = 2.0;
  ...
  public PriorityQueue<ScenarioCheckerMessage> runScenarioCheckerTest(Scenario scenario) {
    ...
    String mainModelstring =
        scenario.getScenarioStore().getAttributesPsychology().getPsychologyLayer().getCognition();

    if (mainModelstring != null
        && mainModelstring.equals(SocialDistancingCognitionModel.class.getSimpleName())) {

      List<StimulusInfo> stimulusInfos =
          scenario.getScenarioStore().getStimulusInfoStore().getStimulusInfos();

      int counter = 0;
      for (StimulusInfo stimulusInfo : stimulusInfos) {

        List<Stimulus> socialDistance =
            stimulusInfo
                .getStimuli()
                .stream()
                .filter(stimulus -> stimulus instanceof DistanceRecommendation)
        ...
        if (socialDistance.size() == 1) {
          ...
          if (socialDistanceVal < 1.25 || socialDistanceVal > 2.0) {
            ...
            messages.add(
                msgBuilder
                    .perceptionAttrError()
                    .reason(
                        ScenarioCheckerReason.SOCIAL_DISTANCING,
                        String.format(
                            " [%3.2f - %3.2f] current value: %3.2f",
                            LOW_BOUND, HIGH_BOUND, socialDistanceVal))
                    .build());
          }
          ...
          messages.add(
              msgBuilder
                  ...
                  .reason(
                      ScenarioCheckerReason.SOCIAL_DISTANCING,
                      "In each time frame, only one stimulus of type DistanceRecommendation is allowed. "
                          + socialDistance.size()
                          + " stimuli found in "
                          + stimulusInfo.toString())
                  .build());
        }
      }
        ...
        messages.add(
            msgBuilder
                ...
                .reason(
                    ScenarioCheckerReason.SOCIAL_DISTANCING,
                    mainModelstring
                        + " is defined as CognitionModel. No stimuli of type DistanceRecommendation under the perception tab.")
                .build());
      }
    }
    return messages;
