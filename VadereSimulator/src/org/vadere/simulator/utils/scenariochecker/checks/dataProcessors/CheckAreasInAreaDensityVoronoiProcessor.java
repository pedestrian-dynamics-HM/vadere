...
public class CheckAreasInAreaDensityVoronoiProcessor extends AbstractScenarioCheck {
  ...
  public PriorityQueue<ScenarioCheckerMessage> runScenarioCheckerTest(Scenario scenario) {
    ...
    List<DataProcessor> areaVoronoiProc =
        processors
            .stream()
            .filter(
                dataProcessor -> dataProcessor.getClass().equals(AreaDensityVoronoiProcessor.class))
    ...
    if (!areaVoronoiProc.isEmpty()) {
      ...
      for (int i = 0; i < areaVoronoiProc.size(); i++) {
        AreaDensityVoronoiProcessor voronoiProcessor =
            (AreaDensityVoronoiProcessor) areaVoronoiProc.get(i);
        AttributesAreaDensityVoronoiProcessor voronoiAtt =
            (AttributesAreaDensityVoronoiProcessor) voronoiProcessor.getAttributes();
        int voronoiMeasId = voronoiAtt.getVoronoiMeasurementAreaId();
        int voronoiDiagramId = voronoiAtt.getMeasurementAreaId();

        MeasurementArea voronoiMeasArea =
            scenario.getTopography().getMeasurementArea(voronoiMeasId);
        MeasurementArea voronoiDiagramArea =
            scenario.getTopography().getMeasurementArea(voronoiDiagramId);
        if (!voronoiMeasArea.isRectangular() || !voronoiDiagramArea.isRectangular()) {
          ret.add(
              msgBuilder
                  .dataProcessorAttrError()
                  .reason(
                      ScenarioCheckerReason.MEASUREMENT_AREA_NOT_RECTANGULAR,
                      String.format(" [Processor id: %d]", voronoiProcessor.getId()))
                  .build());
        }
        ...
        if (!voronoiDiagramArea.getShape().contains((Rectangle2D) voronoiMeasArea.getShape())) {
          ret.add(
              msgBuilder
                  .dataProcessorAttrError()
                  .reason(
                      ScenarioCheckerReason.AREAS_DENSITY_VORONOI_PROCESSOR_MISMATCH,
                      String.format(" [Processor id: %d]", voronoiProcessor.getId()))
                  .build());
        }
      }
