...
/**
 * @author hm-mgoedel Warnings if settings of scenario do not match with cellular automaton
 *     requirements todo: Add methods to change spwaning mode of source
 */
  ...
  public PriorityQueue<ScenarioCheckerMessage> runScenarioCheckerTest(Scenario scenario) {
    ...
    if (mainModelstring != null && mainModelstring.equals(CellularAutomaton.class.getName())) {
        ...
        if (!source.getAttributes().getSpawnerAttributes().isEventPositionGridCA()) {
          messages.add(
              msgBuilder
                  ...
                  .target(source)
                  .reason(
                      ScenarioCheckerReason.CA_SPAWNING,
                      "If the cellular automaton model is used, the CA spawning should be activated. Otherwise agents will not be placed on a well-defined grid.")
                  .build());
        }
      }
      ...
      */
      ...
      if (scenario.getAttributesPedestrian().getSpeedDistributionStandardDeviation()
          > 0.0) { // || scenario.getAttributesPedestrian().getSpeedDistributionMean() != 1.0) {
        messages.add(
            msgBuilder
                .simulationAttrError()
                .reason(
                    ScenarioCheckerReason.CA_SPAWNING,
                    "If the cellular automaton model is used, the free-flow speed should be set to 1.0 m/s and standard deviation to 0.0 m/s. At the moment, individual speeds through event-driven setup is not supported. If you want to change the speed of the pedestrians, adapt the parameter simTimeStepLength in Simulation.")
                .build());
      }
      ...
      if (scenario.getAttributesPedestrian().getRadius() != 0.2) {
        messages.add(
            msgBuilder
                ...
                .reason(
                    ScenarioCheckerReason.CA_SPAWNING,
                    "If the cellular automaton model is used, the radius should be set to 0.2 m.")
                .build());
      }

      /*dynamical elements */
      for (DynamicElement e :
          scenario.getTopography().getPedestrianDynamicElements().getInitialElements()) {
        if (e.getType() == ScenarioElementType.PEDESTRIAN) {
          ...
          messages.add(
              msgBuilder
                  .simulationAttrWarning()
                  .target(e)
                  .reason(
                      ScenarioCheckerReason.CA_SPAWNING,
                      "Make sure that the dynamical elements are set on the CA grid to avoid overlaps and other issues.")
                  .build());
          ...
          if (att_e.getSpeedDistributionMean() != 1.0
              || att_e.getSpeedDistributionStandardDeviation() != 0.0
              || ((Agent) e).getFreeFlowSpeed() != 1.0) {
            messages.add(
                msgBuilder
                    .simulationAttrError()
                    .target(e)
