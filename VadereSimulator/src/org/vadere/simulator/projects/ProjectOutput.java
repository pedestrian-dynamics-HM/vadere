...
public class ProjectOutput {
  ...
  /**
   * This function does not check if cached {@link SimulationOutput} is dirty. It is assumed {@link
   * #update()} was called prior to this function.
   *
    ...
    try {
      ...
      for (Map.Entry<String, SimulationOutput> entry : simulationOutputs.entrySet()) {
        if (!entry.getValue().isDirty()) {
          ...
          if (hash1.equals(hash2)) out.add(project.getOutputDir().resolve(entry.getKey()).toFile());
        }
      }
  ...
  /**
   * If the output directory is present it is marked dirty and will be re-check in the next call of
   * {@link #update()}
   *
   * @param dirName directory name of {@link SimulationOutput}
   */
  public synchronized void markDirty(String dirName) {
  ...
  public void update() {
    try {
      Files.walkFileTree(
          ...
          new SimpleFileVisitor<Path>() {
            ...
            public FileVisitResult preVisitDirectory(Path dir, BasicFileAttributes attrs) {
              ...
              if (dir.endsWith(IOUtils.CORRUPT_DIR)) return FileVisitResult.SKIP_SUBTREE;

              // ignore output directory but continue with subtree
              if (dir.endsWith(IOUtils.OUTPUT_DIR)) return FileVisitResult.CONTINUE;

              Optional<SimulationOutput> outDir = getSimulationOutput(dirName);

              // only re-check existing SimulationOutput if they are dirty
              if (outDir.isPresent()) {
                if (!outDir.get().isDirty()) return FileVisitResult.CONTINUE;

                Optional<SimulationOutput> newSim =
                    IOOutput.getSimulationOutput(project, dir.toFile());
                if (newSim.isPresent()) {
      ...
      logger.info(
          String.format(
              "output directory '%s' of project '%s' is not valid.",
              project.getOutputDir(), project.getName()));
    }
  }
}
