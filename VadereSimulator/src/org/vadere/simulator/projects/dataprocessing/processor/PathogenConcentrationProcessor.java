...
public class PathogenConcentrationProcessor extends DataProcessor<TimeGridKey, Double> {
  ...
  protected void doUpdate(SimulationState state) {
    ...
    AttributesPathogenConcentrationProcessor attr =
        (AttributesPathogenConcentrationProcessor) getAttributes();
    ...
    if (state.getSimTimeInSec() > nextProcessingTime) {
      for (double yPos = topoY + gridResolution / 2;
          yPos <= topoY + topoHeight - gridResolution / 2;
          yPos = yPos + gridResolution) {
        for (double xPos = topoX + gridResolution / 2;
            xPos <= topoX + topoWidth - gridResolution / 2;
            xPos = xPos + gridResolution) {
          double pathogenConcentration = 0;
          VPoint node = new VPoint(xPos, yPos);
          Collection<AerosolCloud> cloudsAtNode =
              state
                  .getTopography()
                  .getAerosolClouds()
                  .stream()
                  .filter(c -> c.getShape().contains(node))
          ...
          this.putValue(
              new TimeGridKey(step, node.x, node.y, gridResolution * gridResolution),
