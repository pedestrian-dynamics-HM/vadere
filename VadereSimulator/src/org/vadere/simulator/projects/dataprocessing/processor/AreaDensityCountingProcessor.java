...
/**
 * @author Daniel Lehmberg Processor counts number of pedestrians in a measurement area of any
 *     shape.
 *     <p>To avoid unneccessary complicated structures this class does *not* extend
 *     from @AreaDensityProcessor (which requires an intern class or a separate class implementing
 *     the density algorithm). However, AreaDensityProcessor is a better description of what the
 *     processor does. See e.g. AreaDensityVoronoiProcessor / AreaDensityVoronoiAlgorithm
 */
...
public class AreaDensityCountingProcessor extends AreaDataProcessor<Integer>
    implements CompoundObjectProvider {
  ...
  protected void doUpdate(final SimulationState state) {
    ...
    // Here could also be another processor. However, because a processor uses more memory, all
    // pedestrians
  ...
  public CompoundObject provide(CompoundObjectBuilder builder) {
    ...
    return builder
        ...
        .build(measurementAreaId, getLastKey().getTimestep(), lastValue);
  }
}
