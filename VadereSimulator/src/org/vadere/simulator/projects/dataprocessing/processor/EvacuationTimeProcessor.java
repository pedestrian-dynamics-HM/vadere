...
public class EvacuationTimeProcessor extends NoDataKeyProcessor<Double> {
  ...
  public void postLoop(final SimulationState state) {
    ...
    // seems like the topography is empty after the simulation, so I saved the last value in
    // numberOfAgentsInScenario
    if (numberOfAgentsInScenario == 0) {
      if (this.pedEvacTimeProc.getValues().size() > 0) {
        result =
            this.pedEvacTimeProc.getValues().stream().anyMatch(tevac -> tevac == Double.NaN)
                ? Double.NaN
