...
/** @author Maxim Dudin */
@DataProcessorClass()
public class PedestriansNearbyProcessor
    extends DataProcessor<TimestepPedestriansNearbyIdKey, PedestriansNearbyData> {
  ...
  protected void doUpdate(final SimulationState state) {
    ...
    for (Pedestrian ped : peds) {
      ...
      List<DynamicElement> dynElemNneighbours =
          getDynElementsAtPosition(state.getTopography(), ped.getPosition(), maxDistance);
      List<PedestriansNearbyData> pedsNearby =
          dynElemNneighbours
              .parallelStream()
              ...
              .map(
                  p -> {
                    ...
                    return new PedestriansNearbyData(
                        ped.getId(), p.getId(), sampleEveryNthStep, timeStep, trajectory);
                  })
              .collect(Collectors.toList());
      pedsNearby.forEach(
          o ->
              this.putValue(
                  new TimestepPedestriansNearbyIdKey(timeStep, o.getPedId1(), o.getPedId2()), o));
    }
  }
  ...
  protected void putValue(
      final TimestepPedestriansNearbyIdKey key, final PedestriansNearbyData value) {
  ...
  public void init(final ProcessorManager manager) {
    super.init(manager);
    AttributesPedestrianNearbyProcessor att =
        (AttributesPedestrianNearbyProcessor) this.getAttributes();
