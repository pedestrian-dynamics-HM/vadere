...
public class FootStepSelfCategoryProcessor extends DataProcessor<EventtimePedestrianIdKey, String> {
    ...
    for (Pedestrian p : pedestrians) {
      ...
      if ((selfCat == SelfCategory.WAIT) || (selfCat == SelfCategory.CHANGE_TARGET)) {
        if (traj.getFootSteps().size() == 0) {
          if (state.getStep() == 1) {
            this.putValue(
                new EventtimePedestrianIdKey(state.getSimTimeInSec(), p.getId()),
                selfCategoryString);
          } else {
      ...
      for (FootStep fs : traj.getFootSteps()) {
        this.putValue(
            new EventtimePedestrianIdKey(fs.getStartTime(), p.getId()), selfCategoryString);
      }
    }
  }
  ...
  private EventtimePedestrianIdKey getKey(int pedId) {
    ...
    // TODO check whether we can use stream instead? currenlty not working, but seems to be more
    // effective
    // key = this.getKeys().stream().filter(k -> k.getPedestrianId() ==
    // pedId).max(Comparator.comparing(EventtimePedestrianIdKey::getSimtime)).orElseThrow(NoSuchElementException::new);
    return key;
  }
