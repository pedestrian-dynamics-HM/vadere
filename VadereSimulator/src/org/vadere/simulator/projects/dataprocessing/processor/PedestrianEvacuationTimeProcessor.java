 ...
 *
 * <p>- evacuation time is saved on every update (inefficient) - only works if agents are deleted at
 * their targets
 *
 * <p>A more better way could be implemented using target listener. In this case it is important to
 * check that the target is the agent's final (last) target.
 *
...
public class PedestrianEvacuationTimeProcessor extends DataProcessor<PedestrianIdKey, Double> {
  ...
  public void init(final ProcessorManager manager) {
    super.init(manager);
    AttributesPedestrianEvacuationTimeProcessor att =
        (AttributesPedestrianEvacuationTimeProcessor) this.getAttributes();
    pedStartTimeProc =
        (PedestrianStartTimeProcessor)
            manager.getProcessor(att.getPedestrianStartTimeProcessorId());
  }
  ...
  //				final double timeUntilTargetReached = state.getSimTimeInSec() -
  // pedStartTimeProc.getValue(pedKey);
  ...
  protected void doUpdate(final SimulationState state) {
    ...
    state
        ...
        .forEach(
            key -> this.putValue(key, state.getSimTimeInSec() - pedStartTimeProc.getValue(key)));
  }
