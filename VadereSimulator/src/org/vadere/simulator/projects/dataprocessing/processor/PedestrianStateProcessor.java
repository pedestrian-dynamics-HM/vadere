 ...
 *
 * <p>The possible states are: - c = Pedestrian was created - m = Pedestrian has moved - d =
...
public class PedestrianStateProcessor extends DataProcessor<TimestepPedestrianIdKey, String> {
  ...
  protected void doUpdate(SimulationState state) {
    ...
    pedsInThisState
        ...
        .forEach(
            id -> {
              boolean pedEntryExists =
                  getKeys().stream().filter(ped -> ped.getPedestrianId().equals(id)).count() > 0;
    ...
    List<Integer> activePedsInPreviousState =
        getKeys()
            .stream()
    ...
    List<Integer> deletedPeds =
        activePedsInPreviousState
            .stream()
