...
/** @author Marion Goedel Evaluates the flow based on FundamentalDiagramAProcessor (zhang-2011) */
@DataProcessorClass()
public class MeanFlowProcessor extends NoDataKeyProcessor<Double> {
  ...
  public void init(final ProcessorManager manager) {
    ...
    this.fundamentalDiagramAProcessor =
        (FundamentalDiagramAProcessor)
            manager.getProcessor(att.getPedestrianFundamentalDiagramAProcessorId());
    this.evacTimeProcessor =
  ...
  public void postLoop(final SimulationState state) {
    ...
    // choose only values before evacuation time exceeds (to ignore zero entries after all
    // pedestrians have left scenario)
    // todo: make sure that infinity only occurs at the end (why does it occur anyway?)
    fundamentalDiagramAProcessor
        .getValues()
        .stream()
        .filter(v -> v.get(index_time) < evacTime && Double.isInfinite(v.get(index_flow)))
        .forEach(v -> flow.put(v.get(index_time), v.get(index_flow)));
    ...
    // putValue(NoDataKey.key(), evacTimes.parallelStream().reduce(0.0, (val1, val2) -> val1 + val2)
    // / evacTimes.size());
  }
