...
public class TestOptimizationMetricNelderMeadProcessor extends TestProcessor {
  ...
  public void init(@NotNull final ProcessorManager manager) {
    ...
    pedestrianMetricOptimizationProcessor =
        (PedestrianMetricOptimizationProcessor)
            manager.getProcessor(att.getOptimizationMetricProcessorId());
  ...
  public void postLoop(SimulationState state) {
    ...
    for (OptimizationMetric singleMetric : processorData.values()) {
      ...
      pointDistanceL2Values.add(
          singleMetric.getOptimalPoint().distance(singleMetric.getFoundPoint()));

      // Insert all values for difference in the function values.
      differenceFuncValues.add(
          singleMetric.getFoundFuncValue() - singleMetric.getOptimalFuncValue());
    }
    ...
    String msg =
        getCompareValuesString(
            "mean difference in point distance",
            metricStatistics.get("meanPointDistance"),
            attr.getMaxMeanPointDistance());

    handleAssertion(
        metricStatistics.get("meanPointDistance") <= attr.getMaxMeanPointDistance(), msg);

    msg =
        getCompareValuesString(
            "mean difference in function value",
            metricStatistics.get("meanDifferenceFuncValue"),
            attr.getMaxMeanPointDistance());
    handleAssertion(
        metricStatistics.get("meanDifferenceFuncValue") <= attr.getMaxMeanDifferenceFuncValue(),
        msg);
  }
  ...
  private String getCompareValuesString(String valueName, double newValue, double referenceValue) {
    ...
    if (newValue < referenceValue) {
      ...
      msg =
          "NEUTRAL  -- The statistics '"
              + valueName
              + "' is equal to the reference value. Value = "
              + newValue;
    }
    return msg;
  }
  ...
  private void printStatistics(HashMap<String, Double> statistics) {
    ...
    System.out.println(
        getCompareValuesString(
            "mean function difference",
            statistics.get("meanDifferenceFuncValue"),
            attr.getMaxMeanDifferenceFuncValue()));
    System.out.println(
        getCompareValuesString(
            "mean point distance",
            statistics.get("meanPointDistance"),
            attr.getMaxMeanPointDistance()));
    ...
    System.out.println(
        getCompareValuesString(
            ...
            statistics.get("minPointDistanceL2"),
            attr.getInfoMinPointDistanceL2()));
    System.out.println(
        getCompareValuesString(
            "maximum point distance",
            statistics.get("maxPointDistanceL2"),
            attr.getInfoMaxPointDistanceL2()));
    System.out.println(
        getCompareValuesString(
            "standard deviation point distance",
            statistics.get("stddevPointDistance"),
            attr.getInfoStddevPointDistance()));

    System.out.println(
        getCompareValuesString(
            "minimum function difference",
            statistics.get("minDifferenceFuncValue"),
            attr.getInfoMinFuncDifference()));
    System.out.println(
        getCompareValuesString(
            "maximum function difference",
            statistics.get("maxDifferenceFuncValue"),
            attr.getInfoMaxFuncDifference()));
    System.out.println(
        getCompareValuesString(
            "standard deviation function difference",
            statistics.get("stddevDifferenceFuncValue"),
            attr.getInfoStddevDifferenceFuncValue()));

    System.out.println("\n In JSON format (for copying into TestProcessor");

    System.out.println("\"maxMeanPointDistance\" : " + statistics.get("meanPointDistance") + ",");
    System.out.println(
        "\"maxMeanDifferenceFuncValue\" : " + statistics.get("meanDifferenceFuncValue") + ",");
    System.out.println(
        "\"infoMinPointDistanceL2\" : " + statistics.get("minPointDistanceL2") + ",");
    System.out.println(
        "\"infoMaxPointDistanceL2\" : " + statistics.get("maxPointDistanceL2") + ",");
    System.out.println(
        "\"infoMinFuncDifference\" : " + statistics.get("minDifferenceFuncValue") + ",");
    System.out.println(
        "\"infoMaxFuncDifference\" : " + statistics.get("maxDifferenceFuncValue") + ",");
    System.out.println(
        "\"infoStddevPointDistance\" : " + statistics.get("stddevPointDistance") + ",");
    System.out.println(
        "\"infoStddevDifferenceFuncValue\" : " + statistics.get("stddevDifferenceFuncValue"));
  ...
  private HashMap<String, Double> computeStatistics(
      ArrayList<Double> pointDistanceL2Values, ArrayList<Double> differenceFuncValues) {
    ...
    for (int i = 0; i < numberElements; ++i) {
      stddevPointDistance +=
          Math.pow((pointDistanceL2Values.get(i) - statistics.get("meanPointDistance")), 2);
      stddevDifferenceFuncValue +=
          Math.pow((differenceFuncValues.get(i) - statistics.get("meanDifferenceFuncValue")), 2);
    }
