...
@DataProcessorClass
public class FootStepGroupIDProcessor extends DataProcessor<EventtimePedestrianIdKey, Integer>
  ...
  protected void doUpdate(SimulationState state) {
    getModel(state, CentroidGroupModel.class)
        .ifPresent(
            m -> { // find CentroidGroupModel
              CentroidGroupModel model = (CentroidGroupModel) m;
              model
                  .getGroupsById()
                  .forEach(
                      (gId, group) -> { // for each group
                        group
                            .getMembers()
                            .forEach(
                                ped -> { // for each member in group
                                  ped.getTrajectory()
                                      .getFootSteps()
                                      .forEach(
                                          fs -> {
                                            this.putValue(
                                                new EventtimePedestrianIdKey(
                                                    fs.getStartTime(), ped.getId()),
                                                gId);
                                          });
                                });
                      });
  ...
  public String[] toStrings(EventtimePedestrianIdKey key) {
    ...
    if (i == null) {
      logger.warn(
          String.format("FootstepGroupIDProcessor does not have Data for Key: %s", key.toString()));
      i = -1;
    }
