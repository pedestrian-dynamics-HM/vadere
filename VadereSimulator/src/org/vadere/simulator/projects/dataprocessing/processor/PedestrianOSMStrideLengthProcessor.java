...
@DataProcessorClass()
public class PedestrianOSMStrideLengthProcessor
  ...
  protected void doUpdate(final SimulationState state) {
    ...
    if (osm != null) {
      List<PedestrianOSM> osmPeds =
          peds.stream().map(p -> ((PedestrianOSM) p)).collect(Collectors.toList());
      osmPeds.forEach(
          ped -> {
      ...
      peds.forEach(
          ped ->
              this.putValue(new TimestepPedestrianIdKey(state.getStep(), ped.getId()), Double.NaN));
    }
  }
  ...
  public void init(final ProcessorManager manager) {
    ...
    if (model instanceof OptimalStepsModel) this.osm = (OptimalStepsModel) model;
  }
}
