...
public class PedestrianFlowProcessor extends DataProcessor<TimestepPedestrianIdKey, Double> {
  ...
  protected void doUpdate(final SimulationState state) {
    ...
    Set<TimestepPedestrianIdKey> keys =
        this.pedVelProc
            .getKeys()
            .stream()
            .filter(key -> key.getTimestep() == state.getStep())
  ...
  public void init(final ProcessorManager manager) {
    super.init(manager);
    AttributesPedestrianFlowProcessor att =
        (AttributesPedestrianFlowProcessor) this.getAttributes();

    this.pedVelProc =
        (PedestrianVelocityProcessor) manager.getProcessor(att.getPedestrianVelocityProcessorId());
    this.pedDensProc =
        (PedestrianDensityProcessor) manager.getProcessor(att.getPedestrianDensityProcessorId());
  }
