...
@DataProcessorClass()
public class VoronoiPolygonProcessor extends DataProcessor<TimestepPositionKey, VPolygon>
  ...
  protected void doUpdate(final SimulationState state) {
    ...
    for (Face face : faces) {
      ...
      if (capCell == null) {
        logger.info(
            "no voronoidiagramm for timestep: "
                + state.getStep()
                + "and area: "
                + voronoiMeasurementArea.getId());
  ...
  public void init(final ProcessorManager manager) {
    super.init(manager);
    AttributesVoronoiPolygonProcessor att =
        (AttributesVoronoiPolygonProcessor) this.getAttributes();
