  ...
  public double getDensity(final VPoint pos, final SimulationState state) {
    if (state.getStep() > this.lastStep) {
      ...
      if (!this.filteredObstacles && this.isObstacleDensity) {
        this.obstacleFilter =
            IGaussianFilter.create(state.getTopography(), this.scale, this.standDev);
        this.obstacleFilter.filterImage();
        this.filteredObstacles = true;
      }

      this.pedestrianFilter =
    ...
    double density = 0.0;
    if (this.isObstacleDensity) density = this.obstacleFilter.getFilteredValue(pos.x, pos.y);
