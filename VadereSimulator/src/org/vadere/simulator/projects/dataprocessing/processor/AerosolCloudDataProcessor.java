...
/**
 * This processor writes out the shape and the pathogen load of all {@link AerosolCloud}s. Each row
 * corresponds to one aerosol cloud at a given simulation time. The shape of an aerosol cloud is
 * described by the radius and its center ([x, y]-coordinates).
 *
 * <p>To reduce the size of the output file, one can reduce the sample frequency, that is the
 * processor writes out only every {@param sampleEveryNthSimStep}.
 *
 * @author Simon Rahn
 */
@DataProcessorClass()
public class AerosolCloudDataProcessor extends DataProcessor<TimestepIdDataKey, String> {
  ...
  public void init(ProcessorManager manager) {
    super.init(manager);
    AttributesAerosolCloudDataProcessor attr =
        (AttributesAerosolCloudDataProcessor) this.getAttributes();
