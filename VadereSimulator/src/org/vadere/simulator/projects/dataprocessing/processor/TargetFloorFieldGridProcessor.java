				...
				if(optPed.isPresent()) {
					...
					for (double y = bound.y; y < bound.y + bound.height; y += att.getResolution()) {
						...
						for (double x = bound.x; x < bound.x + bound.width; x += att.getResolution()) {
							floorFieldGridRow.setValue(col++, pft.getTargetPotential(targetIds, new VPoint(x, y), optPed.get()));
						}
						this.setValue(new TimestepRowKey(state.getStep(), row++), floorFieldGridRow);
					}
