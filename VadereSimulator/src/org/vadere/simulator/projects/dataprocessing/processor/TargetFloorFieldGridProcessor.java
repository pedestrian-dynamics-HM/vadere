...
@DataProcessorClass()
public class TargetFloorFieldGridProcessor
    extends DataProcessor<TimestepRowKey, FloorFieldGridRow> {
  ...
  protected void doUpdate(final SimulationState state) {
    ...
    if (optMainModel.isPresent() && optMainModel.get() instanceof PotentialFieldModel) {
      IPotentialFieldTarget pft =
          ((PotentialFieldModel) optMainModel.get()).getPotentialFieldTarget();
      ...
      if (!hasOnceProcessed || pft.needsUpdate()) {
         ...
         */
        Optional<Pedestrian> optPed =
            state.getTopography().getPedestrianDynamicElements().getElements().stream().findAny();
        ...
        if (optPed.isPresent()) {
          ...
          for (int i = 0; i < widthMax; i++) {
            ...
            FloorFieldGridRow floorFieldGridRow =
                new FloorFieldGridRow((int) Math.floor(bound.width / att.getResolution()));
            int col = 0;
      ...
      logger.warn(
          "could not process, main model is missing or is not the instance of "
              + PotentialFieldModel.class.getName());
    }
  }
