...
public class MeanPedestrianEvacuationTimeProcessor extends NoDataKeyProcessor<Double> {
  ...
  public void init(final ProcessorManager manager) {
    super.init(manager);
    AttributesMeanPedestrianEvacuationTimeProcessor att =
        (AttributesMeanPedestrianEvacuationTimeProcessor) this.getAttributes();
  ...
  public void postLoop(final SimulationState state) {
    ...
    putValue(
        NoDataKey.key(),
        evacTimes.parallelStream().reduce(0.0, (val1, val2) -> val1 + val2) / evacTimes.size());
  }
