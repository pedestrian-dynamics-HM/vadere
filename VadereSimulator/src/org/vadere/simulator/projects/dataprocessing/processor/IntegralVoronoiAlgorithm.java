...
/**
 * Given a Simulation state this Algorithm computes the Voronoi density and Voronoi velocity defined
 * in zhang-2011 (doi:10.1088/1742-5468/2011/06/P06004) section 3.4 (Method D), equations (7, 8, 9)
 *
 * @author Benedikt Zoennchen
 */
public class IntegralVoronoiAlgorithm extends AreaDensityAlgorithm
    implements IAreaVelocityAlgorithm {
  ...
  public IntegralVoronoiAlgorithm(
      @NotNull final Function<TimestepPedestrianIdKey, Double> agentVelocityFunc,
      @NotNull final MeasurementArea measurementArea,
      @NotNull final MeasurementArea voronoiMeasurementArea) {
    super("areaVoronoi");
  ...
  private VPolygon computeIntersection2(@NotNull final VPolygon cell) {
    try {
      WeilerAtherton weilerAtherton =
          new WeilerAtherton(Arrays.asList(cell, measurementAreaPolygon));
  ...
  private List<Face> generateFaces(@NotNull final SimulationState state) {
    ...
    List<VPoint> pedestrianPositions =
        Agent.getPositions(state.getTopography().getElements(Agent.class));
  ...
  public double getVelocity(SimulationState state) {
    ...
    for (Face face : faces) {
      ...
      if (intersectMeasurementArea(face)) {
        ...
        velocity +=
            (computeIntersection2(face.toPolygon()).getArea() * agentVelocityFunc.apply(key));
      }
    }
