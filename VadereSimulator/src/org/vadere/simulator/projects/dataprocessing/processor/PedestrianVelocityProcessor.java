...
/**
 * This processor computes the velocity based on pedestrian positions and the simulation time. Let p
 * be the position of a pedestrian at the current time step s. And let q be the position of the same
 * pedestrian at the time step s minus <tt>backSteps</tt> then the velocity of this pedestrian at
 * the time of time step s is defined by: the length of the vector q-p divided by the duration
 * between s and (s minus <tt>backSteps</tt>).
 *
...
public class PedestrianVelocityProcessor extends APedestrianVelocityProcessor {
  ...
  public void init(final ProcessorManager manager) {
    super.init(manager);
    AttributesPedestrianVelocityProcessor attVelProc =
        (AttributesPedestrianVelocityProcessor) getAttributes();
    this.pedestrianPositionProcessor =
        (PedestrianPositionProcessor)
            manager.getProcessor(attVelProc.getPedestrianPositionProcessorId());
  ...
  private double getVelocity(int timeStep, double currentSimTime, int pedId) {
    ...
    if (timeStep > 1) {
      ...
      VPoint pastPosition =
          pedestrianPositionProcessor.getValue(new TimestepPedestrianIdKey(pastStep, pedId));
      VPoint position =
          pedestrianPositionProcessor.getValue(new TimestepPedestrianIdKey(timeStep, pedId));
      ...
      if (pastPosition != null) {
        ...
        velocity =
            position.subtract(pastPosition).scalarMultiply(1.0 / duration).distanceToOrigin();
      }
    }
