...
public class QueueWidthProcessor extends DataProcessor<TimestepKey, Double> {
  ...
  private VPoint
      direction; /* can be either in x-direction [1, 0] or [-1, 0] or in y-direction [0, 1], [0,-1] */
  ...
  protected void doUpdate(final SimulationState state) {
    ...
    if (Math.abs(direction.getX()) > 0) {
      double minX =
          Math.min(referencePoint.getX(), referencePoint.getX() + direction.getX() * maxDist);
      double maxX =
          Math.max(referencePoint.getX(), referencePoint.getX() + direction.getX() * maxDist);

      List<Pedestrian> pedQueue =
          state
              .getTopography()
      ...
      if (pedQueue.size() == 0) {
        ...
        for (int i_ped = 0; i_ped < pedQueue.size(); i_ped++) {
          double delta_y =
              Math.abs(
                  pedQueue.get(i_ped).getPosition().getX()
                      - referencePoint
                          .getX()); // X and Y are interchanged - compare supplementary material
          double delta_x =
              Math.abs(pedQueue.get(i_ped).getPosition().getY() - referencePoint.getY());
