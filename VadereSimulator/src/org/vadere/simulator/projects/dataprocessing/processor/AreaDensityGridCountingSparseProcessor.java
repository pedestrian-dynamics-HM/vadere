...
public class AreaDensityGridCountingSparseProcessor
    extends AreaDensityGridCountingProcessor<TimeGridKey, Integer> {
  ...
  protected void doUpdate(SimulationState state) {
    ...
    AttributesAreaDensityGridCountingProcessor attr =
        (AttributesAreaDensityGridCountingProcessor) getAttributes();
    LinkedCellsGrid<Pedestrian> cellsElements =
        new LinkedCellsGrid<Pedestrian>(
    ...
    state
        ...
        .getElements()
        .forEach(cellsElements::addObject);
    ...
    for (int r = 0; r < count.length; r++) {
      for (int c = 0; c < count[r].length; c++) {
        ...
        if (pedCount > 0) {
          TimeGridKey key =
              new TimeGridKey(
                  step, r * attr.getCellSize(), c * attr.getCellSize(), attr.getCellSize());
