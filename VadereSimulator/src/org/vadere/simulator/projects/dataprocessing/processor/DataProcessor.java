 ...
 *
 * <p>This class contains all common functionality for all data processors. It provides access to
 * the internal data map for saving concrete data of type <tt>V</tt> with key type <tt>K</tt>.
 *
 * <p>The methods <tt>preLoop</tt> and <tt>postLoop</tt> are called at corresponding points in time
 * related to the simulation loop.
 *
 * <p>The method <tt>doUpdate</tt> gets called after every simulation step with the current
 ...
 * data by using the <tt>putValue</tt> method.
 *
 * <p>To get specific attributes defined in JSON or access to the <tt>MainModel</tt>, one has to use
  ...
  public Optional<Model> getSubModel(SimulationState state, Class clazz) {
    ...
    if (mainModel.isPresent()) {
      Optional<Model> model =
          mainModel.get().getSubmodels().stream().filter(clazz::isInstance).findAny();
      return model;
    } else {
