...
/**
 * This processor computes the fundamental diagram by computing an (average) velocity and the
 * density for each pedestrian over a certain area <tt>measurementArea</tt>. The <tt>velocity</tt>
 * of a pedestrian is the distance walked inside the <tt>measurementArea</tt> divided by the
 * required time. The <tt>density</tt> of a pedestrian is the integral of the number of pedestrians
 * inside the <tt>measurementArea</tt> (integrated over the time) divided by the area of the
 * <tt>measurementArea</tt> and the time required to walk through the <tt>measurementArea</tt>. The
 * bound of integration is the time the pedestrians enters the <tt>measurementArea</tt> and the time
 * the pedestrian exits the <tt>measurementArea</tt>.
 *
 * <p>For more details see zhang-2011 (doi:10.1088/1742-5468/2011/06/P06004) Method B.
 *
 * <p>Note that this processor does only work if pedestrians do not move multiple times through
 * <tt>measurementArea</tt>
 *
 * @author Benedikt Zoennchen
 */
@DataProcessorClass()
public class FundamentalDiagramBProcessor
  ...
  public void init(final ProcessorManager manager) {
    super.init(manager);
    AttributesFundamentalDiagramBProcessor att =
        (AttributesFundamentalDiagramBProcessor) this.getAttributes();
    pedestrianTrajectoryProcessor =
        (PedestrianTrajectoryProcessor)
            manager.getProcessor(att.getPedestrianTrajectoryProcessorId());
  ...
  public void postLoop(SimulationState state) {
    ...
    /**
     * (2) Compute for each pedestrian its velocity v_i and its density \rho_i see zhang-2011 Method
     * B
     */
  ...
  private double density(
      @NotNull final PedestrianIdKey key,
      @NotNull final Map<PedestrianIdKey, VTrajectory> cutTrajectoryMap) {
    ...
    double densityIntegral =
        cutTrajectoryMap
            ...
            .map(
                trajectory ->
                    trajectory.cut(
                        pedTrajectory.getStartTime().get(), pedTrajectory.getEndTime().get()))
            ...
            .mapToDouble(
                trajectory -> (trajectory.getEndTime().get() - trajectory.getStartTime().get()))
            .sum();
