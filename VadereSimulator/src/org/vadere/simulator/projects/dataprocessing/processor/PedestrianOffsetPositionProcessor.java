...
@DataProcessorClass()
public class PedestrianOffsetPositionProcessor
    extends DataProcessor<TimestepPedestrianIdKey, VPoint> {
  ...
  Map<PedestrianIdKey, VPoint> getPositions(TimestepKey timestepKey) {
    return this.getData()
        ...
        .collect(
            Collectors.toMap(
                e -> new PedestrianIdKey(e.getKey().getPedestrianId()), Map.Entry::getValue));
  }
  ...
  protected void doUpdate(final SimulationState state) {
    ...
    for (Pedestrian p : state.getTopography().getElements(Pedestrian.class)) {
      this.putValue(
          new TimestepPedestrianIdKey(timeStep, p.getId()), p.getPosition().add(attr.getOffset()));
    }
  }
