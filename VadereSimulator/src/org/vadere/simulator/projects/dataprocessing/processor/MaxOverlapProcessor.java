...
/**
 * This processor saves the largest overlap (2*pedRadius - distance between the pedestrian's
 * centers) for one simulation. The processor depends on the PedestrianOverlapDistProcessor. It only
 * works if all pedestrians in the simulation have the same radius.
 *
 * @author Marion Gödel
 */
@DataProcessorClass()
public class MaxOverlapProcessor extends NoDataKeyProcessor<Double> {
  ...
  public void postLoop(final SimulationState state) {
    ...
    OverlapData maximumOverlap =
        this.pedOverlapProc
            .getData()
            .values()
            .stream()
            .max(OverlapData::maxDist)
            .orElse(OverlapData.noOverLap);
  ...
  public void init(final ProcessorManager manager) {
    ...
    this.pedOverlapProc =
        (PedestrianOverlapProcessor) manager.getProcessor(att.getPedestrianOverlapProcessorId());
  }
