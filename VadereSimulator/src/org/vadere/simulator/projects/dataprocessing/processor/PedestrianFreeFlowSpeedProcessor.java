...
/**
 * @author Marion Gödel Save free-flow speeds for each pedestrian in the simulation to evaluate how
 *     well the sample fits the defined distribution. In the entry (pedestrianIdKey) "-1" the
 *     average is saved In the entry (pedestrianIdKey) "-2" the std of the samples is saved
 */
@DataProcessorClass()
public class PedestrianFreeFlowSpeedProcessor extends DataProcessor<PedestrianIdKey, Double> {
  ...
  private void update(PedestrianIdKey pedIdKey, double freeFlowSpeed) {
    ...
    if (!keys.contains(pedIdKey)) this.putValue(pedIdKey, freeFlowSpeed);
  }
  ...
  public void postLoop(final SimulationState state) {
    ...
    double tmp_sum =
        this.getValues()
            .stream()
            .mapToDouble(speed -> Math.pow(speed - meanVal.getAsDouble(), 2))
            .sum();
