...
public class ProcessorManager {
  ...
  public ProcessorManager(
    ...
    for (DataProcessor<?, ?> proc : dataProcessors) this.processorMap.put(proc.getId(), proc);
  ...
  public MeasurementArea getMeasurementArea(int measurementAreaId, boolean requireRectangular) {
    ...
    if (measurementArea == null) {
      throw new RuntimeException(
          String.format("MeasurementArea with index %d does not exist.", measurementAreaId));
    }

    if (requireRectangular && !measurementArea.isRectangular()) {
      throw new RuntimeException(
          String.format(
              "Measurement area for %d is required to be rectangular", measurementAreaId));
    }
    ...
    this.outputFiles.forEach(
        file ->
            file.setAbsoluteFileName(
                Paths.get(directory, String.format("%s", new File(file.getFileName()).getName()))
                    .toString()));
  }
