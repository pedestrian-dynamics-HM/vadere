...
public class ExceptionIncident extends Incident {
    ...
    while (true) {
      try {
        ...
        if (errMsg.startsWith("Unrecognized field")) { // UnrecognizedFieldExceptionIncident
          ...
          tree.deleteUnrecognizedField(
              referenceChainNodes.getLeft(), referenceChainNodes.getRight(), log, this);
        } else if (errMsg.startsWith(
            "Can not coerce a floating-point")) { // EnforceIntegerExceptionIncident
          Pair<String, String> referenceChainNodes = getReferenceChainNodes(errMsg);
          tree.enforceIntegerValue(
              referenceChainNodes.getLeft(), referenceChainNodes.getRight(), log, this);
          ...
          String logMsg =
              "Can't automatically deal with this JsonProcessException: \n" + e.getMessage();

          if (e.getMessage().startsWith("Can't parse \"false\" as boolean")
              || e.getMessage().startsWith("Can't parse \"true\" as boolean")) {
            logMsg =
                "One or more (the migration was stopped at the first occurrence) boolean-value "
  ...
  private Pair<String, String> getReferenceChainNodes(String errMsg) {
    String referenceChain =
        errMsg.substring(
            errMsg.indexOf("through reference chain: ") + "through reference chain: ".length());
    String parentKey =
        referenceChain.split("\"")[0].substring(0, referenceChain.split("\"")[0].length() - 1);
