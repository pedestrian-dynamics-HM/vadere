...
public class MigrationUtil {
  ...
  public MigrationUtil() {
    ...
    treeMarker =
        createList(
            "DO_NOT_MIGRATE_TREE", ".DO_NOT_MIGRATE_TREE", IOUtils.CORRUPT_DIR, IOUtils.LEGACY_DIR);
  ...
  public void generateNewVersionTransform(Path resourceDir, String newVersionLabel)
      throws URISyntaxException, IOException {

    Path oldTransform =
        JoltTransformation.getTransformationFileFromFileSystem(resourceDir, Version.latest());
    Path oldIdentity =
        JoltTransformation.getIdenityFileFromFileSystem(resourceDir, Version.latest());

    String newTransformString =
        JoltTransformation.getTransforamtionResourcePath(
            Version.latest().label('-'), newVersionLabel);
    String newIdenityString = JoltTransformation.getIdentiyResoucrePath(newVersionLabel);
    ...
    json =
        json.replace(
            "\"release\": \"" + Version.latest().label('-') + "\",",
            "\"release\": \"" + newVersionLabel + "\",");
    IOUtils.writeTextFile(newIdenity.toString(), json);

    json =
        json.replace(
            "\"release\": \"&\",",
            "// no release here to overwrite it with default at the default operation down below");
    IOUtils.writeTextFile(newTransform.toString(), json);
  }

  public void migrateDirectoryTree(Path p, Version targetVersion, boolean recursive)
    ...
    FileVisitor<Path> visitor =
        getVisitor(
            ...
            dirMarker,
            recursive,
            path -> migrate(path, targetVersion));
    this.err = null;
    try {
    ...
    if (this.err != null) {
      throw new MigrationException(
          "topographyError while processing MigrationTaskHandler in walkFileTree", this.err);
    }
  }

  public void revertDirectoryTree(Path p, boolean recursive) throws MigrationException {
    FileVisitor<Path> visitor =
        getVisitor(new ArrayList<>(), treeMarker, dirMarker, recursive, this::revert);
    this.err = null;
    try {
      Files.walkFileTree(p, visitor);
    } catch (IOException e) {
   ...
   * FileTreeWalk the calling funtion will check if an topographyError occurred and will throw the
   * Throwable upstream.
   */
  ...
  FileVisitor<Path> getVisitor(
      ArrayList<String> ignoreDirs,
      ArrayList<String> treeMarker,
      ArrayList<String> dirMarker,
      boolean recursive,
      MigrationTaskHandler func) {
    ...
    return new FileVisitor<Path>() {
      ...
      public FileVisitResult preVisitDirectory(Path dir, BasicFileAttributes attrs) {
        ...
        if (ignoreDirs.contains(dir.getFileName().toString())) { // skip ignored directories-trees.
          ...
          if (dir.resolve(IOUtils.LEGACY_DIR).toFile().exists()) {
            throw new RuntimeException(
                "Cannot migrate directory "
                    + dir
                    + " due to a legacy folder. Delete the legacy folder and re-run the migration.");
          }
        ...
        if (recursive) return FileVisitResult.CONTINUE;
      ...
      public FileVisitResult visitFile(Path file, BasicFileAttributes attrs) {
        ...
        if (!file.getFileName().toString().endsWith(".scenario")) return FileVisitResult.CONTINUE;

        // if dirMarker is set do not migrate any scenario files in this dir
        // but continue with sub-dirs.
        boolean hasDirMarker =
            dirMarker.stream().anyMatch(m -> file.getParent().resolve(m).toFile().exists());
        if (hasDirMarker) {
          return FileVisitResult.CONTINUE;
        }
