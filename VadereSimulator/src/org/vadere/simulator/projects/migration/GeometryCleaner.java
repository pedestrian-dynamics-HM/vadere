...
public class GeometryCleaner {
	...
	private List<VPolygon> magnet() {
		...
		IncrementalTriangulation<PVertex, PHalfEdge, PFace> dt = IncrementalTriangulation.fromBuilderFactory(PTriangleMeshBuilder::new, boundingBox);
		...
		for(int i = 0; i < polygons.size(); i++) {
			...
			for(int j = 0; j < points.size(); j++) {
				...
				IPoint insertedPoint = dt.getMesh().edges().endToPoint(edge);

				PVertex v = dt.getMesh().vertices().getEndOf(edge);
				PVertex umin = null;
				double len = Double.MAX_VALUE;
				for(PVertex u : dt.getMesh().vertices().adjacentIterableFor(v)) {
					if(len > u.distance(v)) {
						umin = u;
						len = u.distance(v);
					}
	...
	private Pair<VPolygon, List<VPolygon>> mergePolygons(@NotNull final Collection<VLine> lines, @NotNull final List<VPolygon> polygons) {
		...
		var faces = triangulation.getMesh().faces();
		var vertices = triangulation.getMesh().vertices();
		var changeConnectivity = triangulation.getMeshBuilder().changeConnectivity();
		...
		for (PFace hole : faces.getHoles()) {
			VPolygon poly = GeometryUtils.toPolygon(vertices.streamVerticesOf(hole).map(v -> vertices.toPoint(v)).collect(Collectors.toList()));
			result.add(poly);
		}
