...
/**
 * This is the Base version for the JsonTransformationFactory which automatically build by a
 * Annotation Processor at build time. Do not directly use this class
 */
public class JsonTransformationBaseFactory
    extends BaseFactory<JsonTransformation, FactoryObject<JsonTransformation>> {

  private static final Logger logger = Logger.getLogger(JsonTransformationBaseFactory.class);
  ...
  public JsonTransformation getInstanceOf(String key) throws ClassNotFoundException {
    if (transformationMap.containsKey(key)) {
      ...
      if (supplierMap.containsKey(key)) {
        ...
        logger.infof(
            "No specific transformation fount for key: %s. Using default transformation (version increment only)",
            key);
        JsonTransformation tmp = supplierMap.get("default").getSupplier().get();
        transformationMap.put(
            key, tmp); // save default transformation for other occurrences for given version.
        return tmp;
      }
