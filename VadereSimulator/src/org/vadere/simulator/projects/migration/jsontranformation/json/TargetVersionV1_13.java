...
public class TargetVersionV1_13 extends SimpleJsonTransformation {
  ...
  private JsonNode targetChangerAlgorithm(JsonNode node) throws MigrationException {
    // targetChangers may be missing if an old scenario was never opened since the targetChangers
    // where introduced
    ...
    if (!path(node, "scenario/topography/targetChangers").isMissingNode()) {
      ...
      while (iter.hasNext()) {
        ...
        if (pedAsTarget) {
          ...
          ArrayList<Double> probabilityToChangeTarget =
              getDoubleList(changer, "probabilityToChangeTarget");
