...
public interface JsonNodeExplorer {
  ...
  default JsonNode remove(JsonNode root, String childName) throws MigrationException {
    ...
    if (removed == null) {
      throw new MigrationException(
          "Cannot delete childElement '" + childName + "' from parent " + root.asText());
    }
    return removed;
  }
  ...
  default JsonNode setVersionFromTo(JsonNode scenarioFile, Version from, Version to)
      throws MigrationException {
    JsonNode version = pathMustExist(scenarioFile, "release");
    if (!version.asText().equals(from.label()))
      throw new MigrationException(
          String.format(
              "Previous version is wrong. Expected %s but found %s",
  ...
  default JsonNode pathMustExist(JsonNode root, String path) throws MigrationException {
    ...
    if (ret.isMissingNode()) {
      throw new MigrationException(
          "Json Path Erro: The path '" + path + "' should be accessible from " + root.asText());
    }
    return ret;
  }
   ...
   */
  default Iterator<JsonNode> iteratorProcessorsByType(JsonNode node, String processorType)
      throws MigrationException {
    JsonNode processors = pathMustExist(node, "processWriters/processors");
    return new JsonFilterIterator(
        processors,
        n -> {
  ...
  default Iterator<JsonNode> iteratorMeasurementArea(JsonNode node, int id)
      throws MigrationException {
  ...
  default ArrayList<MeasurementArea> deserializeMeasurementArea(JsonNode node, ObjectMapper mapper)
      throws MigrationException {
    ArrayNode jsonMeasurementArea =
        (ArrayNode) pathMustExist(node, "scenario/topography/measurementAreas");
  ...
  default void createArrayNodeIfNotExist(JsonNode scenarioFile, String relPath, String fieldName)
      throws MigrationException {
   ...
   * @param shapeNode Shape used in the {@link
   ...
   * @return The id of the {@link MeasurementArea} to use in the {@link
   *     org.vadere.simulator.projects.dataprocessing.processor.DataProcessor}
   * @throws MigrationException
   */
  default int transformShapeToMeasurementArea(
      JsonNode scenarioFile, JsonNode shapeNode, ObjectMapper mapper) throws MigrationException {
    ...
    MeasurementArea area =
        measurementAreas
            .stream()
            .filter(a -> a.compareByShape(newArea))
            .findFirst()
            .orElse(newArea);
    if (area == newArea) {
      // if a new area is included ensure a unique id. Important: The id is only unique within the
      // MeasurementAreas
      int newId =
          measurementAreas.stream().map(MeasurementArea::getId).max(Integer::compareTo).orElse(0)
              + 1;
      area.setId(newId);
  ...
  default void replaceMeasurementAreas(
      JsonNode scenarioFile, ArrayList<MeasurementArea> measurementAreas, ObjectMapper mapper)
      throws MigrationException {
    ArrayNode jsonMeasurementArea =
        (ArrayNode) pathMustExist(scenarioFile, "scenario/topography/measurementAreas");
  ...
  /**
   * Create Iterator over all files in the scenario which contain the specified id.
   *
   * @param node root of scenario
   ...
   */
  default Iterator<JsonNode> iteratorFilesForProcessorId(JsonNode node, String id)
      throws MigrationException {
  ...
  default ArrayList<JsonNode> getFilesForProcessorId(JsonNode node, String id)
      throws MigrationException {
  ...
  default void renameFileFieldName(
      JsonNode rootNode, String fieldKey, String oldName, String newName)
      throws MigrationException {
    String childName = "files";
    renameProcessWritersChildField(rootNode, childName, fieldKey, oldName, newName);
  }

  default void renameProcessorFieldName(
      JsonNode rootNode, String fieldKey, String oldName, String newName)
      throws MigrationException {
  ...
  private void renameProcessWritersChildField(
      JsonNode rootNode, String childName, String fieldKey, String oldName, String newName)
      throws MigrationException {
  ...
  default ArrayList<Double> getDoubleList(JsonNode root, String path) throws MigrationException {
    ...
    if (!arrList.isArray()) throw new MigrationException("Node should be an Array Node.");
  ...
  default void createNewOutputFile(JsonNode root, String type, String... pIds)
      throws MigrationException {
