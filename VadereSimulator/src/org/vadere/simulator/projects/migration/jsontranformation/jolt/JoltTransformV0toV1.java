...
public class JoltTransformV0toV1 extends JoltTransformation {
  ...
  private JsonNode findMainModel(JsonNode node) throws MigrationException {
    ...
    if (!node.path("vadere")
        .path("attributesModel")
        .path(AttributesOSM.class.getName())
        .isMissingNode()) {
    ...
    if (!node.path("vadere")
        .path("attributesModel")
        .path(AttributesGNM.class.getName())
        .isMissingNode()) {
    ...
    if (!node.path("vadere")
        .path("attributesModel")
        .path(AttributesBHM.class.getName())
        .isMissingNode()) {
      if (!mainModelValue.equals("")) {
        throw new MigrationException(
            "can't automatically determine the mainModel - more than one mainModel-suitable model is present");
    ...
    if (mainModelValue.equals("")) {
      throw new MigrationException(
          "could not automatically determine the mainModel based on the present models OSM, GNM, SFM, BHM");
    }
   ...
   * Transformation is context sensitive thus handle this with a hook and not within the Jolt
   * transformation
   */
  private JsonNode attributesPotentialCompactVSosmIncident(JsonNode node)
      throws MigrationException {
    JsonNode osmAttr =
        node.path("vadere").path("attributesModel").path(AttributesOSM.class.getCanonicalName());
    if (osmAttr.isMissingNode()) return node;

    JsonNode potentialCompactAttr =
        node.path("vadere")
            .path("attributesModel")
            .path(AttributesPotentialCompact.class.getCanonicalName());
    JsonNode potentialOSMAttr =
        node.path("vadere")
            .path("attributesModel")
            .path(AttributesPotentialOSM.class.getCanonicalName());

    if (!potentialCompactAttr.isMissingNode() && !potentialOSMAttr.isMissingNode()) {
      throw new MigrationException(
          "[AttributesPotentialCompact] and [AttributesPotentialOSM] are both present, that is not allowed.");
    }
    ...
    if (!potentialCompactAttr.isMissingNode()) {
      addToObjectNode(
          osmAttr, "pedestrianPotentialModel", PotentialFieldPedestrianCompact.class.getName());
      addToObjectNode(
          osmAttr, "obstaclePotentialModel", PotentialFieldObstacleCompact.class.getName());
      if (!beforeChange.equals(osmAttr.toString())) {
        logger.info(
            "\t- AttributesOSM: since AttributesPotentialCompact is present, set [pedestrianPotentialModel] to PotentialFieldPedestrianCompact "
    ...
    if (!potentialOSMAttr.isMissingNode()) {
      addToObjectNode(
          osmAttr, "pedestrianPotentialModel", PotentialFieldPedestrianOSM.class.getName());
      addToObjectNode(osmAttr, "obstaclePotentialModel", PotentialFieldObstacleOSM.class.getName());
      if (!beforeChange.equals(osmAttr.toString())) {
        logger.info(
            "\t- AttributesOSM: since AttributesPotentialOSM is present, set [pedestrianPotentialModel] to PotentialFieldPedestrianOSM "
  ...
  private JsonNode moveSpawnDelayIntoDistributionParametersIncident(JsonNode node) {
    ...
    if (sources.isMissingNode()) return node;
