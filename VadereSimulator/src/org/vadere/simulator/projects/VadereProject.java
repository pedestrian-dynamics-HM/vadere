...
public class VadereProject {
  ...
  private final BlockingQueue<ProjectFinishedListener> projectFinishedListener =
  ...
  public VadereProject(
      final String name, final Iterable<Scenario> scenarios, Path vadereProjectDirectory) {
    this.name = name;
  ...
  public boolean hasUnsavedChanges() {
    for (Scenario srm : getScenarios()) {
      if (srm.hasUnsavedChanges()) return true;
    }
    return false;
  }
  ...
  public void runScenarios(final Collection<Scenario> scenariosToRun) {
    // TODO [priority=normal] [task=bugfix] this is a bug: scenariosLeft may be overwritten even if
    // there are still scenarios in it
  ...
  private void notifySimulationListenersSimulationError(
      final Scenario scenario, final Throwable ex) {
    for (SingleScenarioFinishedListener l : singleScenarioFinishedListener) {
  ...
  private ScenarioRun prepareNextScenario() {
    ...
    final ScenarioRun scenarioRun =
        new ScenarioRun(
            nextScenario,
            scenarioFinishedListener,
            projectDirectory
                .resolve(IOUtils.SCENARIO_DIR)
                .resolve(nextScenario.getName() + IOUtils.SCENARIO_FILE_EXTENSION),
            loadCache(nextScenario));
  ...
  private RunnableFinishedListener scenarioFinishedListener =
      new RunnableFinishedListener() {
        @Override
