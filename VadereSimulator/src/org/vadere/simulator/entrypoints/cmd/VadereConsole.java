...
/**
 * Provides the possibility to start Vadere in console mode. Do not use Logging in this Class! The
 * Logging framework needs information generated here to configure itself.
 */
public class VadereConsole {
  ...
  private static void addSubCommandsToParser(ArgumentParser parser) {
    ...
    projectRun
        .addArgument("--project-dir", "-p")
        .required(true)
        .type(String.class)
        .dest("project-dir")
        .help("Path to project directory.");
    projectRun
        .addArgument("--scenario-checker")
        .required(false)
        .type(String.class)
    ...
    scenarioRun
        .addArgument("--override-timestep-setting")
    ...
    migrationAssistant
        ...
        .dest("paths")
        .help(
            "The scenario files or directories on to operate on. Directories containing"
    ...
    migrationAssistant
        ...
        .help(
            "Default: "
                ...
                + " Use one of the shown version strings to indicate the target version."
                + " If not specified the last version is used.");

    migrationAssistant
        .addArgument("--output-file", "-o")
        .required(false)
        .type(String.class)
        .metavar("OUTPUT-PATH")
        .dest("output-path")
        .help(
            "Write new version to this directory. If not specified backup input file and overwrite.");

    migrationAssistant
        .addArgument("--revert-migration")
        .required(false)
        .action(Arguments.storeTrue())
        .dest("revert-migration")
        .help(
            "If set vadere will search for a <scenario-file>.legacy and will replace the current version with this backup."
                + " The Backup must be in the same directory.");

    migrationAssistant
        .addArgument("--recursive", "-r")
        .required(false)
        .action(Arguments.storeTrue())
        .dest("recursive")
        .setDefault(false)
        .help(
            "If PATH contains a directory instead of a scenario file recursively search "
                + "the directory tree for scenario files and apply the command.");

    migrationAssistant
        .addArgument("--consider-projects-only")
        .required(false)
        .dest("consider-projects-only")
        .action(Arguments.storeTrue())
        .setDefault(false)
        .help(
            "If set only directories containing a vadere project will be migrated. "
                + "The migraion will use the legacy folder in the project.");

    migrationAssistant
        .addArgument("--create-new-version")
        .required(false)
        .type(String.class)
