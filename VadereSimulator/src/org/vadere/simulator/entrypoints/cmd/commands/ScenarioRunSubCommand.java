...
public class ScenarioRunSubCommand implements SubCommandRunner {
  private static final Logger logger = Logger.getLogger(ScenarioRunSubCommand.class);
  ...
  private boolean checkScenario(final Scenario scenario, String scenarioCheckerSwitch) {
    ...
    ConsoleScenarioCheckerMessageFormatter formatter =
        new ConsoleScenarioCheckerMessageFormatter(scenario);
  ...
  public void run(Namespace ns, ArgumentParser parser) {
    ...
    if (!outputDir.toFile().exists()) {
      if (!outputDir.toFile().mkdirs()) {
        logger.error(
            "Could not create all necessary directories: " + outputDir.toFile().toString());
        System.exit(-1);
      } else {
    ...
    try {
      ...
      if (checkScenario(scenario, scenarioCheckerSwitch)) {
        ScenarioCache cache =
            ScenarioCache.load(scenario, scenarioFile.toAbsolutePath().getParent());
        new ScenarioRun(
                scenario,
                outputDir.toFile().toString(),
                overrideTimeStepSetting,
                null,
                scenarioFile,
                cache)
            .run();
