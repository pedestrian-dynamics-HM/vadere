...
public class SuqSubCommand implements SubCommandRunner {
  private static final Logger logger = Logger.getLogger(SuqSubCommand.class);
  ...
  public void run(Namespace ns, ArgumentParser parser) {
    ...
    try {
      ...
      new ScenarioRun(scenario, outputDir.toFile().toString(), true, null, scenarioFile, cache)
          .run();
    } catch (Throwable e) {
      e.printStackTrace();
      logger.error(e);
