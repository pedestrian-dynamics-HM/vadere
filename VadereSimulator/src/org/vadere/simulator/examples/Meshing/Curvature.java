...
/**
 * Code to test the iterative eikonal solver discussed in the PhD thesis of B. Zoennchen (p. 187,
 * Alg. 20).
 *
...
public class Curvature {
  ...
  public static void interativeEikonalSolver() throws InterruptedException, IOException {
    ...
    for (int i = 0; i < 4; i++) {
      ...
      list.stream()
          .forEach(
              v -> curTri.getMesh().getAdjacentVertexIt(v).forEach(u -> initialVertices.add(u)));

      Set<PVertex> initialVertices2 = new HashSet<>();
      initialVertices
          .stream()
          .forEach(
              v -> curTri.getMesh().getAdjacentVertexIt(v).forEach(u -> initialVertices2.add(u)));
      ...
      for (var v : tri.getMesh().getVertices()) {
        double[] result =
            GeometryUtilsMesh.curvature(tri.getMesh(), v, vertex -> solver.getPotential(vertex));
      ...
      final var pTri = tri;
      Predicate<PHalfEdge> edgePredicate =
          e -> {
            ...
            double curvature =
                InterpolationUtil.barycentricInterpolation(x, y, z, totalArea, p.getX(), p.getY());
      ...
      GenRegularRefinement<PVertex, PHalfEdge, PFace> refinement =
          new GenRegularRefinement<>(triangulation, edgePredicate, i + 1);
  ...
  public static void adaptoveEikonalSolver() throws InterruptedException, IOException {
    ...
    for (var e : boundaryEdges) {
      ...
      VPoint p2 =
          improver
              .getTriangulation()
              .getMesh()
              .toPoint(improver.getTriangulation().getMesh().getNext(edge));
      VPoint p3 =
          improver
              .getTriangulation()
              .getMesh()
              .toPoint(improver.getTriangulation().getMesh().getPrev(edge));
    ...
    var solver =
        new MeshEikonalSolverFMMIterative<>(
  ...
  public static void adaptoveEikonalSolverSine() throws InterruptedException, IOException {
    ...
    ITimeCostFunction timeCostFunction =
        p -> {
          return 1.0
              / (0.8 * Math.sin(2 * Math.PI * p.getX()) * Math.sin(2 * Math.PI * p.getY()) + 1.0);
        };
    ...
    // (1) generate basic mesh
    var improver =
        new GenEikMesh<PVertex, PHalfEdge, PFace>(
            distanceFunction,
            p -> 0.1,
            Arrays.asList(q),
            0.1,
