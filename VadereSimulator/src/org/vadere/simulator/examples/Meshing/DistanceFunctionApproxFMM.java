...
public class DistanceFunctionApproxFMM implements IDistanceFunctionCached {
  ...
  public DistanceFunctionApproxFMM(
    ...
    var ruppertsTriangulator =
        new PRuppertsTriangulator(boundedPSLG, circumRadiusFunc, 10, false, false);
    ...
    for (var v : triangulation.getMesh().getVertices()) {
      ...
      if (pslg.getSegmentBound().contains(v)
          && pslg.getHoles().stream().noneMatch(poly -> poly.contains(p))) {
  ...
  /**
   * public EikonalSolverFMMTriangulation(@NotNull final ITimeCostFunction
   * timeCostFunction, @NotNull final IIncrementalTriangulation<V, E, F> triangulation, @NotNull
   * final Collection<V> targetVertices
   *
   * @param pslg
   */
  ...
  public void printPython() {
    System.out.println(
        triangulation.getMesh().toPythonTriangulation(v -> eikSolver.getPotential(v)));
