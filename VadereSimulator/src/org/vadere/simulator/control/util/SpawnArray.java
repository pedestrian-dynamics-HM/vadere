  ...
  public SpawnArray(
    ...
    if (xDim * yDim <= 0) {
      ...
      eX =
          (xDim == 1)
              ? bound.getCenterX()
              : spawnElementBound.x + spawnElementBound.width / 2 + SPAWN_BUFFER;
      eY =
          (yDim == 1)
              ? bound.getCenterY()
              : spawnElementBound.y + spawnElementBound.height / 2 + SPAWN_BUFFER;
      logger.info(
          String.format(
    ...
    for (int i = 0; i < (xDim * yDim); i++) {
      VPoint candidatePoint =
          firstSpawnPoint.add(new VPoint(2 * eX * (i % xDim), 2 * eY * (i / xDim)));
