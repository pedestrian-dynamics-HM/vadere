 ...
 * <li>NoneOverlapping: With this strategy a group spawns do not overlap. E.g The first 2x2 Group
 *     will start at (0,0) and the second at (2,0) under the assumption the underlying source is big
 *     enough to hold two 2x2 groups in the x dimension.
 * <li>Overlapping: With this strategy the second group will start at (1,0)
 */
public class GroupPlacementHelper {
  ...
  public GroupPlacementHelper(
      int boundedShapeGridCellsX,
      int boundedShapeGridCellsY,
      int groupSize,
      HashMap<Integer, Integer> validSpawnPointMapInBoundShape) {
    ...
    // dimGy set to minimize lost space in resulting rectangle (or square if groupSize is a square
    // number)
   ...
   * @return true if all positions within the group are contained within the source shape.
   */
  boolean isGridCellWithinSource(
      HashMap<Integer, Integer> validSpawnPointMapInBoundShape, int groupIndex) {
    for (int pedIndexInGroup = 0; pedIndexInGroup < groupSize; pedIndexInGroup++) {
      boolean isValid =
          validSpawnPointMapInBoundShape.containsKey(
              getOverlappingIndex(groupIndex, pedIndexInGroup));
      if (!isValid) {
        return false;
      }
    }
  ...
  public int getOverlappingStart(int groupNumber) {
    return (groupNumber % groupPlacementCountX)
        + // offset in x
        (groupNumber / groupPlacementCountX)
            * boundedShapeGridCellsX; // offset in y (groupDimY not needed)
  }
