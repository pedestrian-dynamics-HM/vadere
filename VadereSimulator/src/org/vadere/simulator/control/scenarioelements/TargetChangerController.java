 ...
 *
 * <p>{@link TargetChanger}'s attributes contain two important parameters to control the changing
 * behavior:
 *
 * <ul>
 *   <li>"changeTargetProbability": This defines how many percent of the agents, who enter the area,
 *       should change their target.
 *   <li>If "nextTargetIsPedestrian == false", assign a new static target. Otherwise, randomly
 *       choose a pedestrian (with given target id) to follow.
 * </ul>
 */
public class TargetChangerController extends ScenarioElementController {
  ...
  // Constructors
  public TargetChangerController(
      Topography topography, TargetChanger targetChanger, Random random) {
  ...
  private Collection<DynamicElement> getDynamicElementsNearTargetChangerArea() {
    ...
    final double reachRadius =
        Math.max(areaBounds.getHeight(), areaBounds.getWidth()) + reachDistance;

    List<Pedestrian> pedestriansNearArea =
        topography.getSpatialMap(Pedestrian.class).getObjects(areaCenter, reachRadius);
