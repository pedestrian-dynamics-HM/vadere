...
public class TopographyController extends OfflineTopographyController
    implements ControllerEventProvider<Agent, TopographyController> {
  ...
  protected ArrayList<ControllerEventListener<Agent, TopographyController>> eventListeners =
      new ArrayList<>();

  public TopographyController(
      Domain domain, DynamicElementFactory dynamicElementFactory, final Random random) {
    super(domain, random);
    this.domain = domain;
  ...
  public void preLoop(double simTimeInSec, List<Attributes> attributesList) {
    // If there is no background mesh these attributes are used to constrcut a distance function by
    // using a cellgrid of size defined by AttributesFloorField.
    ...
    try {
      ...
      logger.warn(
          "no "
              + AttributesFloorField.class.getName()
              + " found, the default values are used instead.");
  ...
  private void createAgentWrapperPedestrians(double simTimeInSec) {
    for (Pedestrian agentWrapper : domain.getTopography().getInitialElements(Pedestrian.class)) {
      ...
      Pedestrian createdPedestrian =
          (Pedestrian)
              dynamicElementFactory.createElement(
                  agentWrapper.getPosition(), agentWrapper.getId(), Pedestrian.class);
  ...
  private void applyAttributesFromAgentWrapper(Pedestrian agentWrapper, Pedestrian newPedestrian) {
    ...
    if (!Double.isNaN(agentWrapper.getVelocity().x)
        && !Double.isNaN(agentWrapper.getVelocity().y)) {
    ...
    agentWrapper
        .getTrajectory()
        .getFootSteps()
        .forEach(footStep -> newPedestrian.addFootStepToTrajectory(footStep));
