...
public class SortedSubListTargetChanger extends BaseTargetChangerAlgorithm {
  ...
  public void throwExceptionOnInvalidInput(TargetChanger targetChanger) {
    ...
    if (!inputIsValid) {
      throw new IllegalArgumentException(
          String.format(
              "The size of \"probabilitiesToChangeTarget\" and \"nextTarget\" must be equal for %s algorithm",
              TargetChangerAlgorithmType.SORTED_SUB_LIST));
    }
  }
  ...
  public void init(Random rnd) {
    ...
    for (Double probability : targetChanger.getAttributes().getProbabilitiesToChangeTarget()) {
      ...
      binomialDistributions.add(
          new BinomialDistribution(
              randomGenerator, BINOMIAL_DISTRIBUTION_SUCCESS_VALUE, probability));
    }
  }
