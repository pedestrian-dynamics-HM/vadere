...
public class FollowPedestrianTargetChanger extends BaseTargetChangerAlgorithm {
  ...
  public void throwExceptionOnInvalidInput(TargetChanger targetChanger) {
    ...
    if (!inputIsValid) {
      throw new IllegalArgumentException(
          String.format(
              "The size of \"probabilitiesToChangeTarget\" must be 1 for. %s",
              TargetChangerAlgorithmType.FOLLOW_PERSON));
    }
  }
  ...
  public boolean setAgentTargetList(Agent agent) {
    ...
    // check probability
    if (binomialDistribution.sample() != BINOMIAL_DISTRIBUTION_SUCCESS_VALUE) return false;

    int nextTarget =
        (targetChanger.getAttributes().getNextTarget().size() > 0)
    ...
    List<Pedestrian> pedsWithCorrectTargetId =
        allPedestrians
            .stream()
    ...
    if (pedsWithCorrectTargetId.size() > 0) {
      ...
      List<Pedestrian> pedsWithFollowers =
          pedsWithCorrectTargetId
              .stream()
              .filter(pedestrian -> pedestrian.getFollowers().isEmpty() == false)
              .collect(Collectors.toList());

      Pedestrian pedToFollow =
          (pedsWithFollowers.isEmpty()) ? pedsWithCorrectTargetId.get(0) : pedsWithFollowers.get(0);
