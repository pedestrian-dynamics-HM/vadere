 ...
 *
 * <p>Take following attributes into account when absorbing agents: - {@link
 * org.vadere.state.attributes.scenario.AttributesAbsorbingArea#getDeletionDistance()}
 *
 * <p>
 */
public class AbsorbingAreaController extends ScenarioElementController {
  ...
  private Collection<DynamicElement> getDynamicElementsNearAbsorbingArea() {
    ...
    final VPoint centerOfAbsorbingArea =
        new VPoint(absorbingAreaBounds.getCenterX(), absorbingAreaBounds.getCenterY());

    final double deletionDistance = absorbingArea.getAttributes().getDeletionDistance();
    final double deletionRadius =
        Math.max(absorbingAreaBounds.getHeight(), absorbingAreaBounds.getWidth())
            + deletionDistance;

    final Collection<DynamicElement> elementsNearAbsorbingArea = new LinkedList<>();

    List<Pedestrian> pedestriansNearAbsorbingArea =
        topography
            .getSpatialMap(Pedestrian.class)
            .getObjects(centerOfAbsorbingArea, deletionRadius);
