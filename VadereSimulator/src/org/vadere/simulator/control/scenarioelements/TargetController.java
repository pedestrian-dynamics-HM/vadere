...
public class TargetController extends ScenarioElementController {
  ...
  public TargetController(Topography topography, Target target, Random random) {
    ...
    if (targetAttributes.isWaiting()) {
      try {
        distribution =
            DistributionFactory.create(
                targetAttributes.getWaiterAttributes().getDistribution(),
                new JDKRandomGenerator(random.nextInt()));

      } catch (Exception e) {
        throw new IllegalArgumentException(
            "Problem with scenario parameters for target: "
                + "waitingTimeDistribution and/or distributionParameters. See causing Excepion herefafter.",
            e);
      }
    }
  }
  ...
  public void update(double simTimeInSec) {
    ...
    for (DynamicElement element : getPrefilteredDynamicElements()) {
      ...
      final boolean agentHasReachedThisTarget =
          isNextTargetForAgent(agent) && hasAgentReachedThisTarget(agent);
  ...
  private Collection<DynamicElement> getPrefilteredDynamicElements() {
    final double reachedDistance =
        target.getAttributes().getAbsorberAttributes().getDeletionDistance();
  ...
  private void handleArrivingAgent(
      Agent agent, double simTimeInSec, Map<Integer, Double> leavingTimes) {
    final int agentId = agent.getId();
    final int waitingSpots = target.getParallelWaiters();

    final boolean targetHasFreeWaitingSpots =
        waitingSpots <= 0 || (waitingSpots > 0 && leavingTimes.size() < waitingSpots);
  ...
  private <T extends DynamicElement> List<T> getObjectsInCircle(
      final Class<T> clazz, final VPoint center, final double radius) {
  ...
  private boolean isNextTargetForAgent(Agent agent) {
    ...
    if (agent.hasNextTarget()) {
      if (agent.getNextTargetId() == target.getId() && !agent.isCurrentTargetAnAgent())
