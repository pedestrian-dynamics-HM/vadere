 ...
 *
 * <p>ChangeTargetScripted > ChangeTarget > Threat > Wait > WaitInArea > ElapsedTime
 */
public class SimplePerceptionModel extends PerceptionModel {
  ...
  public void update(HashMap<Pedestrian, List<Stimulus>> pedSpecificStimuli) {
    for (Pedestrian pedestrian : pedSpecificStimuli.keySet()) {
      Stimulus mostImportantStimulus =
          rankChangeTargetAndThreatHigherThanWait(pedSpecificStimuli.get(pedestrian), pedestrian);
  ...
  private Stimulus rankChangeTargetAndThreatHigherThanWait(
      List<Stimulus> stimuli, Pedestrian pedestrian) {
    ...
    List<String> attr =
        attributes.getSortedPriorityQueue().values().stream().collect(Collectors.toList());
    Collections.reverse(attr);
    for (String typeName : attr) {
      Stimulus stimulus = StimulusFactory.stringToStimulus(typeName);
      if (stimulus != null) {
        stimuliSorted.addAll(
            stimuli
                .stream()
                .filter(s -> s.getClass() == stimulus.getClass())
                .collect(Collectors.toList()));
      }
    }
