 ...
 *
 * <p>Check following conditions for a pedestrian:
 *
 * <ol>
 *   <li>Is pedestrian inside threat area.
 *   <li>Is pedestrian outside threat area.
 *   <li>If pedestrian is outside threat area, test if other pedestrians are nearby who have
 *       perceived the threat. If so, imitate their behavior if they are in-group members.
 * </ol>
 */
public class ThreatCognitionModel implements ICognitionModel {
  ...
  private void handleThreat(Pedestrian pedestrian, Stimulus stimulus) {
    ...
    // Current stimulus is a threat => store it and make clear that pedestrian is inside threat
    // area.
  ...
  private void handleElapsedTime(Pedestrian pedestrian) {
    ...
    if (latestThreat != null) {
      ...
      if (pedestrian.getGroupMembership() == GroupMembership.OUT_GROUP) {
        ...
        throw new IllegalArgumentException(
            "Can only process \"IN_GROUP\" and \"OUT_GROUP\" group membership!");
      }
    }
  }
  ...
  private void testIfInsideOrOutsideThreatArea(Pedestrian pedestrian, Threat latestThreat) {
    VPoint threatOrigin =
        topography.getTarget(latestThreat.getOriginAsTargetId()).getShape().getCentroid();
    ...
    boolean pedestrianIsBlockedByObstacle =
        TopographyHelper.pedestrianIsBlockedByObstacle(pedestrian, topography);
  ...
  /**
   * If a threatened ingroup pedestrian is nearby, use the same reaction as if the current
   * "pedestrian" would have perceived the same threat. I.e, imitate the behavior of the perceived
   * and threatened ingroup member:
   *
   * <ol>
   *   <li>Firstly, accelerate and get out of threat area.
   *   <li>Then, search for a safe zone.
   * </ol>
   *
  ...
  private void imitateThreatenedNeighborIfPresent(Pedestrian pedestrian) {
    List<Pedestrian> threatenedNeighbors =
        TopographyHelper.getNeighborsWithSelfCategory(
            pedestrian, SelfCategory.COMMON_FATE, topography);
    List<Pedestrian> threatenedIngroupNeighbors =
        threatenedNeighbors
            .stream()
