...
/**
 * The {@link CooperativeCognitionModel} makes a pedestrian cooperative if its average speed falls
 * below a certain threshold. I.e., usually the agent could not move for some time steps. For
 * example, in case of other counter-flowing agents.
 *
 * <p>{@link SelfCategory#COOPERATIVE} should motivate pedestrians to swap places instead of blindly
 * walking to a target and colliding with other pedestrians.
 */
  ...
  public void update(Collection<Pedestrian> pedestrians) {
    for (Pedestrian pedestrian : pedestrians) {
      if (pedestrianCannotMove(pedestrian)) {
        ...
        // Maybe, check if area directed to target is free for a step (only then change to
        // "TARGET_ORIENTED").
