...
/**
 * The {@link CounterflowCognitionModel} enables a pedestrian to evade if counter-flowing agents are
 * detected. The locomotion layer must implemenet the specific evasion behavior (e.g. tangentially).
 *
 * <p>Note: Maybe, combine this behavior also with {@link SelfCategory#COOPERATIVE} to avoid jams.
 */
  ...
  public void update(Collection<Pedestrian> pedestrians) {
    for (Pedestrian pedestrian : pedestrians) {
      ...
      if (pedestrian.hasNextTarget()) {
        Pedestrian neighborCloserToTarget =
            TopographyHelper.getNeighborCloserToTargetCentroid(pedestrian, topography);

        if (neighborCloserToTarget != null) {
          if (TopographyHelper.walkingDirectionDiffers(
              pedestrian, neighborCloserToTarget, topography)) {
            pedestrian.setSelfCategory(SelfCategory.EVADE);
          } else if (neighborCloserToTarget.getSelfCategory()
              == SelfCategory.EVADE) { // Imitate behavior
            pedestrian.setSelfCategory(SelfCategory.EVADE);
          }
        }
      }
