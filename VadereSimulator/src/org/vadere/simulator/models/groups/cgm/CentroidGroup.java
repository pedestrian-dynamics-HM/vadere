...
public class CentroidGroup implements Group {
  ...
  public static final int POTENTIAL_DISTANCE_THRESHOLD = 1000;
  public static final int MAX_NO_VISION_OF_LEADER = 25;

  private final int id;
  private final int size;

  protected final ArrayList<Pedestrian> members;
  ...
  public CentroidGroup(int id, int size, CentroidGroupModel model) {
    this.id = id;
    this.size = size;
    this.model = model;
  ...
  public Optional<Pedestrian> getMember(int pedId) {
    return members.stream().filter(ped -> ped.getId() == pedId).findAny();
  }
  ...
  private void initGroupVelocity() {
    double minVelocity =
        getMembers().stream().map(Agent::getFreeFlowSpeed).min(Double::compareTo).orElseThrow();
  ...
  public boolean isCentroidWithinObstacle() {
    if (members.size() - lostMembers.size() <= 2) {
      return getPairIntersectObstacle()
          ...
          .filter(
              pair ->
                  !isLostMember(pair.getKey().getLeft()) && !isLostMember(pair.getKey().getRight()))
  ...
  private void reevaluateAll() {
    if (this.lostMembers.size() > this.members.size() / 2) {
      ...
      if (membersMostCommonTarget.size() > this.members.size() / 2) {
        ...
        Map<Boolean, List<Pedestrian>> partitions =
            membersMostCommonTarget
                .stream()
                .collect(
                    Collectors.partitioningBy(
                        p -> Math.abs(getRelativeDistanceCentroid(p, true, false)) < 8));
        if (partitions.get(true).size() > lostMembers.size()) {
          ...
          lostMembers.addAll(
              members
                  .stream()
                  .filter(m -> !membersMostCommonTarget.contains(m))
  ...
  private List<Pedestrian> getMembersMostCommonTarget() {
    List<Integer> targetIds =
        members.stream().map(m -> m.getNextTargetId()).collect(Collectors.toList());
  ...
  public boolean isGroupTarget(int targetId) {
    Optional<Pedestrian> ped = members.stream().filter(memb -> !isLostMember(memb)).findAny();
    return ped.map(pedestrian -> pedestrian.getNextTargetId() == targetId).orElse(true);
  }

  /**
   * The method returns the maximal euclidean distance to another group member. The other group
   * member is the Key (left element) of the Pair.
   *
  ...
  public Pair<Pedestrian, Double> getMaxDistPedIdInGroup(Pedestrian ped) {
    Pair<PedestrianPair, Double> maxDist =
        getEuclidDist()
            .stream()
            // only compare member-Pairs in which the ped is a part of. Either left or right
            .filter(
                pair -> pair.getKey().getLeft().equals(ped) || pair.getKey().getRight().equals(ped))
  ...
  /**
   * Calculates the distance of the given pedestrian to the centroid of the group based on the sum
   * of the potential fields of all other group members.
   *
   * @param ped Pedestrian
   * @return Distance to group centroid
   */
  public double getRelativeDistanceCentroid(
      Pedestrian ped, boolean includeLostMembers, boolean useGroupTarget) {
    double result = 0.0;
  ...
  public void setGroupTargetList(LinkedList<Integer> targetIds, int agentId) {
    ...
    if (this.members
            .stream()
            .filter(ped -> !isLostMember(ped))
            .map(ped -> ped.getNextTargetId())
      ...
      log.error(
          "Group "
              + this.getID()
              + ": all groups members should have same target after targetchange but did not.");
    }
  }

  /**
   * if the targets of one group member are changed, also changed the targets of the other group
   * members.
   *
   * @param targetIds
   */
  ...
  /**
   * if one group member has reached its target and is assigned the next target, also assign the
   * next target for the other members. Important for speed adjusting and TargetPotentials which do
   * only work, if targets are shared.
   *
   * @param nextSpeed
   */
  @Override
    ...
    if (element instanceof TargetChanger) {
      ...
      boolean eventPending =
          this.getMembers()
                  ...
                  .filter(
                      p -> p.getElementsEncountered(TargetChanger.class).contains(element.getId()))
                  .count()
              == 1;
