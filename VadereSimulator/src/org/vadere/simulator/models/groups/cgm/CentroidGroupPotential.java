...
public class CentroidGroupPotential implements PotentialFieldAgent {
  ...
  public CentroidGroupPotential(
      CentroidGroupModel groupCollection,
  ...
  public double getAgentPotential(
    ...
    result += getPedestrianRepulsionPotential(ped, pos, closePedestrians);
  ...
  private double getPedestrianGroupPotential(Pedestrian ped, IPoint pos) {
    ...
    if (leader != null) {
      ...
      final double[] distanceToCentroid = {
        pos.getX() - leaderPoint.getX(), pos.getY() - leaderPoint.getY()
      };

      result =
          attributesCGM.getLeaderAttractionFactor() * Math.pow(leaderPotential - pedPotential, 4);
    }

    return result;
  }

  private double getPedestrianRepulsionPotential(
      Pedestrian ped, IPoint pos, Collection<? extends Agent> closePedestrians) {
    double potential = 0;
  ...
  public double getAgentPotential(IPoint pos, Agent pedestrian, Agent otherPedestrian) {
    ...
    double potential = potentialFieldPedestrian.getAgentPotential(pos, pedestrian, otherPedestrian);
