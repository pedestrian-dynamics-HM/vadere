 ...
 * Automaton' (bib-key: seitz-2014). The basic idea from seitz-2014: <quote> Firstly members walking
 * ahead slow down and members falling behind slightly speed up to reach the group. </quote>
 *
 * <p>Agents which are added to the topography (before the simulation starts) are assigned to groups
 * directly in the {@link #preLoop(double)}. Agents which are spawned later on, are assigned to
 * groups in the callback {@link #elementAdded(Pedestrian)}. The actual behavior of the central
 * group model is implemented in the helper class {@link CentroidGroup}.
 */
@ModelClass
