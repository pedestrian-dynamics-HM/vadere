...
public abstract class ODEModel<
        T extends DynamicElement, TAttributes extends AttributesDynamicElement>
  ...
  public ODEModel(
      ...
      AbstractModelEquations<T> equations,
      TAttributes elementAttributes,
      Random random) {
    super();
    this.type = type;
    this.random = random;
  ...
  public void initializeODEModel(
      Class<T> type,
      FirstOrderIntegrator integrator,
      AbstractModelEquations<T> equations,
      TAttributes elementAttributes,
  ...
  public void update(final double simTimeInSec) {
    ...
    List<T> orderedDynamicElements =
        domain.getTopography().getElements(type).stream().collect(Collectors.toList());
    List<VPoint> positions =
        orderedDynamicElements.stream().map(ped -> ped.getPosition()).collect(Collectors.toList());
      ...
      for (int i = 0; i < orderedDynamicElements.size(); i++) {
        ...
        if (element.getType() == ScenarioElementType.PEDESTRIAN) {
          ...
          FootStep currentFootstep =
              new FootStep(
                  positions.get(i), pedestrian.getPosition(), lastSimTimeInSec, simTimeInSec);
   ...
   * double vector. The {@link AbstractModelEquations} are used to get the correct positions from
   * the vector.
   */
  public static <T extends DynamicElement> void updateElementPositions(
      Class<T> type,
      double t,
      Topography topography,
      AbstractModelEquations equations,
      double[] y) {
    ...
    for (DynamicElement element : dynamicElements) {
      ...
      if (topography.hasTeleporter()) {
        if (newPosition[0] > topography.getTeleporter().getTeleporterPosition().x) {
          newPosition[0] += topography.getTeleporter().getTeleporterShift().x;
        }
        if (newPosition[0]
            < topography.getTeleporter().getTeleporterPosition().x
  ...
  private static void setVelocity(DynamicElement element, Vector2D vector2d) {
    ...
    } else throw new IllegalArgumentException("Element is neither a car nor a pedestrian.");
  }
