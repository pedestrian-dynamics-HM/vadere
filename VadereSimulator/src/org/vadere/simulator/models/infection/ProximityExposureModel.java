...
/**
 * ProximityExposureModel describes the degree of exposure of <code>Pedestrian</code>s to another
 * infectious one simply by their mutual distance.
 *
 * <p>
 *
 ...
 *
 * <ul>
 *   <li>Each pedestrian obtains a {@link BasicExposureModelHealthStatus health status} after being
 *       inserted into the topography.
 *   <li>Any pedestrian that approaches an infectious pedestrian so that the mutual distance falls
 *       below a defined threshold becomes exposed. The threshold is defined in {@link
 *       AttributesProximityExposureModel}.
 * </ul>
 */
@ModelClass
public class ProximityExposureModel extends AbstractExposureModel {
  ...
  public void initialize(
    ...
    this.attributesProximityExposureModel =
        Model.findAttributes(attributesList, AttributesProximityExposureModel.class);
  ...
  public void update(double simTimeInSec) {
    ...
    Collection<Pedestrian> infectiousPedestrians =
        pedestrians.stream().filter(Pedestrian::isInfectious).collect(Collectors.toSet());

    if (!infectiousPedestrians.isEmpty()) {
      for (Pedestrian infectiousPedestrian : infectiousPedestrians) {
        Collection<Pedestrian> exposedPedestrians =
            getPedestriansNearbyInfectiousPedestrian(pedestrians, infectiousPedestrian);
  ...
  @NotNull
  private Collection<Pedestrian> getPedestriansNearbyInfectiousPedestrian(
      Collection<Pedestrian> pedestrians, Pedestrian infectiousPedestrian) {
    VPoint position = infectiousPedestrian.getPosition();
    VShape areaOfExposure =
        new VCircle(position, attributesProximityExposureModel.getExposureRadius());
    ...
    Collection<Pedestrian> exposedPedestrians =
        pedestrians
            ...
            .filter(
                p ->
                    (areaOfExposure.contains(p.getPosition()))
                        && p.getId() != infectiousPedestrian.getId())
            .collect(Collectors.toSet());
    return exposedPedestrians;
  }

  /**
   * This simple approach allows only 0 or {@link #MAX_DEG_OF_EXPOSURE}. The degree of exposure is
   * increased only once.
   */
  @Override
  ...
  public Agent sourceControllerEvent(
    ...
    AttributesExposureModelSourceParameters sourceParameters =
        defineSourceParameters(controller, attributesProximityExposureModel);
  ...
  public Pedestrian topographyControllerEvent(
    ...
    if (attributesProximityExposureModel
        .getInfectiousPedestrianIdsNoSource()
        .contains(agent.getId())) {
