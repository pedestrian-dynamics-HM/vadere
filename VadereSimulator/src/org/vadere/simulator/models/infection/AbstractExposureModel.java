...
/**
 * AbstractExposureModel is the abstract base class for all exposure models which allow to describe
 * a <code>Pedestrian</code>'s degree of exposure, more precisely, its {@link
 * ExposureModelHealthStatus health status}.
 *
 * <p>An exposure model can be included in the simulation by defining the desired model in the
 * scenario file in the list of <code>submodels</code>.
 */
public abstract class AbstractExposureModel implements Model {
  ...
  abstract void updatePedestrianDegreeOfExposure(
      final Pedestrian pedestrian, double degreeOfExposure);
  ...
  public AttributesExposureModelSourceParameters defineSourceParameters(
      SourceController controller, AttributesExposureModel attributes) {
    int sourceId = controller.getSourceId();
    int defaultSourceId = -1;
    Optional<AttributesExposureModelSourceParameters> sourceParameters =
        attributes
    ...
    // if sourceId not set by user, check if the user has defined default attributes by setting
    // sourceId = -1
    if (sourceParameters.isEmpty()) {
      sourceParameters =
          attributes
              .getExposureModelSourceParameters()
              .stream()
              .filter(s -> s.getSourceId() == defaultSourceId)
              .findFirst();

      // if no user defined default values: use attributesAirTransmissionModel default values
      if (sourceParameters.isPresent()) {
        logger.infof(
            ">>>>>>>>>>>defineSourceParameters: sourceId %d not set explicitly exposureModelSourceParameters. Source uses default exposureModelSourceParameters defined for sourceId: %d",
            sourceId, defaultSourceId);
      } else {
        logger.errorf(
            ">>>>>>>>>>>defineSourceParameters: sourceId %d is not set in exposureModelSourceParameters",
            sourceId);
      }
    }
  ...
  /**
   * Assures that each pedestrian that is spawned by sources obtains properties related to {@link
   * AttributesExposureModel} when pedestrian is added to topography.
   */
  protected abstract Agent sourceControllerEvent(
   ...
   */
  protected abstract Pedestrian topographyControllerEvent(
