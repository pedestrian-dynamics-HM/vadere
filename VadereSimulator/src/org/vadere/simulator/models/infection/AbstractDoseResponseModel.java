...
/**
 * AbstractDoseResponseModel is the abstract base class for all dose response models which allow to
 * describe a <code>Pedestrian</code>'s probability of infection, more precisely its {@link
 * DoseResponseModelInfectionStatus}.
 *
 * <p>A dose response model can be included in the simulation by defining the desired model in the
 * scenario file in the list of <code>submodels</code>.
 *
 * <p>Any dose response model requires that an exposure model is defined.
 *
...
public abstract class AbstractDoseResponseModel implements Model {
   ...
   */
  protected void checkIfExposureModelDefined(List<Attributes> attributesList)
      throws AttributesNotFoundException {
    Set<Attributes> result =
        attributesList
            .stream()
            .filter(a -> AttributesExposureModel.class.isAssignableFrom(a.getClass()))
            .collect(Collectors.toSet());
    if (result.size() < 1) {
      throw new RuntimeException(
          this.getClass()
              + " requires any exposure model defined by "
              + AttributesExposureModel.class);
    }
  }

  /**
   * Assures that each pedestrian that is spawned by sources obtains properties related to {@link
   * AttributesDoseResponseModel} when pedestrian is added to topography.
   */
