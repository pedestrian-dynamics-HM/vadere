...
public class PotentialFieldPedestrianCompact implements PotentialFieldAgent {
  ...
  public Collection<Pedestrian> getRelevantAgents(
      @NotNull final VCircle stepDisc, Agent pedestrian, Topography scenario) {
  ...
  public Vector2D getAgentPotentialGradient(
    ...
    for (Agent neighbor : otherPedestrians) {
      if (neighbor != pedestrian) {
        gradient = gradient.add(getAgentPotentialGradient(pos, pedestrian, neighbor));
      }
    }
  ...
  public Vector2D getAgentPotentialGradient(IPoint pos, Agent pedestrian, Agent otherPedestrian) {
