...
public class PotentialFieldObstacleCompact implements PotentialFieldObstacle {
  ...
  private void init(
      final AttributesPotentialCompact attributes, final Domain domain, final Random random) {
    this.attributes = attributes;
    this.domain = domain;
    this.obstacles = new ArrayList<>(domain.getTopography().getObstacles());
    this.random = random;
    this.width =
        attributes.getObstPotentialWidth()
  ...
  public double getObstaclePotential(IPoint pos, Agent pedestrian) {
    ...
    if (potential < currentPotential) potential = currentPotential;
  ...
  private Vector2D getObstaclePotentialGradient(
    ...
    if (distance >= 0 && distance < this.width) {
      ...
      double dp =
          -2 * height * distance * width * width / Math.pow(distance * distance - width * width, 2);
  ...
  public PotentialFieldObstacle copy() {
    PotentialFieldObstacleCompact potentialFieldObstacleCompact =
        new PotentialFieldObstacleCompact();
