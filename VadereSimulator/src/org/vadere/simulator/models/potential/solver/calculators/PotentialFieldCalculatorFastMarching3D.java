...
/**
 * Uses the fast-marching algorithm to solve the Eikonal equation (F(x) * |grad phi|=1) in a given
 * 3D room.
 */
public class PotentialFieldCalculatorFastMarching3D {
  ...
  private Vector3D[] neighborPoints =
      new Vector3D[] {
  ...
  private Vector3D neighbors(int index) {
    return new Vector3D(neighborPoints[index].x, neighborPoints[index].y, neighborPoints[index].z);
  }

  /**
   * Initializes the FM potential calculator with a time cost function F > 0.
   *
   * @param timeCostFunction an ITimeCostFunction, that defines the time cost ( > 0 ) at every point
   *     on the grid.
   */
  public PotentialFieldCalculatorFastMarching3D(ITimeCostFunction3D timeCostFunction) {
  ...
  public double[][][] recalculate(
      double[][][] potential, CellStateFD[][][] elements, List<Vector3D> targets) {
    ...
    while (narrowBand.size() > 0) {
      ...
      addNearPoints(nearPoint.p, narrowBand, frozenPoints, elements, potential);
    }
  ...
  /**
   * Recalculates the potential at the given point, using the neigbor points and the high accuracy
   * version.
   *
   ...
   */
  private double recalculateAt(Vector3D p, TreeSet<Vector3D> frozenPoints, double[][][] potential) {
    int x = (int) (p.x);
    int y = (int) (p.y);
    int z = (int) (p.z);
    ...
    for (int j = 0; j < 3; j++) {
      ...
      for (int i = 0; i < 2; i++) {
        ...
        if (pk.x >= 0 && pk.y >= 0 && pk.z >= 0 && pk.x < width && pk.y < height && pk.z < depth) {
          if (frozenPoints.contains(pk)) {
            ...
            if (_val1 < val1) {
              ...
              if (pk2.x >= 0
                ...
                if (frozenPoints.contains(new Vector3D((int) pk2.x, (int) pk2.y, (int) pk2.z))
                    && _val2 <= _val1) {
                  val2 = _val2;
                } else {
                  val2 = Double.MAX_VALUE;
  ...
  /**
   * add all near points (in the neighborhood) to the given point. the neighbors potential is
   * evaluated using the recalculateAt function.
   *
  ...
  private void addNearPoints(
      Vector3D t,
      TreeSet<DistPoint3D> narrowBand,
      TreeSet<Vector3D> frozenPoints,
      CellStateFD[][][] elements,
      double[][][] potential) {
    int x = (int) (t.x);
    int y = (int) (t.y);
    int z = (int) (t.z);
    ...
    for (Vector3D n : neighborPoints) {
      ...
      if (i >= 0
        ...
        if (elements[i][j][k] != CellStateFD.OBSTACLE && !frozenPoints.contains(p)) {
