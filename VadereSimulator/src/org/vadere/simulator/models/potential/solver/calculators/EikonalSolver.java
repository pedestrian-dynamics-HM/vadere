...
public interface EikonalSolver {
  ...
  /**
   * Computes potentials on basis of the given data. Should be called from outside only once for
   * initialization.
   */
  void solve();

  /**
   * Recomputes the potentials. May be called every simulation step. May contain an empty
   * implementation for static floor field initializers.
   */
  default void update() {}

  /**
   * Returns true if the potential field needs an update. The value indicates the type of
   * initializer: static or dynamic floor field. Initializer of static floor fields may return
   * false, initializers of dynamic floor fields may return true.
   */
  ...
  default double getPotential(
      final IPoint pos, final double unknownPenalty, final double weight, final Object caller) {
    return getPotential(pos, unknownPenalty, weight);
  }

  /**
   * Returns a copy of the current (for the current F which might change over time) solution of the
   * eikonal equation.
   *
  ...
  /**
   * Load floor field from given file path. This method does not check if the cached floor field is
   * up to date with the current state of the scenario. This must be checked by the caller
   * beforehand.
   *
   * <p>The default behavior does not support caching. To support caching override this method and
   * implement the needed logic. see {@link
   * org.vadere.simulator.models.potential.solver.calculators.cartesian.GridEikonalSolver} for
   * implementation.
   *
