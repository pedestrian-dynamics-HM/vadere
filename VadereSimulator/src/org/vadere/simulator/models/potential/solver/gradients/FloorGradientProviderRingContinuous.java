...
public class FloorGradientProviderRingContinuous implements GradientProvider {
  ...
  public FloorGradientProviderRingContinuous(
      VPoint center, double radiusSmall, double radiusBig, double xStretch, double yStretch) {
  ...
  public void gradient(double t, int currentTargetId, double[] x, double[] grad) {
    ...
    if (Math.abs(x[0] - this.center.x) <= this.xStretch) {
      ...
      grad[1] +=
          Math.signum(localPos[1])
              * OBSTACLE_HEIGHT
              * MathUtil.cutExp(distToRadiusBig, FloorGradientProviderRingContinuous.CUTOFF);
      double distToRadiusSmall = Math.abs(distance - radiusSmall);
      grad[1] +=
          -Math.signum(localPos[1])
              * OBSTACLE_HEIGHT
              * MathUtil.cutExp(distToRadiusSmall, FloorGradientProviderRingContinuous.CUTOFF);
    ...
    if (distance > this.radiusBig - FloorGradientProviderRingContinuous.CUTOFF) {
      double distToRadius = Math.abs(distance - radiusBig);
      grad[0] +=
          (localPos[0])
              * OBSTACLE_HEIGHT
              * MathUtil.cutExp(distToRadius, FloorGradientProviderRingContinuous.CUTOFF);
      grad[1] +=
          (localPos[1])
              * OBSTACLE_HEIGHT
              * MathUtil.cutExp(distToRadius, FloorGradientProviderRingContinuous.CUTOFF);
    ...
    if (distance < this.radiusSmall + FloorGradientProviderRingContinuous.CUTOFF) {
