 ...
 *
 */
public interface IMeshEikonalSolver<V extends IVertex, E extends IHalfEdge, F extends IFace> extends EikonalSolver {
	...
	default double computePotential(@NotNull final V point, V point1, V point2, double cosPhi, @NotNull final LocalSover localSover) {
		...
		VPoint p = getMesh().vertices().toPoint(point);
		VPoint p1 = getMesh().vertices().toPoint(point1);
		VPoint p2 = getMesh().vertices().toPoint(point2);
	...
	default double computeCosPhi(@NotNull final V p1, @NotNull final V p, @NotNull final V p2) {
		...
		return Math.cos(GeometryUtils.angle(vertices.getX(p1), vertices.getY(p1), vertices.getX(p), vertices.getY(p), vertices.getX(p2), vertices.getY(p2)));
	}
