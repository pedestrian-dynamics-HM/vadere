...
/**
 * The Informed Fast Iterative Method on a Cartesian Grid, compare PhD thesis B. Zoennchen Section
 * 9.5
 *
...
public class EikonalSolverIFIM extends AGridEikonalSolver {
  ...
  private void loop() {
    ...
    while (!activeList.isEmpty()) {
      ...
      while (activeListIterator.hasNext()) {
        ...
        Triple<Double, Point, Point> r =
            computeGodunovDifferenceAndDep(activePoint, cellGrid, Direction.ANY);
        if (r.getLeft() < p) {
          ...
          for (Point neighbour : cellGrid.getLegitNeumannNeighborhood(activePoint)) {
            if (cellGrid.getValue(neighbour).tag != PathFindingTag.NARROW
              ...
              if (isReady(neighbour)) {
                ...
                Triple<Double, Point, Point> r2 =
                    computeGodunovDifferenceAndDep(neighbour, cellGrid, Direction.ANY);
    ...
    // System.arraycopy(definingVertices, 0, prefDefiningVertices, 0, definingVertices.length *
    // definingVertices[0].length * definingVertices[0][0].length);
    // if(i==0) {
    ...
    // cellGrid.pointStream().filter(p -> cellGrid.getValue(p).tag ==
    // PathFindingTag.Reached).count());
    nUpdates = 0;
    i++;
  }
    ...
    if (isReady(p)) {
      ...
      while (!notReady.isEmpty()) {
        ...
        if (pp.equals(new Point(148, 22))) {
          Triple<Double, Point, Point> r2 =
              computeGodunovDifferenceAndDep(pp, cellGrid, Direction.ANY);
  ...
  private boolean isReady(Point p) {
    boolean result =
        prefDefiningVertices == null
            || (isValid(prefDefiningVertices[p.x][p.y][0])
                && isValid(prefDefiningVertices[p.x][p.y][1]));
    /*if(!result) {
    	System.out.println(p.x + ", " + p.y + ": " + prefDefiningVertices[p.x][p.y][0] + " / " + prefDefiningVertices[p.x][p.y][1]);
    }*/
    return prefDefiningVertices == null
        || (isValid(prefDefiningVertices[p.x][p.y][0])
            && isValid(prefDefiningVertices[p.x][p.y][1]));
  }
