...
public class PotentialFieldCalculatorDijkstra extends AGridEikonalSolver {
  ...
  public void solve() {
    ...
    ComparatorPotentialFieldValue comparator = new ComparatorPotentialFieldValue(potentialField);
    PriorityQueue<Point> priorityQueue = new PriorityQueue<Point>(50, comparator);
    ...
    while (!priorityQueue.isEmpty()) {
      ...
      for (Point neighbor : neighbors) {
        ...
        if (neighborTag == PathFindingTag.Reachable) {
          value =
              potentialField.getValue(currentPoint).potential
                  + potentialField.pointDistance(currentPoint, neighbor);
          ...
          potentialField.setValue(neighbor, new CellState(value, PathFindingTag.Reachable));
        }
      }
    }
