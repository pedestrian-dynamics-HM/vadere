...
/**
 * This class computes the traveling time T using a (multi threaded) lock free variant of the fast
 * iterative method for arbitrary triangulated meshes. The quality of the result depends on the
 * quality of the triangulation. For a high accuracy the triangulation should not contain too many
 * non-acute triangles.
 *
 ...
 */
public class MeshEikonalSolverFIMLockFree<V extends IVertex, E extends IHalfEdge, F extends IFace>
   ...
   */
  public MeshEikonalSolverFIMLockFree(
                    ...
                    .forEach(
                        i -> {
                          partition
                              .get(i)
                              .stream()
                              .flatMap(v -> getMesh().streamVertices(v))
                              .forEach(
                                  v -> {
                                    if (isUndefined(v)) {
                                      updatePotential(v, i);
