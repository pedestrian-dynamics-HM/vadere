...
/**
 * Provides the potential values in a configuration space based on 2D geometry and 1D rotation.
 *
 * <p>This class was part of a test whether rigid objects can be moved through a difficult geometry.
 * Based on Sethian-1999, p180ff. The test was successful.
 */
public class ConfigurationSpacePotentialProvider {
  ...
  public ConfigurationSpacePotentialProvider(
      Geometry geometry, Shape floorshape, List<VShape> targetShapes, VPolygon shape) {
    TimeCost2D1ConfigurationSpace timecostcs =
        new TimeCost2D1ConfigurationSpace(geometry, floorshape, shape, cellSize, cellSizeAngle);
    ...
    for (int r = 0; r < gridsize; r++) {
      for (int c = 0; c < gridsize; c++) {
        ...
        for (VShape targetShape : targetShapes) {
          if (targetShape.contains(new VPoint(c * cellSize, r * cellSize))) {
