...
/**
 * This code solves the eikonal equation by iteratively refine the mesh as described in the PhD
 * thesis of B. Zoennchen (Section 9.4.3). The implementation is experimental and not jet ready for
 * general usage, i.e. the code is still not very well structured and rather hard to read.
 *
 ...
 * @author Benedikt Zoennchen
 */
public class MeshEikonalSolverFMMIterative<V extends IVertex, E extends IHalfEdge, F extends IFace>
  ...
  private IIncrementalTriangulation<V, E, F> refine(
      @NotNull final IIncrementalTriangulation<V, E, F> triangulation) {

    // (1) copy triangulation
    IIncrementalTriangulation<V, E, F> clone =
        new IncrementalTriangulation<>(triangulation.getMesh().clone(), e -> true);

    // (2) refine if necessary
    final Predicate<E> predicate =
        new PredicateRefinement<>(triangulation, clone, MIN_EDGE_LEN, 0.3);
    // final Predicate<E> predicate = new PredicateEdgeRefinement<>(solver, MIN_EDGE_LEN,
    // MAX_CURVATURE);
  ...
  public void solve() {
    ...
    if (!calculationFinished) {
      ...
      solver.solve();
      System.out.println(
          solver.getTriangulation().getMesh().toPythonTriangulation(v -> solver.getPotential(v)));
      level = 1;
      double lastMaxCurvature = 0;
      maxCurrentCurvature = Double.MAX_VALUE;

      while (level < MAX_ITERATIONS) {
        // while(maxCurrentCurvature > MAX_CURVATURE && !hasConverged(lastMaxCurvature,
        // maxCurrentCurvature) && level < MAX_ITERATIONS) {
        lastMaxCurvature = maxCurrentCurvature;
        // System.out.println(solver.getTriangulation().getMesh().toPythonTriangulation(v ->
        // solver.getPotential(v)));
        ...
        // solver = new MeshEikonalSolverFMM<>(getTimeCostFunction(), refinedTriangulation,
        // refinedTriangulation.getMesh().getBoundaryVertices());
        //
        if (targetShapes == null) {
          solver =
              new MeshEikonalSolverFMM<>(getTimeCostFunction(), pointList, refinedTriangulation);
        } else {
          solver =
              new MeshEikonalSolverFMM<>("", targetShapes, timeCostFunction, refinedTriangulation);
        }
        solver.solve();
    ...
    for (var v : background.getMesh().getVertices()) {
      double[] result =
          GeometryUtilsMesh.curvature(
              background.getMesh(), v, vertex -> solver.getPotential(vertex));
