...
public interface GridEikonalSolver extends EikonalSolver {
  ...
  default double getPotential(
    ...
    if (weightOfKnown > 0.00001) {
      tmpPotential = tmpPotential / weightOfKnown + (1 - weightOfKnown) * unknownPenalty;
  ...
  default double getPotential(
      final double x, final double y, final double unknownPenalty, final double weight) {
  ...
  default Direction computeDirection(final Point point, final CellGrid cellGrid) {
    ...
    if (isValidPoint(cellGrid, new Point(point.x + 1, point.y))
        && (!isValidPoint(cellGrid, new Point(point.x - 1, point.y))
            || (cellGrid.getValue(new Point(point.x + 1, point.y)).potential
                < cellGrid.getValue(new Point(point.x - 1, point.y)).potential))) {
      posX = true;
    }
    ...
    if (isValidPoint(cellGrid, new Point(point.x, point.y + 1))
        && (!isValidPoint(cellGrid, new Point(point.x, point.y - 1))
            || (cellGrid.getValue(new Point(point.x, point.y + 1)).potential
                < cellGrid.getValue(new Point(point.x, point.y - 1)).potential))) {
      posY = true;
    }
  ...
  default Point[] getPointByDirection(
      final Point point, final CellGrid cellGrid, final Direction direction) {
    Point xPoint, yPoint;
  ...
  default double computeGodunovDifference(
      final Point point, final CellGrid cellGrid, final Direction direction) {
    ...
    if ((xVal != Double.MAX_VALUE ^ yVal != Double.MAX_VALUE)
        || Math.abs(xVal - yVal) >= distance) {
  ...
  default Triple<Double, Point, Point> computeGodunovDifferenceAndDep(
      final Point point, final CellGrid cellGrid, final Direction direction) {
  ...
  default double computeGodunovDifference(final Point point, final CellGrid cellGrid) {
    ...
    for (int j = 0; j < 2; j++) {
      ...
      for (int i = 0; i < 2; i++) {
        Point pni =
            new Point(point.x + neighbors.get(2 * j + i).x, point.y + neighbors.get(2 * j + i).y);
        Point pni2 =
            new Point(
                point.x + neighbors.get(2 * j + i).x * 2, point.y + neighbors.get(2 * j + i).y * 2);
        ...
        if (isValidPoint(cellGrid, pni) && cellGrid.getValue(pni).tag.frozen) {
          ...
          if (val1n < val1) {
            ...
            if (isValidPoint(cellGrid, pni2)) {
              ...
              if (cellGrid.getValue(pni2).tag.frozen && val2n <= val1n) {
                val2 = val2n;
              } else {
    ...
    java.util.List<Double> solutions = MathUtil.solveQuadratic(coeff2, coeff1, coeff0);
    int numberOfSolutions = solutions.size();
  ...
  default boolean loadCachedFloorField(ICacheObject cacheObject) {
    // loadFromFilesystem floor field from cache. If it succeeds return true to indicate that the
    // floor field
    // is initialized.
