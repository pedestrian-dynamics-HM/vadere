...
/**
 * Provides a floor gradient. It is computed by the directions to the targets of the respective
 * pedestrian, regardless of obstacles (i.e. the direct way). The point x with d(x,target)=0 would
 * give problems when calculating the direction: x/norm(x-target). The mollified version (Koester et
 * al. 2012) is this: x/sqrt((x-target).^2+epsilon^2) with a small epsilon.
 */
public class FloorGradientProviderEuclideanMollified implements GradientProvider {
  ...
  public void gradient(double t, int currentTargetId, double[] x, double[] grad) {
    ...
    posTarget = targetShapes.get(currentTargetId).closestPoint(new VPoint(x[0], x[1]));
    ...
    double norm = Math.sqrt(grad[0] * grad[0] + grad[1] * grad[1] + epsilonSquared);
    if (norm > 0) {
      grad[0] /= norm;
      grad[1] /= norm;
    }
