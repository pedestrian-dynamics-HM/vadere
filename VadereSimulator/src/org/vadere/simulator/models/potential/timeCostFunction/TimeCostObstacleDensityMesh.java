 ...
 * @param <F>
 */
public class TimeCostObstacleDensityMesh<V extends IVertex, E extends IHalfEdge, F extends IFace>
  ...
  public double costAt(@NotNull final V v, @Nullable final Object caller) {
    return timeCostFunction.costAt(v, caller)
        + attributesTimeCost.getObstacleDensityWeight() * densities.getValue(v);
  }
  ...
  public void update() {
    ...
    if (!updated) {
      triangulation
          ...
          .filter(
              v -> -distanceFunction.apply(triangulation.getMesh().toPoint(v)) < influenceRadius)
          .forEach(
              v -> {
                double dist =
                    Math.max(0, -distanceFunction.apply(triangulation.getMesh().toPoint(v)));
