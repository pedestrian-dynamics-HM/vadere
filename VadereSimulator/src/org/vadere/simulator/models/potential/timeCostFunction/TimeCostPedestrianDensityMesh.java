 ...
 * @param <F>
 */
public class TimeCostPedestrianDensityMesh<V extends IVertex, E extends IHalfEdge, F extends IFace>
  ...
  private boolean coarse(@NotNull final V vertex) {
    ...
    for (Pedestrian pedestrian : topography.getPedestrianDynamicElements().getElements()) {
      if (pedestrian.getPosition().distanceSq(triangulation.getMesh().toPoint(vertex))
  ...
  private boolean refine(@NotNull final E e) {
    ...
    if (!triangulation.getMesh().isBoundary(e)) {
      ...
      if (
      /*!refiner.isGreen(e) || */ triangulation.getMesh().toLine(e).length() > 1.0) {
        for (Pedestrian pedestrian : topography.getPedestrianDynamicElements().getElements()) {
        ...
        for (V v : closeVertices) {
          double density =
              densities.getValue(v)
                  + loadingStrategy.calculateLoading(element)
                      / (influenceRadius * influenceRadius * Math.PI);
              ...
              .toPythonValues(
                  v ->
                      densities.getValue(v)
                          + triangulation
                              .getMesh()
                              .getDoubleData(v, TimeCostObstacleDensityMesh.nameObstacleDensity)));
