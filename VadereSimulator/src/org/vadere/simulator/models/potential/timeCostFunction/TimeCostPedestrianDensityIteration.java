...
/**
 * TimeCostPedestrianDensityIteration return cost for the pedestrian density by using the gaussian
 * formula from Seitz paper. We currently use the getSpatialMapPeds for getting the neighbours (3 *
 * size x size * 3), with size = 3. The function is for the purpose of navigation around groups.
 *
 * <p>This implementation is not efficient, but was used to compare the values and the performance
 * of the different implementations. Use the TimeCostPedestrianDensity instead.
 */
@Deprecated
public class TimeCostPedestrianDensityIteration implements ITimeCostFunction {
  ...
  private static Logger logger = Logger.getLogger(TimeCostPedestrianDensityIteration.class);
  ...
  TimeCostPedestrianDensityIteration(
    ...
    this.floor = floor;
    this.varianz = attributes.getStandardDeviation() * attributes.getStandardDeviation();
    this.scaleFactor =
    ...
    loadingStrategy =
        IPedestrianLoadingStrategy.create(floor, attributes, attributesPedestrian, targetId);
    logger.info("torso:  " + attributesPedestrian.getRadius() * 2);
    logger.info("standard derivation:  " + attributes.getStandardDeviation());
  ...
  public void update() {
    ...
    logger.info(
        "runtime: " + runtime + " number of peds " + floor.getElements(Pedestrian.class).size());
    runtime = 0;
  ...
  private double calculatePedestrianDensity(final IPoint position) {
    ...
    double radius = 4;
    Collection<Pedestrian> pedestrianBodies =
        floor.getSpatialMap(Pedestrian.class).getObjects(new VPoint(position), radius);
    ...
    if (!pedestrianBodies.isEmpty()) {
      for (Pedestrian pedestrianBody : pedestrianBodies) {
        ...
        densitySum +=
            Math.exp((-distance * distance * 0.5) / varianz)
