...
/**
 * A TimeCostFunction which reduces the travelling speed decreases with the pedestrian density. At
 * the moment the code to dynamically refine the mesh is disabled (compare the unused method {@link
 * TimeCostGaussianPedestrianDensityMesh#refineMesh}).
 *
 * <p>Enabling the dynamic refinement is experimental code! It is not clear how often one should
 * refine and how large the refinement cutoff should be used.
 *
 * @param <V>
 * @param <E>
 * @param <F>
 ...
 */
public class TimeCostGaussianPedestrianDensityMesh<
        V extends IVertex, E extends IHalfEdge, F extends IFace>
    implements ITimeCostFunctionMesh<V> {
  ...
  // the radius in meter for which a pedestrian can contribute to the density, i.e. the cutoff
  // radius.
  ...
  private boolean refine(@NotNull final E e) {
    ...
    if (!triangulation.getMesh().isBoundary(e)) {
      ...
      if (
      /*!refiner.isGreen(e) || */ triangulation.getMesh().toLine(e).length() > h_max) {
        for (Pedestrian pedestrian : topography.getPedestrianDynamicElements().getElements()) {
          // influenceRadius is cutoff radius for the mesh refinement is 4 times the density cutoff
          // radius, i.e. we have to refine every time an agent travles more than the
          // influenceRadius.
          if (pedestrian.getPosition().distanceSq(triangle.midPoint())
              < influenceRadius * influenceRadius * 4) {
            return true;
          }
        }
      }
  ...
  public double costAt(@NotNull final IPoint p) {
    ...
    if (!triangulation.getMesh().isBoundary(face)) {
      cost =
          GeometryUtilsMesh.barycentricInterpolation(
              face, triangulation.getMesh(), v -> densities.getValue(v), p.getX(), p.getY());
    }
        ...
        Predicate<V> predicate =
            v ->
                GeometryUtils.lengthSq(
                        mesh.getX(v) - element.getPosition().x,
                        mesh.getY(v) - element.getPosition().y)
