...
public class LoadingFactory {
  /**
   * Creates and returns the laoding constant laoding strategy (that is discribed in hartmanns
   * paper).
   *
   ...
   * @param sameFlooLoading the loading for pedestrians of that have the same target = the target of
   *     the generated potential field (c_D in hartmann-2012)
  ...
  public static IPedestrianLoadingStrategy createConstantPedestrianLoading(
      final int floorFieldTargetId, final double sameFlooLoading, final double otherFloorLoading) {
    return new PedestrianLoadingStrategyConstant(
        floorFieldTargetId, sameFlooLoading, otherFloorLoading);
  }
   ...
   * @param floorFieldTargetId the target id that is the target of the generated potential field
   *     that uses this loading strategy
   * @param loading the loading that will be multiplied to the dynamic loading (c_p in the bachelor
   *     thesis of Benedikt Zoennchen)
   ...
   * @return the laoding constant laoding strategy
   */
  public static IPedestrianLoadingStrategy createUnitPedestrianLoading(final double loading) {
