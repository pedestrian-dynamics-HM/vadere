...
public class PotentialFieldObstacleOSM implements PotentialFieldObstacle {
  ...
  public void initialize(
    ...
    AttributesPotentialOSM attributesPotentialOSM =
        Model.findAttributes(attributesList, AttributesPotentialOSM.class);
  ...
  public double getObstaclePotential(IPoint pos, Agent pedestrian) {
    ...
    double distance =
        domain.getTopography().distanceToObstacle(pos, pedestrian) - pedestrian.getRadius();
    ...
    if (distance <= 0) {
      ...
      repulsion =
          Math.exp(-Math.pow(distance, attributes.getBObsOSM()) * attributes.getAObsOSM())
  ...
  public Vector2D getObstaclePotentialGradient(VPoint pos, Agent pedestrian) {
    ...
    for (Obstacle obstacle : obstacles) {
      ...
      // Shapes of pedestrians are assumed to be circles.
      double distance = obstacle.getShape().distance(pos) - pedestrian.getRadius();
    ...
    if (closestObstacle != null) {
      result = getObstaclePotentialGradient(pos, closestObstacle, pedestrian, closestDistance);
    } else {
  ...
  private Vector2D getObstaclePotentialGradient(
      VPoint pos, Obstacle obs, Agent pedestrian, double distance) {
    ...
    if (distance >= 0 && distance < attributes.getObstacleRepulsionWidth()) {
      ...
      double vu =
          -attributes.getAObsOSM()
              ...
              * Math.exp(
                  -attributes.getAObsOSM() * Math.pow(distance, attributes.getBObsOSM() / 2.0))
              * attributes.getObstacleRepulsionStrength();
