...
public class TargetDirectionGeoOptimumBruteForce implements TargetDirection {
  ...
  /**
   * This method computes the target direction by a brute force optimization evaluating many
   * equidistant points an the step circle of the this agent which is rather slow.
   *
  ...
  public VPoint getTargetDirection(@NotNull final Target target) {
    ...
    List<VPoint> possibleNextPositions =
        GeometryUtils.getDiscDiscretizationPoints(
            new VCircle(position, stepLength), 1, nPoints, angle, 2 * Math.PI);
    VPoint nextOptimalPos =
        possibleNextPositions
            .stream()
            // .filter(p -> !collidesWithObstacle(p))
            .min(
                Comparator.comparingDouble(
                    p -> targetPotentialField.getPotential(p, pedestrianBHM)))
            .get();
