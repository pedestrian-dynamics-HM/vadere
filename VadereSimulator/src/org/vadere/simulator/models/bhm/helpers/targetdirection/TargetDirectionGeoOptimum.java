...
public class TargetDirectionGeoOptimum implements TargetDirection {
  ...
  public TargetDirectionGeoOptimum(
    ...
    this.fallBackStrategy =
        new TargetDirectionGeoOptimumBruteForce(pedestrianBHM, targetPotentialField);
  }

  /**
   * This method computes the target direction by a golden section search of the target potential
   * function on the step circle of this agent.
   *
  ...
  public VPoint getTargetDirection(@NotNull final Target target) {
    ...
    Function<Double, Double> f =
        rad -> targetPotentialField.getPotential(bestArg.rotate(rad).add(position), pedestrianBHM);
    ...
    if (firstGuess < bestVal && firstGuess < currentPotential) {
      ...
      // in this case this optimization failed i.e. the agent would go backwards! => back to brute
      // force!
