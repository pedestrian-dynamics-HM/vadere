...
public class BehaviouralHeuristicsModel implements MainModel {
    ...
    try {
      potentialFieldTarget =
          IPotentialFieldTargetGrid.createPotentialField(
              ...
              PotentialFieldTargetGrid.class.getCanonicalName());
  ...
  @Override
  public <T extends DynamicElement> PedestrianBHM createElement(
  ...
  public <T extends DynamicElement> PedestrianBHM createElement(
    ...
    AttributesAgent pedAttributes =
        new AttributesAgent(aAttr, registerDynamicElementId(topography, id));
  ...
  private PedestrianBHM createElement(
      VPoint position, @NotNull final AttributesAgent pedAttributes) {
    PedestrianBHM pedestrian =
        new PedestrianBHM(topography, pedAttributes, attributesBHM, random, potentialFieldTarget);
    pedestrian.setPosition(position);
    return pedestrian;
  }
  ...
  public VShape getDynamicElementRequiredPlace(@NotNull final VPoint position) {
    return new VCircle(
        position,
        new AttributesAgent(attributesPedestrian, -1).getRadius()
            + new AttributesBHM().getSpaceToKeep());
  }
