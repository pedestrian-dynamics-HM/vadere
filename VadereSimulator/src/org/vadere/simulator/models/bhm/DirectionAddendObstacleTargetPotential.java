...
/**
 * Computes a vector to add to the target direction such that collisions with obstacles will be
 * avoided.
 *
...
public class DirectionAddendObstacleTargetPotential implements DirectionAddend {
  ...
  public VPoint getDirectionAddend(@NotNull final VPoint targetDirection) {
    ...
    Optional<Obstacle> closeObstacles =
        me.detectClosestObstacleProximity(nextPosition, me.getRadius() + GeometryUtils.DOUBLE_EPS);

    // if there is none, there is no need to change the target direction
    if (closeObstacles.isPresent()) {
      closeObstacles =
          me.detectClosestObstacleProximity(
              me.getPosition(),
              me.getRadius() + footStep.distanceToOrigin() + GeometryUtils.DOUBLE_EPS);
      ...
      IPoint p =
          GeometryUtils.projectOnto(
              targetDirection.getX(), targetDirection.getY(), normal.x, normal.y);

      // if the target direction points away from the obstacle don't adjust it
      if (!p.equals(
          VPoint.ZERO) /* && p.norm().distance(normal.norm()) < GeometryUtils.DOUBLE_EPS*/) {
    ...
    closeObstacles =
        me.detectClosestObstacleProximity(me.getPosition().add(newFootStep), me.getRadius());
