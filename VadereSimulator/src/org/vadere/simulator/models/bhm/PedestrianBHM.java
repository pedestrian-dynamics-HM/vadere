...
public class PedestrianBHM extends Pedestrian {
  ...
  public PedestrianBHM(
      Topography topography,
      AttributesAgent attributesPedestrian,
      AttributesBHM attributesBHM,
      Random random) {
  ...
  public PedestrianBHM(
      ...
      AttributesBHM attributesBHM,
      Random random,
      @Nullable IPotentialFieldTarget potentialFieldTarget) {
    ...
    this.stepLength =
        attributesBHM.getStepLengthIntercept()
            + stepDeviation
  ...
  /**
   * Realizes the next step by setting member variable position from super class Pedestrian, and
   * setting member variable lastPosition=position. Also updates super member velocity.
   *
   * <p>This method sets the member variables and thus has side effects!
   */
  ...
  public VPoint stepAwayFromCollision(final Pedestrian collisionPed) {
    return getPosition()
        .subtract(collisionPed.getPosition())
        .norm()
        .scalarMultiply(stepLength)
        .add(getPosition());
  }
  ...
  VPoint makeSmallerStep(final VPoint position, final Pedestrian collisionPed) {
    ...
    for (int i = smallStepResolution; i > 0; i--) {
      ...
      result =
          getPosition()
              .add(direction.scalarMultiply(stepLength * stepFraction * smallStepResolution));
   ...
   *
   * <p>This method sets the member variable targetDirection and thus has side effects! Therefore
   * this method should always stay private.
   */
  ...
  public VPoint computeMovementProjection() {
    return getPosition()
        .add(
            getTargetDirection().scalarMultiply(stepLength * attributesBHM.getPlannedStepsAhead()));
  }
  ...
  public boolean collidesWithPedestrian(VPoint position, double spaceToKeep) {
    ...
    for (Pedestrian other : topography.getElements(Pedestrian.class)) {
      if (other.getId() != getId()) {
        ...
        double distance =
            position.distance(other.getPosition()) - other.getRadius() - getRadius() - spaceToKeep;
  ...
  public boolean collidesWithPedestrianOnPath(VPoint position) {
    ...
    if (collision != null
        && TopographyHelper.otherPedestrianIsCloserToTarget(topography, this, collision)) {
      isCollision = true;
    }
  ...
  /**
   * If findAny is false, return the first collision on the path to position. If findAny is true,
   * return the first collision that was found (could be any).
   */
  public Pedestrian findCollisionPedestrian(VPoint position, boolean findAny) {
    ...
    for (Pedestrian other :
      ...
      if (other.getId() != getId()) {
        ...
        double distance =
            stepLine.distance(other.getPosition())
                - other.getRadius()
                - getRadius()
        ...
        if (distance < 0) {
          ...
          if (findAny) {
            return other;
              ...
              || (UtilsBHM.angleBetweenTargetDirection(this, other)
                  > attributesBHM.getOnlyEvadeContraFlowAngle())) {
  ...
  Optional<Obstacle> detectClosestObstacleProximity(
      @NotNull final VPoint position, double proximity) {
