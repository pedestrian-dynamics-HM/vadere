...
public class UtilsBHM {
  ...
  public static List<VPoint> getTangentialPoints(final VPoint pointOutside, final double radius) {
    ...
    if (Double.isNaN(x1) || Double.isNaN(y1) || Double.isNaN(x2) || Double.isNaN(y2)) {
      logger.error(
          "Could not generate tangential point at ("
              + pointOutside
              + ") with radius="
              + radius
              + ".");
  ...
  public static double angleBetweenTarget(PedestrianBHM me, Pedestrian other) {
    return UtilsBHM.angle(me.getTargetDirection(), other.getPosition().subtract(me.getPosition()));
  }

  /**
   * Filters the list of pedestrians such that only pedestrians will remain which form the convex
   * hull of all the pedestrians in the list (with repect to their position).
   *
  ...
  private static List<Pedestrian> convexHull(@NotNull final List<Pedestrian> cluster) {
    ...
    GrahamScan grahamScan =
        new GrahamScan(cluster.stream().map(ped -> ped.getPosition()).collect(Collectors.toList()));
    Set<VPoint> convexHull = new HashSet<>(grahamScan.getConvexHull());
    return cluster
        .stream()
        .filter(ped -> convexHull.contains(ped.getPosition()))
        .collect(Collectors.toList());
  }

  /**
   * Given the fractions f1, f2, ..., fn this method returns 1 or 2 ... or n with probability f1 /
   * sum, f2 / sum, ..., fn / sum for sum = f1 + f2 + ... + fn.
   *
   * @param fractions
   * @param random
   * @return a randomly chosen index in [0, fraction.size()) with probabilities depending on the
   *     fractions
   */
  public static int randomChoice(
      @NotNull final List<Double> fractions, @NotNull final Random random) {
