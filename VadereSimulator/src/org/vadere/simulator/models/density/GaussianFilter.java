...
abstract class GaussianFilter implements IGaussianFilter {
  ...
  /**
   * the scale of the images respect to the scenario width and heigt and based on the gridresolution
   * of the potential field grid.
   */
  ...
  GaussianFilter(
      final Rectangle2D scenarioBounds,
      final double scale,
      final BiFunction<Integer, Integer, Float> f,
  ...
  public double getMaxFilteredValue() {
    return IntStream.range(0, outputMatrix.length)
        .mapToDouble(i -> outputMatrix[i])
        .max()
        .orElse(0.0);
  }
