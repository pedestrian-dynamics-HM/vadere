...
public class UpdateSchemeCLParallel extends UpdateSchemeParallel {
  ...
  public UpdateSchemeCLParallel(
      @NotNull final Topography topography,
      @NotNull final CLParallelOSMLocalMem clOptimalStepsModel) {
    super(topography);
  ...
  public void update(double timeStepInSec, double currentTimeInSec) {
    try {
      ...
      List<PedestrianOSM> pedestrianOSMList =
      ...
      if (counter == 0) {
        ...
        for (int i = 0; i < pedestrianOSMList.size(); i++) {
          ...
          CLParallelOSMLocalMem.PedestrianOpenCL pedestrian =
              new CLParallelOSMLocalMem.PedestrianOpenCL(
      ...
      for (int i = 0; i < pedestrianOSMList.size(); i++) {
        // logger.info("not equals for index = " + i + ": " + pedestrianOSMList.get(i).getPosition()
        // + " -> " + result.get(i));
