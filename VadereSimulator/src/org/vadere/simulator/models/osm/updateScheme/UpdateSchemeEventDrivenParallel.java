...
public class UpdateSchemeEventDrivenParallel extends UpdateSchemeEventDriven {
  ...
  private static final Logger logger = Logger.getLogger(UpdateSchemeEventDrivenParallel.class);
  ...
  public UpdateSchemeEventDrivenParallel(
      @NotNull final Topography topography, @NotNull final double pedestrianPotentialWidth) {
    super(topography);
  ...
  public void update(final double timeStepInSec, final double currentTimeInSec) {
    topography
        .getElements(PedestrianOSM.class)
        .parallelStream()
        .forEach(pedestrianOSM -> pedestrianOSM.clearStrides());

    double maxStepSize =
        topography
            .getElements(PedestrianOSM.class)
            .parallelStream()
            .mapToDouble(ped -> ped.getDesiredStepSize())
            .max()
            .orElse(0);
    double maxDesiredSpeed =
        topography
            .getElements(PedestrianOSM.class)
            .parallelStream()
            .mapToDouble(ped -> ped.getDesiredSpeed())
            .max()
            .orElse(0);

    double stepSize = Math.max(maxStepSize, maxDesiredSpeed * timeStepInSec);
    // this formula is slightly different than the formula in the PhD of B. Zoennchen (p. 63, eq.
    // 5.3)
    ...
    do {
      ...
      while (!pedestrianEventsQueue.isEmpty()
          && pedestrianEventsQueue.peek().getTimeOfNextStep() < currentTimeInSec) {
