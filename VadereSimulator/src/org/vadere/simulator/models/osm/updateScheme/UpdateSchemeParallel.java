...
/**
 * The implementation of the parallel update scheme described in 'How update schemes influence crowd
 * simulations' i.e. seitz-2014b. The implementation is not based on the time credit concept but on
 * event times similar to the event driven update scheme. The difference is that all events which
 * starts within a time span of <tt>currentTimeInSec</tt> - <tt>timeStepInSec</tt> and
 * <tt>currentTimeInSec</tt> will be performed in parallel on the bases of the situation (i.e.
 * agents position) at <tt>currentTimeInSec</tt> - <tt>timeStepInSec</tt>.
 */
public class UpdateSchemeParallel implements UpdateSchemeOSM {
  ...
  public void update(double timeStepInSec, double currentTimeInSec) {
    ...
    do {
      ...
      CallMethod[] callMethods = {
        CallMethod.SEEK, CallMethod.MOVE, CallMethod.CONFLICTS, CallMethod.STEPS
      };
      List<Future<?>> futures;
      ...
      for (CallMethod callMethod : callMethods) {
        ...
        for (final PedestrianOSM pedestrian :
            CollectionUtils.select(topography.getElements(Pedestrian.class), PedestrianOSM.class)) {
  ...
  protected void update(
      ...
      final double timeStepInSec,
      final double currentTimeInSec,
      CallMethod callMethod) {

    // At the moment no other events are supported for the parallel update scheme!
    assert pedestrian.getMostImportantStimulus() instanceof ElapsedTime
        && pedestrian.getSelfCategory() == SelfCategory.TARGET_ORIENTED;
    switch (callMethod) {
      case SEEK:
   ...
   */
  protected void updateParallelSeek(
      @NotNull final PedestrianOSM pedestrian,
      final double currentTimeInSec,
      final double timeStepInSec) {
  ...
  /**
   * Sets the last and (current) position of the pedestrian. The velocity and the timeCredit will be
   * updated later since the move operation might reverted by {@link
   * UpdateSchemeParallel#updateParallelConflicts(PedestrianOSM)}.
   *
   * @param pedestrian the pedestrian
   */
  private void updateParallelMove(
      @NotNull final PedestrianOSM pedestrian, final double timeStepInSec) {
  ...
  /**
   * Resolves conflicts: If there is any overlapping pedestrian with a smaller timeCredit, the
   * pedestrians position will be set to his last position i.e. a rollback of the move step.
   *
  ...
  protected void updateParallelSteps(@NotNull final PedestrianOSM pedestrian) {
    if (movePedestrians.contains(pedestrian)) {
      if (undoPedestrians.contains(pedestrian)) {
        pedestrian.setTimeOfNextStep(
            pedestrian.getTimeOfNextStep() + pedestrian.getDurationNextStep());
      } else {
