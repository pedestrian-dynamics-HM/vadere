...
public class StepCircleOptimizerGradient extends StepCircleOptimizer {
  ...
  public StepCircleOptimizerGradient(
  ...
  public VPoint getNextPosition(PedestrianOSM ped, Shape reachableArea) {
    ...
    GradientProvider floorGradient =
        new FloorGradientProviderDiscrete(
            potentialFieldTarget.getCellGrids(), topography.getBounds(), targetIds);
    ...
    for (int i = 0; i <= attributesOSM.getStepCircleResolution(); i++) {
      ...
      nextPos = new VPoint(position.x - nextMove.x, position.y - nextMove.y);
