...
public class HimmelblauFunction extends PotentialEvaluationFunction {
  ...
  public double value(double arg) throws FunctionEvaluationException {
    VPoint newPos =
        new VPoint(
            this.getPedestrian().getPosition().x + stepsize * Math.cos(arg),
  ...
  public double value(double[] posi) {
    ...
    if (Math.pow(pos[0] - position.x, 2) + Math.pow(pos[1] - position.y, 2)
        <= Math.pow(stepsize, 2) + 0.0001) {
      result =
          Math.pow((pos[0] * pos[0] + pos[1] - 11), 2)
