...
public class StepCircleOptimizerBrent extends StepCircleOptimizer {
  private static Logger logger = Logger.getLogger(StepCircleOptimizerBrent.class);
  ...
  public VPoint getNextPosition(PedestrianOSM ped, Shape reachableArea) {
    ...
    try {
      minimum = -1;
      minimumValue =
          potentialEvaluationFunction.value(potentialEvaluationFunction.pointToArray(curPos));
      int counter = 0;
      ...
      while (counter < bound) {
        ...
        newMinimum =
            optimizer.optimize(
                ...
                0,
                2 * Math.PI,
                2 * Math.PI / bound * (counter + randOffset));
        newMinimumValue = potentialEvaluationFunction.value(newMinimum);

        counter++;

        if (minimumValue > newMinimumValue
            || (Math.abs(minimumValue - newMinimumValue) <= 0.00001 && random.nextBoolean())) {
    ...
    if (minimum == -1) {
      ...
      VPoint newPos =
          new VPoint(
              stepSize * Math.cos(minimum) + curPos.x, stepSize * Math.sin(minimum) + curPos.y);
      return newPos;
    }
  }
