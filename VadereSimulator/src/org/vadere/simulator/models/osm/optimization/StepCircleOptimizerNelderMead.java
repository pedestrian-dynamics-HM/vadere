...
public class StepCircleOptimizerNelderMead extends StepCircleOptimizer {
  ...
  private static Logger logger = Logger.getLogger(StepCircleOptimizerNelderMead.class);
  ...
  public VPoint getNextPosition(PedestrianOSM pedestrian, Shape reachableArea) {
    ...
    try {
      ...
      newMinimum =
          optimizer.optimize(potentialEvaluationFunction, GoalType.MINIMIZE, position).getPoint();
      ...
      if ((minimumValue > newMinimumValue
          && Math.abs(minimumValue - newMinimumValue) > threshold)) {
      ...
      while (counter < bound) {
        ...
        double innerDistance = pedestrian.getPosition().distance((positions.get(counter)));
        VPoint innerDirection =
            pedestrian
                .getPosition()
        ...
        newMinimum =
            optimizer
                .optimize(potentialEvaluationFunction, GoalType.MINIMIZE, newPosition)
                .getPoint();
        newMinimumValue = potentialEvaluationFunction.value(newMinimum);
    ...
    // lastSolution.put(pedestrian, new VPoint(minimum[0]-pedestrian.getPosition().getX(),
    // minimum[1]-pedestrian.getPosition().getY()));

    if (getIsComputeMetric()) {
      // See merge request !65
      this.computeAndAddBruteForceSolutionMetric(
          pedestrian, new SolutionPair(new VPoint(minimum[0], minimum[1]), minimumValue));
    }
  ...
  private boolean isLocalMinimum(
      PotentialEvaluationFunction evaluationFunction, VCircle stepDisc, VPoint optimum)
