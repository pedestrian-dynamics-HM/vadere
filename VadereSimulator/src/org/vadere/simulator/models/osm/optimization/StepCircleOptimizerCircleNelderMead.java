...
public class StepCircleOptimizerCircleNelderMead extends StepCircleOptimizer {
  ...
  private static Logger logger = Logger.getLogger(StepCircleOptimizerCircleNelderMead.class);
  private static double threshold = 0.01;

  private final Random random;
  private final AttributesOSM attributesOSM;

  public StepCircleOptimizerCircleNelderMead(
      @NotNull final Random random, @NotNull final AttributesOSM attributesOSM) {
    this.random = random;
    this.attributesOSM = attributesOSM;
  }

  private VPoint getNextPosition1D(
      @NotNull final PedestrianOSM ped, @NotNull final Shape reachableArea) {
  ...
  public VPoint getNextPosition(
    ...
    if (getIsComputeMetric()) {
      ...
      this.computeAndAddBruteForceSolutionMetric(
          ped, new SolutionPair(resultPoint.clone(), minFuncValue));
    }

    return resultPoint;
  }

  private List<Pair<Double, Double>> generate1DSimplexes(
      @NotNull final PedestrianOSM ped, @NotNull final VCircle stepCircle) {
  ...
  private List<VTriangle> generate2DSimplexes(
      @NotNull final PedestrianOSM ped, @NotNull final VCircle stepCircle) {
    double step = stepCircle.getRadius() / 3.0;
    assert ped.getAttributesOSM().getNumberOfCircles() == 1;

    List<VPoint> positions =
        StepCircleOptimizerDiscrete.getReachablePositions(ped, stepCircle, random);
  ...
  // List<VPoint> positions = StepCircleOptimizerDiscrete.getReachablePositions(pedestrian,
  // (VCircle)reachableArea, random);
  private VPoint getNextPosition2D(
      @NotNull final PedestrianOSM ped, @NotNull final Shape reachableArea) {

    final VCircle stepCircle = (VCircle) reachableArea;
    Function<IPoint, Double> eval =
        pos -> {
