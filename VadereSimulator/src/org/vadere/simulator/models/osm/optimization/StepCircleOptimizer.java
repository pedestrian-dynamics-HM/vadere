 ...
 *
 * <p>The abstract functions need to be implemented by every StepCircleOptimizer. The additional
 * functions only serve to compute a true solution (obtained via computationally expensive brute
 * force). This allows to compute a metric that measures the quality of the respective concrete
 * subclass of StepCircleOptimizer.
 *
 * <p>Currently, only the StepCircleOptimizerNelderMead uses the metric functionality. To compute
 * the brute force solution only the "computeAndAddBruteForceSolutionMetric" function has to be
 * called (NOTE: depending on the setting in "getReachablePositions" this can be very expensive.
 */
public abstract class StepCircleOptimizer {
  ...
  protected StepCircleOptimizer() {
    ...
    this.computeMetric =
        VadereConfig.getConfig()
            .getBoolean("Testing.stepCircleOptimization.compareBruteForceSolution");
  ...
  /**
   * The following functions are to compute the "true" optimal value via brute force. This allows to
   * check the quality of a optimization algorithm.
   */
  ...
  protected void computeAndAddBruteForceSolutionMetric(
      final PedestrianOSM pedestrian, final SolutionPair foundSolution) {

    var bruteForceSolution =
        new StepCircleOptimizerDiscrete(0.0, null).computeBruteForceSolution(pedestrian);

    var optimizationMetric =
        new OptimizationMetric(
            pedestrian.getId(),
            pedestrian.getTimeOfNextStep(),
            bruteForceSolution.point,
            bruteForceSolution.funcValue,
            foundSolution.point,
            foundSolution.funcValue);
  ...
  public static StepCircleOptimizer create(
    ...
    switch (type) {
      ...
      case PATTERN_SEARCH:
        ...
        break;
      case GRADIENT:
        result = new StepCircleOptimizerGradient(topography, potentialFieldTarget, attributesOSM);
        break;
      case DISCRETE:
      case NONE:
      default:
