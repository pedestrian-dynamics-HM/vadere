...
/** The Class StepCircleOptimizerDiscrete. Simple discrete optimizer, described in [Seitz, 2012] */
public class StepCircleOptimizerDiscrete extends StepCircleOptimizer {
  ...
  private static final Logger log = Logger.getLogger(StepCircleOptimizerDiscrete.class);
  ...
  public VPoint getNextPosition(
    ...
    for (VPoint currentPosition : positions) {
      try {
        ...
        Logger.getLogger(StepCircleOptimizerDiscrete.class)
            .error("Potential evaluation threw an error: " + e.getMessage());
      }
    }

    // pedestrian.getTargetPotential(nextPos) > 0 => agent is not jet on his target otherwise the
    // agent would wait forever
    if (curPosPotential - potential <= movementThreshold
        && pedestrian.getTargetPotential(nextPos) > 0) {
      nextPos = curPos;
    }
  ...
  public SolutionPair computeBruteForceSolution(final PedestrianOSM pedestrian) {
    ...
    VPoint optimalPoint =
        getNextPosition(
            pedestrian, getBruteForcePointsCircles(reachableArea), reachableArea.getRadius());
  ...
  public static List<VPoint> getReachablePositions(
      @NotNull final PedestrianOSM pedestrian,
      @NotNull VCircle reachableArea,
    ...
    if (attributesOSM.getNumberOfCircles() < 0) {
      throw new IllegalArgumentException(
          "number of circles is negative (" + attributesOSM.getNumberOfCircles() + ")");
  ...
  private static List<VPoint> getBruteForcePointsCircles(VCircle reachableArea) {
    // NOTE: numberPointsOfLargestCircle and numberOfCircles are parameters with a trade off between
    // runtime and
    // precision of brute force solution

    return GeometryUtils.getDiscDiscretizationPoints(
        null, false, reachableArea, 100, 2000, 0, 2.0 * Math.PI);
  }
  ...
  private static List<VPoint> getBruteForcePointsLines(VCircle reachableArea) {
    ...
    VPoint refPoint =
        new VPoint(
            centerPoint.x - reachableArea.getRadius(), centerPoint.y + reachableArea.getRadius());
  ...
  /**
   * The maximum deviation from the last movement direction given the current speed. See seitz-2016
   * PhD-thesis equation 4.6
   */
