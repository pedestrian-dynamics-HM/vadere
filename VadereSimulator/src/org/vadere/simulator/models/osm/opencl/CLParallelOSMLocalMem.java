...
public class CLParallelOSMLocalMem {
  ...
  public CLParallelOSMLocalMem(
    ...
    double radius = 0.2;
    this.maxNumberOfElementsPerCell =
        (int) Math.ceil((cellSize + radius) * (cellSize + radius) / (Math.PI * radius * radius));
    init();
  }
  ...
  public List<VPoint> update() throws OpenCLException {
    try (MemoryStack stack = stackPush()) {
      ...
      clMaxPedCountInCell(
          clMaxPedCountInCell, clCellStarts, clCellEnds, clGridSize, numberOfElements);
    ...
    try (MemoryStack stack = stackPush()) {
      ...
      long maxWorkGroupSize =
          CLUtils.getMaxWorkGroupSizeForKernel(
              clDevice,
              clSeek,
              4 * maxNumberOfElementsPerCell * 9,
              max_work_group_size,
      ...
      CLInfo.checkCLError(
          clSetKernelArg1f(clSeek, 13, (float) attributesFloorField.getPotentialFieldResolution()));
      ...
      CLInfo.checkCLError(
          clSetKernelArg(
              clSeek, 16, maxNumberOfElementsPerCell * circlePositionList.size() * 5 * 4));
      CLInfo.checkCLError(
          clSetKernelArg(clSeek, 17, maxNumberOfElementsPerCell * circlePositionList.size() * 4));
  ...
  private void clMaxPedCountInCell(
    ...
    try (MemoryStack stack = stackPush()) {
      ...
      CLInfo.checkCLError(
          (int)
              enqueueNDRangeKernel(
                  "clCount", clQueue, clCount, 1, null, clGlobalWorkSize, null, null, null));
    }
  }
