...
public class CLOptimalStepsModel {
  ...
  /**
   * The data structure representing the linked cell. The elements of cell i between
   * (reorderedPedestrians[cellStart[i]*2], reorderedPedestrians[cellStart[i]*2+1]) and
   * (reorderedPedestrians[(cellEnds[i]-1)*2], reorderedPedestrians[(cellEnds[i]-1)*2+1]).
   */
  public class LinkedCell {
    ...
    /**
     * the mapping between the unordered (original) pedestrians and the reorderedPedestrians, i.e.
     * reorderedPedestrians[i] == pedestrians[indices[i]]
     */
    public int[] indices;
  ...
  public List<PedestrianOpenCL> getNextSteps(
      @NotNull final List<PedestrianOpenCL> pedestrians, final double cellSize)
      throws OpenCLException {
    ...
    try (MemoryStack stack = stackPush()) {
      ...
      // clFindCellBoundsAndReorder(clCellStarts, clCellEnds, clReorderedPedestrians, clHashes,
      // clIndices, clPedestrians, numberOfElements, numberOfGridCells);
   ...
   * mapping of the rearrangement. This method exists to test the bitonic sort algorithm on the GPU.
   *
   * @param positions the pedestrians which will be hashed.
   * @return the sorted hashes.
   * @throws OpenCLException
   */
  public int[] calcSortedHashes(@NotNull final List<PedestrianOpenCL> positions)
      throws OpenCLException {
  ...
  /**
   * Computes all the hash values, i.e. cells of each position. This method exists to test the hash
  ...
  /**
   * Returns the gridSizes of the linked cell, i.e. result[0] is the x and result[1] the y
   * direction.
   *
  ...
  private void allocDeviceMemory(final int numberOfElements) {
    try (MemoryStack stack = stackPush()) {
      ...
      clReorderedPedestrians =
          clCreateBuffer(clContext, CL_MEM_READ_WRITE, 3 * 4 * numberOfElements, errcode_ret);
      clPedestrians =
          clCreateBuffer(clContext, CL_MEM_READ_WRITE, 3 * 4 * numberOfElements, errcode_ret);
      clPedestrianNextPositions =
          clCreateBuffer(clContext, CL_MEM_READ_WRITE, 2 * 4 * numberOfElements, errcode_ret);
  ...
  private void clNextPosition(
    ...
    try (MemoryStack stack = stackPush()) {
      ...
      long maxWorkGroupSize =
          getMaxWorkGroupSizeForKernel(clDevice, clNextPositions, 0); // local 4 byte (integer)
      ...
      CLInfo.checkCLError(
          clSetKernelArg1f(
              clNextPositions, 12, (float) attributesFloorField.getPotentialFieldResolution()));
      ...
      try (MemoryStack stack = stackPush()) {
        ...
        log.info(
            name
                ...
                + eventAddr
                + ": "
                + ((double) endTime.get() - startTime.get()) / 1_000_000.0
                + " [ms]");
        endTime.clear();
        startTime.clear();
        return result;
  ...
  private long getMaxWorkGroupSizeForKernel(long clDevice, long clKernel, long workItemMem)
      throws OpenCLException {
    try (MemoryStack stack = stackPush()) {
      LongBuffer pp = stack.mallocLong(1);
      CLInfo.checkCLError(
          clGetKernelWorkGroupInfo(clKernel, clDevice, CL_KERNEL_LOCAL_MEM_SIZE, pp, null));
      ...
      long maxWorkGroupSizeForLocalMemory =
          workItemMem == 0 ? max_work_group_size : (max_local_memory_size / (workItemMem));
      PointerBuffer ppp = stack.mallocPointer(1);
      CLInfo.checkCLError(
          clGetKernelWorkGroupInfo(clKernel, clDevice, CL_KERNEL_WORK_GROUP_SIZE, ppp, null));

      long maxWorkGroupSizeForPrivateMemory = ppp.get(0);
      logger.debug(
          "CL_KERNEL_WORK_GROUP_SIZE (" + clKernel + ") = " + maxWorkGroupSizeForPrivateMemory);
      // return Math.min(max_work_group_size, Math.min(maxWorkGroupSizeForLocalMemory,
      // maxWorkGroupSizeForPrivateMemory));
      return Math.min(
          max_work_group_size,
          Math.min(maxWorkGroupSizeForLocalMemory, maxWorkGroupSizeForPrivateMemory));
    }
  }
  ...
  private void clBitonicSort(
    ...
    try (MemoryStack stack = stackPush()) {
      ...
      IntBuffer errcode_ret = stack.callocInt(1);
      long maxWorkGroupSize =
          getMaxWorkGroupSizeForKernel(
              clDevice, clBitonicMergeLocal, 8); // local memory for key and values (integer)
