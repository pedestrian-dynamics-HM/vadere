 ...
 *     <p>This class offers the methods to compute an array based linked-cell which contains
 *     2D-coordinates i.e. {@link VPoint} using the GPU (see. green-2007 Building the Grid using
 *     Sorting).
 */
public class CLParallelEventDrivenOSM {
  ...
  public CLParallelEventDrivenOSM(
    ...
    this.iGridSize =
        new int[] {
          (int) Math.ceil(bound.getWidth() / cellSize),
          (int) Math.ceil(bound.getHeight() / cellSize)
        };
  ...
  /**
   * Set's new set of agents which we want to simulate. This will remove all other agents. This
   * method will free old data from the device memory and transfer required data to the device as
   * well as reserve new device memory.
   *
   * @param pedestrians the list of pedestrians / agents
   * @throws OpenCLException
   */
  public void setPedestrians(@NotNull final List<PedestrianOpenCL> pedestrians)
      throws OpenCLException {
    ...
    try (MemoryStack stack = stackPush()) {
      ...
      clPedestrians =
          clCreateBuffer(
              clContext, CL_MEM_READ_WRITE, OFFSET * 4 * pedestrians.size(), errcode_ret);
      clPositions =
          clCreateBuffer(
              clContext, CL_MEM_READ_WRITE, COORDOFFSET * 4 * pedestrians.size(), errcode_ret);
      ...
      clReorderedEventTimes =
          clCreateBuffer(clContext, CL_MEM_READ_WRITE, 4 * pedestrians.size(), errcode_ret);
      clIds =
          clCreateBuffer(
              clContext, CL_MEM_READ_WRITE, iGridSize[0] * iGridSize[1] * 4, errcode_ret);
      clEventTimesData =
          clCreateBuffer(clContext, CL_MEM_READ_WRITE, 4 * pedestrians.size(), errcode_ret);
  ...
  public boolean update(float simTimeInSec) throws OpenCLException {
    try (MemoryStack stack = stackPush()) {
      ...
      clEventTimes(clIds, clReorderedEventTimes, clCellStarts, clCellEnds);
      ...
      clSwapIndex(clGlobalIndexOut, clGlobalIndexIn, clIndices, numberOfElements);
  ...
  public void readFromDevice() {
    ...
    clEnqueueReadBuffer(
        clQueue, swap ? clGlobalIndexOut : clGlobalIndexIn, true, 0, memIndices, null, null);
    ...
    float[] positionsAndRadi =
        CLUtils.toFloatArray(memNextPositions, numberOfElements * COORDOFFSET);
  ...
  private FloatBuffer allocPedestrianEventTimeMemory(
      @NotNull final List<PedestrianOpenCL> pedestrians) {
  ...
  /**
   * Allocates the host memory for objects which do not change during the simulation e.g. the static
   * potential field. Therefore this initialization is done once for a simulation.
   */
  ...
  /**
   * Allocates the device memory for objects which do not change during the simulation e.g. the
   * static potential field. Therefore this initialization is done once for a simulation.
   */
      ...
      try (MemoryStack stack = stackPush()) {
        ...
        clPotentialFieldSize =
            clCreateBuffer(
                clContext,
                CL_MEM_READ_ONLY | CL_MEM_ALLOC_HOST_PTR | CL_MEM_COPY_HOST_PTR,
                memPotentialFieldSize,
                errcode_ret);
        clPotentialFieldGridSize =
            clCreateBuffer(
                clContext,
                CL_MEM_READ_ONLY | CL_MEM_ALLOC_HOST_PTR | CL_MEM_COPY_HOST_PTR,
                memPotentialFieldGridSize,
                errcode_ret);
        clWorldOrigin =
            clCreateBuffer(
                clContext,
        ...
        clTargetPotential =
            clCreateBuffer(
                clContext,
                CL_MEM_READ_ONLY | CL_MEM_ALLOC_HOST_PTR | CL_MEM_COPY_HOST_PTR,
                memTargetPotentialField,
                errcode_ret);
        clObstaclePotential =
            clCreateBuffer(
                clContext,
                CL_MEM_READ_ONLY | CL_MEM_ALLOC_HOST_PTR | CL_MEM_COPY_HOST_PTR,
                memObstaclePotentialField,
                errcode_ret);
        clCirclePositions =
            clCreateBuffer(
                clContext,
                ...
                errcode_ret);
        clCellStarts =
            clCreateBuffer(clContext, CL_MEM_READ_WRITE, 4 * numberOfGridCells, errcode_ret);
        clCellEnds =
            clCreateBuffer(clContext, CL_MEM_READ_WRITE, 4 * numberOfGridCells, errcode_ret);
      }
    }
      ...
      CLInfo.checkCLError(
          (int)
              enqueueNDRangeKernel(
                  "clMemSet", clQueue, clResetCells, 1, null, clGlobalWorkSize, null, null, null));
    }
  }
  ...
  private void clCalcHash(
      ...
      final int numberOfElementsPower)
      throws OpenCLException {
    try (MemoryStack stack = stackPush()) {
      PointerBuffer clGlobalWorkSize = stack.callocPointer(1);
      ...
      CLInfo.checkCLError(
          (int)
              enqueueNDRangeKernel(
                  "clCalcHash", clQueue, clCalcHash, 1, null, clGlobalWorkSize, null, null, null));
    }
  }
  ...
  private void clMove(
    ...
    try (MemoryStack stack = stackPush()) {
      ...
      PointerBuffer clGlobalWorkSize = stack.callocPointer(1);
      // long maxWorkGroupSize = CLUtils.getMaxWorkGroupSizeForKernel(clDevice, clMove, 0,
      // max_work_group_size, max_local_memory_size); // local 4 byte (integer)
      ...
      CLInfo.checkCLError(
          clSetKernelArg1f(clMove, 14, (float) attributesFloorField.getPotentialFieldResolution()));
      ...
      CLInfo.checkCLError(
          (int)
              enqueueNDRangeKernel(
                  "clMove", clQueue, clMove, 1, null, clGlobalWorkSize, null, null, null));
    }
  }
  ...
  private void clSwap(
    ...
    try (MemoryStack stack = stackPush()) {
      ...
      CLInfo.checkCLError(
          (int)
              enqueueNDRangeKernel(
                  "clSwap", clQueue, clSwap, 1, null, clGlobalWorkSize, null, null, null));
    }
  }
  ...
  private void clMinEventTime(
      final long clMinEventTime, final long clEventTimes, final int numberOfElements)
      throws OpenCLException {
    ...
    try (MemoryStack stack = stackPush()) {
      ...
      long maxWorkGroupSize =
          CLUtils.getMaxWorkGroupSizeForKernel(
              clDevice,
              clFindCellBoundsAndReorder,
              4,
              max_work_group_size,
              max_local_memory_size); // local 4 byte (integer)
    ...
    try (MemoryStack stack = stackPush()) {
      ...
      CLInfo.checkCLError(
          clSetKernelArg(
              clFindCellBoundsAndReorder,
              10,
              (Math.min(numberOfElements + 1, maxWorkGroupSize)) * 4)); // local memory
  ...
  private long enqueueNDRangeKernel(
    ...
    if (profiling) {
      try (MemoryStack stack = stackPush()) {
        ...
        CLInfo.checkCLError(
            clGetEventProfilingInfo(eventAddr, CL_PROFILING_COMMAND_START, startTime, null));
        CLInfo.checkCLError(
            clGetEventProfilingInfo(eventAddr, CL_PROFILING_COMMAND_END, endTime, null));
        clEvent.clear();
        // in nanaSec
        ...
        if (print) {
          log.info(
              name + " event time " + "0x" + eventAddr + ": " + toMillis(executionTime) + " [ms]");
        }
        endTime.clear();
        startTime.clear();
        return result;
    ...
    try (MemoryStack stack = stackPush()) {
      ...
      long maxWorkGroupSize =
          CLUtils.getMaxWorkGroupSizeForKernel(
              ...
              8,
              max_work_group_size,
              max_local_memory_size); // local memory for key and values (integer)
      ...
      if (numberOfElements <= maxWorkGroupSize) {
        ...
        CLInfo.checkCLError(
            clSetKernelArg(clBitonicSortLocal, 6, numberOfElements * 4)); // local memory
        CLInfo.checkCLError(
            clSetKernelArg(clBitonicSortLocal, 7, numberOfElements * 4)); // local memory
        ...
        CLInfo.checkCLError(
            clSetKernelArg(clBitonicSortLocal1, 4, maxWorkGroupSize * 4)); // local memory
        CLInfo.checkCLError(
            clSetKernelArg(clBitonicSortLocal1, 5, maxWorkGroupSize * 4)); // local memory
        ...
        for (int size = (int) (2 * maxWorkGroupSize); size <= numberOfElements; size <<= 1) {
          for (int stride = size / 2; stride > 0; stride >>= 1) {
            if (stride >= maxWorkGroupSize) {
              ...
              CLInfo.checkCLError(
                  clSetKernelArg(clBitonicMergeLocal, 8, maxWorkGroupSize * 4)); // local memory
              CLInfo.checkCLError(
                  clSetKernelArg(clBitonicMergeLocal, 9, maxWorkGroupSize * 4)); // local memory
              ...
              try {
                log.debug(
                    "The cl_program [0x"
                        + program
                        + "] was built "
                        + (CLInfo.getProgramBuildInfoInt(program, clDevice, CL_PROGRAM_BUILD_STATUS)
                                == CL_SUCCESS
                            ? "successfully"
                            : "unsuccessfully"));
  ...
  private void buildProgram() throws OpenCLException {
    try (MemoryStack stack = stackPush()) {
      ...
      log.debug(
          InfoUtils.getProgramBuildInfoStringASCII(clProgram, clDevice, CL_PROGRAM_BUILD_LOG));
      ...
      CLInfo.checkCLError(errcode_ret);
      clFindCellBoundsAndReorder =
          clCreateKernel(clProgram, "findCellBoundsAndReorder", errcode_ret);
      CLInfo.checkCLError(errcode_ret);
  ...
  private int getPotentialFieldWidth() {
    return (int) Math.floor(bound.getWidth() / attributesFloorField.getPotentialFieldResolution())
        + 1;
  }

  private int getPotentialFieldHeight() {
    return (int) Math.floor(bound.getHeight() / attributesFloorField.getPotentialFieldResolution())
        + 1;
  }
  ...
  private FloatBuffer generatePotentialFieldApproximation(
      @NotNull final EikonalSolver eikonalSolver) {
  ...
  public static class PedestrianOpenCL {
    ...
    public PedestrianOpenCL(
        final VPoint position, final float stepRadius, final float freeFlowSpeed) {
