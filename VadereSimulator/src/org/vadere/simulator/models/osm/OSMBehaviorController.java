 ...
 *
 * <p>This class can be used by {@link OptimalStepsModel} to react to environmental stimuli (see
 * {@link Stimulus}) and how an agent has categorized itself in regard to other agents (see {@link
 * SelfCategory}).
 *
 * <p>For instance:
 *
...
public class OSMBehaviorController {
  ...
  // Methods
  public void makeStepToTarget(
      @NotNull final PedestrianOSM pedestrian, @NotNull final Topography topography) {
   ...
   *
   * <p>!IMPORTANT! this function calls movePedestrian which must be called ONLY ONCE for each
  ...
  public void makeStep(
    ...
    if (nextPosition.equals(currentPosition)) {
      ...
      Vector2D pedVelocity =
          new Vector2D(nextPosition.x - currentPosition.x, nextPosition.y - currentPosition.y)
              .multiply(1.0 / stepTime);
    ...
    FootStep currentFootstep =
        new FootStep(currentPosition, nextPosition, stepStartTime, stepEndTime);
    pedestrian.getTrajectory().add(currentFootstep);
    pedestrian.getFootstepHistory().add(currentFootstep);
  }

  /**
   * This operation undo the last foot step of an agent. This is required to resolve conflicts by
   * the {@link org.vadere.simulator.models.osm.updateScheme.UpdateSchemeParallel}.
   *
   ...
   *
   * <p>Watch out: The focus is the behavioral change here and not the exact speed-up. The exact
   * speed-up factor requires empirical data.
   *
   * <p>In future: Requires data for calibration.
   *
   ...
   */
  public void changeToTargetRepulsionStrategyAndIncreaseSpeed(
  ...
  /**
   * In dangerous situation humans tend to escape to familiar places (safe zones). A pedestrian
   * selects the target which is closest to its source as safe zone. Or if pedestrian has no target,
   * select closest target as safe zone.
   *
   * <p>Watch out: This is our current definition of a safe zone. Maybe, this mus be adjusted in the
   * future when more empirical data about safe zones is available.
   */
  public void changeTargetToSafeZone(PedestrianOSM pedestrian, Topography topography) {
    ...
    ScenarioElement searchPosition =
        (pedestrian.getSource() == null) ? pedestrian : pedestrian.getSource();
    Target closestTarget =
        TopographyHelper.findClosestTargetToSource(
            topography, searchPosition, pedestrian.getThreatMemory().getLatestThreat());
  ...
  public PedestrianOSM findSwapCandidate(PedestrianOSM pedestrian, Topography topography) {
    ...
    List<Pedestrian> neighborsCloserToTarget =
        TopographyHelper.getNeighborsCloserToTarget(pedestrian, topography);
    ...
    if (neighborsCloserToTarget.size() > 0) {
      for (Pedestrian neighbor : neighborsCloserToTarget) {
        if (neighbor.hasNextTarget()) {
          ...
          boolean walkingDirectionDiffers =
              TopographyHelper.walkingDirectionDiffers(pedestrian, neighbor, topography);
   ...
   *
   * <p>Watch out: This method manipulates pedestrian2 which is contained in a queue sorted by
   * timeOfNextStep! The calling code must re-add pedestrian2 after invoking this method.
   */
  public void swapPedestrians(
      PedestrianOSM pedestrian1, PedestrianOSM pedestrian2, Topography topography) {
  ...
  public void changeRepulsion(PedestrianOSM pedestrian) {
    pedestrian.setCombinedPotentialStrategy(
        CombinedPotentialStrategy.PEDESTRIAN_REPULSION_STRATEGY);
  }
}
