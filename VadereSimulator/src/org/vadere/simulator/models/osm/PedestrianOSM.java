...
public class PedestrianOSM extends Pedestrian {
  ...
  public PedestrianOSM(
    ...
    this.combinedPotentialStrategy =
        new TargetAttractionStrategy(
            potentialFieldTarget, potentialFieldObstacle, potentialFieldPedestrian);
    ...
    this.stepLength =
        attributesOSM.getStepLengthIntercept()
            + this.stepDeviation
  ...
  public void updateNextPosition() {
    ...
    if (PotentialFieldTargetRingExperiment.class.equals(potentialFieldTarget.getClass())) {
      ...
      // }
        ...
        || getDurationNextStep() > getAttributesOSM().getMaxStepDuration()) {
      this.nextPosition = getPosition();
    } else if (isCurrentTargetAnAgent() == false
        && topography.getTarget(getNextTargetId()).getShape().contains(getPosition())) {
      this.nextPosition = getPosition();
    } else {
      ...
      if (stairs == null) { // --> agent is on area
        ...
        if (attributesOSM.isMinimumStepLength()
            && getPosition().distance(nextPosition) < minStepLength) {
  ...
  /**
   * Returns the step size the agent is currently trying to achieve. This step size is dynamic i.e.
   * it might be influenced by the situation via a dynamic desired speed influenced by {@link
   * SpeedAdjuster}. If this step size is larger or smaller than {@link #getFreeFlowStepSize()} the
   * agent tries to accelerate or decelerate respectively.
   *
  ...
  public double getDesiredStepSize() {
    ...
    if (attributesOSM.isDynamicStepLength()) {
      double step =
          attributesOSM.getStepLengthIntercept()
              + attributesOSM.getStepLengthSlopeSpeed() * getDesiredSpeed()
              + stepDeviation;
      return step;
    }
  ...
  /**
   * Returns the desired speed an agent is currently trying to achieve. This speed is dynamic i.e.
   * it might be influenced by the situation via a dynamic desired speed influenced by {@link
   * SpeedAdjuster}.
   *
  ...
  public Vector2D getPedestrianGradient(VPoint pos) {
    return potentialFieldPedestrian.getAgentPotentialGradient(
        pos, new Vector2D(0, 0), this, relevantPedestrians);
  }
  ...
  public void refreshRelevantPedestrians() {
    VCircle reachableArea = new VCircle(getPosition(), getFreeFlowStepSize());
    setRelevantPedestrians(
        potentialFieldPedestrian.getRelevantAgents(reachableArea, this, getTopography()));
  }

  // Setters...

  public void setRelevantPedestrians(
      @NotNull final Collection<? extends Agent> relevantPedestrians) {
  ...
  public void setCombinedPotentialStrategy(CombinedPotentialStrategy newStrategy) {
    if (newStrategy == CombinedPotentialStrategy.TARGET_ATTRACTION_STRATEGY) {
      ...
      this.combinedPotentialStrategy =
          new TargetRepulsionStrategy(
              this.potentialFieldTarget,
              this.potentialFieldObstacle,
              this.potentialFieldPedestrian);
    } else if (newStrategy == CombinedPotentialStrategy.PEDESTRIAN_REPULSION_STRATEGY) {
      this.combinedPotentialStrategy =
          new PedestrianRepulsionStrategy(
              this.potentialFieldTarget,
              this.potentialFieldObstacle,
              this.potentialFieldPedestrian,
              (AttributesPedestrianRepulsionPotentialStrategy)
                  this.attributesCombinedPotentialStrategy);
    } else {
  ...
  public double getDurationNextStep() {
    return Math.min(
        getAttributesOSM().getMaxStepDuration(), getDesiredStepSize() / getDesiredSpeed());
  }
  ...
  public ArrayList<OptimizationMetric> getOptimizationMetricElements() {
    // Function that can be called by a processor to obtain metric data
    ...
    if (this.getGroupIds() != null) {
      if (this.getGroupIds().size() == 1) {
        for (int i : getGroupIds()) {
          Collection<Pedestrian> pp =
              getTopography()
                  .getPedestrianDynamicElements()
                  .getElements()
                  .stream()
                  .filter(p -> p.getGroupIds().getFirst() == i && p.getId() != getId())
  ...
  public void setCombinedPotentialStrategyAttributes(
      AttributesCombinedPotentialStrategy attributes) {
