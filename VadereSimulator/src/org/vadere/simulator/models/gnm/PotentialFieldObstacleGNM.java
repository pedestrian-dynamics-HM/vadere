...
/**
 * Provides gradients for obstcles in a continous manner. The gradient is computed by using a point
 * a little ahead of the given pedestrian and calculating the distance to that point.
 */
@ModelClass
public class PotentialFieldObstacleGNM implements GradientProvider, PotentialFieldObstacle {
  ...
  public void gradient(double t, int targetID, double[] x, double[] completeGrad) {
    ...
    for (Obstacle obstacle : obstacles) {
      ...
      pot =
          attributesPotential.getObstacleBodyPotential()
              * MathUtil.cutExp(distance, attributesPotential.getObstacleRepulsionStrength()); // /
      // Math.sin(phiB)*1.18167);

      // compute and normalize the gradient length to the potential
      double normDV = Math.sqrt(distanceVec[0] * distanceVec[0] + distanceVec[1] * distanceVec[1]);
      if (normDV > epsDV) {
