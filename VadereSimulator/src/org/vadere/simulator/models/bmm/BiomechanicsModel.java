...
public class BiomechanicsModel implements MainModel {
  ...
  @Override
  public <T extends DynamicElement> PedestrianBMM createElement(
  ...
  private PedestrianBMM createElement(
      @NotNull final VPoint position, @NotNull final AttributesAgent pedAttributes) {
    PedestrianBMM pedestrian =
        new PedestrianBMM(
            position, topography, pedAttributes, attributesBMM, attributesBHM, random);
    return pedestrian;
  }
  ...
  public void update(final double simTimeInSec) {
    ...
    List<VPoint> positions =
        pedestriansBMM.stream().map(ped -> ped.getPosition()).collect(Collectors.toList());

    UnsupportedSelfCategoryException.throwIfPedestriansNotTargetOrientied(
        pedestriansBMM, this.getClass());
    ...
    for (int i = 0; i < pedestriansBMM.size(); i++) {
      ...
      FootStep currentFootstep =
          new FootStep(positions.get(i), agent.getPosition(), lastSimTimeInSec, simTimeInSec);
