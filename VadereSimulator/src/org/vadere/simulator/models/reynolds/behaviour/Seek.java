...
public class Seek {
  ...
  public Vector2D nextStep(double simTime, Vector2D currentMov, PedestrianReynolds ped) {
    ...
    double simTimeStepLength =
        0.4; // TODO [priority=low] [task=refactoring] get this attribute from AttributePedestrians

    VPoint pos = ped.getPosition();
    // VPoint target = model.getScenario().getTargets().get(0).getShape().closestPoint(pos);
    VPoint target =
        model.getScenario().getTarget(ped.getTargets().getFirst()).getShape().closestPoint(pos);
