 ...
 *
 * <p>For creation of submodels, see {@link SubModelBuilder}! The SubModelBuilder should be used in
 * the {@link MainModel#initialize} method.
 */
public class MainModelBuilder {
  ...
  public MainModelBuilder(
      @NotNull final ScenarioStore scenarioStore,
      @Nullable final AMesh floorFieldMesh,
      @Nullable final AMesh backgroundMesh) {
  ...
  private MainModel instantiateMainModel(Random random) {
    ...
    mainModel.initialize(
        scenarioStore.getAttributesList(),
        getDomain(),
        scenarioStore.getTopography().getAttributesPedestrian(),
        random);
    return mainModel;
  }
}
