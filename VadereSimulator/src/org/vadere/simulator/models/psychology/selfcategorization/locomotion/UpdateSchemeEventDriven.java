...
public class UpdateSchemeEventDriven
    implements DynamicElementAddListener, DynamicElementRemoveListener {
  ...
  private void clearStrides(@NotNull final Topography topography) {
     ...
     */
    for (PedestrianSelfCatThreat pedestrian :
        topography.getElements(PedestrianSelfCatThreat.class)) {
  ...
  private void update(
      @NotNull final PedestrianSelfCatThreat pedestrian,
    ...
    // TODO: Maybe, use a state table with function pointers to a template function myFunc(ped,
    // topography, time)
    if (selfCategory == SelfCategory.TARGET_ORIENTED) {
      ...
      // TODO: Check if "changeToTargetRepulsionStrategyAndIncreaseSpeed()" is really necessary
      // here.
