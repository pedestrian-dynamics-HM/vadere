...
@ModelClass
public class PotentialFieldTargetQueuingGrid
    implements IPotentialFieldTargetGrid,
        DynamicElementRemoveListener<Pedestrian>,
  ...
  public PotentialFieldTargetQueuingGrid(
    ...
    this.competitiveField =
        new PotentialFieldTargetGrid(
            domain, attributesPedestrian, attributesQueuingGame.getNavigationFloorField());
    this.gentleField =
        new PotentialFieldTargetGrid(
            domain, attributesPedestrian, attributesQueuingGame.getQueuingFloorField());
    ...
    for (Obstacle obstacle : domain.getTopography().getObstacles()) {
      FloorDiscretizer.setGridValuesForShapeCentered(
          cellGrid, obstacle.getShape(), new CellState(Double.MAX_VALUE, PathFindingTag.Obstacle));
    }
    ...
    this.queues =
        domain
            ...
            .map(t -> t.getId())
            .distinct()
            .map(targetId -> new Queue(domain.getTopography(), targetId, detector))
            .collect(Collectors.toList());
  }
  ...
  public void update(double simTimeInSec) {
    ...
    // update lifetimes
    Map<PedestrianAttitudeType, List<Pedestrian>> pedGroup =
        pedestrians
            .stream()
            .collect(
                Collectors.groupingBy(
                    p -> p.getModelPedestrian(QueueingGamePedestrian.class).getAttituteType()));

    if (pedGroup.get(PedestrianAttitudeType.GENTLE) != null) {
      pedGroup
          .get(PedestrianAttitudeType.GENTLE)
          .stream()
          .filter(
              p ->
                  changeAttitude(
    ...
    if (pedGroup.get(PedestrianAttitudeType.COMPETITIVE) != null) {
      pedGroup
          .get(PedestrianAttitudeType.COMPETITIVE)
          .stream()
          .filter(
              p ->
                  changeAttitude(
    ...
    logger.info(
        "#competitive peds = " + numberOfCompetitivePeds + ", #gentle peds: " + numberOfGentlePeds);
  }
  ...
  public boolean changeAttitude(
      final Pedestrian ped,
      final double lifeTimeInSec,
      final double lastLifeTimeInSec,
  ...
  private static boolean isValidArguments(
      final Topography topography, final AttributesQueuingGame attributesQueuingGame) {
    ...
    TimeCostFunctionType competitiveTimeCostFunctionType =
        competitiveAttributes.getTimeCostAttributes().getType();
    TimeCostFunctionType gentleTimeCostFunctionType =
        gentleAttributes.getTimeCostAttributes().getType();
  ...
  public double getPotential(IPoint pos, Agent agent) {
    ...
    if (pedestrianAttitudeMap.containsKey(ped)
        && queues.stream().anyMatch(queue -> queue.isQueued(ped))) {
