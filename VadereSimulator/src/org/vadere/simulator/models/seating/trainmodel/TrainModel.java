...
public class TrainModel {
  ...
  private final Comparator<ScenarioElement> scenarioElementComperatorX =
      new Comparator<ScenarioElement>() {
        @Override
  ...
  public TrainModel(Topography topography, TrainGeometry trainGeometry) {
    ...
    if (seatRows
            .stream()
            .anyMatch(l -> l.size() % 4 != 0) // there must be 4 seat groups per entrance area
    ...
    Rectangle2D longAisle =
        createFilterRect(
            leftmostCompartment.getMinY() + trainGeometry.getBenchWidth(),
            trainGeometry.getAisleWidth());

    // Sort compartment target by x coordinate. Right-most target must be the first in the list.
    compartmentTargets =
        findTargets(longAisle)
            .stream()
            .sorted(
                (a, b) ->
                    Double.compare(
                        a.getShape().getBounds().getX(), b.getShape().getBounds().getX()))
    ...
    Rectangle2D leftDoorsRect =
        createFilterRect(
            topography.getBounds().getMinY(),
            leftmostCompartment.getMinY() - topography.getBounds().getMinY() + innerOffset);
    Rectangle2D rightDoorsRect =
        createFilterRect(
            leftmostCompartment.getMaxY() - innerOffset,
  ...
  /**
   * Return a new list of all pedestrians in the scenario. Pedestrians that are already defined in
   * the topography are not counted until the simulation has started.
   */
  ...
  private <T extends ScenarioElement> List<T> findScenarioElement(
      List<T> scenarioElementList, Rectangle2D box) {
    return scenarioElementList
        .stream()
  ...
  /**
   * Calculate the seat number within a compartment. The seat number corresponds to the numbering
   * used in the data collection app and data processing.
   *
   * @param longRowIndex the index of a "long row" of seats which goes from the front to the rear.
   *     Long rows are indexed from left to right.
   * @param indexInLongRow the index of a seat within a long row.
   */
  static int calculateSeatNumberWithinCompartment(int longRowIndex, int indexInLongRow) {
    ...
    else offset = (indexInLongRow - 2) % 4; // row of 4 abreast seats in compartment
  ...
  public int getEntranceAreaIndexForPerson(Pedestrian p) {
    ...
    if (source == null) throw new RuntimeException("Person's source is null.");

    final int index = getEntranceAreaIndexOfSource(source);
    if (index == -1) throw new RuntimeException("Person is not spawned at one of the doors.");
    return index;
  }
