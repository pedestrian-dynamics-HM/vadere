...
/**
 * The Resource class is for loading, changing, adding and manipulating properties in the property
 * file [applicationName]_config.properties. The global_config.properties file plays a special role.
 * If a property is not found in the specific file e. g. postvisualization_config.properties than
 * the global_config.properties will be used as second step. Note: The global_config.properties are
 * read only!
 */
public class Resources {
  ...
  private Properties getProperties() {
    if (properties == null) {
      ...
      try {
        ...
        e.printStackTrace();
        logger.error(
            "topographyError while loading properties for application: " + applicationName);
      } finally {
        try {
          in.close();
        ...
        bout =
            new BufferedOutputStream(
                new FileOutputStream(
                    location.getFile() + "config/" + applicationName + "_config.properties"));
  ...
  public Icon getIcon(final String name, final int iconWidth, final int iconHeight) {
    ...
    Image img =
        icon.getImage()
            .getScaledInstance(iconWidth, iconHeight, java.awt.Image.SCALE_AREA_AVERAGING);
  ...
  private static Color stringToColor(final String sColor) {
    return new Color(
        Integer.parseInt(sColor.substring(1, 3)),
        Integer.parseInt(sColor.substring(3, 5)),
