...
public class ActionGenerateMesh extends AbstractAction {
  ...
  public ActionGenerateMesh(
      final String name, Icon icon, String shortDescription, final ProjectViewModel model) {
    super(name, icon);
  ...
  public void actionPerformed(final ActionEvent e) {
    ...
    ActionTranslateTopographyDialog dialog =
        new ActionTranslateTopographyDialog(0.5, 5.0, "hmin, hmax");
    ...
    if (dialog.getValue()) {
      ...
      CompletableFuture.supplyAsync(() -> constructor.pslgToAdaptivePMesh(pslg, hmin, hmax, true))
          .thenAccept(mesh -> saveFloorFieldMesh(mesh, ""))
          .exceptionally(
              ex -> {
                ex.printStackTrace();
                return null;
      ...
      CompletableFuture.supplyAsync(
              () -> constructor.pslgToCoarsePMesh(pslg, p -> Double.POSITIVE_INFINITY, true))
          .thenAccept(mesh -> saveFloorFieldMesh(mesh, IOUtils.BACKGROUND_MESH_ENDING))
          .exceptionally(
              ex -> {
                ex.printStackTrace();
                return null;
  ...
  private void saveFloorFieldMesh(
      @NotNull final IMesh<PVertex, PHalfEdge, PFace> mesh, final String ending) {
    ...
    File meshDir =
        new File(
            model
                ...
                .concat("/" + IOUtils.SCENARIO_DIR + "/" + IOUtils.MESH_DIR));
    File outputFile =
        new File(
            meshDir.getAbsoluteFile()
                + "/"
                + model.getCurrentScenario().getName()
                + ending
                + ".poly");
