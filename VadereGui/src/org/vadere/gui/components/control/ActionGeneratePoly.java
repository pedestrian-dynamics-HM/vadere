...
public class ActionGeneratePoly extends AbstractAction {
  ...
  public ActionGeneratePoly(
      final String name,
      Icon icon,
      String shortDescription,
      final DefaultModel<? extends DefaultSimulationConfig> model) {
    super(name, icon);
  ...
  public void actionPerformed(final ActionEvent e) {
    JFileChooser fileChooser =
        new JFileChooser(CONFIG.getString("SettingsDialog.snapshotDirectory.path"));

    Date todaysDate = new java.util.Date();
    SimpleDateFormat formatter =
        new SimpleDateFormat(CONFIG.getString("SettingsDialog.dataFormat"));
    String formattedDate = formatter.format(todaysDate);

    File outputFile =
        new File(Messages.getString("FileDialog.filenamePrefix") + formattedDate + ".poly");
    ...
    if (returnVal == JFileChooser.APPROVE_OPTION) {
      ...
      outputFile =
          fileChooser.getSelectedFile().toString().endsWith(".poly")
