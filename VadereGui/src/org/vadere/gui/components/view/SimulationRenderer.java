...
public abstract class SimulationRenderer extends DefaultRenderer {
  ...
  protected void renderPreTransformation(Graphics2D graphics2D, int width, int height) {
    if (model.isFloorFieldAvailable()
        && (model.config.isShowTargetPotentialField() || model.config.isShowPotentialField())) {
      synchronized (model) {
        renderPotentialFieldOnViewport(
            ...
            0,
            (int)
                (Math.min(model.getTopographyBound().width, model.getViewportBound().width)
                    * model.getScaleFactor()),
            (int)
                (Math.min(model.getTopographyBound().height, model.getViewportBound().height)
                    * model.getScaleFactor()));
      }
    }
  ...
  public void renderPostTransformation(
    ...
    if (model.config.isShowObstacles()) {
      renderScenarioElement(
          model.getTopography().getObstacles(), graphics, model.config.getObstacleColor());
    }

    if (model.config.isShowMeasurementArea()) {
      renderMeasurementAreas(
          model.getTopography().getMeasurementAreas(),
          graphics,
          model.config.getMeasurementAreaColor());
    }
    ...
    if (model.config.isShowTargets()) {
      renderScenarioElement(
          model.getTopography().getTargets(), graphics, model.config.getTargetColor());
    }

    if (model.config.isShowTargetChangers()) {
      renderScenarioElement(
          model.getTopography().getTargetChangers(),
          graphics,
          model.config.getTargetChangerColor());
    }

    if (model.config.isShowAbsorbingAreas()) {
      renderScenarioElement(
          model.getTopography().getAbsorbingAreas(),
          graphics,
    ...
    if (model.config.isShowAerosolClouds()) {
      renderAerosolClouds(
          model.getTopography().getAerosolClouds(), graphics, model.config.getAerosolCloudColor());
    }

    if (model.config.isShowDroplets()) {
      // ToDo use renderScenarioElement() instead?
      renderAllDroplets(
          model.getTopography().getDroplets(), graphics, model.config.getDropletsColor());
    }

    if (model.config.isShowSources()) {
      renderScenarioElement(
          model.getTopography().getSources(), graphics, model.config.getSourceColor());
    }
  ...
  protected void renderTrajectory(
      final Graphics2D g, final java.util.List<VPoint> points, final Pedestrian pedestrain) {
    renderTrajectory(g, points.stream(), pedestrain);
  }

  protected void renderTrajectory(
      final Graphics2D g, final Stream<VPoint> points, final Pedestrian pedestrain) {
    ...
    VPoint endPos =
        model.config.isInterpolatePositions()
            ? pedestrain.getInterpolatedFootStepPosition(model.getSimTimeInSec())
            : pedestrain.getPosition();
    Path2D.Double path = new Path2D.Double();
    path.moveTo(endPos.getX(), endPos.getY());
    points.forEachOrdered(p -> path.lineTo(p.getX(), p.getY()));
  ...
  private void renderDensity(final Graphics2D g) {
    CLGaussianCalculator densityCalculator =
        new CLGaussianCalculator(model, model.config.getDensityScale());
    /*
  ...
  private void renderPotentialFieldOnViewport(
      final Graphics2D g, final int xPos, final int yPos, final int width, final int height) {
    ...
    double maxDistance =
        Math.sqrt(
            model.getTopography().getBounds().getWidth()
                    * model.getTopography().getBounds().getWidth()
                + model.getTopography().getBounds().getHeight()
                    * model.getTopography().getBounds().getHeight());
      ...
      for (int y = 0; y < potentialFieldImage.getHeight(); y++) {
        ...
        VPoint pos =
            new VPoint(
                ...
                viewportBound.getMinY()
                    + (potentialFieldImage.getHeight() - 1 - y) / model.getScaleFactor());
  ...
  private void renderPotentialField(
      final Graphics2D g, final int xPos, final int yPos, final int width, final int height) {
  ...
  public Color getPedestrianColor(@NotNull final Agent agent) {
    ...
    switch (model.config.getAgentColoring()) {
      ...
      case INFORMATION_STATE:
        if (agent instanceof Pedestrian) {
          Pedestrian pedestrian = (Pedestrian) agent;
          return model.config.getInformationStateColor(
              pedestrian.getKnowledgeBase().getInformationState());
        }
      case PREDICATE:
        {
      ...
      case HEALTH_STATUS:
        {
          if (agent instanceof Pedestrian) {
            Pedestrian pedestrian = (Pedestrian) agent;
            return model.config.getHealthStatusColor(
                pedestrian.isInfectious(), pedestrian.getDegreeOfExposure());
          }
        }
      default:
        return model.config.getPedestrianColor();
    }
  }
}
