...
public class JsonValidIndicator extends JPanel {
  ...
  public JsonValidIndicator() {
    ...
    labelJsonInvalid =
        new JLabel(
            "<html>"
                + Messages.getString("TextView.lbljsoninvalid.text")
                + " <font color=gray size=-1><a href=#>"
                + Messages.getString("TextView.lbljsoninvalid.showErrorText")
                + "</a></font></html>");
    add(labelJsonInvalid);
    labelJsonInvalid.setIcon(new ImageIcon(Resources.class.getResource("/icons/Error.gif")));
    labelJsonInvalid.addMouseListener(
        new MouseAdapter() {
          @Override
