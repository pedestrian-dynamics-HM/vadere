...
public class RecordTriangulationMovie {
  ...
  public static void main(String... args) throws IOException {
    ...
    Function<AFace, Color> colorFunction1 =
        f -> {
          float q = Math.max(0.0f, Math.min(1.0f, (float) meshImprover.faceToQuality(f)));
          return new Color(q, q, q);
        };

    Function<AFace, Color> colorFunction2 =
        f -> {
          return ColorHelper.numberToHurColor(
              (float) f.getId() / meshImprover.getMesh().getNumberOfFaces());
        };
    ...
    MeshRenderer<AVertex, AHalfEdge, AFace> meshRenderer =
        new MeshRenderer<>(meshImprover.getMesh(), f -> false, colorFunction1);

    MeshPanel<AVertex, AHalfEdge, AFace> distmeshPanel =
        new MeshPanel<>(meshRenderer, bbound.getWidth() * 1000, bbound.getHeight() * 1000);
    ...
    while (nSteps < 300) {
      ...
      if (!meshImprover.isInitialized()) {
        addPictures(
            ...
            meshRenderer,
            10,
            (int) bbound.getWidth() * 1000,
            (int) bbound.getHeight() * 1000);
      } else if (finished) {
        finished = false;
        addPictures(
            recorder,
