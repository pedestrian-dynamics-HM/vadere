...
public class ClassFinder {
  ...
  public static List<String> getAttributesNames() {
    ...
    return getClassNamesWithTagInPackage(
        AttributesOSM.class.getPackage().getName(), Attributes.class);
  }
  ...
  public static List<String> getModelNames() {
    List<String> modelNames =
        getClassNamesWithTagInPackage(Model.class.getPackage().getName(), Model.class);
  ...
  public static Map<String, List<String>> groupPackages(List<String> classNamesInPackageNotation) {
    ...
    Map<String, List<String>> groupNamesToMembers =
        sortClassNamesIntoGroups(classNamesInPackageNotation, groupNames);
  ...
  public static List<Class<?>> getOutputFileClasses() {
    return findSubclassesInPackage(OutputFile.class.getPackage().getName(), OutputFile.class)
        .stream()
        .filter(cfile -> !Modifier.isAbstract(cfile.getModifiers()))
  ...
  public static List<Class<?>> getProcessorClasses(Type keyType) {
    return findSubclassesInPackage(DataProcessor.class.getPackage().getName(), DataProcessor.class)
        ...
        .filter(
            cproc ->
                findGenericProcessorSuperclass(cproc).getActualTypeArguments()[0].equals(keyType))
        .collect(Collectors.toList());
  }
  ...
  private static List<String> getClassNamesWithTagInPackage(
      String packageName, Class<?> baseClassOrInterface) {
    return findSubclassesInPackage(packageName, baseClassOrInterface)
        .stream()
        .map(Class::getName)
        .collect(Collectors.toList());
  }

  private static List<Class<?>> findSubclassesInPackage(
      String packageName, Class<?> baseClassOrInterface) {
    try {
   ...
   * and subpackages.
   *
   * <p>Deprecated since this method is slow if we have to access jar file, which is the case if we
   * cup the project via vadere.jar!
   *
  ...
  private static List<String> deriveGroupNamesFromPackageNames(
      List<String> classNamesInPackageNotation) {
  ...
  private static Map<String, List<String>> sortClassNamesIntoGroups(
      List<String> classNamesInPackageNotation, List<String> groupNames) {
    TreeMap<String, List<String>> groupNamesToMembers = new TreeMap<>();

    for (String groupName : groupNames) {
      List<String> groupMembers =
          classNamesInPackageNotation
              .stream()
    ...
    List<String> modelNamesWithoutPackage =
        classNamesInPackageNotation
            .stream()
