...
public class ActionSeeDiscardChanges extends AbstractAction {
  ...
  public ActionSeeDiscardChanges(
      final String name, final ProjectViewModel model, final VTable scenarioTable) {
    super(name);
    this.model = model;
  ...
  public void actionPerformed(final ActionEvent event) {
    ...
    if (selectedRows.length > 1) singularPlural = "plural";

    int choice =
        JOptionPane.showOptionDialog(
            ProjectView.getMainWindow(),
            VDialogManager.getPanelWithBodyAndTextArea(
                "<html>"
                    + Messages.getString(
                            "ActionSeeDiscardChanges.changes." + singularPlural + ".text")
                        .replace("%VAR%", String.valueOf(selectedRows.length))
                    + ":<br><br></html>",
                model.getDiffOfSelectedScenarios(selectedRows)),
    ...
    if (choice == JOptionPane.YES_OPTION) model.saveSelectedScenarios(selectedRows);

    if (choice == JOptionPane.NO_OPTION) model.discardChangesOfSelectedScenarios(selectedRows);
