...
public abstract class ActionAbstractSaveProject extends AbstractAction {
  ...
  static boolean askSaveUnlessUserCancels(ProjectViewModel model) throws IOException {
    if (!model.hasProjectChanged()) return true;
  ...
  /**
   * Save the project. Before that, if necessary, show the save dialog and give the user the option
   * to cancel.
   *
   * @param model
  ...
  static boolean saveProjectUnlessUserCancels(ProjectViewModel model) throws IOException {
    if (model.getCurrentProjectPath() == null && !doesUserChooseToSave(model)) return false;
  ...
  /**
   * Show project save dialog to let the user choose the project path. If the user clicks save, set
   * the model's project path to the selected path.
   *
   * @return false only if the user canceled the save dialog
   */
  ...
  static void saveProject(ProjectViewModel model) throws IOException {
    if (model.isProjectAvailable()) {
      try {
        ...
        IOUtils.errorBox(
            Messages.getString("SaveFileErrorMessage.text")
      ...
      IOUtils.infoBox(
          Messages.getString("EmptyProjectErrorMessage.text"),
  ...
  private static void updateRecentProjectPreferences(ProjectViewModel model) {
    ...
    List<String> values =
        VadereConfig.getConfig().getList(String.class, key, Collections.EMPTY_LIST);
