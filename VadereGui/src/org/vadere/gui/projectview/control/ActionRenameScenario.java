  ...
  public void actionPerformed(final ActionEvent event) {
    ...
    String newName =
        JOptionPane.showInputDialog(
            null, Messages.getString("listMenuRenameButtonAction.title"), oldName);
    ...
    if (newName.length() > 0 && !model.isScenarioNameInConflict(newName)) {
      try {
        ...
        model
            ...
            .replace(
                scenario,
                new VadereScenarioTableModel.VadereDisplay(scenario, VadereState.INITIALIZED));
        ...
        logger.error(
            "IO operations failed regarding renaming scenario "
                + oldName
                + " to "
                + newName
                + ":\n"
