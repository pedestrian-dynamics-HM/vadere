...
public class ActionRunRepeatedlyScenarios extends AbstractAction {
  ...
  public ActionRunRepeatedlyScenarios(
      final String name,
      final ProjectViewModel model,
      final VTable scenarioTable,
    ...
    if (model.runScenarioIsOk()) {
      ...
      if (scenarios.size() > 1) {
        throw new IllegalArgumentException(
            "Please select a single scenario for the stochastic evaluation");
      }
      Scenario singleScenario = scenarios.iterator().next();
      for (int i_rep = 0; i_rep < repetitions; i_rep++) scenarios.add(singleScenario);
