...
/**
 * Ask the user to select a scenario-file from an output directory to convert this file to a {@link
 * org.vadere.simulator.projects.Scenario} scenario.
 */
public class ActionGenerateScenarioFromOutputFile extends ActionAbstractAddScenario {
  ...
  protected Scenario generateVadere(final String name) throws IOException {
    try {
      ...
      String path =
          IOUtils.chooseFile(
              Messages.getString("LoadOutputText"), model.getCurrentProjectPath(), filter);
      if (path == null) {
        return null;
      }
