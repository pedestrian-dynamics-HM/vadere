...
public class ActionCreateProject extends AbstractAction {
  ...
  public void actionPerformed(final ActionEvent event) {
    try {
      if (!ActionAbstractSaveProject.askSaveUnlessUserCancels(model)) return;

      // get the name of the new project
      String newProjectName =
          JOptionPane.showInputDialog(
              null,
              Messages.getString("NewProjectName.text"),
      ...
      if (newProjectName == null || newProjectName.trim().length() == 0) {
        ...
        if (projectDirectory == null) return; // do not create project without saving it first.

        VadereProject project =
            new VadereProject(newProjectName, new ArrayList<>(), Paths.get(projectDirectory));
