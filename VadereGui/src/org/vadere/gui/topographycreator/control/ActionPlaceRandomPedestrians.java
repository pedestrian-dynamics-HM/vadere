...
public class ActionPlaceRandomPedestrians extends TopographyAction {
  ...
  public ActionPlaceRandomPedestrians(
      String name, String icon, IDrawPanelModel panelModel, UndoableEditSupport undoSupport) {
  ...
  public void actionPerformed(ActionEvent e) {
    ...
    if (dialog.showDialog() && dialog.isValid()) {
      ...
      SimpleReachablePointProvider pointProvider =
          SimpleReachablePointProvider.uniform(
              random, legalBound, topography.getObstacleDistanceFunction());

      double groupMembershipRatio = dialog.getGroupMembershipRatio();
      BinomialDistribution binomialDistribution =
          new BinomialDistribution(BINOMIAL_DISTRIBUTION_SUCCESS_VALUE, groupMembershipRatio);
      ...
      for (int i = 0; i < numOfPeds; i++) {
        ...
        if (!checkOverlap(newPosition)) {
          ...
          Pedestrian pedestrian =
              createPedestrian(dialog, topography, random, binomialDistribution, point, pedId);
  ...
  private Pedestrian createPedestrian(
      ActionRandomPedestrianDialog dialog,
      Topography topography,
      Random random,
      BinomialDistribution binomialDistribution,
      IPoint point,
      int id) {
    AttributesAgent attributesAgent = new AttributesAgent(topography.getAttributesPedestrian(), id);
  ...
  private LinkedList<Integer> getTargetList(
      TARGET_OPTION selectedOption, LinkedList<Integer> dialogList) {
  ...
  private void showWarning(int numOfPeds, int createdPeds) {
    String message =
        String.format(
            "%s: %d\n%s: %d",
            Messages.getString(
                "TopographyCreator.PlaceRandomPedestrians.couldNotPlaceAllPeds.requested.text"),
            numOfPeds,
            Messages.getString(
