...
/**
 * This AttributeEditor Class is used by JAttributeTable as the default fallback for any type which
 * has no preregistered editor and is an abstract type. It lists all subclasses as items in the
 * combobox and creates a new JPropertyPane for the selected type right under the table
 */
...
public class AbstractTypeCellEditor extends AttributeEditor {
  ...
  public AbstractTypeCellEditor(
      AttributeTreeModel.TreeNode model, JPanel contentPanel, Object initialValue) {
  ...
  private void initializeRunnableRegistry() {
    ...
    this.runnableRegistry.registerAction(
        "[null]",
        () -> {
          ...
          if (value instanceof Attributes) {
            try {
              ...
              throw new RuntimeException(
                  "This should not happen registered distribution not found" + value);
            }
          }
  ...
  private DefaultComboBoxModel<Object> initializeComboBoxModel(ArrayList<Class<?>> classesModel) {
    ...
    for (var clazz : classesModel) {
      try {
        ...
        System.out.println(
            getSimpleName(clazz) + " does not implement a default constructor. will be skipped");
      }
    }
