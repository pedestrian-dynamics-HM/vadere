...
/**
 * AbstrNode represents an object field which can hold an instance of a subtype of the given field
 * type. The AbstrNode does not notify structure listeners but value listeners if the object
 * instance changes to a different class structure.
 */
public class AbstrNode extends AttributeTreeModel.TreeNode {
  ...
  public AbstrNode(AttributeTreeModel.TreeNode parent, String fieldName, Class fieldType) {
    ...
    this.classRegistry =
        cache
            .getSubTypeOff(fieldType)
            .stream()
            .collect(
                Collectors.toMap(
                    aClass -> aClass,
                    aClass ->
                        AttributeTreeModel.parseClassTree(getParent(), fieldName, (Class) aClass)));
  }
  ...
  @Override
  public void updateParentsFieldValue(String field, Object object)
    ...
    if (object != null) {
      ...
      if (nextNode == null) {
        throw new RuntimeException(
            "the object "
                + object
                + " which was tried to be assigned to AbstrNode is not a subclass of "
                + getFieldType());
      }
