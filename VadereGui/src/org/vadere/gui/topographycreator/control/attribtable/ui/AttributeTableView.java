...
public class AttributeTableView extends JPanel implements ViewListener {
  ...
  public void selectionChange(@NotNull Object object) {
    ...
    if (this.activePage == null) {
      throw new RuntimeException(
          "AttributeTableView did not have a valid page for "
              + object.getClass()
              + ". The method buildPageFor("
              + object.getClass()
              + ") probably has not been called before this.");
    }
  ...
  public void viewChanged(Object object) {
    if (viewListener != null) viewListener.viewChanged(object);
  }
  ...
  public void buildPageFor(Class clazz) {
    ...
    var page =
        new AttributeTablePage(
            tree,
            AttributeTablePage.generateHeaderName(clazz),
            new AttributeTablePage.TableStyler(tree));
  ...
  public void buildPageFor(AttributeTreeModel.TreeNode tree) {
    var page =
        new AttributeTablePage(
            tree,
            AttributeTablePage.generateHeaderName(tree.getFieldType()),
            new AttributeTablePage.TableStyler(tree));
    var f = new ManualAttributeTableFocus();
    page.applyFocusPolicy(f);
