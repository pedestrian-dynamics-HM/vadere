...
/**
 * ChildObjectCellEditor is an editor used for any Object not registered and not abstract. It
 * displays all fields of the model inside a AttributeTablePage displayed inside the contentPanel
 * inherited by AttributeEditor. If the user clicks on the button displaying the object class type
 * the contentPanel will be displayed/hidden.
 */
public class ChildObjectCellEditor extends AttributeEditor {
  ...
  protected AttributeTablePage createInternalPropertyPane(Object newObject) {
    return new AttributeTablePage(
        model,
        AttributeTablePage.generateHeaderName(model.getFieldType()),
        new AttributeTablePage.TableStyler(model));
  }
}
