...
public class AttributeTablePage extends JPanel implements AttributeTreeModel.ValueListener {
  ...
  /**
   * container is used as a parent container for the class attribute table. All attributes can be
   * hidden when the user clicks on the header of the panel. It displays the classes simple name in
   * the header.
   */
  ...
  public AttributeTablePage(
      AttributeTreeModel.TreeNode model, String title, JAttributeTable.Styler pageStyler) {
    super(new BorderLayout());
    this.setBackground(Color.white);

    this.model = model;

    container = new JCollapsablePanel(title, JCollapsablePanel.Style.HEADER);
  ...
  /**
   * TableStyler is an implementation of a JAttributeTable to display every field in the model in a
   * | name | editor | structure. ---------------- | name | editor |
   */
  public static class TableStyler extends JAttributeTable.Styler {
    ...
    public JTable rowDelegateStyle(String id, AttributeEditor editor) {
      ...
      DefaultTableModel tableModel =
          new DefaultTableModel(new Object[] {"id", "attr"}, 1) {
            @Override
       ...
       * AttributeHelpView so that we are aware of the field that correspons to the cell clicked on
       * *
       */
            ...
            public void mouseClicked(MouseEvent e) {
              SwingUtilities.invokeLater(
                  () -> {
                    ...
                    if (field != null) AttributeHelpView.getInstance().loadHelpFromField(field);
                  });
            }
          });

      /** this section is supposed enable the resizing of the editor cells* */
      style.addComponentListener(
