...
/**
 * AttributeTableContainer is the root element for the attribute table component stack. It serves as
 * teh interface between the TopographyCreatorModel and the AttributeTree nodes. When instantiated
 * it will scan the project directory 'org.vadere' to find any class subclassing
 * AttributesScenarioElement and make the AttributeTableView precreate a Model/Page for each class.
 *
 * <p>AttributeTableContainer has two swing subcomponents: the AttributeTableView which is a view
 * displaying a AttributeTablePage for the currently selected ScenarioElement received from the
 * TopographyCreatorModel and a JTextPane which displays the help text for the currently selected
 * attribute in the UI
 */
public class AttributeTableContainer extends JPanel
    implements ISelectScenarioElementListener, Observer, ViewListener {
  ...
  /**
   * Initializes the AttributeTableView and Help panel. Creates a AttributeTablePage for each Class
   * subclassing AttributesScenarioElement Adds itself as a listener and observer to the
  ...
  public AttributeTableContainer(final TopographyCreatorModel defaultModel) {
    ...
    var minimalHelpViewSize =
        new Dimension(1, Toolkit.getDefaultToolkit().getScreenSize().height / 10);

    helpView.setMinimumSize(minimalHelpViewSize);
    helpView.setBorder(new LineBorder(UIManager.getColor("Component.borderColor")));

    helpView.addComponentListener(
  ...
  /**
   * This method implements the necessary method of the ISelectScenarioElementListener interface. It
   * will be called when the user selects a new ScenarioElement in the Editor and will delegate
   * hiding/changing the current Page to the AttributeTableView
   *
  ...
  public void selectionChange(ScenarioElement scenarioElement) {
    ...
    if (isNoElementSelected(scenarioElement)) attrView.clear();
  ...
  /**
   * This method implements the necessary method of the Observer interface. It will be called
   * everytime something changes in the editor. (which is probably very inefficient e.g. this
   * package does not need to update on mouse movement in the viewport). It delegates all calls tho
   * the AttributeTableView.
   */
  @Override
   ...
   * TreeAdapter which is a subclass of TreeNode which is used as a root node and stores a reference
   * to an Object implementing this method via the Revalidatable interface will call this method
   * once updateParentsFieldValue was called on it from a child node.
   */
  @Override
