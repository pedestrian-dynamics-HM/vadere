...
public class ActionMergeObstacles extends TopographyAction {
  ...
  public void actionPerformed(final ActionEvent e) {
    ...
    GeometryCleaner topographyCleaner =
        new GeometryCleaner(
    ...
    // polygons = WeilerAtherton.magnet(polygons, new
    // VRectangle(getScenarioPanelModel().getBounds()));
    ...
    getScenarioPanelModel()
        .removeObstacleIf(
            obstacle ->
                obstacle.getShape() instanceof VPolygon
                    || obstacle.getShape() instanceof VRectangle);
    ...
    // WeilerAtherton weilerAtherton = new WeilerAtherton(Arrays.asList(mergedPolygons.getLeft(),
    // new VPolygon(getScenarioPanelModel().getBounds())));
    // weilerAtherton.cap().get();

    // getScenarioPanelModel().addShape(new Obstacle(new AttributesObstacle(-1,
    // weilerAtherton.cap().get())));
