...
public class SelectElementMode extends DefaultSelectionMode {
  ...
  public SelectElementMode(
      final IDrawPanelModel panelModel, final UndoableEditSupport undoSupport) {
    super(panelModel);
    ...
    if (isMouseOnSelectedElement()) {
      ...
      resizeElement =
          panelModel
              .getSelectedElement()
              .getShape()
              .atBorder(panelModel.translateVectorCoordinates(startPoint));
      isModifying = true;
    } else {
  ...
  public void mouseDragged(final MouseEvent e) {
    if (isMouseOnPrototypeShape() || isModifying) {
      // VShape shape = panelModel.translate(new Point(e.getPoint().x - startPoint.x, e.getPoint().y
      // - startPoint.y));
      VShape shape =
          resizeElement
              ? panelModel.resize(startPoint, e.getPoint())
              : panelModel.translate(
                  new Point(e.getPoint().x - startPoint.x, e.getPoint().y - startPoint.y));
  ...
  private boolean isMouseOnSelectedElement() {
    ...
    return element != null
        && element.getShape().intersects(cursor.x - 0.01, cursor.y - 0.01, 0.02, 0.02);
  ...
  private boolean isMouseOnPrototypeShape() {
    ...
    VPoint cursor = panelModel.getMousePosition();
    return panelModel.isPrototypeVisble()
        && shape.intersects(cursor.x - 0.01, cursor.y - 0.01, 0.02, 0.02);
  }

  @Override
  public Cursor getCursor() {
    VShape selectedShape =
        panelModel.getSelectedElement() == null ? null : panelModel.getSelectedElement().getShape();
    VPoint mousePosition = panelModel.getMousePosition();
    boolean directionalCursorCondition =
        isMouseOnSelectedElement() && (resizeElement || selectedShape.atBorder(mousePosition));
