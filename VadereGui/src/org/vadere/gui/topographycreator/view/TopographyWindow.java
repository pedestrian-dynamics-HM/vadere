...
public class TopographyWindow extends JPanel {
  ...
  private void setTopography(
      final TopographyCreatorModel panelModel, @NotNull final ProjectViewModel model) {
    ...
    ScenarioElementView scenarioElementView =
        new ScenarioElementView(panelModel, jsonValidIndicator, selectedElementLabel);
    ...
    tabbedInfoPanel.addChangeListener(
        e -> {
    ...
    Action mergeObstaclesAction =
        new ActionMergeObstacles(
            "mergeObstacles",
    ...
    FormLayout layout =
        new FormLayout(
            "2dlu, default:grow(0.75), 2dlu, default:grow(0.25), 2dlu", // col
    ...
    scrollPane
        .getViewport()
        .addChangeListener(
            new JViewportChangeListener(panelModel, scrollPane.getVerticalScrollBar()));
    scrollPane.setPreferredSize(new Dimension(1, windowHeight));
    IViewportChangeListener viewportChangeListener =
        new ViewportChangeListener(panelModel, scrollPane);
    panelModel.addViewportChangeListener(viewportChangeListener);
    panelModel.addScrollPane(scrollPane);

    mainPanel =
        new TopographyPanel(panelModel, new TopographyCreatorRenderer(panelModel), scrollPane);
            ...
            new MouseAdapter() {
              ...
              public void mouseClicked(MouseEvent e) {
                if (e.getClickCount() == 2) {
                  int newLocation =
                      splitPane.getSize().width
                          - (scenarioElementView.textWidth()
                              + 65); // 65 magic number to take gutter into account
                  if (newLocation > 0) splitPane.setDividerLocation(newLocation);
                }
              }
            });
    ...
    /* select dot action */
    TopographyAction selectDotModeAction =
        new ActionSwitchSelectionMode(
            "dot selection mode",
            panelModel,
            new DrawDotMode(panelModel, undoSupport),
            basicAction);
    ...
    TopographyAction selectRectangleAction =
        new ActionSwitchSelectionMode(
            "rect selection mode",
            panelModel,
            new DrawRectangleMode(panelModel, undoSupport),
            basicAction);

    /* switch category to obstacle action */
    TopographyAction switchToObstacleAction =
        new ActionSwitchCategory(
            "switch to obstacle", panelModel, ScenarioElementType.OBSTACLE, selectRectangleAction);

    /* switch category to target action */
    TopographyAction switchToTargetAction =
        new ActionSwitchCategory(
            "switch to targets", panelModel, ScenarioElementType.TARGET, selectRectangleAction);

    /* switch category to target changer action */
    TopographyAction switchToTargetChangerAction =
        new ActionSwitchCategory(
            "switch to target changer",
            panelModel,
            ScenarioElementType.TARGET_CHANGER,
            selectRectangleAction);

    /* switch category to absorbing areas action */
    TopographyAction switchToAbsorbingAreaAction =
        new ActionSwitchCategory(
            "switch to absorbing areas",
            panelModel,
            ScenarioElementType.ABSORBING_AREA,
            selectRectangleAction);

    /* switch category to stairs action */
    TopographyAction switchToStairsAction =
        new ActionSwitchCategory(
            "switch to stairs", panelModel, ScenarioElementType.STAIRS, selectRectangleAction);

    /* switch category to measurement area action */
    TopographyAction switchToMeasurementAreaAction =
        new ActionSwitchCategory(
            "switch to measurement area",
            panelModel,
            ScenarioElementType.MEASUREMENT_AREA,
            selectRectangleAction);

    /* subtract obstacles from measurementArea */
    Action subtractMeasurementAreaAction =
        new ActionSubtractMeasurementArea(
            Messages.getString("TopographyCreator.btnSubtractMeasurementArea.label"),
            ICONS_SUBTRACT_PNG,
            panelModel,
            undoSupport);

    /* Place Random Pedestrians */
    Action placeRandomPedestrians =
        new ActionPlaceRandomPedestrians(
    ...
    Action pen =
        new ActionSwitchSelectionMode(
            ...
            "",
            panelModel,
            new DrawConvexHullMode(panelModel, undoSupport),
            basicAction);
    Action pen2 =
        new ActionSwitchSelectionMode(
            Messages.getString("TopographyCreator.btnSimplePolygon.label"),
            "/icons/simplePolygon.png",
            "",
            panelModel,
            new DrawSimplePolygonMode(panelModel, undoSupport),
            basicAction);
    Action rectangle =
        new ActionSwitchSelectionMode(
            Messages.getString("TopographyCreator.btnRectangle.label"),
            "/icons/paint_method_rectangle_icon.png",
            "",
            panelModel,
            new DrawRectangleMode(panelModel, undoSupport),
            basicAction);
    Action dot =
        new ActionSwitchSelectionMode(
            ...
            "/icons/paint_method_circle_icon.png",
            "",
            panelModel,
            new DrawDotMode(panelModel, undoSupport),
            basicAction);
    ...
    TopographyAction openObstacleDialog =
        new ActionOpenDrawOptionMenu(
            "Obstacle",
    ...
    TopographyAction openTargetDialog =
        new ActionOpenDrawOptionMenu(
            "Target",
    ...
    TopographyAction openStairsDialog =
        new ActionOpenDrawOptionMenu(
            "Stairs",
    ...
    TopographyAction openMeasurementAreaDialog =
        new ActionOpenDrawOptionMenu(
            "MeasurementArea",
    ...
    /* pedestrians */
    TopographyAction switchToPedestrianAction =
        new ActionSwitchCategory(
            "switch to pedestrian",
            panelModel,
            ScenarioElementType.PEDESTRIAN,
            selectDotModeAction);

    TopographyAction openPedestrianDialog =
        new ActionOpenDrawOptionMenu(
            "Pedestrian",
    ...
    TopographyAction switchToSourceAction =
        new ActionSwitchCategory(
            "switch to source", panelModel, ScenarioElementType.SOURCE, selectDotModeAction);

    /* source */
    JButton sourceButton = new JButton();
    TopographyAction openSourceDialog =
        new ActionOpenDrawOptionMenu(
            "Source",
    ...
    ActionSelectSelectShape selectShape =
        new ActionSelectSelectShape(
            "select shape mode",
    ...
    /* resize Topography */
    TopographyAction resizeTopographyBound =
        new ActionResizeTopographyBound(
    ...
    TopographyAction simplifyObstacle =
        new ActionSimplifyObstacles(
            "Simplify",
    ...
    TopographyAction translateTopography =
        new ActionTranslateTopography(
            "TranslateTopography",
    ...
    TopographyAction translateElements =
        new ActionTranslateElements(
            "TranslateElements",
    ...
    AbstractAction polyImg =
        new ActionGeneratePoly(
    ...
    AbstractAction generateMesh =
        new ActionGenerateMesh(
            Messages.getString("ProjectView.btnGenerateMesh.tooltip"),
    ...
    overlayToolBar.addSections(
        new ScenarioToolBarSection(selectShape, eraseMode),
        new ScenarioToolBarSection(zoomInAction, zoomOutAction, maximizeAction));
    ...
    var sectionE =
        new ScenarioToolBarSection(resetScenarioAction, saveScenarioAction, selectCutAction);
    var sectionF = new ScenarioToolBarSection(undoAction, redoAction);
    var sectionA = new ScenarioToolBarSection(polyImg, generateMesh);
    ...
    getInputMap(JComponent.WHEN_IN_FOCUSED_WINDOW)
        .put(KeyStroke.getKeyStroke(KeyEvent.VK_DELETE, 0), "delete-element");
    ...
    getInputMap(JComponent.WHEN_IN_FOCUSED_WINDOW)
        .put(KeyStroke.getKeyStroke("ESCAPE"), "deselect");
