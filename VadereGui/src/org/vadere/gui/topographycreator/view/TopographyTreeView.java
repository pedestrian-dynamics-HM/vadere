...
public class TopographyTreeView extends JPanel implements ISelectScenarioElementListener, Observer {
  ...
  public TopographyTreeView(final IDrawPanelModel<DefaultConfig> panelModel) {
    ...
    this.jTree.addTreeSelectionListener(
        e -> {
          if (isVisible()) {
            ...
            if (newPath != null) {
              ...
              if (wrapper.isType(TreeElementWrapper.TreeElementType.LEAF)
                  && !wrapper.compareWithScenarioElement(curr)) {
                wrapper
                    .getElement()
                    .ifPresent(
                        element -> {
  ...
  private void loadTree() {
    ...
    for (ScenarioElementType type : ScenarioElementType.values()) {
      DefaultMutableTreeNode c =
          new DefaultMutableTreeNode(TreeElementWrapper.category(type.name()));
