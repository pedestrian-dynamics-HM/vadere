 ...
 * it can no longer modified but the TopographyBuilder can be modified (there is no possibility to
 * remove/set members). To solve this problem, use this TopographyBuilder but only in the
 * topographycreator-package! Each build Topography is a new Object. The references of the members
 * of two Topography-Object can be the same because they wont be cloned.
 */
public class TopographyBuilder implements Iterable<ScenarioElement> {
  ...
  public TopographyBuilder(final Topography topography) {
    try {
      ...
      LinkedList<Pedestrian> pedStores =
          new LinkedList<>(topography.getInitialElements(Pedestrian.class));
      pedestrians = new LinkedList<>();
  ...
  private void setIds() {
    Set<Integer> usedIds =
        topographyElements
            .stream()
            .map(ScenarioElement::getId)
            .filter(id -> id != Attributes.ID_NOT_SET)
  ...
  private static Object getPrivateFieldValueFromTopography(
      final String fieldName, Topography topography)
  ...
  public Topography build() {
    ...
    for (Obstacle obstacle : obstacles) topography.addObstacle(obstacle);

    for (Stairs stairs : this.stairs) topography.addStairs(stairs);

    for (Source source : sources) topography.addSource(source);

    for (Target target : targets) topography.addTarget(target);

    for (TargetChanger targetChanger : targetChangers) topography.addTargetChanger(targetChanger);

    for (AbsorbingArea absorbingArea : absorbingAreas) topography.addAbsorbingArea(absorbingArea);
  ...
  public void translateElements(final double x, final double y) {
    ...
    if (attributes.getReferenceCoordinateSystem() != null) {
      VPoint newTranslation =
          attributes.getReferenceCoordinateSystem().getTranslation().add(-x, -y);
  ...
  public ScenarioElement selectElement(final VPoint position) {
    for (ScenarioElement element : topographyElements)
      if (element
          .getShape()
          .intersects(new Rectangle2D.Double(position.x - 0.1, position.y - 0.1, 0.2, 0.2)))
        return element;
    return null;
  }
  ...
  public void removeObstacleIf(@NotNull final Predicate<Obstacle> predicate) {
    topographyElements.removeIf(
        scenarioElement ->
            scenarioElement instanceof Obstacle && predicate.test((Obstacle) scenarioElement));
  ...
  public void removeMeasurementAreaIf(@NotNull final Predicate<MeasurementArea> predicate) {
    topographyElements.removeIf(
        scenarioElement ->
            scenarioElement instanceof MeasurementArea
                && predicate.test((MeasurementArea) scenarioElement));
