...
public class TopographyCreatorModel extends DefaultModel implements IDrawPanelModel {
  ...
  /**
   * container of all ScenarioElements, separates the real panelModel (Topography) from the
   * PanelModel (that holds information about selection, color, scaling and so on).
   */
  private TopographyBuilder topographyBuilder;

  /**
   * this is the scalingfactor for the obstacles, targets and sources (if the user want to scale the
   * topography). Not only a view-attribute.
   */
  ...
  /** Deletes the last added shape if any shape was added before, otherwise nothing will happen. */
  @Override
  ...
  /**
   * Deletes the last added shape, if it there (otherwise nothing will happen) specified by the
   * categoryKey and the type.
   *
  ...
  public void translateTopography(final double x, final double y) {
    ...
    setTopographyBound(
        new VRectangle(x, y, getTopographyBound().getWidth(), getTopographyBound().getHeight()));
    setChanged();
  }
