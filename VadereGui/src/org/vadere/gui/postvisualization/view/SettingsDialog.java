...
public class SettingsDialog extends org.vadere.gui.components.view.SettingsDialog {
  ...
  public void initComponents() {
    ...
    int row =
        18; // TODO this is hard coded; must be adapted when further elements are inserted above
    CellConstraints cc = new CellConstraints();
    JRadioButton chShowEvacTimeColor =
        new JRadioButton(Messages.getString("PostVis.chShowEvacTimeColor.text"));
    agentColorSettingsPane.add(chShowEvacTimeColor, cc.xyw(2, row += NEXT_CELL, 9));
    chShowEvacTimeColor.addItemListener(
        e -> {
          model.setAgentColoring(AgentColoring.EVACUATION_TIMES);
          model.notifyObservers();
        });

    JRadioButton chShowCriteriaColor =
        new JRadioButton(Messages.getString("PostVis.chShowCriteriaColor.text") + ":");
    PedestrianColorPanel pedestrianColorPanel = new PedestrianColorPanel(model);
    agentColorSettingsPane.add(
        chShowCriteriaColor, cc.xy(2, row += NEXT_CELL, CellConstraints.LEFT, CellConstraints.TOP));
    agentColorSettingsPane.add(pedestrianColorPanel, cc.xyw(4, row, 7));
    chShowCriteriaColor.addItemListener(
        e -> {
  ...
  protected JLayeredPane getAdditionalOptionPanel() {
    ...
    JCheckBox chCleanTrajecties =
        new JCheckBox(Messages.getString("PostVis.chbHideTrajAtTarget.text"));

    chCleanPed.setSelected(!model.config.isShowFaydedPedestrians());
    chCleanPed.addItemListener(
        e -> {
    ...
    chCleanTrajecties.addItemListener(
        e -> {
