...
public class PostvisualizationWindow extends JPanel implements Observer, DropTargetListener {
  ...
  private PostvisualizationWindow(
      final boolean loadTopographyInformationsOnly, final String projectPath) {
    ...
    addToggleActionToToolbar(
        ...
        new ActionVisualization(
            "show_pedestrian", resources.getIcon("pedestrian.png", iconWidth, iconHeight), model) {
          @Override
    ...
    addToggleActionToToolbar(
        toolbar,
        new ActionVisualization(
            "show_groups", resources.getIcon("group.png", iconWidth, iconHeight), model) {
          @Override
          public void actionPerformed(ActionEvent e) {
    ...
    addToggleActionToToolbar(
        toolbar,
        new ActionVisualization(
            "show_contacts", resources.getIcon("contacts.png", iconWidth, iconHeight), model) {
          @Override
          public void actionPerformed(ActionEvent e) {
            ...
            if (!model.config.isAerosolCloudsRecorded()) {
              JOptionPane.showMessageDialog(
                  ...
                  Messages.getString("PostVis.ShowAerosolCloudsErrorMessage.text")
                      ...
                      + "\n"
                      + TableAerosolCloudData.TABLE_NAME
                      + ".txt");
            } else {
    ...
    addToggleActionToToolbar(
        toolbar,
        new ActionSwapSelectionMode(
            "draw_voronoi_diagram", resources.getIcon("voronoi.png", iconWidth, iconHeight), model),
        "ProjectView.btnDrawVoronoiDiagram.tooltip");
    addToggleActionToToolbar(
        toolbar,
        new ActionVisualization(
            "show_grid", resources.getIcon("grid.png", iconWidth, iconHeight), model) {
          @Override
          public void actionPerformed(ActionEvent e) {
    ...
    addToggleActionToToolbar(
        toolbar,
        new ActionVisualization(
            "show_density", resources.getIcon("density.png", iconWidth, iconHeight), model) {
          @Override
          public void actionPerformed(ActionEvent e) {
    ...
    ActionRecording recordAction =
        new ActionRecording(
            "record", resources.getIcon("record.png", iconWidth, iconHeight), renderer);
    ...
    ActionVisualizationMenu imgDialog =
        new ActionVisualizationMenu(
            "camera_menu",
            resources.getIcon("camera.png", iconWidth, iconHeight),
            model,
            null,
            imgOptions);
    addActionMenuToToolbar(
        toolbar, imgDialog, Messages.getString("ProjectView.btnSnapshot.tooltip"));
    ...
    addActionToToolbar(
        toolbar,
        new ActionVisualization(
            "settings", resources.getIcon("settings.png", iconWidth, iconHeight), model) {
          @Override
          public void actionPerformed(ActionEvent e) {
    ...
    JMenuItem miCloseFloorFile =
        new JMenuItem(
            new ActionRemoveFloorFieldFile(
    ...
    String[] paths = VadereConfig.getConfig().getString("recentlyOpenedFiles", "").split(",");
    ...
    miGlobalSettings.addActionListener(
        e -> DialogFactory.createSettingsDialog(model).setVisible(true));
  ...
  private void buildKeyboardShortcuts() {
    Action spaceKeyReaction =
        new ActionVisualization("Typed Space Key Reaction", model) {
  ...
  public void loadOutputFile(
      final File trajectoryFile,
      final HashMap<String, File> additionalFiles,
      ...
      for (HashMap.Entry<String, File> entry : additionalFiles.entrySet()) {
        switch (entry.getKey()) {
          case ContactData.TABLE_NAME:
            additionalTables.put(
                entry.getKey(), IOOutput.readContactData(entry.getValue().toPath()));
          case TableAerosolCloudData.TABLE_NAME:
            additionalTables.put(
                entry.getKey(), IOOutput.readAerosolCloudData(entry.getValue().toPath()));
        }
      }
      model.init(
          IOOutput.readTrajectories(trajectoryFile.toPath()),
          additionalTables,
          scenario,
          trajectoryFile.getParent());
      model.notifyObservers();
    } catch (Exception ex) {
      JOptionPane.showMessageDialog(
          this, ex.getMessage(), Messages.getString("Error.text"), JOptionPane.ERROR_MESSAGE);
    }
  }

  public void loadOutputFile(final File trajectoryFile, final Scenario scenario)
      throws IOException {
    Player.getInstance(model).stop();
    try {
      model.init(
          IOOutput.readTrajectories(trajectoryFile.toPath()), scenario, trajectoryFile.getParent());
      model.notifyObservers();
    } catch (Exception ex) {
      JOptionPane.showMessageDialog(
          this, ex.getMessage(), Messages.getString("Error.text"), JOptionPane.ERROR_MESSAGE);
  ...
  private static JButton addActionToToolbar(
      final JToolBar toolbar, final Action action, final String toolTipProperty) {
    return SwingUtils.addActionToToolbar(toolbar, action, Messages.getString(toolTipProperty));
  }

  private static JToggleButton addToggleActionToToolbar(
      final JToolBar toolbar, final Action action, final String toolTipProperty) {
    return SwingUtils.addToggleActionToToolbar(
        toolbar, action, Messages.getString(toolTipProperty));
  }

  private static JButton addActionMenuToToolbar(
      final JToolBar toolbar,
      final ActionVisualizationMenu menuAction,
      final String toolTipProperty) {
    JButton btn =
        SwingUtils.addActionToToolbar(toolbar, menuAction, Messages.getString(toolTipProperty));
          ...
          if (model.hasOutputChanged()) {
            ...
            if (paths != null) {
              ...
              for (String path : paths) {
                if (path.length() > 0) {
                  mRecentFiles.add(
                      new ActionOpenFile("[" + i + "]" + " " + path, null, model, path));
                  i++;
                }
              }
            }
  ...
  public void openScenarioAndTrajectoryFile(@NotNull File scenarioOrTrajectoryFile) {
    VadereConfig.getConfig()
        .setProperty("SettingsDialog.outputDirectory.path", scenarioOrTrajectoryFile.getParent());
    VadereConfig.getConfig()
        .setProperty("SettingsDialog.outputDirectory.path", scenarioOrTrajectoryFile.getParent());
        ...
        () -> {
          ...
          try {
            ...
            if (trajectoryFile.isPresent() && scenarioFile.isPresent()) {
              ...
              model.init(
                  IOOutput.readTrajectories(trajectoryFile.get().toPath()),
                  vadereScenario,
                  trajectoryFile.get().getParent());
              model.notifyObservers();
              dialog.dispose();
            } else {
              String errorMessage =
                  String.format(
                      "%s\n%s\n%s",
                      Messages.getString("Data.TrajectoryOrScenarioFile.NoData.text"),
                      trajectoryFile,
                      scenarioFile);
