...
public class PostvisualizationRenderer extends SimulationRenderer {
  ...
  protected void renderSimulationContent(final Graphics2D g) {
    ...
    if (!model.isEmpty()) {
      ...
      Table slice =
          (model.config.isShowAllTrajectories())
              ? model.getAppearedPedestrians()
              : model.getAlivePedestrians();
  ...
  private void renderPedestrians(
      Graphics2D g, Collection<Pedestrian> pedestrians, Map<Integer, Color> agentColors) {
    ...
    if (model.config.isShowPedestrians()) {
      for (Pedestrian pedestrian : pedestrians) {
        ...
        if (model.config.isShowWalkdirection()
            && (model.config.isShowFaydedPedestrians()
                || model.getTrajectories().getDeathTime(pedestrian.getId())
                    > model.getSimTimeInSec())) {
  ...
  private void renderWalkingDirection(Graphics2D g, Pedestrian pedestrian) {
    ...
    if (lastPosition != null) {
      ...
      if (lastPosition.distance(position) < MIN_ARROW_LENGTH) {
        ...
        direction =
            new VPoint(
                lastPosition.getX() - position.getX(), lastPosition.getY() - position.getY());
  ...
  private void renderConnectingLinesByContact(Graphics2D g) {
    ...
    if (showContacts) {
      ...
      Table pairs =
          model.getContactData().getPairsOfPedestriansInContactAt(model.getSimTimeInSec());
      ...
      for (Row row : pairs) {
        ...
        if (model.config.isShowPedestrians()) {
          agents
              .stream()
              .filter(a -> a.getId() == id1 || a.getId() == id2)
              .forEach(a -> getAgentRender().render(a, Color.red, g));
        }
      }
  ...
  private void renderAerosolCloudData(Graphics2D g) {
    boolean showAerosolClouds =
        model.config.isShowAerosolClouds() && !model.getTableAerosolCloudData().isEmpty();

    if (showAerosolClouds) {
      Collection<AerosolCloud> aerosolClouds =
          model.getTableAerosolCloudData().toAerosolCloudCollection(getModel().getStep());
