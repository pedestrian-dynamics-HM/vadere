...
public interface IPostvisualizationModel {
  ...
  /**
   * Sets the simulation time for which the postVis displays all objects. This can be any value. If
   * the value is smaller 0 or greater than the overall simulation time it will be adjusted
   * accordingly.
   *
  ...
  /**
   * Returns a dt in seconds by which the postVis will be stepped forward and backwards. This can be
   * different from the dt which is defined by the actual output i.e. the
   * <tt>simTimeStepLength</tt>.
   *
  ...
  /**
   * Returns a dt in seconds by which the simulation updated its dynamic elements such as its
   * dynamic floor fields, its sources and targets and so on.
   *
   ...
   * <tt>simTimeStepLength</tt> that is if t is the simulation time than ceil(t /
   * <tt>simTimeStepLength</tt>) is the simulation {@link Step}.
   *
  ...
  /**
   * Returns the first simulation step for which an agent is present. A simulation step is defined
   * by <tt>simTimeStepLength</tt> that is if t is the simulation time than ceil(t /
   * <tt>simTimeStepLength</tt>) is the simulation {@link Step}.
   *
  ...
  default Function<IPoint, Double> getPotentialField() {
    ...
    Optional<PotentialFieldContainer> optionalPotentialFieldContainer =
        getPotentialFieldContainer();
    if (optionalPotentialFieldContainer.isPresent()) {
      ...
      try {
        final CellGrid potentialField =
            container.getPotentialField(
                Step.toFloorStep(getVisTimeStepLength(), getSimTimeStepLength()));
