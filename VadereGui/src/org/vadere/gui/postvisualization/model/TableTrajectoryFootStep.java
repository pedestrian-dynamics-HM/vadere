...
public class TableTrajectoryFootStep {
  ...
  public TableTrajectoryFootStep(@NotNull final Table dataFrame) {
    ...
    if (!isEmpty()) {
      this.startTime =
          agentDataFrame.summarize(birthTimeColName, min).apply().doubleColumn(0).get(0);
  ...
  protected Agent toAgent(
      @NotNull final Row row, @NotNull final AttributesAgent attributesAgent, final double time) {
    ...
    VPoint position =
        FootStep.interpolateFootStep(startX, startY, endX, endY, startTime, endTime, time);
  ...
  public void setSlice(final double startTime, final double endTime) {
    currentSlice =
        trajectoryDataFrame /*.where(trajectoryDataFrame.doubleColumn(startTimeCol).isLessThan(endTime)
                            .and(trajectoryDataFrame.doubleColumn(endTimeCol).isGreaterThanOrEqualTo(startTime)))*/;
  }

  /**
   * Returns all footsteps <tt>fs</tt> for which <tt>fs.startTime</tt> is smaller than
   * <tt>endTime</tt> and <tt>fs.endTime</tt> is greater or equals <tt>startTime</tt>.
   *
   * @param startTime
   * @param endTime
  ...
  public Table getAgents(final double startTime, final double endTime) {
    return currentSlice.where(
        getStartTime().isLessThan(endTime).and(getEndTime().isGreaterThanOrEqualTo(startTime)));
  }

  /**
   * Returns all footsteps <tt>fs</tt> for which <tt>fs.startTime</tt> is smaller than
   * <tt>endTime</tt> and <tt>fs.endTime</tt> is greater or equals <tt>startTime</tt> for all agents
   * which are alive in between, that is: their fist step starts after <tt>startTime</tt> and their
   * last step ends before <tt>endTime</tt>.
   *
   * @param startTime
   * @param endTime
   * @return multiple foosteps for each agent
  ...
  public Table getAgents(final double simTimeInSec) {
    return currentSlice
        .where(
            getStartTime()
                .isLessThanOrEqualTo(simTimeInSec)
  ...
  public Table getAgentsWithDisappearedAgents(final double simTimeInSec) {
    ...
    Table deadAgents =
        currentSlice.where(
            getPedId()
                .isNotIn(getPedId(aliveAgents).asObjectArray())
    ...
    return agentDataFrame
        .where(
            getBirthTime()
                ...
                .and(getDeathTime().isGreaterThanOrEqualTo(endTime)))
  ...
  public double getBirthTime(final int pedId) {
    return agentDataFrame
        .where(agentDataFrame.intColumn(pedIdCol).isEqualTo(pedId))
