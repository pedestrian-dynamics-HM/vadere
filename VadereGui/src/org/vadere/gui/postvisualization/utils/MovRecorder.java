...
public class MovRecorder implements IRecorder {
  ...
  private void addPicture() throws IOException {
    synchronized (model) {
      if (!this.finished) {
        ...
        logger.info(
            this + " add picture " + bi.getWidth() + " (width), " + bi.getHeight() + " (height).");
        enc.encodeImage(bi);
  ...
  public void startRecording() throws IOException {
    synchronized (model) {
      ...
      outputFile =
          new File(Messages.getString("FileDialog.filenamePrefix") + formattedDate + ".mov");
      ...
      if (returnVal == JFileChooser.APPROVE_OPTION) {
        ...
        outputFile =
            fileChooser.getSelectedFile().toString().endsWith(".mov")
