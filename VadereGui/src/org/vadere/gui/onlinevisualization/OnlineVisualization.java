...
public class OnlineVisualization implements PassiveCallback {
  ...
  /**
   * Holds a snapshot of the observation area of a frame. This class is used to provide simulation
   * data for visualization to the draw thread. To avoid threading issues, the class holds a partial
   * copy of the original scenario.
   */
  ...
  /**
   * Pushes (by copy) required data from current simulation into data queues for being displayed by
   * draw thread (thread-safe). These may be for example the physical world representation and
   * potential field of perception.
   */
  private void pushDrawData(double simTimeInSec) {
    ...
    synchronized (model.getDataSynchronizer()) {
      ...
      IPotentialField pft =
          (model.config.isShowTargetPotentialField() && potentialFieldTarget != null)
              ? potentialFieldTarget.getSolution()
              : null;
      Function<Agent, IMesh<?, ?, ?>> discretizations =
          (model.config.isShowTargetPotentielFieldMesh() && potentialFieldTarget != null)
              ? potentialFieldTarget.getDiscretization()
              : null;
      ...
      if (model.config.isShowPotentialField() && selectedAgent != null && potentialField != null) {
        pedPotentialField =
            IPotentialField.copyAgentField(
                potentialField, selectedAgent, new VRectangle(model.getTopographyBound()), 0.1);
      }

      ObservationAreaSnapshotData data =
          new ObservationAreaSnapshotData(
              simTimeInSec, domain.clone(), pft, pedPotentialField, selectedAgent, discretizations);
      model.pushObservationAreaSnapshot(data);
    }
  }

  // [issue 280] show OnlineVisualization Window and remove mouse Listeners. This is necessary to
  // ensure
