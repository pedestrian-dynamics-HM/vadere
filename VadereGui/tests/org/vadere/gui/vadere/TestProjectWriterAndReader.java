...
public class TestProjectWriterAndReader {
    ...
    tests.add(
        new Scenario(
            new ScenarioStore(
                testName + "1",
                "",
                OptimalStepsModel.class.getName(),
                attributes,
                new AttributesSimulation(),
                new Topography())));
    tests.add(
        new Scenario(
            new ScenarioStore(
  ...
  /**
   * Test method for {@link org.vadere.simulator.projects.io.IOVadere#readProject(java.lang.String)}
   * .
   *
   * @throws IOException
   * @throws SAXException
  ...
  public void testWriteReadProject()
    ...
    assertEquals(
        projectJson
            .getScenarios()
            .stream()
            .map(scenario -> JsonConverter.serializeScenarioRunManager(scenario))
